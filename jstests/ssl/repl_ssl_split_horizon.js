(function() {
    ...
    try {
        if (runMongoProgram(
                "env", "HOSTALIASES=" + hostsFile, "getent", "hosts", "splithorizon1") != 0) {
            jsTestLog(
                `HOSTALIASES does not seem to work as expected on this system, skipping this test.`);
            removeFile(hostsFile);
            return;
        }
    } catch (e) {
        jsTestLog(`Failed the check for HOSTALIASES support, skipping this test.`);
        removeFile(hostsFile);
        return;
    }
