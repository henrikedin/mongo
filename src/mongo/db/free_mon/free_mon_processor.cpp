...
void FreeMonProcessor::run() {
    try {
        ...
        LOGV2_WARNING("Uncaught exception in '{}' in free monitoring subsystem. Shutting down the "
...
void FreeMonProcessor::doAsyncRegisterComplete(
    ...
    if (!s.isOK()) {
        LOGV2_WARNING("Free Monitoring registration halted due to {}", "s"_attr = s);
...
void FreeMonProcessor::doAsyncRegisterFail(
    ...
    if (!_registrationRetry->incrementError()) {
        ...
        LOGV2_WARNING("Free Monitoring is abandoning registration after excess retries");
        return;
    }

    LOGV2_DEBUG(1, "Free Monitoring Registration Failed with status '{}', retrying in {}", "msg_getPayload"_attr = msg->getPayload(), "registrationRetry_getNextDuration"_attr = _registrationRetry->getNextDuration());
...
void FreeMonProcessor::doAsyncMetricsComplete(
    ...
    if (!s.isOK()) {
        LOGV2_WARNING("Free Monitoring metrics uploading halted due to {}", "s"_attr = s);
...
void FreeMonProcessor::doAsyncMetricsFail(
    ...
    if (!_metricsRetry->incrementError()) {
        // We have exceeded our retry
        LOGV2_WARNING("Free Monitoring is abandoning metrics upload after excess retries");
        return;
    }

    LOGV2_DEBUG(1, "Free Monitoring Metrics upload failed with status {}, retrying in {}", "msg_getPayload"_attr = msg->getPayload(), "metricsRetry_getNextDuration"_attr = _metricsRetry->getNextDuration());
...
void FreeMonProcessor::doNotifyOnUpsert(
    ...
    try {
        ...
        LOGV2_WARNING("Uncaught exception in '{}' in free monitoring op observer. Shutting down the "
                     "free monitoring subsystem.", "exceptionToStatus"_attr = exceptionToStatus());
    }
}
