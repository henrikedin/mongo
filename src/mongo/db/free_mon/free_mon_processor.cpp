...
void FreeMonProcessor::run() {
    try {
        ...
        LOGV2_WARNING(20576, "Uncaught exception in '{exceptionToStatus}' in free monitoring subsystem. Shutting down the "
                     "free monitoring subsystem.", "exceptionToStatus"_attr = exceptionToStatus());
    }
}
...
void FreeMonProcessor::doAsyncRegisterComplete(
    ...
    if (!s.isOK()) {
        LOGV2_WARNING(20577, "Free Monitoring registration halted due to {s}", "s"_attr = s);
    ...
    LOGV2(20572, "Free Monitoring is Enabled. Frequency: {resp_getReportingInterval} seconds", "resp_getReportingInterval"_attr = resp.getReportingInterval());
...
void FreeMonProcessor::doAsyncRegisterFail(
    ...
    if (!_registrationRetry->incrementError()) {
        ...
        LOGV2_WARNING(20578, "Free Monitoring is abandoning registration after excess retries");
        return;
    }

    LOGV2_DEBUG(20573, 1, "Free Monitoring Registration Failed with status '{msg_getPayload}', retrying in {registrationRetry_getNextDuration}", "msg_getPayload"_attr = msg->getPayload(), "registrationRetry_getNextDuration"_attr = _registrationRetry->getNextDuration());
...
void FreeMonProcessor::doCommandUnregister(
    ...
    LOGV2(20574, "Free Monitoring is Disabled");
...
void FreeMonProcessor::doAsyncMetricsComplete(
    ...
    if (!s.isOK()) {
        LOGV2_WARNING(20579, "Free Monitoring metrics uploading halted due to {s}", "s"_attr = s);
...
void FreeMonProcessor::doAsyncMetricsFail(
    ...
    if (!_metricsRetry->incrementError()) {
        // We have exceeded our retry
        LOGV2_WARNING(20580, "Free Monitoring is abandoning metrics upload after excess retries");
        return;
    }

    LOGV2_DEBUG(20575, 1, "Free Monitoring Metrics upload failed with status {msg_getPayload}, retrying in {metricsRetry_getNextDuration}", "msg_getPayload"_attr = msg->getPayload(), "metricsRetry_getNextDuration"_attr = _metricsRetry->getNextDuration());
...
void FreeMonProcessor::doNotifyOnUpsert(
    ...
    try {
        ...
        LOGV2_WARNING(20581, "Uncaught exception in '{exceptionToStatus}' in free monitoring op observer. Shutting down the "
                     "free monitoring subsystem.", "exceptionToStatus"_attr = exceptionToStatus());
    }
}
