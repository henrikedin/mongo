...
StatusWith<bool> extractMetricsFromDocument(const BSONObj& referenceDoc,
    ...
    while (itCurrent.more()) {
        ...
        if (matches && !itReference.more()) {
            LOGV2_DEBUG(20589, 4, "full-time diagnostic data capture schema change: currrent document is "
        ...
        if (matches) {
            ...
            if ((currentElement.type() != referenceElement.type()) &&
                ...
                LOGV2_DEBUG(20591, 4, "full-time diagnostic data capture  schema change: field type change for "
                          "field '{referenceElement_fieldNameStringData}' from '{static_cast_int_referenceElement_type}' to '{static_cast_int_currentElement_type}'", "referenceElement_fieldNameStringData"_attr = referenceElement.fieldNameStringData(), "static_cast_int_referenceElement_type"_attr = static_cast<int>(referenceElement.type()), "static_cast_int_currentElement_type"_attr = static_cast<int>(currentElement.type()));
                matches = false;
            }
        }
    ...
    if (matches && itReference.more()) {
        LOGV2_DEBUG(20592, 4, "full-time diagnostic data capture schema change: reference document is longer "
                  "then current");
        matches = false;
    }
