...
boost::optional<long long> _parseDroppedCollectionCount(const OplogEntry& oplogEntry) {
    ...
    if (!obj2) {
        LOGV2_WARNING("Unable to get collection count from {} without the o2 "
                     "field. oplog op: {}", "desc"_attr = desc, "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
        return boost::none;
    }
    ...
    if (!status.isOK()) {
        LOGV2_WARNING("Failed to parse {} for collection count: {}. oplog op: {}", "desc"_attr = desc, "status"_attr = status, "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
        return boost::none;
    }

    if (count < 0) {
        LOGV2_WARNING("Invalid collection count found in {}: {}. oplog op: {}", "desc"_attr = desc, "count"_attr = count, "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
        return boost::none;
    }
...
void RollbackImpl::_runPhaseFromAbortToReconstructPreparedTxns(
    ...
    LOGV2("Rollback reverted {} insert operations, {} update operations and {} delete operations.", "observerInfo_rollbackCommandCounts_kInsertCmdName"_attr = _observerInfo.rollbackCommandCounts[kInsertCmdName], "observerInfo_rollbackCommandCounts_kUpdateCmdName"_attr = _observerInfo.rollbackCommandCounts[kUpdateCmdName], "observerInfo_rollbackCommandCounts_kDeleteCmdName"_attr = _observerInfo.rollbackCommandCounts[kDeleteCmdName]);
    ...
    for (const auto& uiCount : _newCounts) {
        ...
        if (kCollectionScanRequired == newCount) {
            ...
            if (PlanExecutor::IS_EOF != state) {
                ...
                LOGV2_WARNING("Failed to set count of {} ({}) [{}] due to failed collection scan: {}", "nss_ns"_attr = nss.ns(), "uuid_toString"_attr = uuid.toString(), "ident"_attr = ident, "exec_statestr_state"_attr = exec->statestr(state));
                continue;
            }
        ...
        if (!status.isOK()) {
            // We ignore errors here because crashing or leaving rollback would only leave
            // collection counts more inaccurate.
            LOGV2_WARNING("Failed to set count of {} ({}) [{}] to {}. Received: {}", "nss_ns"_attr = nss.ns(), "uuid_toString"_attr = uuid.toString(), "ident"_attr = ident, "newCount"_attr = newCount, "status"_attr = status);
        } else {
...
Status RollbackImpl::_findRecordStoreCounts(OperationContext* opCtx) {
    ...
    for (const auto& uiCount : _countDiffs) {
        ...
        if (oldCount > static_cast<uint64_t>(std::numeric_limits<long long>::max())) {
            LOGV2_WARNING("Count for {} ({}) was {} which is larger than the maximum int64_t value. Not attempting to fix "
        ...
        if (newCount < 0) {
            LOGV2_WARNING("Attempted to set count for {} ({}) to {} but set it to 0 instead. This is likely due to the count previously "
                         "becoming inconsistent from an unclean shutdown or a rollback that could "
                         "not fix the count correctly. Old count: {}. Count change: {}", "nss_ns"_attr = nss->ns(), "uuid_toString"_attr = uuid.toString(), "newCount"_attr = newCount, "oldCount"_attr = oldCount, "countDiff"_attr = countDiff);
            newCount = 0;
        }
...
Status RollbackImpl::_processRollbackOp(OperationContext* opCtx, const OplogEntry& oplogEntry) {
    ...
    if (opType == OpTypeEnum::kInsert) {
        ...
        if (serverGlobalParams.clusterRole == ClusterRole::ShardServer &&
            ...
            LOGV2_WARNING("Shard identity document rollback detected. oplog op: {}", "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
            ...
            LOGV2_WARNING("Config version document rollback detected. oplog op: {}", "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
        }
...
Status RollbackImpl::_checkAgainstTimeLimit(
    ...
    if (topOfOplogWallTime >= firstOpWallClockTimeAfterCommonPoint) {
        ...
        LOGV2_WARNING("Wall clock times on oplog entries not monotonically increasing. This "
               "might indicate a backward clock skew. Time at first oplog after common point: {}. Time at top of oplog: {}", "firstOpWallClockTimeAfterCommonPoint"_attr = firstOpWallClockTimeAfterCommonPoint, "topOfOplogWallTime"_attr = topOfOplogWallTime);
    }
...
void RollbackImpl::_summarizeRollback(OperationContext* opCtx) const {
    ...
    LOGV2("\tstart time: {}", "rollbackStats_startTime"_attr = _rollbackStats.startTime);
    LOGV2("\tend time: {}", "opCtx_getServiceContext_getFastClockSource_now"_attr = opCtx->getServiceContext()->getFastClockSource()->now());
    LOGV2("\tsync source: {}", "remoteOplog_hostAndPort_toString"_attr = _remoteOplog->hostAndPort().toString());
    ...
    if (_rollbackStats.rollbackId) {
        LOGV2("\trollback id: {}", "rollbackStats_rollbackId"_attr = *_rollbackStats.rollbackId);
    }
    if (_rollbackStats.lastLocalOptime) {
        LOGV2("\tlast optime on branch of history rolled back: {}", "rollbackStats_lastLocalOptime"_attr = *_rollbackStats.lastLocalOptime);
    }
    if (_rollbackStats.commonPoint) {
        LOGV2("\tcommon point optime: {}", "rollbackStats_commonPoint"_attr = *_rollbackStats.commonPoint);
    }
    ...
    if (_rollbackStats.truncateTimestamp) {
        LOGV2("\ttruncate timestamp: {}", "rollbackStats_truncateTimestamp"_attr = *_rollbackStats.truncateTimestamp);
    }
    if (_rollbackStats.stableTimestamp) {
        LOGV2("\tstable timestamp: {}", "rollbackStats_stableTimestamp"_attr = *_rollbackStats.stableTimestamp);
    }
    LOGV2("\tshard identity document rolled back: {}", "observerInfo_shardIdentityRolledBack"_attr = _observerInfo.shardIdentityRolledBack);
    LOGV2("\tconfig server config version document rolled back: {}", "observerInfo_configServerConfigVersionRolledBack"_attr = _observerInfo.configServerConfigVersionRolledBack);
    LOGV2("\taffected sessions: {}", "observerInfo_rollbackSessionIds_empty_none"_attr = (_observerInfo.rollbackSessionIds.empty() ? "none" : ""));
    for (const auto& sessionId : _observerInfo.rollbackSessionIds) {
        LOGV2("\t\t{}", "sessionId"_attr = sessionId);
    }
    LOGV2("\taffected namespaces: {}", "observerInfo_rollbackNamespaces_empty_none"_attr = (_observerInfo.rollbackNamespaces.empty() ? "none" : ""));
    for (const auto& nss : _observerInfo.rollbackNamespaces) {
        LOGV2("\t\t{}", "nss_ns"_attr = nss.ns());
    }
    LOGV2("\tcounts of interesting commands rolled back: {}", "observerInfo_rollbackCommandCounts_empty_none"_attr = (_observerInfo.rollbackCommandCounts.empty() ? "none" : ""));
    ...
    LOGV2("\ttotal number of entries rolled back (including no-ops): {}", "observerInfo_numberOfEntriesObserved"_attr = _observerInfo.numberOfEntriesObserved);
}
