...
Status Reporter::trigger() {
    ...
    if (!_status.isOK()) {
        LOGV2_DEBUG(2, "Reporter failed to schedule callback to prepare and send update command: {}", "status"_attr = _status);
        return _status;
    }
...
void Reporter::_sendCommand_inlock(BSONObj commandRequest, Milliseconds netTimeout) {
    LOGV2_DEBUG(2, "Reporter sending slave oplog progress to upstream updater {}: {}", "target"_attr = _target, "commandRequest"_attr = commandRequest);
    ...
    _status = scheduleResult.getStatus();
    if (!_status.isOK()) {
        LOGV2_DEBUG(2, "Reporter failed to schedule with status: {}", "status"_attr = _status);
...
void Reporter::_processResponseCallback(
    const executor::TaskExecutor::RemoteCommandCallbackArgs& rcbd) {
    {
        ...
        if (_status == ErrorCodes::InvalidReplicaSetConfig &&
            ...
            LOGV2_DEBUG(1, "Reporter found newer configuration on sync source: {}. Retrying.", "target"_attr = _target);
            _status = Status::OK();
