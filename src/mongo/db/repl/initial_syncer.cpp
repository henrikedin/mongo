...
void pauseAtInitialSyncFuzzerSyncronizationPoints(std::string msg) {
    ...
    if (MONGO_unlikely(initialSyncFuzzerSynchronizationPoint1.shouldFail())) {
        LOGV2(20933, "{msg}", "msg"_attr = msg);
        LOGV2(20934, "initialSyncFuzzerSynchronizationPoint1 fail point enabled.");
        initialSyncFuzzerSynchronizationPoint1.pauseWhileSet();
    }

    if (MONGO_unlikely(initialSyncFuzzerSynchronizationPoint2.shouldFail())) {
        LOGV2(20935, "initialSyncFuzzerSynchronizationPoint2 fail point enabled.");
...
BSONObj InitialSyncer::_getInitialSyncProgress_inlock() const {
    try {
        ...
        LOGV2(20936, "Error creating initial sync progress object: {e_toString}", "e_toString"_attr = e.toString());
    }
    BSONObjBuilder bob;
    _appendInitialSyncProgressMinimal_inlock(&bob);
    return bob.obj();
...
void InitialSyncer::_setUp_inlock(OperationContext* opCtx, std::uint32_t initialSyncMaxAttempts) {
    ...
    LOGV2_DEBUG(20937, 1, "Creating oplogBuffer.");
...
void InitialSyncer::_startInitialSyncAttemptCallback(
    ...
    LOGV2(20938, "Starting initial sync (attempt {initialSyncAttempt_1} of {initialSyncMaxAttempts})", "initialSyncAttempt_1"_attr = (initialSyncAttempt + 1), "initialSyncMaxAttempts"_attr = initialSyncMaxAttempts);
    ...
    LOGV2_DEBUG(20939, 2, "Resetting sync source so a new one can be chosen for this initial sync attempt.");
    _syncSource = HostAndPort();

    LOGV2_DEBUG(20940, 2, "Resetting all optimes before starting this initial sync attempt.");
    _opts.resetOptimes();
    _lastApplied = {OpTime(), Date_t()};
    _lastFetched = {};

    LOGV2_DEBUG(20941, 2, "Resetting the oldest timestamp before starting this initial sync attempt.");
    ...
    LOGV2_DEBUG(20942, 2, "Resetting feature compatibility version to last-stable. If the sync source is in "
...
void InitialSyncer::_chooseSyncSourceCallback(
    ...
    if (!syncSource.isOK()) {
        ...
        LOGV2_DEBUG(20943, 1, "Error getting sync source: '{syncSource_getStatus}', trying again in {opts_syncSourceRetryWait} at {when_toString}. Attempt {chooseSyncSourceAttempt_1} of {numInitialSyncConnectAttempts_load}", "syncSource_getStatus"_attr = syncSource.getStatus(), "opts_syncSourceRetryWait"_attr = _opts.syncSourceRetryWait, "when_toString"_attr = when.toString(), "chooseSyncSourceAttempt_1"_attr = (chooseSyncSourceAttempt + 1), "numInitialSyncConnectAttempts_load"_attr = numInitialSyncConnectAttempts.load());
    ...
    if (MONGO_unlikely(initialSyncHangBeforeCreatingOplog.shouldFail())) {
        ...
        LOGV2(20944, "initial sync - initialSyncHangBeforeCreatingOplog fail point "
...
Status InitialSyncer::_truncateOplogAndDropReplicatedDatabases() {
    ...
    LOGV2_DEBUG(20945, 1, "About to truncate the oplog, if it exists, ns:{opts_localOplogNS}, and drop all user databases (so that we can clone them).", "opts_localOplogNS"_attr = _opts.localOplogNS);
    ...
    LOGV2_DEBUG(20946, 2, "Truncating the existing oplog: {opts_localOplogNS}", "opts_localOplogNS"_attr = _opts.localOplogNS);
    Timer timer;
    auto status = _storage->truncateCollection(opCtx.get(), _opts.localOplogNS);
    LOGV2(20947, "Initial syncer oplog truncation finished in: {timer_millis}ms", "timer_millis"_attr = timer.millis());
    if (!status.isOK()) {
        // 1a.) Create the oplog.
        LOGV2_DEBUG(20948, 2, "Creating the oplog: {opts_localOplogNS}", "opts_localOplogNS"_attr = _opts.localOplogNS);
    ...
    LOGV2_DEBUG(20949, 2, "Dropping user databases");
...
void InitialSyncer::_getBeginFetchingOpTimeCallback(
    ...
    if (MONGO_unlikely(initialSyncHangAfterGettingBeginFetchingTimestamp.shouldFail())) {
        LOGV2(20950, "initialSyncHangAfterGettingBeginFetchingTimestamp fail point enabled.");
...
void InitialSyncer::_fcvFetcherCallback(const StatusWith<Fetcher::QueryResponse>& result,
    ...
    LOGV2_DEBUG(20951, 2, "Setting begin applying timestamp to {initialSyncState_beginApplyingTimestamp} using last oplog entry: {redact_result_getValue_documents_front}, ns: {opts_localOplogNS} and the begin fetching timestamp to {initialSyncState_beginFetchingTimestamp}", "initialSyncState_beginApplyingTimestamp"_attr = _initialSyncState->beginApplyingTimestamp, "redact_result_getValue_documents_front"_attr = redact(result.getValue().documents.front()), "opts_localOplogNS"_attr = _opts.localOplogNS, "initialSyncState_beginFetchingTimestamp"_attr = _initialSyncState->beginFetchingTimestamp);
    ...
    LOGV2_DEBUG(20952, 2, "Starting OplogFetcher: {oplogFetcher_toString}", "oplogFetcher_toString"_attr = _oplogFetcher->toString());
    ...
    if (MONGO_unlikely(initialSyncHangBeforeCopyingDatabases.shouldFail())) {
        ...
        // This log output is used in js tests so please leave it.
        LOGV2(20953, "initial sync - initialSyncHangBeforeCopyingDatabases fail point "
                 "enabled. Blocking until fail point is disabled.");
    ...
    LOGV2_DEBUG(20954, 2, "Starting AllDatabaseCloner: {initialSyncState_allDatabaseCloner_toString}", "initialSyncState_allDatabaseCloner_toString"_attr = _initialSyncState->allDatabaseCloner->toString());
...
void InitialSyncer::_oplogFetcherCallback(const Status& oplogFetcherFinishStatus,
                                          std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    stdx::lock_guard<Latch> lock(_mutex);
    LOGV2(20955, "Finished fetching oplog during initial sync: {redact_oplogFetcherFinishStatus}. Last fetched optime: {lastFetched_toString}", "redact_oplogFetcherFinishStatus"_attr = redact(oplogFetcherFinishStatus), "lastFetched_toString"_attr = _lastFetched.toString());
    ...
    if (status.isOK()) {
        LOGV2(20956, "Finished fetching oplog fetching early. Last fetched optime: {lastFetched_toString}", "lastFetched_toString"_attr = _lastFetched.toString());
        return;
    }
...
void InitialSyncer::_allDatabaseClonerCallback(
    ...
    std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    LOGV2(20957, "Finished cloning data: {redact_databaseClonerFinishStatus}. Beginning oplog replay.", "redact_databaseClonerFinishStatus"_attr = redact(databaseClonerFinishStatus));
    _client->shutdownAndDisallowReconnect();
    ...
    if (MONGO_unlikely(initialSyncHangAfterDataCloning.shouldFail())) {
        ...
        // we run with multiple threads so it's fine to spin on this thread.
        // This log output is used in js tests so please leave it.
        LOGV2(20958, "initial sync - initialSyncHangAfterDataCloning fail point "
                 "enabled. Blocking until fail point is disabled.");
...
void InitialSyncer::_lastOplogEntryFetcherCallbackForStopTimestamp(
    ...
    // oplog entry to seed the oplog before checking the rollback ID.
    {
        ...
        LOGV2_DEBUG(20959, 2, "Inserting oplog seed document: {oplogSeedDoc}", "oplogSeedDoc"_attr = oplogSeedDoc);
    ...
    LOGV2(20960, "No need to apply operations. (currently at {initialSyncState_stopTimestamp_toBSON})", "initialSyncState_stopTimestamp_toBSON"_attr = _initialSyncState->stopTimestamp.toBSON());
...
void InitialSyncer::_getNextApplierBatchCallback(
    ...
    if (!batchResult.isOK()) {
        LOGV2_WARNING(20970, "Failure creating next apply batch: {redact_batchResult_getStatus}", "redact_batchResult_getStatus"_attr = redact(batchResult.getStatus()));
    ...
    if (MONGO_unlikely(failInitialSyncBeforeApplyingBatch.shouldFail())) {
        LOGV2(20961, "initial sync - failInitialSyncBeforeApplyingBatch fail point enabled. Pausinguntil fail point is disabled, then will fail initial sync.");
...
void InitialSyncer::_multiApplierCallback(const Status& multiApplierStatus,
    ...
    if (MONGO_unlikely(initialSyncFassertIfApplyingBatchFails.shouldFail())) {
        LOGV2(20963, "initialSyncFassertIfApplyingBatchFails fail point enabled.");
        fassert(31210, status);
    }

    if (!status.isOK()) {
        LOGV2_ERROR(20973, "Failed to apply batch due to '{redact_status}'", "redact_status"_attr = redact(status));
        onCompletionGuard->setResultAndCancelRemainingWork_inlock(lock, status);
        return;
    }
...
void InitialSyncer::_rollbackCheckerCheckForRollbackCallback(
    ...
    if (_shouldRetryError(lock, status)) {
        LOGV2_DEBUG(20964, 1, "Retrying rollback checker because of network error {status}", "status"_attr = status);
        _scheduleRollbackCheckerCheckForRollback_inlock(lock, onCompletionGuard);
        return;
    }
...
void InitialSyncer::_finishInitialSyncAttempt(const StatusWith<OpTimeAndWallTime>& lastApplied) {
    ...
    auto finishCallbackGuard = makeGuard([this, &result] {
        ...
        if (!scheduleResult.isOK()) {
            LOGV2_WARNING(20971, "Unable to schedule initial syncer completion task due to {redact_scheduleResult_getStatus}. Running callback on current thread.", "redact_scheduleResult_getStatus"_attr = redact(scheduleResult.getStatus()));
            _finishCallback(result);
        }
    });

    LOGV2(20965, "Initial sync attempt finishing up.");

    stdx::lock_guard<Latch> lock(_mutex);
    LOGV2(20966, "Initial Sync Attempt Statistics: {redact_getInitialSyncProgress_inlock}", "redact_getInitialSyncProgress_inlock"_attr = redact(_getInitialSyncProgress_inlock()));
    ...
    if (MONGO_unlikely(failAndHangInitialSync.shouldFail())) {
        LOGV2(20967, "failAndHangInitialSync fail point enabled.");
    ...
    LOGV2_ERROR(20974, "Initial sync attempt failed -- attempts left: {stats_maxFailedInitialSyncAttempts_stats_failedInitialSyncAttempts} cause: {redact_result_getStatus}", "stats_maxFailedInitialSyncAttempts_stats_failedInitialSyncAttempts"_attr = (_stats.maxFailedInitialSyncAttempts - _stats.failedInitialSyncAttempts), "redact_result_getStatus"_attr = redact(result.getStatus()));
    ...
    if (_stats.failedInitialSyncAttempts >= _stats.maxFailedInitialSyncAttempts) {
        ...
        LOGV2_FATAL(20976, "{err}", "err"_attr = err);
...
void InitialSyncer::_finishCallback(StatusWith<OpTimeAndWallTime> lastApplied) {
    ...
    if (MONGO_unlikely(initialSyncHangBeforeFinish.shouldFail())) {
        // This log output is used in js tests so please leave it.
        LOGV2(20968, "initial sync - initialSyncHangBeforeFinish fail point "
                 "enabled. Blocking until fail point is disabled.");
    ...
    try {
        ...
        LOGV2_WARNING(20972, "initial syncer finish callback threw exception: {redact_exceptionToStatus}", "redact_exceptionToStatus"_attr = redact(exceptionToStatus()));
    }
...
void InitialSyncer::_checkApplierProgressAndScheduleGetNextApplierBatch_inlock(
    ...
    if (_initialSyncState->beginApplyingTimestamp > _initialSyncState->stopTimestamp) {
        std::string msg = str::stream()
            ...
            << _initialSyncState->beginApplyingTimestamp.toBSON();
        LOGV2_ERROR(20975, "{msg}", "msg"_attr = msg);
        onCompletionGuard->setResultAndCancelRemainingWork_inlock(
    ...
    if (_lastApplied.opTime.isNull()) {
        ...
        LOGV2(20969, "Writing to the oplog and applying operations until {initialSyncState_stopTimestamp_toBSON} before initial sync can complete. (started fetching at {initialSyncState_beginFetchingTimestamp_toBSON} and applying at {initialSyncState_beginApplyingTimestamp_toBSON})", "initialSyncState_stopTimestamp_toBSON"_attr = _initialSyncState->stopTimestamp.toBSON(), "initialSyncState_beginFetchingTimestamp_toBSON"_attr = _initialSyncState->beginFetchingTimestamp.toBSON(), "initialSyncState_beginApplyingTimestamp_toBSON"_attr = _initialSyncState->beginApplyingTimestamp.toBSON());
