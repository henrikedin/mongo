...
void InitialSyncer::_chooseSyncSourceCallback(
    ...
    if (!syncSource.isOK()) {
        ...
        LOGV2_DEBUG(1, "Error getting sync source: '{}', trying again in {} at {}. Attempt {} of {}", "syncSource_getStatus"_attr = syncSource.getStatus(), "opts_syncSourceRetryWait"_attr = _opts.syncSourceRetryWait, "when_toString"_attr = when.toString(), "chooseSyncSourceAttempt_1"_attr = (chooseSyncSourceAttempt + 1), "numInitialSyncConnectAttempts_load"_attr = numInitialSyncConnectAttempts.load());
...
Status InitialSyncer::_truncateOplogAndDropReplicatedDatabases() {
    ...
    LOGV2_DEBUG(1, "About to truncate the oplog, if it exists, ns:{}, and drop all user databases (so that we can clone them).", "opts_localOplogNS"_attr = _opts.localOplogNS);
    ...
    LOGV2_DEBUG(2, "Truncating the existing oplog: {}", "opts_localOplogNS"_attr = _opts.localOplogNS);
    Timer timer;
    ...
    if (!status.isOK()) {
        // 1a.) Create the oplog.
        LOGV2_DEBUG(2, "Creating the oplog: {}", "opts_localOplogNS"_attr = _opts.localOplogNS);
...
void InitialSyncer::_fcvFetcherCallback(const StatusWith<Fetcher::QueryResponse>& result,
    ...
    LOGV2_DEBUG(2, "Starting OplogFetcher: {}", "oplogFetcher_toString"_attr = _oplogFetcher->toString());
    ...
    LOGV2_DEBUG(2, "Starting AllDatabaseCloner: {}", "initialSyncState_allDatabaseCloner_toString"_attr = _initialSyncState->allDatabaseCloner->toString());
...
void InitialSyncer::_oplogFetcherCallback(const Status& oplogFetcherFinishStatus,
                                          std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    stdx::lock_guard<Latch> lock(_mutex);
    LOGV2("Finished fetching oplog during initial sync: {}. Last fetched optime: {}", "redact_oplogFetcherFinishStatus"_attr = redact(oplogFetcherFinishStatus), "lastFetched_toString"_attr = _lastFetched.toString());
    ...
    if (status.isOK()) {
        LOGV2("Finished fetching oplog fetching early. Last fetched optime: {}", "lastFetched_toString"_attr = _lastFetched.toString());
        return;
    }
...
void InitialSyncer::_lastOplogEntryFetcherCallbackForStopTimestamp(
    ...
    LOGV2("No need to apply operations. (currently at {})", "initialSyncState_stopTimestamp_toBSON"_attr = _initialSyncState->stopTimestamp.toBSON());
...
void InitialSyncer::_getNextApplierBatchCallback(
    ...
    if (!batchResult.isOK()) {
        LOGV2_WARNING("Failure creating next apply batch: {}", "redact_batchResult_getStatus"_attr = redact(batchResult.getStatus()));
...
void InitialSyncer::_multiApplierCallback(const Status& multiApplierStatus,
    ...
    if (!status.isOK()) {
        LOGV2_ERROR("Failed to apply batch due to '{}'", "redact_status"_attr = redact(status));
        onCompletionGuard->setResultAndCancelRemainingWork_inlock(lock, status);
        return;
    }
...
void InitialSyncer::_finishInitialSyncAttempt(const StatusWith<OpTimeAndWallTime>& lastApplied) {
    ...
    auto finishCallbackGuard = makeGuard([this, &result] {
        ...
        if (!scheduleResult.isOK()) {
            LOGV2_WARNING("Unable to schedule initial syncer completion task due to {}. Running callback on current thread.", "redact_scheduleResult_getStatus"_attr = redact(scheduleResult.getStatus()));
    ...
    stdx::lock_guard<Latch> lock(_mutex);
    LOGV2("Initial Sync Attempt Statistics: {}", "redact_getInitialSyncProgress_inlock"_attr = redact(_getInitialSyncProgress_inlock()));
    ...
    LOGV2_ERROR("Initial sync attempt failed -- attempts left: {} cause: {}", "stats_maxFailedInitialSyncAttempts_stats_failedInitialSyncAttempts"_attr = (_stats.maxFailedInitialSyncAttempts - _stats.failedInitialSyncAttempts), "redact_result_getStatus"_attr = redact(result.getStatus()));
...
void InitialSyncer::_finishCallback(StatusWith<OpTimeAndWallTime> lastApplied) {
    ...
    try {
        ...
        LOGV2_WARNING("initial syncer finish callback threw exception: {}", "redact_exceptionToStatus"_attr = redact(exceptionToStatus()));
    }
