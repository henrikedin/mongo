...
bool DataReplicatorExternalStateImpl::shouldStopFetching(
    ...
    if (_replicationCoordinator->shouldChangeSyncSource(source, replMetadata, oqMetadata)) {
        ...
        // change sync sources.
        if (oqMetadata) {
            LOGV2(20926, "Canceling oplog query due to OplogQueryMetadata. We have to choose a new "
                     "sync source. Current source: {source}, OpTime {oqMetadata_getLastOpApplied}, its sync source index:{oqMetadata_getSyncSourceIndex}", "source"_attr = source, "oqMetadata_getLastOpApplied"_attr = oqMetadata->getLastOpApplied(), "oqMetadata_getSyncSourceIndex"_attr = oqMetadata->getSyncSourceIndex());

        } else {
            LOGV2(20927, "Canceling oplog query due to ReplSetMetadata. We have to choose a new sync "
                     "source. Current source: {source}, OpTime {replMetadata_getLastOpVisible}, its sync source index:{replMetadata_getSyncSourceIndex}", "source"_attr = source, "replMetadata_getLastOpVisible"_attr = replMetadata.getLastOpVisible(), "replMetadata_getSyncSourceIndex"_attr = replMetadata.getSyncSourceIndex());
        }
        return true;
    }
    return false;
