...
void SyncSourceFeedback::forwardSlaveProgress() {
    {
        ...
        if (_reporter) {
            ...
            if (!triggerStatus.isOK()) {
                LOGV2_WARNING("unable to forward slave progress to {}: {}", "_reporter_getTarget"_attr = _reporter->getTarget(), "triggerStatus"_attr = triggerStatus);
            }
        }
    }
}
...
Status SyncSourceFeedback::_updateUpstream(Reporter* reporter) {
    ...
    if (!triggerStatus.isOK()) {
        LOGV2_WARNING("unable to schedule reporter to update replication progress on {}: {}", "syncTarget"_attr = syncTarget, "triggerStatus"_attr = triggerStatus);
