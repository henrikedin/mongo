...
bool MemberData::setUpValues(Date_t now, ReplSetHeartbeatResponse&& hbResponse) {
    ...
    if (_lastResponse.getState() != hbResponse.getState()) {
        LOGV2_OPTIONS(20989, {logv2::LogTag::kRS}, "Member {hostAndPort_toString} is now in state {hbResponse_getState_toString}", "hostAndPort_toString"_attr = _hostAndPort.toString(), "hbResponse_getState_toString"_attr = hbResponse.getState().toString());
    }
...
void MemberData::setDownValues(Date_t now, const std::string& heartbeatMessage) {
    ...
    if (_lastResponse.getState() != MemberState::RS_DOWN) {
        LOGV2_OPTIONS(20990, {logv2::LogTag::kRS}, "Member {hostAndPort_toString} is now in state RS_DOWN - {redact_heartbeatMessage}", "hostAndPort_toString"_attr = _hostAndPort.toString(), "redact_heartbeatMessage"_attr = redact(heartbeatMessage));
    }
...
void MemberData::setAuthIssue(Date_t now) {
    ...
    if (_lastResponse.getState() != MemberState::RS_UNKNOWN) {
        LOGV2_OPTIONS(20991, {logv2::LogTag::kRS}, "Member {hostAndPort_toString} is now in state RS_UNKNOWN due to authentication issue.", "hostAndPort_toString"_attr = _hostAndPort.toString());
    }
...
void MemberData::setLastDurableOpTimeAndWallTime(OpTimeAndWallTime opTime, Date_t now) {
    ...
    if (_lastAppliedOpTime < opTime.opTime) {
        ...
        LOGV2(20992, "Durable progress ({opTime_opTime}) is ahead of the applied progress ({lastAppliedOpTime}. This is likely due to a "
                 "rollback. memberid: {memberId}{hostAndPort_toString} previous durable progress: {lastDurableOpTime}", "opTime_opTime"_attr = opTime.opTime, "lastAppliedOpTime"_attr = _lastAppliedOpTime, "memberId"_attr = _memberId, "hostAndPort_toString"_attr = _hostAndPort.toString(), "lastDurableOpTime"_attr = _lastDurableOpTime);
    } else {
