...
public:
     ...
     */
    // Status setLastAppliedOptime_forTest(long long cfgVer, long long memberId, const OpTime&
    // opTime);
    // Status setLastDurableOptime_forTest(long long cfgVer, long long memberId, const OpTime&
    // opTime);
...
private:
    ...
    //    // It is invalid to call notify_inlock() unless holding
    ...
    ////   1) My last applied optime reaches the target optime, if we've received a heartbeat from
    /// all
    ////      nodes.
    ...
    // *
    // * If the writeConcern is 'majority', also waits for _currentCommittedSnapshot to be newer
    // than
    // * minSnapshot.
    // */
    ...
    // Status _checkIfWriteConcernCanBeSatisfied_inlock(const WriteConcernOptions& writeConcern)
    // const;

    ///**
    // * Wakes up threads in the process of handling a stepdown request based on whether the
    // * TopologyCoordinator now believes enough secondaries are caught up for the stepdown request
    // to
    // * complete.
    // */
    ...
    // */
    // void _scheduleHeartbeatToTarget_inlock(const HostAndPort& target, int targetIndex, Date_t
    // when);
    ...
    // */
    // void _handleHeartbeatResponse(const executor::TaskExecutor::RemoteCommandCallbackArgs&
    // cbData,
    //                              int targetIndex);
    ...
    // * replica set member and so long as the config version in the metadata matches the replica
    // set
    ...
    ///**
    // * Callback which marks downed nodes as down, triggers a stepdown if a majority of nodes are
    // no
    ...
    // * _handleLivenessTimeout callback and calls _scheduleNextLivenessUpdate to schedule a new
    // one.
    ...
    ///**
    // * Waits until the deadline or until the optime of the current node is at least the
    // clusterTime
    ...
    //// The OpTimes and SnapshotNames for all snapshots newer than the current commit point, kept
    /// in
