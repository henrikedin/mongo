...
Status ReplicationProcess::refreshRollbackID(OperationContext* opCtx) {
    ...
    if (kUninitializedRollbackId == _rbid) {
        LOGV2(21260, "Rollback ID is {rbidResult_getValue}", "rbidResult_getValue"_attr = rbidResult.getValue());
    } else {
        LOGV2(21261, "Rollback ID is {rbidResult_getValue} (previously {rbid})", "rbidResult_getValue"_attr = rbidResult.getValue(), "rbid"_attr = _rbid);
    }
...
int ReplicationProcess::getRollbackID() const {
    ...
    if (kUninitializedRollbackId == _rbid) {
        ...
        LOGV2_WARNING(21264, "Rollback ID is not initialized yet.");
    }
    return _rbid;
}
...
Status ReplicationProcess::initializeRollbackID(OperationContext* opCtx) {
    ...
    if (initRbidSW.isOK()) {
        LOGV2(21262, "Initialized the rollback ID to {initRbidSW_getValue}", "initRbidSW_getValue"_attr = initRbidSW.getValue());
        _rbid = initRbidSW.getValue();
        invariant(kUninitializedRollbackId != _rbid);
    } else {
        LOGV2_WARNING(21265, "Failed to initialize the rollback ID: {initRbidSW_getStatus_reason}", "initRbidSW_getStatus_reason"_attr = initRbidSW.getStatus().reason());
    }
...
Status ReplicationProcess::incrementRollbackID(OperationContext* opCtx) {
    ...
    if (status.isOK()) {
        LOGV2(21263, "Incremented the rollback ID to {status_getValue}", "status_getValue"_attr = status.getValue());
        _rbid = status.getValue();
        invariant(kUninitializedRollbackId != _rbid);
    } else {
        LOGV2_WARNING(21266, "Failed to increment the rollback ID: {status_getStatus_reason}", "status_getStatus_reason"_attr = status.getStatus().reason());
    }
