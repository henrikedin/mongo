...
Status ReplicationProcess::refreshRollbackID(OperationContext* opCtx) {
    ...
    if (kUninitializedRollbackId == _rbid) {
        ...
        LOGV2("Rollback ID is {} (previously {})", "rbidResult_getValue"_attr = rbidResult.getValue(), "rbid"_attr = _rbid);
    }
...
int ReplicationProcess::getRollbackID() const {
    ...
    if (kUninitializedRollbackId == _rbid) {
        ...
        LOGV2_WARNING("Rollback ID is not initialized yet.");
    }
    return _rbid;
}
...
Status ReplicationProcess::initializeRollbackID(OperationContext* opCtx) {
    ...
    if (initRbidSW.isOK()) {
        ...
        LOGV2_WARNING("Failed to initialize the rollback ID: {}", "initRbidSW_getStatus_reason"_attr = initRbidSW.getStatus().reason());
    }
...
Status ReplicationProcess::incrementRollbackID(OperationContext* opCtx) {
    ...
    if (status.isOK()) {
        ...
        LOGV2_WARNING("Failed to increment the rollback ID: {}", "status_getStatus_reason"_attr = status.getStatus().reason());
    }
