...
public:
    ...
    virtual ~LoseElectionGuardV1() {
        ...
        LOGV2("Lost {}election due to internal error", "isDryRun_dry_run"_attr = (_isDryRun ? "dry run " : ""));
...
void ReplicationCoordinatorImpl::_startElectSelfV1_inlock(StartElectionReasonEnum reason) {
    ...
    switch (_rsConfigState) {
        ...
        case kConfigHBReconfiguring:
            ...
            return;
        default:
            LOGV2_FATAL(28641, "Entered replica set election code while in illegal config state {}", "int_rsConfigState"_attr = int(_rsConfigState));
            fassertFailed(28641);
    }
...
void ReplicationCoordinatorImpl::_processDryRunResult(long long originalTerm,
    ...
    if (_topCoord->getTerm() != originalTerm) {
        LOGV2("not running for primary, we have been superseded already during dry run. original term: {}, current term: {}", "originalTerm"_attr = originalTerm, "topCoord_getTerm"_attr = _topCoord->getTerm());
        return;
    }
...
void ReplicationCoordinatorImpl::_writeLastVoteForMyElection(
    ...
    if (_topCoord->getTerm() != lastVote.getTerm()) {
        LOGV2("not running for primary, we have been superseded already while writing our last "
                 "vote. election term: {}, current term: {}", "lastVote_getTerm"_attr = lastVote.getTerm(), "topCoord_getTerm"_attr = _topCoord->getTerm());
        return;
    }
...
void ReplicationCoordinatorImpl::_onVoteRequestComplete(long long newTerm,
    ...
    if (_topCoord->getTerm() != newTerm) {
        LOGV2("not becoming primary, we have been superseded already during election. election term: {}, current term: {}", "newTerm"_attr = newTerm, "topCoord_getTerm"_attr = _topCoord->getTerm());
        return;
    }
    ...
    switch (endResult) {
        ...
        case VoteRequester::Result::kSuccessfullyElected:
            LOGV2("election succeeded, assuming primary role in term {}", "topCoord_getTerm"_attr = _topCoord->getTerm());
            ReplicationMetrics::get(getServiceContext())
