...
void rollback_internal::syncFixUp(OperationContext* opCtx,
    ...
    for (auto&& doc : fixUpInfo.docsToRefetch) {
        ...
        try {
            if (nss) {
                LOGV2_DEBUG(2, "Refetching document, collection: {}, UUID: {}, {}", "nss"_attr = *nss, "uuid"_attr = uuid, "redact_doc_id"_attr = redact(doc._id));
            } else {
                LOGV2_DEBUG(2, "Refetching document, UUID: {}, {}", "uuid"_attr = uuid, "redact_doc_id"_attr = redact(doc._id));
            }
            ...
            LOGV2("Rollback couldn't re-fetch from uuid: {} _id: {} {}/{}: {}", "uuid"_attr = uuid, "redact_doc_id"_attr = redact(doc._id), "numFetched"_attr = numFetched, "fixUpInfo_docsToRefetch_size"_attr = fixUpInfo.docsToRefetch.size(), "redact_ex"_attr = redact(ex));
            throw;
        }
    }
        ...
        for (const auto& idAndDoc : goodVersionsByDocID) {
            ...
            try {
                ...
                if (idAndDoc.second.isEmpty()) {
                    LOGV2_DEBUG(2, "Deleting document with: {}, from collection: {}, with UUID: {}", "redact_doc_id"_attr = redact(doc._id), "doc_ns"_attr = doc.ns, "uuid"_attr = uuid);
                    ...
                    if (collection) {
                        if (collection->isCapped()) {
                            ...
                            try {
                                ...
                                LOGV2_WARNING("Ignoring failure to roll back change to capped collection {} with _id {}: {}", "nss_ns"_attr = nss->ns(), "redact_idAndDoc_first_id_toString_includeFieldName_false"_attr = redact(idAndDoc.first._id.toString(
                    ...
                    LOGV2_DEBUG(2, "Updating document with: {}, from collection: {}, UUID: {}, to: {}", "redact_doc_id"_attr = redact(doc._id), "doc_ns"_attr = doc.ns, "uuid"_attr = uuid, "redact_idAndDoc_second"_attr = redact(idAndDoc.second));
                    // TODO faster...
                    updates++;
