...
bool FixUpInfo::removeRedundantIndexCommands(UUID uuid, std::string indexName) {
    LOGV2_DEBUG(21376, 2, "Attempting to remove redundant index operations from the set of indexes to create "
              "for collection {uuid}, for index '{indexName}'", "uuid"_attr = uuid, "indexName"_attr = indexName);
    ...
    if (indexes == indexesToCreate.end()) {
        LOGV2_DEBUG(21377, 2, "Collection {uuid} has no indexes to create. Not removing any index creation operations for index '{indexName}'.", "uuid"_attr = uuid, "indexName"_attr = indexName);
        return false;
    }
    ...
    if (indexesToCreateForColl->find(indexName) == indexesToCreateForColl->end()) {
        LOGV2_DEBUG(21378, 2, "Index '{indexName}' was not previously set to be created for collection {uuid}. Not removing any index creation operations.", "indexName"_attr = indexName, "uuid"_attr = uuid);
        return false;
    }

    // This index was previously added to the set of indexes to create for this collection, so we
    // remove it from that set.
    LOGV2_DEBUG(21379, 2, "Index '{indexName}' was previously set to be created for collection {uuid}. Removing this redundant index creation operation.", "indexName"_attr = indexName, "uuid"_attr = uuid);
...
Status FixUpInfo::recordDropTargetInfo(const BSONElement& dropTarget,
    ...
    if (!dropTargetUUIDStatus.isOK()) {
        ...
        LOGV2_ERROR(21446, "{message}", "message"_attr = message);
...
Status rollback_internal::updateFixUpInfoFromLocalOplogEntry(OperationContext* opCtx,
    ...
    if (isNestedApplyOpsCommand) {
        LOGV2_DEBUG(21380, 2, "Updating rollback FixUpInfo for nested applyOps oplog entry: {redact_oplogEntry_toBSON}", "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
    }
    ...
    if (oplogEntry.getOpType() == OpTypeEnum::kCommand) {
        ...
        switch (oplogEntry.getCommandType()) {
            ...
            case OplogEntry::CommandType::kDropIndexes: {
                ...
                if (!status.isOK()) {
                    LOGV2_FATAL(21448, "Missing index name in dropIndexes operation on rollback, document: {redact_oplogEntry_toBSON}", "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
                    throw RSFatalException(
            ...
            case OplogEntry::CommandType::kCreateIndexes: {
                ...
                if (!status.isOK()) {
                    LOGV2_FATAL(21449, "Missing index name in createIndexes operation on rollback, document: {redact_oplogEntry_toBSON}", "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
                    throw RSFatalException(
            ...
            case OplogEntry::CommandType::kStartIndexBuild: {
                ...
                if (existingIt != buildsToRestart.end()) {
                    LOGV2_DEBUG(21381, 2, "Index build that was previously marked to be restarted will now be "
                              "dropped due to a rolled-back 'startIndexBuild' oplog entry: {buildUUID}", "buildUUID"_attr = buildUUID);
            ...
            case OplogEntry::CommandType::kAbortIndexBuild: {
                ...
                LOGV2_DEBUG(21382, 2, "Index build will be restarted after a rolled-back 'abortIndexBuild': {buildUUID}", "buildUUID"_attr = buildUUID);
            ...
            case OplogEntry::CommandType::kCommitIndexBuild: {
                ...
                LOGV2_DEBUG(21383, 2, "Index build will be restarted after a rolled-back 'commitIndexBuild': {buildUUID}", "buildUUID"_attr = buildUUID);
            ...
            case OplogEntry::CommandType::kRenameCollection: {
                ...
                if (ns.empty()) {
                    ...
                    LOGV2(21384, "{message}", "message"_attr = message);
            ...
            case OplogEntry::CommandType::kCollMod: {
                for (auto field : obj) {
                    ...
                    LOGV2_FATAL(21450, "{message}{redact_obj}", "message"_attr = message, "redact_obj"_attr = redact(obj));
                ...
                do {
                    if (operations.type() != Array) {
                        std::string message = str::stream()
                            ...
                            << redact(operations);
                        LOGV2_FATAL(21451, "{message}", "message"_attr = message);
                        return Status(ErrorCodes::UnrecoverableRollbackError, message);
                    }
                    for (const auto& subopElement : operations.Array()) {
                        if (subopElement.type() != Object) {
                            std::string message = str::stream()
                                ...
                                << redact(subopElement);
                            LOGV2_FATAL(21452, "{message}", "message"_attr = message);
                            return Status(ErrorCodes::UnrecoverableRollbackError, message);
                        }
            ...
            default: {
                ...
                LOGV2_FATAL(21453, "{message} document: {redact_obj}", "message"_attr = message, "redact_obj"_attr = redact(obj));
                throw RSFatalException(message);
            }
        }
    }
    ...
    if (doc._id.eoo()) {
        ...
        LOGV2_FATAL(21454, "{message}, document: {redact_oplogEntry_toBSON}", "message"_attr = message, "redact_oplogEntry_toBSON"_attr = redact(oplogEntry.toBSON()));
        throw RSFatalException(message);
    }
...
void checkRbidAndUpdateMinValid(OperationContext* opCtx,
    ...
    LOGV2(21385, "Setting minvalid to {minValid}", "minValid"_attr = minValid);
    ...
    if (MONGO_unlikely(rollbackHangThenFailAfterWritingMinValid.shouldFail())) {
        ...
        LOGV2(21386, "rollback - rollbackHangThenFailAfterWritingMinValid fail point "
...
void dropIndex(OperationContext* opCtx,
    ...
    if (!indexDescriptor) {
        LOGV2_WARNING(21442, "Rollback failed to drop index {indexName} in {nss_toString}: index not found.", "indexName"_attr = indexName, "nss_toString"_attr = nss.toString());
        return;
    }
    ...
    if (entry->isReady(opCtx)) {
        ...
        if (!status.isOK()) {
            LOGV2_FATAL(21455, "Rollback failed to drop index {indexName} in {nss_toString}: {redact_status}", "indexName"_attr = indexName, "nss_toString"_attr = nss.toString(), "redact_status"_attr = redact(status));
        }
    } else {
        auto status = indexCatalog->dropUnfinishedIndex(opCtx, indexDescriptor);
        if (!status.isOK()) {
            LOGV2_FATAL(21456, "Rollback failed to drop unfinished index {indexName} in {nss_toString}: {redact_status}", "indexName"_attr = indexName, "nss_toString"_attr = nss.toString(), "redact_status"_attr = redact(status));
        }
    }
    wunit.commit();
}
...
void rollbackCreateIndexes(OperationContext* opCtx, UUID uuid, std::set<std::string> indexNames) {
    ...
    if (!collection) {
        LOGV2_DEBUG(21387, 2, "Cannot find the collection with uuid: {uuid_toString} in CollectionCatalog during roll back of a createIndexes command.", "uuid_toString"_attr = uuid.toString());
        return;
    }
    ...
    if (!indexCatalog) {
        LOGV2_DEBUG(21388, 2, "Cannot find the index catalog in collection with uuid: {uuid_toString} during roll back of a createIndexes command.", "uuid_toString"_attr = uuid.toString());
        return;
    }
    ...
    for (auto itIndex = indexNames.begin(); itIndex != indexNames.end(); itIndex++) {
        ...
        LOGV2(21389, "Dropping index in rollback for collection: {nss}, UUID: {uuid}, index: {indexName}", "nss"_attr = *nss, "uuid"_attr = uuid, "indexName"_attr = indexName);

        dropIndex(opCtx, indexCatalog, indexName, *nss);

        LOGV2_DEBUG(21390, 1, "Dropped index in rollback for collection: {nss}, UUID: {uuid}, index: {indexName}", "nss"_attr = *nss, "uuid"_attr = uuid, "indexName"_attr = indexName);
    }
}
...
void rollbackDropIndexes(OperationContext* opCtx,
    ...
    // If we cannot find the collection, we skip over dropping the index.
    if (!collection) {
        LOGV2_DEBUG(21391, 2, "Cannot find the collection with uuid: {uuid_toString}in CollectionCatalog during roll back of a dropIndexes command.", "uuid_toString"_attr = uuid.toString());
        return;
    }
    ...
    for (auto itIndex = indexNames.begin(); itIndex != indexNames.end(); itIndex++) {
        ...
        LOGV2(21392, "Creating index in rollback for collection: {nss}, UUID: {uuid}, index: {indexName}", "nss"_attr = *nss, "uuid"_attr = uuid, "indexName"_attr = indexName);

        createIndexForApplyOps(opCtx, indexSpec, *nss, OplogApplication::Mode::kRecovering);

        LOGV2_DEBUG(21393, 1, "Created index in rollback for collection: {nss}, UUID: {uuid}, index: {indexName}", "nss"_attr = *nss, "uuid"_attr = uuid, "indexName"_attr = indexName);
    }
}
...
void dropCollection(OperationContext* opCtx,
    ...
    if (RollbackImpl::shouldCreateDataFiles()) {
        ...
        LOGV2(21394, "Rolling back createCollection on {nss}: Preparing to write documents to a rollback file for a collection {nss} with uuid {collection_uuid} to {removeSaver_file_generic_string}", "nss"_attr = nss, "nss"_attr = nss, "collection_uuid"_attr = collection->uuid(), "removeSaver_file_generic_string"_attr = removeSaver.file().generic_string());
        ...
        while (PlanExecutor::ADVANCED == (execState = exec->getNext(&curObj, nullptr))) {
            ...
            if (!status.isOK()) {
                LOGV2_FATAL(21457, "Rolling back createCollection on {nss} failed to write document to remove saver file: {redact_status}", "nss"_attr = nss, "redact_status"_attr = redact(status));
                throw RSFatalException(
        ...
        if (execState != PlanExecutor::IS_EOF) {
            if (execState == PlanExecutor::FAILURE &&
                ...
                LOGV2_FATAL(21458, "Rolling back createCollection on {nss} failed with {redact_errorStatus}. A full resync is necessary.", "nss"_attr = nss, "redact_errorStatus"_attr = redact(errorStatus));
                throw RSFatalException(
                    "Rolling back createCollection failed. A full resync is necessary.");
            } else {
                LOGV2_FATAL(21459, "Rolling back createCollection on {nss} failed. A full resync is necessary.", "nss"_attr = nss);
                throw RSFatalException(
                    "Rolling back createCollection failed. A full resync is necessary.");
            }
        }
...
void renameOutOfTheWay(OperationContext* opCtx, RenameCollectionInfo info, Database* db) {
    ...
    if (!tmpNameResult.isOK()) {
        LOGV2_FATAL(21460, "Unable to generate temporary namespace to rename collection {info_renameTo} out of the way. {tmpNameResult_getStatus_reason}", "info_renameTo"_attr = info.renameTo, "tmpNameResult_getStatus_reason"_attr = tmpNameResult.getStatus().reason());
        throw RSFatalException(
            "Unable to generate temporary namespace to rename collection out of the way.");
    }
    const auto& tempNss = tmpNameResult.getValue();

    LOGV2_DEBUG(21395, 2, "Attempted to rename collection from {info_renameFrom} to {info_renameTo} but {info_renameTo} exists already. Temporarily renaming collection {info_renameTo} with UUID {collection_uuid} out of the way to {tempNss}", "info_renameFrom"_attr = info.renameFrom, "info_renameTo"_attr = info.renameTo, "info_renameTo"_attr = info.renameTo, "info_renameTo"_attr = info.renameTo, "collection_uuid"_attr = collection->uuid(), "tempNss"_attr = tempNss);
    ...
    if (!renameStatus.isOK()) {
        LOGV2_FATAL(21461, "Unable to rename collection {info_renameTo} out of the way to {tempNss}", "info_renameTo"_attr = info.renameTo, "tempNss"_attr = tempNss);
...
void rollbackRenameCollection(OperationContext* opCtx, UUID uuid, RenameCollectionInfo info) {
    ...
    LOGV2(21396, "Attempting to rename collection with UUID: {uuid}, from: {info_renameFrom}, to: {info_renameTo}", "uuid"_attr = uuid, "info_renameFrom"_attr = info.renameFrom, "info_renameTo"_attr = info.renameTo);
    ...
    if (status == ErrorCodes::NamespaceExists) {
        ...
        if (!status.isOK()) {
            LOGV2_FATAL(21462, "Rename collection failed to roll back twice. We were unable to rename collection {info_renameFrom} to {info_renameTo}. {status_toString}", "info_renameFrom"_attr = info.renameFrom, "info_renameTo"_attr = info.renameTo, "status_toString"_attr = status.toString());
            throw RSFatalException(
                "Rename collection failed to roll back twice. We were unable to rename "
                "the collection.");
        }
    } else if (!status.isOK()) {
        LOGV2_FATAL(21463, "Unable to roll back renameCollection command: {status_toString}", "status_toString"_attr = status.toString());
        throw RSFatalException("Unable to rollback renameCollection command");
    }

    LOGV2_DEBUG(21397, 1, "Renamed collection with UUID: {uuid}, from: {info_renameFrom}, to: {info_renameTo}", "uuid"_attr = uuid, "info_renameFrom"_attr = info.renameFrom, "info_renameTo"_attr = info.renameTo);
}
...
Status _syncRollback(OperationContext* opCtx,
    ...
    LOGV2_OPTIONS(21398, {logv2::LogTag::kRS}, "Starting rollback. Sync source: {rollbackSource_getSource}", "rollbackSource_getSource"_attr = rollbackSource.getSource());
    ...
    LOGV2(21399, "Finding the Common Point");
    try {
    ...
    LOGV2(21400, "Rollback common point is {commonPoint}", "commonPoint"_attr = commonPoint);
    ...
    try {
        ...
        if (MONGO_unlikely(rollbackExitEarlyAfterCollectionDrop.shouldFail())) {
            LOGV2(21401, "rollbackExitEarlyAfterCollectionDrop fail point enabled. Returning early "
    ...
    if (MONGO_unlikely(rollbackHangBeforeFinish.shouldFail())) {
        ...
        LOGV2(21402, "rollback - rollbackHangBeforeFinish fail point "
                 "enabled. Blocking until fail point is disabled.");
...
void rollback_internal::syncFixUp(OperationContext* opCtx,
    ...
    LOGV2(21403, "Starting refetching documents");
    ...
    for (auto&& doc : fixUpInfo.docsToRefetch) {
        ...
        try {
            if (nss) {
                LOGV2_DEBUG(21404, 2, "Refetching document, collection: {nss}, UUID: {uuid}, {redact_doc_id}", "nss"_attr = *nss, "uuid"_attr = uuid, "redact_doc_id"_attr = redact(doc._id));
            } else {
                LOGV2_DEBUG(21405, 2, "Refetching document, UUID: {uuid}, {redact_doc_id}", "uuid"_attr = uuid, "redact_doc_id"_attr = redact(doc._id));
            }
            ...
            LOGV2(21406, "Rollback couldn't re-fetch from uuid: {uuid} _id: {redact_doc_id} {numFetched}/{fixUpInfo_docsToRefetch_size}: {redact_ex}", "uuid"_attr = uuid, "redact_doc_id"_attr = redact(doc._id), "numFetched"_attr = numFetched, "fixUpInfo_docsToRefetch_size"_attr = fixUpInfo.docsToRefetch.size(), "redact_ex"_attr = redact(ex));
            throw;
        }
    }

    LOGV2(21407, "Finished refetching documents. Total size of documents refetched: {goodVersions_size}", "goodVersions_size"_attr = goodVersions.size());
    ...
    if (!serverGlobalParams.enableMajorityReadConcern) {
        LOGV2(21408, "Setting initialDataTimestamp to 0 so that we start taking unstable checkpoints.");
        opCtx->getServiceContext()->getStorageEngine()->setInitialDataTimestamp(
            Timestamp::kAllowUnstableCheckpointsSentinel);
    }

    LOGV2(21409, "Checking the RollbackID and updating the MinValid if necessary");
    ...
    LOGV2(21410, "Rolling back createIndexes and startIndexBuild operations");
    ...
    LOGV2(21411, "Rolling back unfinished startIndexBuild operations");
    ...
    LOGV2(21412, "Dropping collections to roll back create operations");
    ...
    for (auto uuid : fixUpInfo.collectionsToDrop) {
        ...
        if (!nss) {
            LOGV2(21413, "This collection does not exist, UUID: {uuid}", "uuid"_attr = uuid);
        } else {
            LOGV2(21414, "Dropping collection: {nss}, UUID: {uuid}", "nss"_attr = *nss, "uuid"_attr = uuid);
            ...
            if (db) {
                ...
                LOGV2_DEBUG(21415, 1, "Dropped collection: {nss}, UUID: {uuid}", "nss"_attr = *nss, "uuid"_attr = uuid);
            }
        }
    }
    ...
    LOGV2(21416, "Rolling back renameCollection commands and collection drop commands.");
    ...
    LOGV2(21417, "Rolling back collections pending being dropped: Removing them from the list of "
    ...
    for (const auto& collPair : fixUpInfo.collectionsToRemoveFromDropPendingCollections) {
        ...
        LOGV2_DEBUG(21418, 1, "Rolling back collection pending being dropped for OpTime: {optime}, collection: {collectionNamespace}", "optime"_attr = optime, "collectionNamespace"_attr = collectionNamespace);
    ...
    if (!fixUpInfo.collectionsToResyncMetadata.empty()) {
        ...
        for (auto uuid : fixUpInfo.collectionsToResyncMetadata) {
            ...
            LOGV2(21419, "Resyncing collection metadata for collection: {nss}, UUID: {uuid}", "nss"_attr = *nss, "uuid"_attr = uuid);
            ...
            if (!infoResult.isOK()) {
                ...
                LOGV2(21420, "{nss_ns} not found on remote host, so we do not roll back collmod "
            ...
            LOGV2_DEBUG(21421, 1, "Resynced collection metadata for collection: {nss}, UUID: {uuid}, with: {redact_info}, to: {redact_DurableCatalog_get_opCtx_getCollectionOptions_opCtx_collection_getCatalogId_toBSON}", "nss"_attr = *nss, "uuid"_attr = uuid, "redact_info"_attr = redact(info), "redact_DurableCatalog_get_opCtx_getCollectionOptions_opCtx_collection_getCatalogId_toBSON"_attr = redact(DurableCatalog::get(opCtx)
        ...
        LOGV2(21422, "Rechecking the Rollback ID and minValid");
        checkRbidAndUpdateMinValid(opCtx, fixUpInfo.rbid, rollbackSource, replicationProcess);
    }

    // Rolls back dropIndexes commands by re-creating the indexes that were dropped.
    LOGV2(21423, "Rolling back dropIndexes commands.");
    ...
    LOGV2(21424, "Restarting rolled-back committed or aborted index builds.");
    IndexBuildsCoordinator::get(opCtx)->restartIndexBuildsForRecovery(
        opCtx, fixUpInfo.indexBuildsToRestart);

    LOGV2(21425, "Deleting and updating documents to roll back insert, update and remove "
             "operations");
    ...
    for (const auto& nsAndGoodVersionsByDocID : goodVersions) {
        ...
        if (RollbackImpl::shouldCreateDataFiles()) {
            ...
            LOGV2(21426, "Preparing to write deleted documents to a rollback file for collection {nss} with uuid {uuid_toString} to {removeSaver_file_generic_string}", "nss"_attr = *nss, "uuid_toString"_attr = uuid.toString(), "removeSaver_file_generic_string"_attr = removeSaver->file().generic_string());
        }
        ...
        for (const auto& idAndDoc : goodVersionsByDocID) {
            ...
            if (now - lastProgressUpdate > progressUpdateGap) {
                LOGV2(21427, "{deletes} delete and {updates} update operations processed out of {goodVersions_size} total operations.", "deletes"_attr = deletes, "updates"_attr = updates, "goodVersions_size"_attr = goodVersions.size());
            ...
            try {
                ...
                if (collection && removeSaver) {
                    ...
                    if (found) {
                        ...
                        if (!status.isOK()) {
                            LOGV2_FATAL(21464, "Rollback cannot write document in namespace {nss_ns} to archive file: {redact_status}", "nss_ns"_attr = nss->ns(), "redact_status"_attr = redact(status));
                            throw RSFatalException(str::stream()
                                                   << "Rollback cannot write document in namespace "
                                                   << nss->ns() << " to archive file.");
                        }
                    } else {
                        LOGV2_ERROR(21447, "Rollback cannot find object: {pattern} in namespace {nss_ns}", "pattern"_attr = pattern, "nss_ns"_attr = nss->ns());
                    }
                }

                if (idAndDoc.second.isEmpty()) {
                    LOGV2_DEBUG(21428, 2, "Deleting document with: {redact_doc_id}, from collection: {doc_ns}, with UUID: {uuid}", "redact_doc_id"_attr = redact(doc._id), "doc_ns"_attr = doc.ns, "uuid"_attr = uuid);
                        ...
                        if (collection->isCapped()) {
                            ...
                            try {
                                ...
                                if (clock->now() - findOneStart > Milliseconds(200))
                                    LOGV2_WARNING(21443, "Roll back slow no _id index for {nss_ns} perhaps?", "nss_ns"_attr = nss->ns());
                                ...
                                LOGV2_WARNING(21444, "Ignoring failure to roll back change to capped collection {nss_ns} with _id {redact_idAndDoc_first_id_toString_includeFieldName_false}: {redact_e}", "nss_ns"_attr = nss->ns(), "redact_idAndDoc_first_id_toString_includeFieldName_false"_attr = redact(idAndDoc.first._id.toString(
                    ...
                    LOGV2_DEBUG(21429, 2, "Updating document with: {redact_doc_id}, from collection: {doc_ns}, UUID: {uuid}, to: {redact_idAndDoc_second}", "redact_doc_id"_attr = redact(doc._id), "doc_ns"_attr = doc.ns, "uuid"_attr = uuid, "redact_idAndDoc_second"_attr = redact(idAndDoc.second));
                    // TODO faster...
                    updates++;
                ...
                LOGV2(21430, "Exception in rollback ns:{nss_ns} {pattern_toString} {redact_e} ndeletes:{deletes}", "nss_ns"_attr = nss->ns(), "pattern_toString"_attr = pattern.toString(), "redact_e"_attr = redact(e), "deletes"_attr = deletes);
                throw;
            }
        }
    }

    LOGV2(21431, "Rollback deleted {deletes} documents and updated {updates} documents.", "deletes"_attr = deletes, "updates"_attr = updates);
    ...
    if (!serverGlobalParams.enableMajorityReadConcern) {
        ...
        LOGV2(21432, "Forcing the stable timestamp to the common point: {fixUpInfo_commonPoint_getTimestamp}", "fixUpInfo_commonPoint_getTimestamp"_attr = fixUpInfo.commonPoint.getTimestamp());
        ...
        auto syncSourceTopOfOplog = OpTime::parseFromOplogEntry(rollbackSource.getLastOperation())
                                        .getValue()
                                        .getTimestamp();
        LOGV2(21433, "Setting initialDataTimestamp to the max of local top of oplog and sync source "
                 "top of oplog. Local top of oplog: {fixUpInfo_localTopOfOplog_getTimestamp}, sync source top of oplog: {syncSourceTopOfOplog}", "fixUpInfo_localTopOfOplog_getTimestamp"_attr = fixUpInfo.localTopOfOplog.getTimestamp(), "syncSourceTopOfOplog"_attr = syncSourceTopOfOplog);
        opCtx->getServiceContext()->getStorageEngine()->setInitialDataTimestamp(
        ...
        LOGV2(21434, "Waiting for an unstable checkpoint");
        const bool stableCheckpoint = false;
        opCtx->recoveryUnit()->waitUntilUnjournaledWritesDurable(opCtx, stableCheckpoint);
    }

    LOGV2(21435, "Truncating the oplog at {fixUpInfo_commonPoint_toString} ({fixUpInfo_commonPointOurDiskloc}), non-inclusive", "fixUpInfo_commonPoint_toString"_attr = fixUpInfo.commonPoint.toString(), "fixUpInfo_commonPointOurDiskloc"_attr = fixUpInfo.commonPointOurDiskloc);
    ...
    if (!serverGlobalParams.enableMajorityReadConcern) {
        ...
        LOGV2(21436, "Setting appliedThrough to the common point: {fixUpInfo_commonPoint}", "fixUpInfo_commonPoint"_attr = fixUpInfo.commonPoint);
        ...
        LOGV2(21437, "Waiting for an unstable checkpoint");
        const bool stableCheckpoint = false;
        opCtx->recoveryUnit()->waitUntilUnjournaledWritesDurable(opCtx, stableCheckpoint);

        // Ensure that appliedThrough is unset in the next stable checkpoint.
        LOGV2(21438, "Clearing appliedThrough");
    ...
    if (!status.isOK()) {
        LOGV2_FATAL(21465, "Failed to reinitialize auth data after rollback: {redact_status}", "redact_status"_attr = redact(status));
...
Status syncRollback(OperationContext* opCtx,
    ...
    LOGV2_OPTIONS(21439, {logv2::LogTag::kRS}, "Rollback finished. The final minValid is: {replicationProcess_getConsistencyMarkers_getMinValid_opCtx}", "replicationProcess_getConsistencyMarkers_getMinValid_opCtx"_attr = replicationProcess->getConsistencyMarkers()->getMinValid(opCtx));
...
void rollback(OperationContext* opCtx,
...

    {
        ...
        if (!status.isOK()) {
            LOGV2(21440, "Cannot transition from {replCoord_getMemberState_toString} to {MemberState_MemberState_RS_ROLLBACK_toString}{causedBy_status}", "replCoord_getMemberState_toString"_attr = replCoord->getMemberState().toString(), "MemberState_MemberState_RS_ROLLBACK_toString"_attr = MemberState(MemberState::RS_ROLLBACK).toString(), "causedBy_status"_attr = causedBy(status));
            return;
        }
    }

    if (MONGO_unlikely(rollbackHangAfterTransitionToRollback.shouldFail())) {
        LOGV2(21441, "rollbackHangAfterTransitionToRollback fail point enabled. Blocking until fail "
    ...
    try {
        ...
        if (ErrorCodes::UnrecoverableRollbackError == status.code()) {
            LOGV2_FATAL(21466, "Unable to complete rollback. A full resync may be needed: {redact_status}", "redact_status"_attr = redact(status));
        ...
        LOGV2_WARNING(21445, "Rollback cannot complete at this time (retrying later): {redact_ex} appliedThrough= {replCoord_getMyLastAppliedOpTime} minvalid= {replicationProcess_getConsistencyMarkers_getMinValid_opCtx}", "redact_ex"_attr = redact(ex), "replCoord_getMyLastAppliedOpTime"_attr = replCoord->getMyLastAppliedOpTime(), "replicationProcess_getConsistencyMarkers_getMinValid_opCtx"_attr = replicationProcess->getConsistencyMarkers()->getMinValid(opCtx));
    ...
    if (ShardIdentityRollbackNotifier::get(opCtx)->didRollbackHappen()) {
        LOGV2_FATAL(21467, "shardIdentity document rollback detected.  Shutting down to clear "
