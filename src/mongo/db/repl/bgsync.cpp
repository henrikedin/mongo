...
void BackgroundSync::_produce() {
    ...
    if (syncSourceResp.syncSourceStatus == ErrorCodes::OplogStartMissing) {
        ...
        if (!status.isOK()) {
            LOGV2_WARNING("Failed to transition into {}. Current state: {}{}", "MemberState_MemberState_RS_RECOVERING"_attr = MemberState(MemberState::RS_RECOVERING), "_replCoord_getMemberState"_attr = _replCoord->getMemberState(), "causedBy_status"_attr = causedBy(status));
            // Do not mark ourselves too stale on errors so we can try again next time.
            return;
        }
