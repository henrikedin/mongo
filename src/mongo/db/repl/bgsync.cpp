...
void BackgroundSync::_produce() {
    ...
    // find a target to sync from the last optime fetched
    {
        ...
        if (!_syncSourceHost.empty()) {
            LOGV2("Clearing sync source {} to choose a new one.", "syncSourceHost"_attr = _syncSourceHost);
        }
    ...
    if (syncSourceResp.syncSourceStatus == ErrorCodes::OplogStartMissing) {
        ...
        if (_replCoord->getMemberState().primary()) {
            LOGV2_WARNING("Too stale to catch up.");
        ...
        LOGV2_ERROR("too stale to catch up -- entering maintenance mode");
        LOGV2("Our newest OpTime : {}", "lastOpTimeFetched"_attr = lastOpTimeFetched);
        ...
        if (!status.isOK()) {
            LOGV2_WARNING("Failed to transition into maintenance mode: {}", "status"_attr = status);
        ...
        if (!status.isOK()) {
            LOGV2_WARNING("Failed to transition into {}. Current state: {}{}", "MemberState_MemberState_RS_RECOVERING"_attr = MemberState(MemberState::RS_RECOVERING), "replCoord_getMemberState"_attr = _replCoord->getMemberState(), "causedBy_status"_attr = causedBy(status));
            // Do not mark ourselves too stale on errors so we can try again next time.
            return;
        }
    ...
    if (_tooStale.swap(false)) {
        ...
        if (!status.isOK()) {
            LOGV2_WARNING("Failed to leave maintenance mode: {}", "status"_attr = status);
        }
    }
    ...
    if (fetcherReturnStatus.code() == ErrorCodes::OplogOutOfOrder) {
        ...
        LOGV2_WARNING("{}", "redact_fetcherReturnStatus"_attr = redact(fetcherReturnStatus));
        ...
        LOGV2_WARNING("Fetcher got invalid BSON while querying oplog. Blacklisting sync source {} for {}.", "source"_attr = source, "blacklistDuration"_attr = blacklistDuration);
        _replCoord->blacklistSyncSource(source, Date_t::now() + blacklistDuration);
    } else if (!fetcherReturnStatus.isOK()) {
        LOGV2_WARNING("Fetcher stopped querying remote oplog with error: {}", "redact_fetcherReturnStatus"_attr = redact(fetcherReturnStatus));
    }
}
...
Status BackgroundSync::_enqueueDocuments(Fetcher::Documents::const_iterator begin,
...

    {
        ...
        LOGV2_DEBUG(3, "batch resetting _lastOpTimeFetched: {}", "lastOpTimeFetched"_attr = _lastOpTimeFetched);
    }
...
void BackgroundSync::_runRollback(OperationContext* opCtx,
    ...
    if (_replCoord->getMemberState().primary()) {
        LOGV2_WARNING("Rollback situation detected in catch-up mode. Aborting catch-up mode.");
        auto status = _replCoord->abortCatchupIfNeeded(
            ReplicationCoordinator::PrimaryCatchUpConclusionReason::kFailedWithError);
        if (!status.isOK()) {
            LOGV2_DEBUG(1, "Aborting catch-up failed with status: {}", "status"_attr = status);
    ...
    LOGV2("Replication commit point: {}", "replCoord_getLastCommittedOpTime"_attr = _replCoord->getLastCommittedOpTime());
...
void BackgroundSync::clearSyncTarget() {
    stdx::unique_lock<Latch> lock(_mutex);
    LOGV2("Resetting sync source to empty, which was {}", "syncSourceHost"_attr = _syncSourceHost);
    _syncSourceHost = HostAndPort();
}
...
void BackgroundSync::start(OperationContext* opCtx) {
    ...
    do {
        ...
        if (_lastOpTimeFetched <= lastAppliedOpTime) {
            LOGV2_DEBUG(1, "Setting bgsync _lastOpTimeFetched={}. Previous _lastOpTimeFetched: {}", "lastAppliedOpTime"_attr = lastAppliedOpTime, "lastOpTimeFetched"_attr = _lastOpTimeFetched);
    ...
    LOGV2_DEBUG(1, "bgsync fetch queue set to: {}", "lastOpTimeFetched"_attr = _lastOpTimeFetched);
}
