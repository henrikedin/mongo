...
void NoopWriter::_writeNoop(OperationContext* opCtx) {
    ...
    if (lastAppliedOpTime != _lastKnownOpTime) {
        LOGV2_DEBUG(1, "Not scheduling a noop write. Last known OpTime: {} != last primary OpTime: {}", "lastKnownOpTime"_attr = _lastKnownOpTime, "lastAppliedOpTime"_attr = lastAppliedOpTime);
    } else {
        if (writePeriodicNoops.load()) {
            const auto logLevel = getTestCommandsEnabled() ? 0 : 1;
            LOGV2_DEBUG(::mongo::logger::LogSeverity(logLevel).toInt(), "Writing noop to oplog as there has been no writes to this replica set in over {}", "writeInterval"_attr = _writeInterval);
            writeConflictRetry(
    ...
    LOGV2_DEBUG(1, "Set last known op time to {}", "lastKnownOpTime"_attr = _lastKnownOpTime);
}
