...
void NoopWriter::_writeNoop(OperationContext* opCtx) {
    ...
    if (!lock.isLocked()) {
        LOGV2_DEBUG(20993, 1, "Global lock is not available skipping noopWrite");
        return;
    }
    ...
    if (!replCoord->canAcceptWritesForDatabase(opCtx, "admin")) {
        LOGV2_DEBUG(20994, 1, "Not a primary, skipping the noop write");
        return;
    }
    ...
    if (lastAppliedOpTime != _lastKnownOpTime) {
        LOGV2_DEBUG(20995, 1, "Not scheduling a noop write. Last known OpTime: {lastKnownOpTime} != last primary OpTime: {lastAppliedOpTime}", "lastKnownOpTime"_attr = _lastKnownOpTime, "lastAppliedOpTime"_attr = lastAppliedOpTime);
    } else {
        if (writePeriodicNoops.load()) {
            const auto logLevel = getTestCommandsEnabled() ? 0 : 1;
            LOGV2_DEBUG(20996, logSeverityV1toV2(logLevel).toInt(), "Writing noop to oplog as there has been no writes to this replica set in over {writeInterval}", "writeInterval"_attr = _writeInterval);
            writeConflictRetry(
    ...
    LOGV2_DEBUG(20997, 1, "Set last known op time to {lastKnownOpTime}", "lastKnownOpTime"_attr = _lastKnownOpTime);
}
