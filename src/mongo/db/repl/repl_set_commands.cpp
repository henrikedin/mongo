...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        LOGV2(21301, "replSetTest command received: {cmdObj_toString}", "cmdObj_toString"_attr = cmdObj.toString());
        ...
        if (cmdObj.hasElement("waitForMemberState")) {
            ...
            LOGV2(21302, "replSetTest: waiting {timeout} for member state to become {expectedState}", "timeout"_attr = timeout, "expectedState"_attr = expectedState);
            ...
            uassertStatusOK(status);
            Milliseconds timeout(timeoutMillis);
            LOGV2(21303, "replSetTest: waiting {timeout} for applier buffer to finish draining", "timeout"_attr = timeout);
...
void parseReplSetSeedList(ReplicationCoordinatorExternalState* externalState,
    ...
    while (1) {
        ...
        if (externalState->isSelf(m, getGlobalServiceContext())) {
            LOGV2_DEBUG(21304, 1, "ignoring seed {m_toString} (=self)", "m_toString"_attr = m.toString());
        } else {
            seeds->push_back(m);
        }
        if (*comma == 0) {
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        if (configObj.isEmpty()) {
            ...
            LOGV2(21305, "initiate : {noConfigMessage}", "noConfigMessage"_attr = noConfigMessage);
            ...
            configObj = b.obj();
            LOGV2(21306, "created this configuration for initiation : {configObj_toString}", "configObj_toString"_attr = configObj.toString());
        }
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        LOGV2(21307, "Attempting to step down in response to replSetStepDown command");

        ReplicationCoordinator::get(opCtx)->stepDown(
            opCtx, force, Seconds(secondaryCatchUpPeriodSecs), Seconds(stepDownForSecs));

        LOGV2(21308, "replSetStepDown command completed");
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        LOGV2(21309, "Received replSetStepUp request");

        const bool skipDryRun = cmdObj["skipDryRun"].trueValue();
        status = ReplicationCoordinator::get(opCtx)->stepUpIfEligible(skipDryRun);

        if (!status.isOK()) {
            LOGV2(21310, "replSetStepUp request failed{causedBy_status}", "causedBy_status"_attr = causedBy(status));
        }
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        Status status = ReplicationCoordinator::get(opCtx)->checkReplEnabledForCommand(&result);
        uassertStatusOK(status);
        LOGV2(21311, "Received replSetAbortPrimaryCatchUp request");
        ...
        if (!status.isOK()) {
            LOGV2(21312, "replSetAbortPrimaryCatchUp request failed{causedBy_status}", "causedBy_status"_attr = causedBy(status));
        }
        uassertStatusOK(status);
        return true;
    }
