...
Future<void> OplogApplier::startup() {
    ...
    auto callback =
        ...
        LOGV2(20998, "Starting oplog application");
        _run(_oplogBuffer);
        LOGV2(20999, "Finished oplog application");
        promise.setWith([] {});
    };
...
void OplogApplier::shutdown() {
    ...
    if (globalFailPointRegistry().find("rsSyncApplyStop")->shouldFail()) {
        LOGV2_FATAL(21001, "Turn off rsSyncApplyStop before attempting clean shutdown");
...
void OplogApplier::enqueue(OperationContext* opCtx,
    ...
    if (sampler.tick()) {
        LOGV2_DEBUG(21000, 2, "oplog buffer has {oplogBuffer_getSize} bytes", "oplogBuffer_getSize"_attr = _oplogBuffer->getSize());
    }
