...
Status SyncSourceResolver::_scheduleFetcher(std::unique_ptr<Fetcher> fetcher) {
    ...
    if (status.isOK()) {
        ...
        LOGV2_ERROR(21492, "Error scheduling fetcher to evaluate host as sync source, host:{fetcher_getSource}, error: {status}", "fetcher_getSource"_attr = fetcher->getSource(), "status"_attr = status);
    }
    return status;
}
...
OpTime SyncSourceResolver::_parseRemoteEarliestOpTime(const HostAndPort& candidate,
    ...
    if (queryResponse.documents.empty()) {
        ...
        LOGV2(21482, "Blacklisting {candidate} due to empty oplog for {kOplogEmptyBlacklistDuration} until: {until}", "candidate"_attr = candidate, "kOplogEmptyBlacklistDuration"_attr = kOplogEmptyBlacklistDuration, "until"_attr = until);
    ...
    if (firstObjFound.isEmpty()) {
        ...
        LOGV2(21483, "Blacklisting {candidate} due to empty first document for {kFirstOplogEntryEmptyBlacklistDuration} until: {until}", "candidate"_attr = candidate, "kFirstOplogEntryEmptyBlacklistDuration"_attr = kFirstOplogEntryEmptyBlacklistDuration, "until"_attr = until);
        _syncSourceSelector->blacklistSyncSource(candidate, until);
        return OpTime();
    }
    ...
    if (!remoteEarliestOpTime.isOK()) {
        ...
        LOGV2(21484, "Blacklisting {candidate} due to error parsing OpTime from the oldest oplog entry for {kFirstOplogEntryNullTimestampBlacklistDuration} until: {until}. Error: {remoteEarliestOpTime_getStatus}, Entry: {redact_firstObjFound}", "candidate"_attr = candidate, "kFirstOplogEntryNullTimestampBlacklistDuration"_attr = kFirstOplogEntryNullTimestampBlacklistDuration, "until"_attr = until, "remoteEarliestOpTime_getStatus"_attr = remoteEarliestOpTime.getStatus(), "redact_firstObjFound"_attr = redact(firstObjFound));
        _syncSourceSelector->blacklistSyncSource(candidate, until);
        return OpTime();
    }
    ...
    if (remoteEarliestOpTime.getValue().isNull()) {
        // First document in remote oplog is empty.
        const auto until = _taskExecutor->now() + kFirstOplogEntryNullTimestampBlacklistDuration;
        LOGV2(21485, "Blacklisting {candidate} due to null timestamp in first document for {kFirstOplogEntryNullTimestampBlacklistDuration} until: {until}", "candidate"_attr = candidate, "kFirstOplogEntryNullTimestampBlacklistDuration"_attr = kFirstOplogEntryNullTimestampBlacklistDuration, "until"_attr = until);
        _syncSourceSelector->blacklistSyncSource(candidate, until);
        return OpTime();
    }
...
void SyncSourceResolver::_firstOplogEntryFetcherCallback(
    ...
    if (!queryResult.isOK()) {
        ...
        LOGV2(21486, "Blacklisting {candidate} due to error: '{queryResult_getStatus}' for {kFetcherErrorBlacklistDuration} until: {until}", "candidate"_attr = candidate, "queryResult_getStatus"_attr = queryResult.getStatus(), "kFetcherErrorBlacklistDuration"_attr = kFetcherErrorBlacklistDuration, "until"_attr = until);
        _syncSourceSelector->blacklistSyncSource(candidate, until);
...
void SyncSourceResolver::_rbidRequestCallback(
    ...
    try {
        ...
        const auto until = _taskExecutor->now() + kFetcherErrorBlacklistDuration;
        LOGV2(21488, "Blacklisting {candidate} due to error: '{ex}' for {kFetcherErrorBlacklistDuration} until: {until}", "candidate"_attr = candidate, "ex"_attr = ex, "kFetcherErrorBlacklistDuration"_attr = kFetcherErrorBlacklistDuration, "until"_attr = until);
        _syncSourceSelector->blacklistSyncSource(candidate, until);
        _chooseAndProbeNextSyncSource(earliestOpTimeSeen).transitional_ignore();
        return;
    }
...
void SyncSourceResolver::_requiredOpTimeFetcherCallback(
    ...
    if (!status.isOK()) {
        ...
        LOGV2_WARNING(21490, "We cannot use {candidate_toString} as a sync source because it does not contain the necessary "
                     "operations for us to reach a consistent state: {status} last fetched optime: {lastOpTimeFetched}. required optime: {requiredOpTime}. Blacklisting this sync source for {kNoRequiredOpTimeBlacklistDuration} until: {until}", "candidate_toString"_attr = candidate.toString(), "status"_attr = status, "lastOpTimeFetched"_attr = _lastOpTimeFetched, "requiredOpTime"_attr = _requiredOpTime, "kNoRequiredOpTimeBlacklistDuration"_attr = kNoRequiredOpTimeBlacklistDuration, "until"_attr = until);
        _syncSourceSelector->blacklistSyncSource(candidate, until);
...
Status SyncSourceResolver::_finishCallback(const SyncSourceResolverResponse& response) {
    try {
        ...
        LOGV2_WARNING(21491, "sync source resolver finish callback threw exception: {exceptionToStatus}", "exceptionToStatus"_attr = exceptionToStatus());
    }
