...
void ReplicationCoordinatorExternalStateImpl::startSteadyStateReplication(
    ...
    LOGV2(21062, "Starting replication fetcher thread");
    _bgSync->startup(opCtx);

    LOGV2(21063, "Starting replication applier thread");
    _oplogApplierShutdownFuture = _oplogApplier->startup();

    LOGV2(21064, "Starting replication reporter thread");
...
void ReplicationCoordinatorExternalStateImpl::_stopDataReplication_inlock(
    ...
    if (oldSSF) {
        LOGV2(21065, "Stopping replication reporter thread");
        _syncSourceFeedback.shutdown();
        oldSSF->join();
    }

    if (oldBgSync) {
        LOGV2(21066, "Stopping replication fetcher thread");
        oldBgSync->shutdown(opCtx);
    }

    if (oldApplier) {
        LOGV2(21067, "Stopping replication applier thread");
        oldApplier->shutdown();
    }
...
void ReplicationCoordinatorExternalStateImpl::startThreads(const ReplSettings& settings) {
    ...
    if (_inShutdown) {
        LOGV2(21068, "Not starting replication storage threads because replication is shutting down.");
        return;
    }

    LOGV2(21069, "Starting replication storage threads");
...
void ReplicationCoordinatorExternalStateImpl::shutdown(OperationContext* opCtx) {
    ...
    LOGV2(21070, "Stopping replication storage threads");
    ...
    LOGV2_DEBUG(21071, 1, "Stopping noop writer");
...
void ReplicationCoordinatorExternalStateImpl::_dropAllTempCollections(OperationContext* opCtx) {
    ...
    for (std::vector<std::string>::iterator it = dbNames.begin(); it != dbNames.end(); ++it) {
        ...
        if (*it == "local")
            continue;
        LOGV2_DEBUG(21072, 2, "Removing temporary collections from {it}", "it"_attr = *it);
            ...
            scheduleWork(
                ...
                [committedOpTime, reaper](const executor::TaskExecutor::CallbackArgs& args) {
                    if (MONGO_unlikely(dropPendingCollectionReaperHang.shouldFail())) {
                        LOGV2(21073, "fail point dropPendingCollectionReaperHang enabled. "
                                 "Blocking until fail point is disabled. "
                                 "committedOpTime: {committedOpTime}", "committedOpTime"_attr = committedOpTime);
