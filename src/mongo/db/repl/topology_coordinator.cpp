...
HostAndPort TopologyCoordinator::chooseNewSyncSource(Date_t now,
    ...
    if (_selfIndex == -1) {
        LOGV2_DEBUG(21494, 1, "Cannot sync from any members because we are not in the replica set config");
        return HostAndPort();
    }
    ...
    if (auto sfp = forceSyncSourceCandidate.scoped(); MONGO_unlikely(sfp.isActive())) {
        ...
        if (!hostAndPortElem) {
            LOGV2_FATAL(21553, "'forceSyncSoureCandidate' parameter set with invalid host and port: {data}", "data"_attr = data);
            fassertFailed(50835);
        }
        ...
        if (syncSourceIndex < 0) {
            LOGV2(21495, "'forceSyncSourceCandidate' failed due to host and port not in "
                     "replica set config: {hostAndPort_toString}", "hostAndPort_toString"_attr = hostAndPort.toString());
            fassertFailed(50836);
        }


        if (_memberIsBlacklisted(_rsConfig.getMemberAt(syncSourceIndex), now)) {
            LOGV2(21496, "Cannot select a sync source because forced candidate is blacklisted: {hostAndPort_toString}", "hostAndPort_toString"_attr = hostAndPort.toString());
            _syncSource = HostAndPort();
            return _syncSource;
        }

        _syncSource = _rsConfig.getMemberAt(syncSourceIndex).getHostAndPort();
        LOGV2(21497, "choosing sync source candidate due to 'forceSyncSourceCandidate' parameter: {syncSource}", "syncSource"_attr = _syncSource);
    ...
    if (_forceSyncSourceIndex != -1) {
        ...
        _forceSyncSourceIndex = -1;
        LOGV2(21498, "choosing sync source candidate by request: {syncSource}", "syncSource"_attr = _syncSource);
    ...
    if (needMorePings > 0) {
        ...
        if (sampler.tick()) {
            LOGV2(21499, "waiting for {needMorePings} pings from other members before syncing", "needMorePings"_attr = needMorePings);
        }
        _syncSource = HostAndPort();
        return _syncSource;
    }
    ...
    if (chainingPreference == ChainingPreference::kUseConfiguration &&
        ...
        if (_currentPrimaryIndex == -1) {
            LOGV2_DEBUG(21500, 1, "Cannot select a sync source because chaining is"
                      " not allowed and primary is unknown/down");
            _syncSource = HostAndPort();
            return _syncSource;
        } else if (_memberIsBlacklisted(*_currentPrimaryMember(), now)) {
            LOGV2_DEBUG(21501, 1, "Cannot select a sync source because chaining is not allowed and primary "
                      "member is blacklisted: {currentPrimaryMember_getHostAndPort}", "currentPrimaryMember_getHostAndPort"_attr = _currentPrimaryMember()->getHostAndPort());
            _syncSource = HostAndPort();
            return _syncSource;
        } else if (_currentPrimaryIndex == _selfIndex) {
            LOGV2_DEBUG(21502, 1, "Cannot select a sync source because chaining is not allowed and we are primary");
            _syncSource = HostAndPort();
            return _syncSource;
        } else if (_memberData.at(_currentPrimaryIndex).getHeartbeatAppliedOpTime() <
                   lastOpTimeFetched) {
            LOGV2_DEBUG(21503, 1, "{Cannot_select_a_sync_source_because_chaining_is_not_allowed_and_the_primary_is_behind_me_Last_oplog_optime_of_primary_my_last_fetched_oplog_optime_format_currentPrimaryMember_getHostAndPort_memberData_at_currentPrimaryIndex_getLastAppliedOpTime_toBSON_lastOpTimeFetched_toBSON}", "Cannot_select_a_sync_source_because_chaining_is_not_allowed_and_the_primary_is_behind_me_Last_oplog_optime_of_primary_my_last_fetched_oplog_optime_format_currentPrimaryMember_getHostAndPort_memberData_at_currentPrimaryIndex_getLastAppliedOpTime_toBSON_lastOpTimeFetched_toBSON"_attr = "Cannot select a sync source because chaining is not allowed and the primary "
            ...
            LOGV2(21504, "chaining not allowed, choosing primary as sync source candidate: {syncSource}", "syncSource"_attr = _syncSource);
    ...
    for (int attempts = 0; attempts < 2; ++attempts) {
        for (std::vector<MemberData>::const_iterator it = _memberData.begin();
            ...
            if (!it->up()) {
                LOGV2_DEBUG(21505, 2, "Cannot select sync source because it is not up: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());
                continue;
            }
            // Candidate must be PRIMARY or SECONDARY state to be considered.
            if (!it->getState().readable()) {
                LOGV2_DEBUG(21506, 2, "Cannot select sync source because it is not readable: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());
                continue;
            }
            ...
            if (attempts == 0) {
                ...
                if (_selfConfig().isVoter() && !itMemberConfig.isVoter()) {
                    LOGV2_DEBUG(21507, 2, "Cannot select sync source because we are a voter and it is not: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());
                    continue;
                }
                // Candidates must not be hidden.
                if (itMemberConfig.isHidden()) {
                    LOGV2_DEBUG(21508, 2, "Cannot select sync source because it is hidden: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());
                    continue;
                }
                // Candidates cannot be excessively behind.
                if (it->getHeartbeatAppliedOpTime() < oldestSyncOpTime) {
                    LOGV2_DEBUG(21509, 2, "Cannot select sync source because it is too far behind.Latest optime of sync candidate {itMemberConfig_getHostAndPort}: {it_getHeartbeatAppliedOpTime}, oldest acceptable optime: {oldestSyncOpTime}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort(), "it_getHeartbeatAppliedOpTime"_attr = it->getHeartbeatAppliedOpTime(), "oldestSyncOpTime"_attr = oldestSyncOpTime);
                    continue;
                }
                // Candidate must not have a configured delay larger than ours.
                if (_selfConfig().getSlaveDelay() < itMemberConfig.getSlaveDelay()) {
                    LOGV2_DEBUG(21510, 2, "Cannot select sync source with larger slaveDelay than ours: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());
                    continue;
                }
            }
            ...
            if (_selfConfig().shouldBuildIndexes()) {
                if (!itMemberConfig.shouldBuildIndexes()) {
                    LOGV2_DEBUG(21511, 2, "Cannot select sync source with shouldBuildIndex differences: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());
                    continue;
                }
            }
            // Only select a candidate that is ahead of me.
            if (it->getHeartbeatAppliedOpTime() <= lastOpTimeFetched) {
                LOGV2_DEBUG(21512, 1, "Cannot select this sync source. Sync source must be ahead of me. Sync candidate: {itMemberConfig_getHostAndPort}, my last fetched oplog optime: {lastOpTimeFetched_toBSON}, latest oplog optime of sync candidate: {it_getHeartbeatAppliedOpTime_toBSON}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort(), "lastOpTimeFetched_toBSON"_attr = lastOpTimeFetched.toBSON(), "it_getHeartbeatAppliedOpTime_toBSON"_attr = it->getHeartbeatAppliedOpTime().toBSON());
                continue;
            }
            ...
            if ((closestIndex != -1) &&
                ...
                LOGV2_DEBUG(21513, 2, "Cannot select sync source with higher latency than the best candidate: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());

                continue;
            }
            // Candidate cannot be blacklisted.
            if (_memberIsBlacklisted(itMemberConfig, now)) {
                LOGV2_DEBUG(21514, 1, "Cannot select sync source which is blacklisted: {itMemberConfig_getHostAndPort}", "itMemberConfig_getHostAndPort"_attr = itMemberConfig.getHostAndPort());
    ...
    if (closestIndex == -1) {
        ...
        if (!_syncSource.empty()) {
            LOGV2_OPTIONS(21515, {logv2::LogTag::kRS}, "{msg}", "msg"_attr = msg);
        }
        setMyHeartbeatMessage(now, msg);
    ...
    LOGV2(21516, "sync source candidate: {syncSource}", "syncSource"_attr = _syncSource);
...
void TopologyCoordinator::blacklistSyncSource(const HostAndPort& host, Date_t until) {
    LOGV2_DEBUG(21517, 2, "blacklisting {host} until {until_toString}", "host"_attr = host, "until_toString"_attr = until.toString());
...
void TopologyCoordinator::unblacklistSyncSource(const HostAndPort& host, Date_t now) {
    ...
    if (hostItr != _syncSourceBlacklist.end() && now >= hostItr->second) {
        LOGV2_DEBUG(21518, 2, "unblacklisting {host}", "host"_attr = host);
...
HeartbeatResponseAction TopologyCoordinator::processHeartbeatResponse(
    ...
    if (hbResponse.isOK() && hbResponse.getValue().hasConfig()) {
        ...
        if (newConfig.getConfigVersion() > currentConfigVersion) {
            ...
            if (newConfig.getConfigVersion() < currentConfigVersion) {
                LOGV2_DEBUG(21519, 1, "Config version from heartbeat was older than ours.");
            } else {
                LOGV2_DEBUG(21520, 2, "Config from heartbeat response was same as ours.");
            }
    ...
    if (_selfIndex == -1) {
        LOGV2_DEBUG(21521, 1, "Could not find ourself in current config so ignoring heartbeat from {target} -- current config: {rsConfig_toBSON}", "target"_attr = target, "rsConfig_toBSON"_attr = _rsConfig.toBSON());
        HeartbeatResponseAction nextAction = HeartbeatResponseAction::makeNoAction();
        nextAction.setNextHeartbeatStartDate(nextHeartbeatStartDate);
        return nextAction;
    }
    ...
    if (memberIndex == -1) {
        LOGV2_DEBUG(21522, 1, "Could not find {target} in current config so ignoring --"
                  " current config: {rsConfig_toBSON}", "target"_attr = target, "rsConfig_toBSON"_attr = _rsConfig.toBSON());
        HeartbeatResponseAction nextAction = HeartbeatResponseAction::makeNoAction();
        nextAction.setNextHeartbeatStartDate(nextHeartbeatStartDate);
        return nextAction;
    }
    ...
    if (!hbResponse.isOK()) {
        ...
        LOGV2_DEBUG(21523, 3, "setUpValues: heartbeat response good for member _id:{member_getId}", "member_getId"_attr = member.getId());
...
HeartbeatResponseAction TopologyCoordinator::checkMemberTimeouts(Date_t now) {
    ...
    if (stepdown) {
        LOGV2(21524, "can't see a majority of the set, relinquishing primary");
...
std::pair<MemberId, Date_t> TopologyCoordinator::getStalestLiveMember() const {
    ...
    for (const auto& memberData : _memberData) {
        ...
        LOGV2_DEBUG(21525, 3, "memberData lastupdate is: {memberData_getLastUpdate}", "memberData_getLastUpdate"_attr = memberData.getLastUpdate());
        if (earliestDate > memberData.getLastUpdate()) {
            earliestDate = memberData.getLastUpdate();
            earliestMemberId = memberData.getMemberId();
        }
    }
    LOGV2_DEBUG(21526, 3, "stalest member {earliestMemberId} date: {earliestDate}", "earliestMemberId"_attr = earliestMemberId, "earliestDate"_attr = earliestDate);
...
StatusWith<bool> TopologyCoordinator::setLastOptime(const UpdatePositionArgs::UpdateInfo& args,
    ...
    if (args.cfgver != _rsConfig.getConfigVersion()) {
        ...
        LOGV2_DEBUG(21528, 1, "{errmsg}", "errmsg"_attr = errmsg);
    ...
    if (!memberData) {
        ...
        LOGV2_DEBUG(21529, 1, "{errmsg}", "errmsg"_attr = errmsg);
...
void TopologyCoordinator::changeMemberState_forTest(const MemberState& newMemberState,
    ...
    switch (newMemberState.s) {
        ...
        case MemberState::RS_STARTUP:
            ...
            break;
        default:
            LOGV2_FATAL(21554, "Cannot switch to state {newMemberState}", "newMemberState"_attr = newMemberState);
            MONGO_UNREACHABLE;
    }
    if (getMemberState() != newMemberState.s) {
        LOGV2_FATAL(21555, "Expected to enter state {newMemberState} but am now in {getMemberState}", "newMemberState"_attr = newMemberState, "getMemberState"_attr = getMemberState());
        MONGO_UNREACHABLE;
    }
    LOGV2(21530, "{newMemberState}", "newMemberState"_attr = newMemberState);
}
...
TopologyCoordinator::prepareFreezeResponse(Date_t now, int secs, BSONObjBuilder* response) {
    if (_role != TopologyCoordinator::Role::kFollower) {
        ...
        LOGV2(21531, "{msg}", "msg"_attr = msg);
        return Status(ErrorCodes::NotSecondary, msg);
    }

    if (secs == 0) {
        _stepDownUntil = now;
        LOGV2(21532, "'unfreezing'");
        ...
        LOGV2(21533, "'freezing' for {secs} seconds", "secs"_attr = secs);
    }
...
void TopologyCoordinator::updateConfig(const ReplSetConfig& newConfig, int selfIndex, Date_t now) {
    ...
    if (!_rsConfig.isInitialized()) {
        ...
        LOGV2_DEBUG(21534, 1, "Updated term in topology coordinator to {term} due to new config", "term"_attr = _term);
    }
    ...
    if (_role == Role::kLeader) {
        if (_selfIndex == -1) {
            LOGV2(21535, "Could not remain primary because no longer a member of the replica set");
        } else if (!_selfConfig().isElectable()) {
            LOGV2(21536, " Could not remain primary because no longer electable");
        } else {
...
std::string TopologyCoordinator::_getUnelectableReasonString(const UnelectableReasonMask ur) const {
    ...
    if (!hasWrittenToStream) {
        LOGV2_FATAL(21556, "Invalid UnelectableReasonMask value 0x{integerToHex_ur}", "integerToHex_ur"_attr = integerToHex(ur));
        fassertFailed(26011);
    }
    ...
    for (std::vector<MemberData>::const_iterator it = _memberData.begin(); it != _memberData.end();
        ...
        if (it->getState().primary() && it->up()) {
            if (remotePrimaryIndex != -1) {
                ...
                LOGV2_WARNING(21552, "two remote primaries (transiently)");
                break;
            }
...
bool TopologyCoordinator::advanceLastCommittedOpTimeAndWallTime(OpTimeAndWallTime committedOpTime,
    ...
    if (_iAmPrimary() && committedOpTime.opTime < _firstOpTimeOfMyTerm) {
        LOGV2_DEBUG(21537, 1, "Ignoring older committed snapshot from before I became primary, optime: {committedOpTime_opTime}, firstOpTimeOfMyTerm: {firstOpTimeOfMyTerm}", "committedOpTime_opTime"_attr = committedOpTime.opTime, "firstOpTimeOfMyTerm"_attr = _firstOpTimeOfMyTerm);
        return false;
    }
    ...
    if (!_selfConfig().isArbiter() &&
        ...
        if (fromSyncSource) {
            ...
            LOGV2_DEBUG(21538, 1, "Ignoring commit point with different term than my lastApplied, since it "
                      "may "
                      "not be on the same oplog branch as mine. optime: {committedOpTime}, my last applied: {getMyLastAppliedOpTimeAndWallTime}", "committedOpTime"_attr = committedOpTime, "getMyLastAppliedOpTimeAndWallTime"_attr = getMyLastAppliedOpTimeAndWallTime());
            return false;
        }
    }
    ...
    if (committedOpTime.opTime < _lastCommittedOpTimeAndWallTime.opTime) {
        LOGV2_DEBUG(21539, 1, "Ignoring older committed snapshot optime: {committedOpTime}, currentCommittedOpTime: {lastCommittedOpTimeAndWallTime}", "committedOpTime"_attr = committedOpTime, "lastCommittedOpTimeAndWallTime"_attr = _lastCommittedOpTimeAndWallTime);
        return false;
    }

    LOGV2_DEBUG(21540, 2, "Updating _lastCommittedOpTimeAndWallTime to {committedOpTime}", "committedOpTime"_attr = committedOpTime);
...
TopologyCoordinator::UpdateTermResult TopologyCoordinator::updateTerm(long long term, Date_t now) {
    ...
    LOGV2_DEBUG(21541, 1, "Updating term from {term} to {term}", "term"_attr = _term, "term"_attr = term);
    _term = term;
...
bool TopologyCoordinator::shouldChangeSyncSource(
    ...
    if (_selfIndex == -1) {
        LOGV2(21542, "Not choosing new sync source because we are not in the config.");
        return false;
    }

    // If the user requested a sync source change, return true.
    if (_forceSyncSourceIndex != -1) {
        LOGV2(21543, "Choosing new sync source because the user has requested to use {rsConfig_getMemberAt_forceSyncSourceIndex_getHostAndPort} as a sync source", "rsConfig_getMemberAt_forceSyncSourceIndex_getHostAndPort"_attr = _rsConfig.getMemberAt(_forceSyncSourceIndex).getHostAndPort());
        return true;
    }

    if (replMetadata.getConfigVersion() != _rsConfig.getConfigVersion()) {
        LOGV2(21544, "Choosing new sync source because the config version supplied by {currentSource}, {replMetadata_getConfigVersion}, does not match ours, {rsConfig_getConfigVersion}", "currentSource"_attr = currentSource, "replMetadata_getConfigVersion"_attr = replMetadata.getConfigVersion(), "rsConfig_getConfigVersion"_attr = _rsConfig.getConfigVersion());
        return true;
    }

    const int currentSourceIndex = _rsConfig.findMemberIndexByHostAndPort(currentSource);
    if (currentSourceIndex == -1) {
        LOGV2(21545, "Choosing new sync source because {currentSource_toString} is not in our config", "currentSource_toString"_attr = currentSource.toString());
        return true;
    }
    ...
    if (syncSourceIndex == -1 && currentSourceOpTime <= myLastOpTime &&
        ...
        LOGV2(21546, "{logMessage_str}", "logMessage_str"_attr = logMessage.str());
        return true;
    }

    if (MONGO_unlikely(disableMaxSyncSourceLagSecs.shouldFail())) {
        LOGV2(21547, "disableMaxSyncSourceLagSecs fail point enabled - not checking the most recent "
                 "OpTime, {currentSourceOpTime_toString}, of our current sync source, {currentSource}, against the OpTimes of the other nodes in this replica set.", "currentSourceOpTime_toString"_attr = currentSourceOpTime.toString(), "currentSource"_attr = currentSource);
    } else {
        ...
        for (std::vector<MemberData>::const_iterator it = _memberData.begin();
            ...
            if (it->up() && (candidateConfig.isVoter() || !_selfConfig().isVoter()) &&
                ...
                LOGV2(21548, "Choosing new sync source because the most recent OpTime of our sync "
                         "source, {currentSource}, is {currentSourceOpTime_toString} which is more than {options_maxSyncSourceLagSecs} behind member {candidateConfig_getHostAndPort_toString} whose most recent OpTime is {it_getHeartbeatAppliedOpTime_toString}", "currentSource"_attr = currentSource, "currentSourceOpTime_toString"_attr = currentSourceOpTime.toString(), "options_maxSyncSourceLagSecs"_attr = _options.maxSyncSourceLagSecs, "candidateConfig_getHostAndPort_toString"_attr = candidateConfig.getHostAndPort().toString(), "it_getHeartbeatAppliedOpTime_toString"_attr = it->getHeartbeatAppliedOpTime().toString());
...
void TopologyCoordinator::processReplSetRequestVotes(const ReplSetRequestVotesArgs& args,
    ...
    if (MONGO_unlikely(voteNoInElection.shouldFail())) {
        LOGV2(21549, "failpoint voteNoInElection enabled");
    ...
    if (MONGO_unlikely(voteYesInDryRunButNoInRealElection.shouldFail())) {
        LOGV2(21550, "failpoint voteYesInDryRunButNoInRealElection enabled");
        if (args.isADryRun()) {
