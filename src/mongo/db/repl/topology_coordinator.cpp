...
HostAndPort TopologyCoordinator::chooseNewSyncSource(Date_t now,
    ...
    if (auto sfp = forceSyncSourceCandidate.scoped(); MONGO_unlikely(sfp.isActive())) {
        ...
        LOGV2("choosing sync source candidate due to 'forceSyncSourceCandidate' parameter: {}", "syncSource"_attr = _syncSource);
    ...
    if (_forceSyncSourceIndex != -1) {
        ...
        _forceSyncSourceIndex = -1;
        LOGV2("choosing sync source candidate by request: {}", "syncSource"_attr = _syncSource);
    ...
    if (chainingPreference == ChainingPreference::kUseConfiguration &&
        ...
        if (_currentPrimaryIndex == -1) {
            ...
            LOGV2_DEBUG(1, "Cannot select a sync source because chaining is not allowed and primary "
                      "member is blacklisted: {}", "currentPrimaryMember_getHostAndPort"_attr = _currentPrimaryMember()->getHostAndPort());
            _syncSource = HostAndPort();
            return _syncSource;
            ...
            LOGV2("chaining not allowed, choosing primary as sync source candidate: {}", "syncSource"_attr = _syncSource);
    ...
    LOGV2("sync source candidate: {}", "syncSource"_attr = _syncSource);
...
void TopologyCoordinator::prepareSyncFromResponse(const HostAndPort& target,
    ...
    if (hbdata.getHeartbeatAppliedOpTime().getSecs() + 10 < lastOpApplied.getSecs()) {
        LOGV2_WARNING("attempting to sync from {}, but its latest opTime is {} and ours is {} so this may not work", "target"_attr = target, "hbdata_getHeartbeatAppliedOpTime_getSecs"_attr = hbdata.getHeartbeatAppliedOpTime().getSecs(), "lastOpApplied_getSecs"_attr = lastOpApplied.getSecs());
...
HeartbeatResponseAction TopologyCoordinator::processHeartbeatResponse(
    ...
    if (_selfIndex == -1) {
        LOGV2_DEBUG(1, "Could not find ourself in current config so ignoring heartbeat from {} -- current config: {}", "target"_attr = target, "rsConfig_toBSON"_attr = _rsConfig.toBSON());
        HeartbeatResponseAction nextAction = HeartbeatResponseAction::makeNoAction();
        nextAction.setNextHeartbeatStartDate(nextHeartbeatStartDate);
        return nextAction;
    }
    ...
    if (memberIndex == -1) {
        LOGV2_DEBUG(1, "Could not find {} in current config so ignoring --"
                  " current config: {}", "target"_attr = target, "rsConfig_toBSON"_attr = _rsConfig.toBSON());
        HeartbeatResponseAction nextAction = HeartbeatResponseAction::makeNoAction();
        nextAction.setNextHeartbeatStartDate(nextHeartbeatStartDate);
        return nextAction;
    }
...
void TopologyCoordinator::changeMemberState_forTest(const MemberState& newMemberState,
    ...
    switch (newMemberState.s) {
        ...
        case MemberState::RS_STARTUP:
            ...
            break;
        default:
            LOGV2_ERROR("Cannot switch to state {}", "newMemberState"_attr = newMemberState);
            MONGO_UNREACHABLE;
    }
    if (getMemberState() != newMemberState.s) {
        LOGV2_ERROR("Expected to enter state {} but am now in {}", "newMemberState"_attr = newMemberState, "getMemberState"_attr = getMemberState());
        MONGO_UNREACHABLE;
    }
...
void TopologyCoordinator::updateConfig(const ReplSetConfig& newConfig, int selfIndex, Date_t now) {
    ...
    if (!_rsConfig.isInitialized()) {
        ...
        LOGV2_DEBUG(1, "Updated term in topology coordinator to {} due to new config", "term"_attr = _term);
    }
    ...
    for (std::vector<MemberData>::const_iterator it = _memberData.begin(); it != _memberData.end();
        ...
        if (it->getState().primary() && it->up()) {
            if (remotePrimaryIndex != -1) {
                ...
                LOGV2_WARNING("two remote primaries (transiently)");
                break;
            }
...
bool TopologyCoordinator::advanceLastCommittedOpTimeAndWallTime(OpTimeAndWallTime committedOpTime,
    ...
    if (_iAmPrimary() && committedOpTime.opTime < _firstOpTimeOfMyTerm) {
        LOGV2_DEBUG(1, "Ignoring older committed snapshot from before I became primary, optime: {}, firstOpTimeOfMyTerm: {}", "committedOpTime_opTime"_attr = committedOpTime.opTime, "firstOpTimeOfMyTerm"_attr = _firstOpTimeOfMyTerm);
        return false;
    }
    ...
    if (committedOpTime.opTime < _lastCommittedOpTimeAndWallTime.opTime) {
        LOGV2_DEBUG(1, "Ignoring older committed snapshot optime: {}, currentCommittedOpTime: {}", "committedOpTime"_attr = committedOpTime, "lastCommittedOpTimeAndWallTime"_attr = _lastCommittedOpTimeAndWallTime);
        return false;
    }
...
TopologyCoordinator::UpdateTermResult TopologyCoordinator::updateTerm(long long term, Date_t now) {
    ...
    LOGV2_DEBUG(1, "Updating term from {} to {}", "term"_attr = _term, "term"_attr = term);
    _term = term;
...
bool TopologyCoordinator::shouldChangeSyncSource(
    ...
    if (_forceSyncSourceIndex != -1) {
        LOGV2("Choosing new sync source because the user has requested to use {} as a sync source", "rsConfig_getMemberAt_forceSyncSourceIndex_getHostAndPort"_attr = _rsConfig.getMemberAt(_forceSyncSourceIndex).getHostAndPort());
        return true;
    }

    if (replMetadata.getConfigVersion() != _rsConfig.getConfigVersion()) {
        LOGV2("Choosing new sync source because the config version supplied by {}, {}, does not match ours, {}", "currentSource"_attr = currentSource, "replMetadata_getConfigVersion"_attr = replMetadata.getConfigVersion(), "rsConfig_getConfigVersion"_attr = _rsConfig.getConfigVersion());
        return true;
    }
