...
TEST_F(AbstractOplogFetcherTest, OplogFetcherStopsRestartingFetcherIfRestartLimitIsReached) {
    ...
    unittest::MONGO_BOOST_LOG << "processing find request from first fetcher";

    _assertFindCommandTimestampEquals(
        ops[0], processNetworkResponse({makeCursorResponse(1, {ops[0], ops[1], ops[2]})}, true));

    unittest::MONGO_BOOST_LOG << "sending error response to getMore request from first fetcher";
    assertRemoteCommandNameEquals(
        "getMore", processNetworkResponse({ErrorCodes::CappedPositionLost, "fail 1"}, true));

    unittest::MONGO_BOOST_LOG << "sending error response to find request from second fetcher";
    _assertFindCommandTimestampEquals(
        ops[2], processNetworkResponse({ErrorCodes::IllegalOperation, "fail 2"}, true));

    unittest::MONGO_BOOST_LOG << "sending error response to find request from third fetcher";
    _assertFindCommandTimestampEquals(
...
TEST_F(AbstractOplogFetcherTest, OplogFetcherResetsRestartCounterOnSuccessfulFetcherResponse) {
    ...
    unittest::MONGO_BOOST_LOG << "processing find request from second fetcher";
    _assertFindCommandTimestampEquals(
        ops[2], processNetworkResponse({makeCursorResponse(1, {ops[2], ops[3], ops[4]})}, true));

    unittest::MONGO_BOOST_LOG << "sending error response to getMore request from second fetcher";
    assertRemoteCommandNameEquals(
    ...
    unittest::MONGO_BOOST_LOG << "sending error response to find request from fourth fetcher";
    _assertFindCommandTimestampEquals(
