...
BaseCloner::AfterStageBehavior CollectionCloner::CollectionClonerStage::run() {
    try {
        ...
        LOGV2(20908, "CollectionCloner ns: '{getCloner_getSourceNss}' uuid: UUID(\"{getCloner_getSourceUuid}\") stopped because collection was dropped on source.", "getCloner_getSourceNss"_attr = getCloner()->getSourceNss(), "getCloner_getSourceUuid"_attr = getCloner()->getSourceUuid());
...
BaseCloner::AfterStageBehavior CollectionCloner::countStage() {
    ...
    if (count < 0) {
        LOGV2_WARNING(20918, "Count command returned negative value. Updating to 0 to allow progress "
...
BaseCloner::AfterStageBehavior CollectionCloner::listIndexesStage() {
    ...
    if (indexSpecs.empty()) {
        LOGV2_WARNING(20919, "No indexes found for collection {sourceNss_ns} while cloning from {getSource}", "sourceNss_ns"_attr = _sourceNss.ns(), "getSource"_attr = getSource());
    }
    ...
    if (!_idIndexSpec.isEmpty() && _collectionOptions.autoIndexId == CollectionOptions::NO) {
        LOGV2_WARNING(20920, "Found the _id_ index spec but the collection specified autoIndexId of false on ns:{this_sourceNss}", "this_sourceNss"_attr = this->_sourceNss);
    }
    return kContinueNormally;
}
...
void CollectionCloner::runQuery() {
    ...
    if (_resumeSupported) {
        if (_resumeToken) {
            ...
            LOGV2_DEBUG(20909, 1, "Collection cloner will resume the last successful query");
            ...
            LOGV2_DEBUG(20910, 1, "Collection cloner will run a new query");
            query = QUERY("query" << BSONObj() << "$readOnce" << true << "$_requestResumeToken"
                                  << true);
        }
    ...
    try {
        ...
        if (!_resumeSupported) {
            ...
            if (_queryStage.isCursorError(status)) {
                LOGV2(20911, "Lost cursor during non-resumable query: {status}", "status"_attr = status);
...
void CollectionCloner::handleNextBatch(DBClientCursorBatchIterator& iter) {
    {
        ...
        if (!getSharedData()->getInitialSyncStatus(lk).isOK()) {
            ...
            LOGV2(20912, "{message}", "message"_attr = message);
    ...
    initialSyncHangCollectionClonerAfterHandlingBatchResponse.executeIf(
        [&](const BSONObj&) {
            while (MONGO_unlikely(
                   ...
                   !mustExit()) {
                LOGV2(20913, "initialSyncHangCollectionClonerAfterHandlingBatchResponse fail point "
                         "enabled for {sourceNss_toString}. Blocking until fail point is disabled.", "sourceNss_toString"_attr = _sourceNss.toString());
                mongo::sleepsecs(1);
            }
        },
        [&](const BSONObj& data) {
...
void CollectionCloner::insertDocumentsCallback(const executor::TaskExecutor::CallbackArgs& cbd) {
...

    {
        ...
        if (_documentsToInsert.size() == 0) {
            LOGV2_WARNING(20921, "insertDocumentsCallback, but no documents to insert for ns:{sourceNss}", "sourceNss"_attr = _sourceNss);
            return;
        }
    ...
    initialSyncHangDuringCollectionClone.executeIf(
        [&](const BSONObj&) {
            LOGV2(20914, "initial sync - initialSyncHangDuringCollectionClone fail point "
...
void CollectionCloner::killOldQueryCursor() {
    ...
    LOGV2_DEBUG(20915, 1, "Attempting to kill old remote cursor with id: {id}", "id"_attr = id);
    try {
        getClient()->runCommand(nss.db().toString(), cmdObj, infoObj);
    } catch (...) {
        LOGV2(20916, "Error while trying to kill remote cursor after transient query error");
    }
...
void CollectionCloner::abortNonResumableClone(const Status& status) {
    ...
    LOGV2(20917, "Error during non-resumable clone: {status}", "status"_attr = status);
    std::string message = str::stream()
