...
void AllDatabaseCloner::postStage() {
    ...
    for (const auto& dbName : _databases) {
        ...
        if (dbStatus.isOK()) {
            ...
            LOGV2_WARNING("database '{}' ({} of {}) clone failed due to {}", "dbName"_attr = dbName, "stats_databasesCloned_1"_attr = (_stats.databasesCloned + 1), "databases_size"_attr = _databases.size(), "dbStatus_toString"_attr = dbStatus.toString());
