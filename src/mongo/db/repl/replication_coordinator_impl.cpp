...
void ReplicationCoordinatorImpl::_finishLoadLocalConfig(
    ...
    if (!isArbiter) {
        if (!lastOpTimeAndWallTimeStatus.isOK()) {
            LOGV2_WARNING("Failed to load timestamp and/or wall clock time of most recently applied "
                         "operation: {}", "lastOpTimeAndWallTimeStatus_getStatus"_attr = lastOpTimeAndWallTimeStatus.getStatus());
        } else {
...
void ReplicationCoordinatorImpl::_stopDataReplication(OperationContext* opCtx) {
    ...
    if (initialSyncerCopy) {
        ...
        if (!status.isOK()) {
            LOGV2_WARNING("InitialSyncer shutdown failed: {}", "status"_attr = status);
        }
    ...
    auto onCompletion = [this, startCompleted](const StatusWith<OpTimeAndWallTime>& opTimeStatus) {
        {
            ...
            if (opTimeStatus == ErrorCodes::CallbackCanceled) {
                ...
                if (_inShutdown) {
                    ...
                    LOGV2_ERROR("Initial sync failed, shutting down now. Restart the server "
    ...
    try {
        {
            ...
            stdx::lock_guard<Latch> lock(_mutex);
            if (_inShutdown) {
                LOGV2("Initial Sync not starting because replication is shutting down.");
                return;
            }
...
void ReplicationCoordinatorImpl::shutdown(OperationContext* opCtx) {
    ...
    std::shared_ptr<InitialSyncer> initialSyncerCopy;
    {
        ...
        if (_rsConfigState == kConfigPreStart) {
            LOGV2_WARNING("ReplicationCoordinatorImpl::shutdown() called before "
...
Status ReplicationCoordinatorImpl::_waitUntilMajorityOpTime(mongo::OperationContext* opCtx,
    ...
    LOGV2_DEBUG(1, "waitUntilOpTime: waiting for optime:{} to be in a snapshot -- current snapshot: {}", "targetOpTime"_attr = targetOpTime, "getCurrentCommittedSnapshotOpTime_inlock"_attr = _getCurrentCommittedSnapshotOpTime_inlock());
    ...
    if (_currentCommittedSnapshot) {
        ...
        // exists.
        LOGV2_DEBUG(3, "Got notified of new snapshot: {}", "currentCommittedSnapshot_toString"_attr = _currentCommittedSnapshot->toString());
    }
    return Status::OK();
}
...
bool ReplicationCoordinatorImpl::_doneWaitingForReplication_inlock(
    ...
    if (writeConcern.wMode == WriteConcernOptions::kMajority) {
        if (_externalState->snapshotsEnabled() && !gTestingSnapshotBehaviorInIsolation) {
            ...
            if (!haveSnapshot) {
                LOGV2_DEBUG(1, "Required snapshot optime: {} is not yet part of the current 'committed' snapshot: {}", "opTime"_attr = opTime, "currentCommittedSnapshot_opTime"_attr = _currentCommittedSnapshot->opTime);
                return false;
            }
...
ReplicationCoordinator::StatusAndDuration ReplicationCoordinatorImpl::awaitReplication(
    ...
    if (getTestCommandsEnabled() && !status.isOK()) {
        stdx::lock_guard lock(_mutex);
        LOGV2("Replication failed for write concern: {}, waiting for optime: {}, opID: {}, progress: {}", "writeConcern_toBSON"_attr = writeConcern.toBSON(), "opTime"_attr = opTime, "opCtx_getOpID"_attr = opCtx->getOpID(), "getReplicationProgress_lock"_attr = _getReplicationProgress(lock));
    }
...
std::shared_ptr<const IsMasterResponse> ReplicationCoordinatorImpl::awaitIsMasterResponse(
    ...
    if (MONGO_unlikely(waitForIsMasterResponse.shouldFail())) {
        ...
        // change.
        LOGV2("waitForIsMasterResponse failpoint enabled.");
    }
...
void ReplicationCoordinatorImpl::_performElectionHandoff() {
    ...
    if (!callbackHandleStatus.isOK()) {
        LOGV2_ERROR("Failed to schedule ReplSetStepUp request to {} for election handoff: {}", "target"_attr = target, "callbackHandleStatus"_attr = callbackHandleStatus);
    }
}
...
Status ReplicationCoordinatorImpl::setMaintenanceMode(bool activate) {
    ...
    if (activate) {
        ...
        LOGV2_WARNING("Attempted to leave maintenance mode but it is not currently active");
...
Status ReplicationCoordinatorImpl::processReplSetReconfig(OperationContext* opCtx,
    ...
    switch (_rsConfigState) {
        ...
        case kConfigHBReconfiguring:
            return Status(ErrorCodes::ConfigurationInProgress,
                          ...
                          "its configuration");
        default:
            LOGV2_FATAL(18914, "Unexpected _rsConfigState {}", "int_rsConfigState"_attr = int(_rsConfigState));
            fassertFailed(18914);
    }
    ...
    if (!status.isOK()) {
        LOGV2_ERROR("replSetReconfig got {} while parsing {}", "status"_attr = status, "newConfigObj"_attr = newConfigObj);
    ...
    if (newConfig.getReplSetName() != _settings.ourSetName()) {
        ...
        LOGV2_ERROR("{}", "std_string_errmsg"_attr = std::string(errmsg));
    ...
    if (!myIndex.isOK()) {
        LOGV2_ERROR("replSetReconfig got {} while validating {}", "myIndex_getStatus"_attr = myIndex.getStatus(), "newConfigObj"_attr = newConfigObj);
...
Status ReplicationCoordinatorImpl::processReplSetInitiate(OperationContext* opCtx,
    ...
    if (!status.isOK()) {
        LOGV2_ERROR("replSet initiate got {} while parsing {}", "status"_attr = status, "configObj"_attr = configObj);
        return Status(ErrorCodes::InvalidReplicaSetConfig, status.reason());
    }
    if (newConfig.getReplSetName() != _settings.ourSetName()) {
        str::stream errmsg;
    ...
    if (!myIndex.isOK()) {
        LOGV2_ERROR("replSet initiate got {} while validating {}", "myIndex_getStatus"_attr = myIndex.getStatus(), "configObj"_attr = configObj);
...
ReplicationCoordinatorImpl::_updateMemberStateFromTopologyCoordinator(WithLock lk,
    ...
    LOGV2_OPTIONS({logv2::LogTag::kRS}, "transition to {} from {}", "newState"_attr = newState, "memberState"_attr = _memberState);
...
void ReplicationCoordinatorImpl::CatchupState::signalHeartbeatUpdate_inlock() {
    ...
    LOGV2("Heartbeats updated catchup target optime to {}", "targetOpTime"_attr = _targetOpTime);
    ...
    auto targetOpTimeCB = [this](Status status) {
        ...
        if (_targetOpTime <= myLastApplied) {
            LOGV2("Caught up to the latest known optime successfully after becoming primary. Target optime: {}. My Last Applied: {}", "targetOpTime"_attr = _targetOpTime, "myLastApplied"_attr = myLastApplied);
...
ReplicationCoordinatorImpl::_setCurrentRSConfig(WithLock lk,
    ...
    LOGV2_OPTIONS({logv2::LogTag::kRS}, "New replica set config in use: {}", "rsConfig_toBSON"_attr = _rsConfig.toBSON());
    _selfIndex = myIndex;
    if (_selfIndex >= 0) {
        LOGV2("This node is {} in the config", "rsConfig_getMemberAt_selfIndex_getHostAndPort"_attr = _rsConfig.getMemberAt(_selfIndex).getHostAndPort());
    } else {
...
void ReplicationCoordinatorImpl::finishRecoveryIfEligible(OperationContext* opCtx) {
    ...
    if (!status.isOK()) {
        LOGV2_WARNING("Failed to transition into {}. Current state: {}{}", "MemberState_MemberState_RS_SECONDARY"_attr = MemberState(MemberState::RS_SECONDARY), "getMemberState"_attr = getMemberState(), "causedBy_status"_attr = causedBy(status));
    }
}
