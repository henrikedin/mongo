...
std::vector<std::string> getAddrsForHost(const std::string& iporhost,
    ...
    if (err) {
        LOGV2_WARNING(20981, "getaddrinfo(\"{iporhost}\") failed: {stringifyError_err}", "iporhost"_attr = iporhost, "stringifyError_err"_attr = stringifyError(err));
        return out;
    }
    ...
    for (addrinfo* addr = addrs; addr != nullptr; addr = addr->ai_next) {
        ...
        if (family == AF_INET || family == AF_INET6) {
            ...
            if (err) {
                LOGV2_WARNING(20982, "getnameinfo() failed: {stringifyError_err}", "stringifyError_err"_attr = stringifyError(err));
                continue;
            }
            out.push_back(host);
        }
    ...
    if (shouldLog(logger::LogSeverity::Debug(2))) {
        ...
        LOGV2_DEBUG(20979, 2, "{builder_str}", "builder_str"_attr = builder.str());
    }
...
bool isSelf(const HostAndPort& hostAndPort, ServiceContext* const ctx) {
    ...
    try {
        ...
        LOGV2_WARNING(20983, "couldn't check isSelf ({hostAndPort}) {e_what}", "hostAndPort"_attr = hostAndPort, "e_what"_attr = e.what());
    }
...
std::vector<std::string> getBoundAddrs(const bool ipv6enabled) {
    ...
    if (err) {
        LOGV2_WARNING(20984, "getifaddrs failure: {errnoWithDescription_err}", "errnoWithDescription_err"_attr = errnoWithDescription(err));
        return out;
    }
    ...
    for (ifaddrs* addr = addrs; addr != nullptr; addr = addr->ifa_next) {
        ...
        if (family == AF_INET || (ipv6enabled && (family == AF_INET6))) {
            err = getnameinfo(
                ...
                NI_NUMERICHOST);
            if (err) {
                LOGV2_WARNING(20985, "getnameinfo() failed: {gai_strerror_err}", "gai_strerror_err"_attr = gai_strerror(err));
                continue;
            }
            out.push_back(host);
        }
    ...
    if (err != NO_ERROR) {
        LOGV2_WARNING(20986, "GetAdaptersAddresses() failed: {errnoWithDescription_err}", "errnoWithDescription_err"_attr = errnoWithDescription(err));
        return out;
    }
    ...
    for (IP_ADAPTER_ADDRESSES* adapter = adapters; adapter != nullptr; adapter = adapter->Next) {
        for (IP_ADAPTER_UNICAST_ADDRESS* addr = adapter->FirstUnicastAddress; addr != nullptr;
            ...
            if (family == AF_INET) {
                ...
                if (ec) {
                    LOGV2_WARNING(20987, "inet_ntop failed during IPv4 address conversion: {ec_message}", "ec_message"_attr = ec.message());
                    continue;
                }
                ...
                if (ec) {
                    LOGV2_WARNING(20988, "inet_ntop failed during IPv6 address conversion: {ec_message}", "ec_message"_attr = ec.message());
                    continue;
                }
                out.push_back(addrstr);
            }
    ...
    if (shouldLog(logger::LogSeverity::Debug(2))) {
        ...
        LOGV2_DEBUG(20980, 2, "{builder_str}", "builder_str"_attr = builder.str());
    }
    return out;
}
