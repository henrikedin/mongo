...
TEST_F(ReplCoordTest, ElectionSucceedsWhenNodeIsTheOnlyElectableNode) {
    ...
    while (net->now() < electionTimeoutWhen) {
        ...
        LOGV2_ERROR("Black holing irrelevant request to {}: {}", "request_target"_attr = request.target, "request_cmdObj"_attr = request.cmdObj);
        net->blackHole(noi);
    }
    net->exitNetwork();
...
TEST_F(ReplCoordTest, NodeWillNotStandForElectionDuringHeartbeatReconfig) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (hbArgs.initialize(request.cmdObj).isOK()) {
            ...
            LOGV2_ERROR("Black holing unexpected request to {}: {}", "request_target"_attr = request.target, "request_cmdObj"_attr = request.cmdObj);
            net->blackHole(noi);
        }
        net->runReadyNetworkOperations();
    }
