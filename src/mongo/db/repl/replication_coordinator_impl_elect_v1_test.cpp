...
TEST_F(ReplCoordTest, ElectionSucceedsWhenNodeIsTheOnlyElectableNode) {
    ...
    LOGV2(21192, "Election timeout scheduled at {electionTimeoutWhen} (simulator time)", "electionTimeoutWhen"_attr = electionTimeoutWhen);
    ...
    while (net->now() < electionTimeoutWhen) {
        ...
        LOGV2_ERROR(21211, "Black holing irrelevant request to {request_target}: {request_cmdObj}", "request_target"_attr = request.target, "request_cmdObj"_attr = request.cmdObj);
        net->blackHole(noi);
    }
    net->exitNetwork();
...
TEST_F(ReplCoordTest, ElectionFailsWhenInsufficientVotesAreReceivedDuringDryRun) {
    ...
    auto electionTimeoutWhen = getReplCoord()->getElectionTimeout_forTest();
    ASSERT_NOT_EQUALS(Date_t(), electionTimeoutWhen);
    LOGV2(21193, "Election timeout scheduled at {electionTimeoutWhen} (simulator time)", "electionTimeoutWhen"_attr = electionTimeoutWhen);
    ...
    while (voteRequests < 2) {
        ...
        LOGV2(21194, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
...
TEST_F(ReplCoordTest, ElectionFailsWhenDryRunResponseContainsANewerTerm) {
    ...
    auto electionTimeoutWhen = getReplCoord()->getElectionTimeout_forTest();
    ASSERT_NOT_EQUALS(Date_t(), electionTimeoutWhen);
    LOGV2(21195, "Election timeout scheduled at {electionTimeoutWhen} (simulator time)", "electionTimeoutWhen"_attr = electionTimeoutWhen);
    ...
    while (voteRequests < 1) {
        ...
        const NetworkInterfaceMock::NetworkOperationIterator noi = net->getNextReadyRequest();
        const RemoteCommandRequest& request = noi->getRequest();
        LOGV2(21196, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
        if (consumeHeartbeatV1(noi)) {
            // The heartbeat has been consumed.
        } else if (request.cmdObj.firstElement().fieldNameStringData() == "replSetRequestVotes") {
...
TEST_F(ReplCoordTest, NodeWillNotStandForElectionDuringHeartbeatReconfig) {
    ...
    for (int i = 0; i < 2; ++i) {
        const NetworkInterfaceMock::NetworkOperationIterator noi = net->getNextReadyRequest();
        const RemoteCommandRequest& request = noi->getRequest();
        LOGV2(21197, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
        ...
        if (hbArgs.initialize(request.cmdObj).isOK()) {
            ...
            LOGV2_ERROR(21212, "Black holing unexpected request to {request_target}: {request_cmdObj}", "request_target"_attr = request.target, "request_cmdObj"_attr = request.cmdObj);
            net->blackHole(noi);
        }
        net->runReadyNetworkOperations();
    }
    ...
    auto electionTimeoutWhen = getReplCoord()->getElectionTimeout_forTest();
    ASSERT_NOT_EQUALS(Date_t(), electionTimeoutWhen);
    LOGV2(21198, "Election timeout scheduled at {electionTimeoutWhen} (simulator time)", "electionTimeoutWhen"_attr = electionTimeoutWhen);
...
TEST_F(ReplCoordTest, ElectionFailsWhenInsufficientVotesAreReceivedDuringRequestVotes) {
    ...
    while (net->hasReadyRequests()) {
        const NetworkInterfaceMock::NetworkOperationIterator noi = net->getNextReadyRequest();
        const RemoteCommandRequest& request = noi->getRequest();
        LOGV2(21199, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
...
TEST_F(ReplCoordTest, ElectionFailsWhenVoteRequestResponseContainsANewerTerm) {
    ...
    while (net->hasReadyRequests()) {
        const NetworkInterfaceMock::NetworkOperationIterator noi = net->getNextReadyRequest();
        const RemoteCommandRequest& request = noi->getRequest();
        LOGV2(21200, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
        if (request.cmdObj.firstElement().fieldNameStringData() != "replSetRequestVotes") {
            net->blackHole(noi);
        } else {
            net->scheduleResponse(
...
TEST_F(ReplCoordTest, ElectionFailsWhenTermChangesDuringActualElection) {
    ...
    while (net->hasReadyRequests()) {
        const NetworkInterfaceMock::NetworkOperationIterator noi = net->getNextReadyRequest();
        const RemoteCommandRequest& request = noi->getRequest();
        LOGV2(21201, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
        if (request.cmdObj.firstElement().fieldNameStringData() != "replSetRequestVotes") {
            net->blackHole(noi);
        } else {
            net->scheduleResponse(
...
private:
    ...
    void _respondToHeartbeatsNow(const ReplSetConfig& config,
        ...
        while (net->hasReadyRequests()) {
            ...
            LOGV2(21202, "{request_target} processing {request_cmdObj} at {net_now}", "request_target"_attr = request.target, "request_cmdObj"_attr = request.cmdObj, "net_now"_attr = net->now());
...
TEST_F(TakeoverTest, CatchupTakeoverDryRunFailsPrimarySaysNo) {
    ...
    while (voteRequests < votesExpected) {
        ...
        const NetworkInterfaceMock::NetworkOperationIterator noi = net->getNextReadyRequest();
        const RemoteCommandRequest& request = noi->getRequest();
        LOGV2(21203, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
        if (request.cmdObj.firstElement().fieldNameStringData() != "replSetRequestVotes") {
            net->blackHole(noi);
        } else {
...
protected:
    ...
    void simulateSuccessfulV1Voting() {
        ...
        ASSERT_NOT_EQUALS(Date_t(), electionTimeoutWhen);
        LOGV2(21204, "Election timeout scheduled at {electionTimeoutWhen} (simulator time)", "electionTimeoutWhen"_attr = electionTimeoutWhen);
        ...
        while (!replCoord->getMemberState().primary()) {
            LOGV2(21205, "Waiting on network in state {replCoord_getMemberState}", "replCoord_getMemberState"_attr = replCoord->getMemberState());
            net->enterNetwork();
            if (net->now() < electionTimeoutWhen) {
                net->runUntil(electionTimeoutWhen);
            }
            ...
            const RemoteCommandRequest& request = noi->getRequest();
            LOGV2(21206, "{request_target_toString} processing {request_cmdObj}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj);
    ...
    void processHeartbeatRequests(NetworkRequestFn onHeartbeatRequest) {
        ...
        while (net->hasReadyRequests()) {
            const NetworkInterfaceMock::NetworkOperationIterator noi = net->getNextReadyRequest();
            const RemoteCommandRequest& request = noi->getRequest();
            LOGV2(21207, "{request_target_toString} processing heartbeat {request_cmdObj} at {net_now}", "request_target_toString"_attr = request.target.toString(), "request_cmdObj"_attr = request.cmdObj, "net_now"_attr = net->now());
            if (ReplSetHeartbeatArgsV1().initialize(request.cmdObj).isOK()) {
                onHeartbeatRequest(noi);
            } else {
                LOGV2(21208, "Black holing unexpected request to {request_target}: {request_cmdObj}", "request_target"_attr = request.target, "request_cmdObj"_attr = request.cmdObj);
                net->blackHole(noi);
            }
            net->runReadyNetworkOperations();
        }
    ...
    void replyHeartbeatsAndRunUntil(Date_t until, NetworkRequestFn onHeartbeatRequest) {
        ...
        while (net->now() < until) {
            while (net->hasReadyRequests()) {
                ...
                auto noi = net->getFrontOfUnscheduledQueue();
                auto& request = noi->getRequest();
                LOGV2(21209, "{request_target} at {net_now} processing {request_cmdObj}", "request_target"_attr = request.target, "net_now"_attr = net->now(), "request_cmdObj"_attr = request.cmdObj);
                if (ReplSetHeartbeatArgsV1().initialize(request.cmdObj).isOK()) {
...
TEST_F(PrimaryCatchUpTest, HeartbeatTimeout) {
    ...
    replyHeartbeatsAndRunUntil(catchupTimeoutTime, [this, time1](const NetworkOpIter noi) {
        const RemoteCommandRequest& request = noi->getRequest();
        if (request.target.host() == "node2") {
            LOGV2(21210, "Black holing heartbeat from {request_target_host}", "request_target_host"_attr = request.target.host());
