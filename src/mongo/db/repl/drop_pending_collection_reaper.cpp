...
void DropPendingCollectionReaper::addDropPendingNamespace(
    ...
    if (std::find_if(lowerBound, upperBound, matcher) != upperBound) {
        LOGV2_FATAL(20932, "Failed to add drop-pending collection {dropPendingNamespace} with drop optime {dropOpTime}: duplicate optime and namespace pair.", "dropPendingNamespace"_attr = dropPendingNamespace, "dropOpTime"_attr = dropOpTime);
...
bool DropPendingCollectionReaper::rollBackDropPendingCollection(
    ...
    const auto pendingNss = collectionNamespace.makeDropPendingNamespace(opTime);
    {
        ...
        auto it = std::find_if(lowerBound, upperBound, matcher);
        if (it == upperBound) {
            LOGV2_WARNING(20930, "Cannot find drop-pending namespace at OpTime {opTime} for collection {collectionNamespace} to roll back.", "opTime"_attr = opTime, "collectionNamespace"_attr = collectionNamespace);
            return false;
        }
    ...
    LOGV2(20928, "Rolling back collection drop for {pendingNss} with drop OpTime {opTime} to namespace {collectionNamespace}", "pendingNss"_attr = pendingNss, "opTime"_attr = opTime, "collectionNamespace"_attr = collectionNamespace);
...
void DropPendingCollectionReaper::dropCollectionsOlderThan(OperationContext* opCtx,
...

    {
        ...
        for (const auto& opTimeAndNamespace : toDrop) {
            ...
            LOGV2(20929, "Completing collection drop for {nss} with drop optime {dropOpTime} (notification optime: {opTime})", "nss"_attr = nss, "dropOpTime"_attr = dropOpTime, "opTime"_attr = opTime);
            ...
            if (!status.isOK()) {
                LOGV2_WARNING(20931, "Failed to remove drop-pending collection {nss} with drop optime {dropOpTime} (notification optime: {opTime}): {status}", "nss"_attr = nss, "dropOpTime"_attr = dropOpTime, "opTime"_attr = opTime, "status"_attr = status);
            }
        }
    }
