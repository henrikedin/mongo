...
AutoGetCollectionForRead::AutoGetCollectionForRead(OperationContext* opCtx,
    ...
    while (auto coll = _autoColl->getCollection()) {
        ...
        if (lastAppliedTimestamp) {
            LOGV2(20545, "tried reading at last-applied time: {lastAppliedTimestamp} on ns: {nss_ns}, but future catalog changes are pending at time {minSnapshot}. Trying again without reading at last-applied time.", "lastAppliedTimestamp"_attr = *lastAppliedTimestamp, "nss_ns"_attr = nss.ns(), "minSnapshot"_attr = *minSnapshot);
...
bool AutoGetCollectionForRead::_shouldReadAtLastAppliedTimestamp(
    ...
    if (opCtx->lockState()->isLockHeldForMode(resourceIdParallelBatchWriterMode, MODE_IS)) {
        LOGV2_DEBUG(20546, 1, "not reading at last-applied because the PBWM lock is held");
        return false;
    }
