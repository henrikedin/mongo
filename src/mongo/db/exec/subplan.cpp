    ...
    for (size_t i = 0; i < _orExpression->numChildren(); ++i) {
        ...
        if (planCache->shouldCacheQuery(*branchResult->canonicalQuery)) {
            ...
            if (auto cachedSol = planCache->getCacheEntryIfActive(planCacheKey)) {
                ...
                LOGV2_DEBUG(5, "Subplanner: cached plan found for child {} of {}", "i"_attr = i, "orExpression_numChildren"_attr = _orExpression->numChildren());
        ...
        if (!branchResult->cachedSolution) {
            ...
            LOGV2_DEBUG(5, "Subplanner: planning child {} of {}", "i"_attr = i, "orExpression_numChildren"_attr = _orExpression->numChildren());
...
Status SubplanStage::choosePlanForSubqueries(PlanYieldPolicy* yieldPolicy) {
    ...
    LOGV2_DEBUG(5, "Subplanner: Composite solution is {}", "redact_compositeSolution_toString"_attr = redact(_compositeSolution->toString()));
