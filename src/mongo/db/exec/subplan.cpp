...
Status SubplanStage::planSubqueries() {
    ...
    for (size_t i = 0; i < _plannerParams.indices.size(); ++i) {
        ...
        LOGV2_DEBUG(20561, 5, "Subplanner: index {i} is {ie}", "i"_attr = i, "ie"_attr = ie);
    }
    ...
    for (size_t i = 0; i < _orExpression->numChildren(); ++i) {
        ...
        if (planCache->shouldCacheQuery(*branchResult->canonicalQuery)) {
            ...
            if (auto cachedSol = planCache->getCacheEntryIfActive(planCacheKey)) {
                ...
                LOGV2_DEBUG(20562, 5, "Subplanner: cached plan found for child {i} of {orExpression_numChildren}", "i"_attr = i, "orExpression_numChildren"_attr = _orExpression->numChildren());
        ...
        if (!branchResult->cachedSolution) {
            ...
            LOGV2_DEBUG(20563, 5, "Subplanner: planning child {i} of {orExpression_numChildren}", "i"_attr = i, "orExpression_numChildren"_attr = _orExpression->numChildren());
            ...
            LOGV2_DEBUG(20564, 5, "Subplanner: got {branchResult_solutions_size} solutions", "branchResult_solutions_size"_attr = branchResult->solutions.size());
        }
    }
...
Status SubplanStage::choosePlanForSubqueries(PlanYieldPolicy* yieldPolicy) {
    ...
    LOGV2_DEBUG(20565, 5, "Subplanner: fully tagged tree is {redact_solnRoot_toString}", "redact_solnRoot_toString"_attr = redact(solnRoot->toString()));
    ...
    LOGV2_DEBUG(20566, 5, "Subplanner: Composite solution is {redact_compositeSolution_toString}", "redact_compositeSolution_toString"_attr = redact(_compositeSolution->toString()));
