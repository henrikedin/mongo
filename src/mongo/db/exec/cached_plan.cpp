...
Status CachedPlanStage::pickBestPlan(PlanYieldPolicy* yieldPolicy) {
    ...
    for (size_t i = 0; i < maxWorksBeforeReplan; ++i) {
        ...
        if (PlanStage::ADVANCED == state) {
            ...
            LOGV2_DEBUG(1, "Execution of cached plan failed, falling back to replan. query: {} planSummary: {} status: {}", "redact_canonicalQuery_toStringShort"_attr = redact(_canonicalQuery->toStringShort()), "Explain_getPlanSummary_child_get"_attr = Explain::getPlanSummary(child().get()), "redact_statusObj"_attr = redact(statusObj));
    ...
    LOGV2_DEBUG(1, "Execution of cached plan required {} works, but was originally cached with only {} works. Evicting cache entry and replanning query: {} plan summary before replan: {}", "maxWorksBeforeReplan"_attr = maxWorksBeforeReplan, "decisionWorks"_attr = _decisionWorks, "redact_canonicalQuery_toStringShort"_attr = redact(_canonicalQuery->toStringShort()), "Explain_getPlanSummary_child_get"_attr = Explain::getPlanSummary(child().get()));
...
Status CachedPlanStage::replan(PlanYieldPolicy* yieldPolicy, bool shouldCache, std::string reason) {
    ...
    LOGV2_DEBUG(1, "Replanning {} resulted in plan with summary: {}, which {} been written to the cache", "redact_canonicalQuery_toStringShort"_attr = redact(_canonicalQuery->toStringShort()), "Explain_getPlanSummary_child_get"_attr = Explain::getPlanSummary(child().get()), "shouldCache_has_has_not"_attr = (shouldCache ? "has" : "has not"));
    return Status::OK();
}
