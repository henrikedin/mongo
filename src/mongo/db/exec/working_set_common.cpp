...
bool WorkingSetCommon::fetch(OperationContext* opCtx,
    ...
    if (!record) {
        ...
        if (member->getState() == WorkingSetMember::RID_AND_IDX &&
            ...
            auto indexKeyEntry = [](const IndexKeyDatum& ikd) {
            ...
            LOGV2_ERROR(
                ...
                "indexKeyData"_attr = logv2::seqLog(
                    boost::make_transform_iterator(member->keyData.begin(), indexKeyEntry),
                    boost::make_transform_iterator(member->keyData.end(), indexKeyEntry)),
