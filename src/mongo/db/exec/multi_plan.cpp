...
Status MultiPlanStage::pickBestPlan(PlanYieldPolicy* yieldPolicy) {
    ...
    if (_cachingMode == CachingMode::SometimesCache) {
        ...
        if (ranking->tieForBest) {
            ...
            LOGV2_DEBUG(1, "Winning plan tied with runner-up. Not caching. query: {} winner score: {} winner summary: {} runner-up score: {} runner-up summary: {}", "redact_query_toStringShort"_attr = redact(_query->toStringShort()), "ranking_scores_0"_attr = ranking->scores[0], "Explain_getPlanSummary_candidates_winnerIdx_root"_attr = Explain::getPlanSummary(_candidates[winnerIdx].root), "ranking_scores_1"_attr = ranking->scores[1], "Explain_getPlanSummary_candidates_runnerUpIdx_root"_attr = Explain::getPlanSummary(_candidates[runnerUpIdx].root));
        }
        ...
        if (alreadyProduced.empty()) {
            ...
            size_t winnerIdx = ranking->candidateOrder[0];
            LOGV2_DEBUG(1, "Winning plan had zero results. Not caching. query: {} winner score: {} winner summary: {}", "redact_query_toStringShort"_attr = redact(_query->toStringShort()), "ranking_scores_0"_attr = ranking->scores[0], "Explain_getPlanSummary_candidates_winnerIdx_root"_attr = Explain::getPlanSummary(_candidates[winnerIdx].root));
        }
    }
