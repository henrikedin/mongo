...
Status IndexBuildInterceptor::drainWritesIntoIndex(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(::mongo::logger::LogSeverity(logLevel).toInt(), "index build: drain applied {} side writes (inserted: {}, deleted: {}) for '{}' in {} ms", "numApplied_appliedAtStart"_attr = (_numApplied - appliedAtStart), "totalInserted"_attr = totalInserted, "totalDeleted"_attr = totalDeleted, "indexCatalogEntry_descriptor_indexName"_attr = _indexCatalogEntry->descriptor()->indexName(), "timer_millis"_attr = timer.millis());
...
Status IndexBuildInterceptor::sideWrite(OperationContext* opCtx,
    ...
    *numKeysOut = keys.size() + (op == Op::kInsert ? multikeyMetadataKeys.size() : 0);
    ...
    LOGV2_DEBUG(2, "recording {} side write keys on index '{}'", "records_size"_attr = records.size(), "indexCatalogEntry_descriptor_indexName"_attr = _indexCatalogEntry->descriptor()->indexName());
