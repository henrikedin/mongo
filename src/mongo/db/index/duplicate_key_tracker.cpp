...
Status DuplicateKeyTracker::recordKeys(OperationContext* opCtx, const std::vector<BSONObj>& keys) {
    ...
    LOGV2_DEBUG(20631, 1, "recording {records_size} duplicate key conflicts on unique index: {indexCatalogEntry_descriptor_indexName}", "records_size"_attr = records.size(), "indexCatalogEntry_descriptor_indexName"_attr = _indexCatalogEntry->descriptor()->indexName());
...
Status DuplicateKeyTracker::checkConstraints(OperationContext* opCtx) const {
    ...
    LOGV2_DEBUG(20632, logSeverityV1toV2(logLevel).toInt(), "index build: resolved {resolved} duplicate key conflicts for unique index: {indexCatalogEntry_descriptor_indexName}", "resolved"_attr = resolved, "indexCatalogEntry_descriptor_indexName"_attr = _indexCatalogEntry->descriptor()->indexName());
    return Status::OK();
}
