...
void abortInProgressTransactions(OperationContext* opCtx) {
    ...
    if (cursor->more()) {
        LOGV2_DEBUG(21699, 3, "Aborting in-progress transactions on stepup.");
    }
    while (cursor->more()) {
        ...
        LOGV2_DEBUG(21700, 3, "Aborting transaction sessionId: {txnRecord_getSessionId_toBSON} txnNumber {txnRecord_getTxnNum}", "txnRecord_getSessionId_toBSON"_attr = txnRecord.getSessionId().toBSON(), "txnRecord_getTxnNum"_attr = txnRecord.getTxnNum());
...
void MongoDSessionCatalog::onStepUp(OperationContext* opCtx) {
...

    {
        ...
        for (const auto& sessionId : sessionIdToReacquireLocks) {
            ...
            LOGV2_DEBUG(21701, 3, "Restoring locks of prepared transaction. SessionId: {sessionId_getId} TxnNumber: {txnParticipant_getActiveTxnNumber}", "sessionId_getId"_attr = sessionId.getId(), "txnParticipant_getActiveTxnNumber"_attr = txnParticipant.getActiveTxnNumber());
