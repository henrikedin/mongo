...
MinVisibleTimestampMap closeCatalog(OperationContext* opCtx) {
    ...
    for (auto&& dbName : allDbs) {
        ...
        for (auto collIt = db->begin(opCtx); collIt != db->end(opCtx); ++collIt) {
            ...
            if (uuid && minVisible) {
                LOGV2_DEBUG(20238, 1, "closeCatalog: preserving min visible timestamp. Collection: {coll_ns} UUID: {uuid} TS: {minVisible}", "coll_ns"_attr = coll->ns(), "uuid"_attr = uuid, "minVisible"_attr = minVisible);
    ...
    LOGV2_DEBUG(20239, 1, "closeCatalog: closing collection catalog");

    // Close all databases.
    LOGV2(20240, "closeCatalog: closing all databases");
    databaseHolder->closeAll(opCtx);

    // Close the storage engine's catalog.
    LOGV2(20241, "closeCatalog: closing storage engine catalog");
...
void openCatalog(OperationContext* opCtx, const MinVisibleTimestampMap& minVisibleTimestampMap) {
    ...
    LOGV2(20242, "openCatalog: loading storage engine catalog");
    auto storageEngine = opCtx->getServiceContext()->getStorageEngine();
    storageEngine->loadCatalog(opCtx);

    LOGV2(20243, "openCatalog: reconciling catalog and idents");
    ...
    for (const auto& entry : nsToIndexNameObjMap) {
        ...
        for (const auto& indexName : entry.second.first) {
            LOGV2(20244, "openCatalog: rebuilding index: collection: {collNss_toString}, index: {indexName}", "collNss_toString"_attr = collNss.toString(), "indexName"_attr = indexName);
        }
    ...
    LOGV2(20245, "openCatalog: reopening all databases");
    auto databaseHolder = DatabaseHolder::get(opCtx);
    ...
    for (auto&& dbName : databasesToOpen) {
        ...
        for (auto&& collNss :
            ...
            // to the oplog.
            if (collNss.isOplog()) {
                LOGV2(20246, "openCatalog: updating cached oplog pointer");
    ...
    LOGV2(20247, "openCatalog: finished reloading collection catalog");
}
}  // namespace catalog
}  // namespace mongo
