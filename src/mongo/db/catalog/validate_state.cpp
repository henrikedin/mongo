...
void ValidateState::initializeCursors(OperationContext* opCtx) {
    ...
    try {
        ...
        LOGV2("Skipping background validation on collection '{}' because the collection is not yet in a checkpoint: {}", "nss"_attr = _nss, "ex"_attr = ex);
        throw;
    }
    ...
    try {
        ...
        invariant(_background);
        LOGV2("Skipping background validation on collection '{}' because the data is not yet in a checkpoint: {}", "nss"_attr = _nss, "ex"_attr = ex);
        throw;
    }
    ...
    while (it->more()) {
        ...
        if (_background && !isIndexDurable) {
            LOGV2("Skipping validation on index '{}' in collection '{}' because the index is not yet in a checkpoint.", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss);
            continue;
        }
        ...
        if (entry->getIdent() != diskIndexIdent) {
            LOGV2("Skipping validation on index '{}' in collection '{}' because the index was recreated and is not yet in a checkpoint.", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss);
            continue;
        }
        ...
        try {
            ...
            LOGV2("Skipping validation on index '{}' in collection '{}' because the index data is not in a checkpoint: {}", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss, "ex"_attr = ex);
            continue;
        }
        ...
        if (_background &&
            ...
            LOGV2("Skipping validation on index '{}' in collection '{}' because the index data is not yet consistent in the checkpoint.", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss);
            continue;
        }
