...
void ValidateState::initializeCursors(OperationContext* opCtx) {
    ...
    try {
        ...
        LOGV2(20373, "Skipping background validation on collection '{nss}' because the collection is not yet in a checkpoint: {ex}", "nss"_attr = _nss, "ex"_attr = ex);
        throw;
    }
    ...
    try {
        ...
        invariant(_background);
        LOGV2(20374, "Skipping background validation on collection '{nss}' because the data is not yet in a checkpoint: {ex}", "nss"_attr = _nss, "ex"_attr = ex);
        throw;
    }
    ...
    while (it->more()) {
        ...
        if (_background && !isIndexDurable) {
            LOGV2(20375, "Skipping validation on index '{desc_indexName}' in collection '{nss}' because the index is not yet in a checkpoint.", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss);
            continue;
        }
        ...
        if (entry->getIdent() != diskIndexIdent) {
            LOGV2(20376, "Skipping validation on index '{desc_indexName}' in collection '{nss}' because the index was recreated and is not yet in a checkpoint.", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss);
            continue;
        }
        ...
        try {
            ...
            LOGV2(20377, "Skipping validation on index '{desc_indexName}' in collection '{nss}' because the index data is not in a checkpoint: {ex}", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss, "ex"_attr = ex);
            continue;
        }
        ...
        if (_background &&
            ...
            LOGV2(20378, "Skipping validation on index '{desc_indexName}' in collection '{nss}' because the index data is not yet consistent in the checkpoint.", "desc_indexName"_attr = desc->indexName(), "nss"_attr = _nss);
            continue;
        }
...
void ValidateState::_relockDatabaseAndCollection(OperationContext* opCtx) {
    ...
    if (MONGO_unlikely(hangDuringYieldingLocksForValidation.shouldFail())) {
        LOGV2(20379, "Hanging on fail point 'hangDuringYieldingLocksForValidation'");
