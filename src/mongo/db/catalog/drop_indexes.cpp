...
Status dropIndexes(OperationContext* opCtx,
    ...
    return writeConflictRetry(opCtx, "dropIndexes", nss.db(), [opCtx, &nss, &cmdObj, result] {
        ...
        if (!serverGlobalParams.quiet.load()) {
            LOGV2(20312, "CMD: dropIndexes {nss}: {cmdObj_kIndexFieldName_toString_false}", "nss"_attr = nss, "cmdObj_kIndexFieldName_toString_false"_attr = cmdObj[kIndexFieldName].toString(false));
        }
