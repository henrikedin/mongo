...
void DatabaseImpl::init(OperationContext* const opCtx) const {
    ...
    if (!status.isOK()) {
        LOGV2_WARNING("tried to open invalid db: {}", "_name"_attr = _name);
...
void DatabaseImpl::clearTmpCollections(OperationContext* opCtx) const {
    ...
    CollectionCatalog::CollectionInfoFn callback = [&](const Collection* collection) {
        try {
            ...
            if (!status.isOK()) {
                LOGV2_WARNING("could not drop temp collection '{}': {}", "collection_ns"_attr = collection->ns(), "redact_status"_attr = redact(status));
            }
            wuow.commit();
        } catch (const WriteConflictException&) {
            LOGV2_WARNING("could not drop temp collection '{}' due to WriteConflictException", "collection_ns"_attr = collection->ns());
