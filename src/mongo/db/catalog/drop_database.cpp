...
void _finishDropDatabase(OperationContext* opCtx,
    ...
    if (MONGO_unlikely(dropDatabaseHangBeforeInMemoryDrop.shouldFail())) {
        LOGV2(20302, "dropDatabase - fail point dropDatabaseHangBeforeInMemoryDrop enabled.");
    ...
    LOGV2(20303, "dropDatabase {dbName} - dropped {numCollections} collection(s)", "dbName"_attr = dbName, "numCollections"_attr = numCollections);
    LOGV2(20304, "dropDatabase {dbName} - finished", "dbName"_attr = dbName);
}
...
Status dropDatabase(OperationContext* opCtx, const std::string& dbName) {
...

    {
        ...
        LOGV2(20305, "dropDatabase {dbName} - starting", "dbName"_attr = dbName);
        ...
        for (auto collIt = db->begin(opCtx); collIt != db->end(opCtx); ++collIt) {
            ...
            LOGV2(20306, "dropDatabase {dbName} - dropping collection: {nss}", "dbName"_attr = dbName, "nss"_attr = nss);

            if (nss.isDropPendingNamespace() && replCoord->isReplEnabled() &&
                opCtx->writesAreReplicated()) {
                LOGV2(20307, "dropDatabase {dbName} - found drop-pending collection: {nss}", "dbName"_attr = dbName, "nss"_attr = nss);
...

    {
        ...
        LOGV2(20308, "dropDatabase {dbName} waiting for {awaitOpTime} to be replicated at {dropDatabaseWriteConcern_toBSON}. Dropping {numCollectionsToDrop} collection(s), with last collection drop at {latestDropPendingOpTime}", "dbName"_attr = dbName, "awaitOpTime"_attr = awaitOpTime, "dropDatabaseWriteConcern_toBSON"_attr = dropDatabaseWriteConcern.toBSON(), "numCollectionsToDrop"_attr = numCollectionsToDrop, "latestDropPendingOpTime"_attr = latestDropPendingOpTime);
        ...
        if (result.status.isOK() && !userWriteConcern.usedDefault) {
            LOGV2(20309, "dropDatabase {dbName} waiting for {awaitOpTime} to be replicated at {userWriteConcern_toBSON}", "dbName"_attr = dbName, "awaitOpTime"_attr = awaitOpTime, "userWriteConcern_toBSON"_attr = userWriteConcern.toBSON());
        ...
        LOGV2(20310, "dropDatabase {dbName} - successfully dropped {numCollectionsToDrop} collection(s) (most recent drop optime: {awaitOpTime}) after {result_duration}. dropping database", "dbName"_attr = dbName, "numCollectionsToDrop"_attr = numCollectionsToDrop, "awaitOpTime"_attr = awaitOpTime, "result_duration"_attr = result.duration);
    }

    if (MONGO_unlikely(dropDatabaseHangAfterAllCollectionsDrop.shouldFail())) {
        LOGV2(20311, "dropDatabase - fail point dropDatabaseHangAfterAllCollectionsDrop enabled. "
