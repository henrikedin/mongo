...
std::map<std::string, int64_t> _validateIndexesInternalStructure(
    ...
    while (it->more()) {
        ...
        LOGV2_OPTIONS(20264, {logComponentV1toV2(LogComponent::kIndex)}, "validating the internal structure of index {descriptor_indexName} on collection {descriptor_parentNS}", "descriptor_indexName"_attr = descriptor->indexName(), "descriptor_parentNS"_attr = descriptor->parentNS());
...
void _validateIndexes(OperationContext* opCtx,
    ...
    for (const auto& index : validateState->getIndexes()) {
        ...
        LOGV2_OPTIONS(20265, {logComponentV1toV2(LogComponent::kIndex)}, "validating index consistency {descriptor_indexName} on collection {descriptor_parentNS}", "descriptor_indexName"_attr = descriptor->indexName(), "descriptor_parentNS"_attr = descriptor->parentNS());
...
void _gatherIndexEntryErrors(OperationContext* opCtx,
    ...
    LOGV2_OPTIONS(20266, {logComponentV1toV2(LogComponent::kIndex)}, "Starting to traverse through all the document key sets.");
    ...
    LOGV2_OPTIONS(20267, {logComponentV1toV2(LogComponent::kIndex)}, "Finished traversing through all the document key sets.");
    LOGV2_OPTIONS(20268, {logComponentV1toV2(LogComponent::kIndex)}, "Starting to traverse through all the indexes.");
    ...
    for (const auto& index : validateState->getIndexes()) {
        ...
        LOGV2_OPTIONS(20269, {logComponentV1toV2(LogComponent::kIndex)}, "Traversing through the index entries for index {descriptor_indexName}.", "descriptor_indexName"_attr = descriptor->indexName());
    ...
    LOGV2_OPTIONS(20270, {logComponentV1toV2(LogComponent::kIndex)}, "Finished traversing through all the indexes.");
...
Status validate(OperationContext* opCtx,
    ...
    try {
        ...
        LOGV2_OPTIONS(20271, {logComponentV1toV2(LogComponent::kIndex)}, "validating collection {validateState_nss}{uuidString}", "validateState_nss"_attr = validateState.nss(), "uuidString"_attr = uuidString);
        ...
        if (MONGO_unlikely(pauseCollectionValidationWithLock.shouldFail())) {
            ...
            LOGV2(20272, "Failpoint 'pauseCollectionValidationWithLock' activated.");
        ...
        if (results->valid) {
            ...
            if (indexConsistency.haveEntryMismatch()) {
                LOGV2_OPTIONS(20273, {logComponentV1toV2(LogComponent::kIndex)}, "Index inconsistencies were detected on collection {validateState_nss}. Starting the second phase of index validation to gather concise errors.", "validateState_nss"_attr = validateState.nss());
        ...
        if (!results->valid) {
            LOGV2_OPTIONS(20274, {logComponentV1toV2(LogComponent::kIndex)}, "Validation complete for collection {validateState_nss}{uuidString}. Corruption found.", "validateState_nss"_attr = validateState.nss(), "uuidString"_attr = uuidString);
        } else {
            LOGV2_OPTIONS(20275, {logComponentV1toV2(LogComponent::kIndex)}, "Validation complete for collection {validateState_nss}{uuidString}. No corruption found.", "validateState_nss"_attr = validateState.nss(), "uuidString"_attr = uuidString);
        }
