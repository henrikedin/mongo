...
Status IndexBuildBlock::init(OperationContext* opCtx, Collection* collection) {
    ...
    auto indexCatalogEntry =
        collection->getIndexCatalog()->createIndexEntry(opCtx, std::move(descriptor), CreateIndexEntryFlags::kNone);
...
const IndexCatalogEntry* IndexBuildBlock::_indexCatalogEntry(OperationContext* opCtx,
...
IndexCatalogEntry* IndexBuildBlock::_indexCatalogEntry(OperationContext* opCtx, Collection* collection) const {
    auto descriptor = collection->getIndexCatalog()->findIndexByName(
        opCtx, _indexName, true /* includeUnfinishedIndexes */);
