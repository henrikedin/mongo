        ...
        *tightnessOut = IndexBoundsBuilder::EXACT;
        ...
        if ("2dsphere" == elt.valueStringDataSafe()) {
            ...
            *tightnessOut = IndexBoundsBuilder::INEXACT_FETCH;
            ...
            LOGV2_WARNING(20735, "Planner error trying to build geo bounds for {elt_toString} index element.", "elt_toString"_attr = elt.toString());
            verify(0);
        }
    } else {
        LOGV2_WARNING(20736, "Planner error, trying to build bounds for expression: {redact_expr_debugString}", "redact_expr_debugString"_attr = redact(expr->debugString()));
        verify(0);
    }
}
...
void IndexBoundsBuilder::alignBounds(IndexBounds* bounds, const BSONObj& kp, int scanDir) {
    ...
    if (!bounds->isValidFor(kp, scanDir)) {
        LOGV2(20734, "INVALID BOUNDS: {redact_bounds_toString}\nkp = {redact_kp}\nscanDir = {scanDir}", "redact_bounds_toString"_attr = redact(bounds->toString()), "redact_kp"_attr = redact(kp), "scanDir"_attr = scanDir);
        MONGO_UNREACHABLE;
    }
}
