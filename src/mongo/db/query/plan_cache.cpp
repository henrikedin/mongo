...
std::unique_ptr<CachedSolution> PlanCache::getCacheEntryIfActive(const PlanCacheKey& key) const {
    ...
    if (res.state == PlanCache::CacheEntryState::kPresentInactive) {
        LOGV2_DEBUG(20737, 2, "Not using cached entry for {redact_res_cachedSolution_toString} since it is inactive", "redact_res_cachedSolution_toString"_attr = redact(res.cachedSolution->toString()));
        return nullptr;
    }
...
PlanCache::NewEntryState PlanCache::getNewEntryState(const CanonicalQuery& query,
    ...
    NewEntryState res;
    if (!oldEntry) {
        LOGV2_DEBUG(20738, 1, "Creating inactive cache entry for query shape {redact_query_toStringShort} queryHash {unsignedIntToFixedLengthHex_queryHash} planCacheKey {unsignedIntToFixedLengthHex_planCacheKey} with works value {newWorks}", "redact_query_toStringShort"_attr = redact(query.toStringShort()), "unsignedIntToFixedLengthHex_queryHash"_attr = unsignedIntToFixedLengthHex(queryHash), "unsignedIntToFixedLengthHex_planCacheKey"_attr = unsignedIntToFixedLengthHex(planCacheKey), "newWorks"_attr = newWorks);
    ...
    if (oldEntry->isActive && newWorks <= oldEntry->works) {
        ...
        LOGV2_DEBUG(20739, 1, "Replacing active cache entry for query {redact_query_toStringShort} queryHash {unsignedIntToFixedLengthHex_queryHash} planCacheKey {unsignedIntToFixedLengthHex_planCacheKey} with works {oldEntry_works} with a plan with works {newWorks}", "redact_query_toStringShort"_attr = redact(query.toStringShort()), "unsignedIntToFixedLengthHex_queryHash"_attr = unsignedIntToFixedLengthHex(queryHash), "unsignedIntToFixedLengthHex_planCacheKey"_attr = unsignedIntToFixedLengthHex(planCacheKey), "oldEntry_works"_attr = oldEntry->works, "newWorks"_attr = newWorks);
        res.shouldBeCreated = true;
        res.shouldBeActive = true;
    } else if (oldEntry->isActive) {
        LOGV2_DEBUG(20740, 1, "Attempt to write to the planCache for query {redact_query_toStringShort} queryHash {unsignedIntToFixedLengthHex_queryHash} planCacheKey {unsignedIntToFixedLengthHex_planCacheKey} with a plan with works {newWorks} is a noop, since there's already a plan with works value {oldEntry_works}", "redact_query_toStringShort"_attr = redact(query.toStringShort()), "unsignedIntToFixedLengthHex_queryHash"_attr = unsignedIntToFixedLengthHex(queryHash), "unsignedIntToFixedLengthHex_planCacheKey"_attr = unsignedIntToFixedLengthHex(planCacheKey), "newWorks"_attr = newWorks, "oldEntry_works"_attr = oldEntry->works);
        ...
        LOGV2_DEBUG(20741, 1, "Increasing work value associated with cache entry for query {redact_query_toStringShort} queryHash {unsignedIntToFixedLengthHex_queryHash} planCacheKey {unsignedIntToFixedLengthHex_planCacheKey} from {oldEntry_works} to {increasedWorks}", "redact_query_toStringShort"_attr = redact(query.toStringShort()), "unsignedIntToFixedLengthHex_queryHash"_attr = unsignedIntToFixedLengthHex(queryHash), "unsignedIntToFixedLengthHex_planCacheKey"_attr = unsignedIntToFixedLengthHex(planCacheKey), "oldEntry_works"_attr = oldEntry->works, "increasedWorks"_attr = increasedWorks);
        ...
        LOGV2_DEBUG(20742, 1, "Inactive cache entry for query {redact_query_toStringShort} queryHash {unsignedIntToFixedLengthHex_queryHash} planCacheKey {unsignedIntToFixedLengthHex_planCacheKey} with works {oldEntry_works} is being promoted to active entry with works value {newWorks}", "redact_query_toStringShort"_attr = redact(query.toStringShort()), "unsignedIntToFixedLengthHex_queryHash"_attr = unsignedIntToFixedLengthHex(queryHash), "unsignedIntToFixedLengthHex_planCacheKey"_attr = unsignedIntToFixedLengthHex(planCacheKey), "oldEntry_works"_attr = oldEntry->works, "newWorks"_attr = newWorks);
...
Status PlanCache::set(const CanonicalQuery& query,
    ...
    if (nullptr != evictedEntry.get()) {
        LOGV2_DEBUG(20743, 1, "{query_nss}: plan cache maximum size exceeded - removed least recently used entry {redact_evictedEntry_toString}", "query_nss"_attr = query.nss(), "redact_evictedEntry_toString"_attr = redact(evictedEntry->toString()));
    }
