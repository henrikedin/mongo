...
StatusWith<std::unique_ptr<QuerySolution>> QueryPlanner::planFromCache(
    ...
    LOGV2_DEBUG(20764, 5, "Tagging the match expression according to cache data: \nFilter:\n{redact_clone_debugString}Cache data:\n{redact_winnerCacheData_toString}", "redact_clone_debugString"_attr = redact(clone->debugString()), "redact_winnerCacheData_toString"_attr = redact(winnerCacheData.toString()));
    ...
    for (size_t i = 0; i < expandedIndexes.size(); ++i) {
        ...
        LOGV2_DEBUG(20765, 5, "Index {i}: {ie_identifier}", "i"_attr = i, "ie_identifier"_attr = ie.identifier);
    }
    ...
    LOGV2_DEBUG(20766, 5, "Tagged tree:\n{redact_clone_debugString}", "redact_clone_debugString"_attr = redact(clone->debugString()));
    ...
    LOGV2_DEBUG(20767, 5, "Planner: solution constructed from the cache:\n{redact_soln_toString}", "redact_soln_toString"_attr = redact(soln->toString()));
    return {std::move(soln)};
}
...
StatusWith<std::vector<std::unique_ptr<QuerySolution>>> QueryPlanner::plan(
    ...
    LOGV2_DEBUG(20768, 5, "Beginning planning...\n=============================\nOptions = {optionString_params_options}\nCanonical query:\n{redact_query_toString}=============================", "optionString_params_options"_attr = optionString(params.options), "redact_query_toString"_attr = redact(query.toString()));

    for (size_t i = 0; i < params.indices.size(); ++i) {
        LOGV2_DEBUG(20769, 5, "Index {i} is {params_indices_i_toString}", "i"_attr = i, "params_indices_i_toString"_attr = params.indices[i].toString());
    }
    ...
    if (!query.getQueryRequest().getHint().isEmpty()) {
        ...
        if (naturalHint) {
            LOGV2_DEBUG(20770, 5, "Forcing a table scan due to hinted $natural");
            if (!canTableScan) {
                return Status(ErrorCodes::NoQueryExecutionPlans,
    ...
    for (auto&& field : fields) {
        LOGV2_DEBUG(20771, 5, "Predicate over field '{field}'", "field"_attr = field);
    }
    ...
    for (size_t i = 0; i < relevantIndices.size(); ++i) {
        LOGV2_DEBUG(20772, 2, "Relevant index {i} is {relevantIndices_i_toString}", "i"_attr = i, "relevantIndices_i_toString"_attr = relevantIndices[i].toString());
    }
    ...
    LOGV2_DEBUG(20773, 5, "Rated tree:\n{redact_query_root_debugString}", "redact_query_root_debugString"_attr = redact(query.root()->debugString()));
    ...
    if (QueryPlannerCommon::hasNode(query.root(), MatchExpression::GEO_NEAR, &gnNode)) {
        ...
        if (!tag || (0 == tag->first.size() && 0 == tag->notFirst.size())) {
            LOGV2_DEBUG(20774, 5, "Unable to find index for $geoNear query.");
        ...
        LOGV2_DEBUG(20775, 5, "Rated tree after geonear processing:{redact_query_root_debugString}", "redact_query_root_debugString"_attr = redact(query.root()->debugString()));
    }
    ...
    if (QueryPlannerCommon::hasNode(query.root(), MatchExpression::TEXT, &textNode)) {
        ...
        LOGV2_DEBUG(20776, 5, "Rated tree after text processing:{redact_query_root_debugString}", "redact_query_root_debugString"_attr = redact(query.root()->debugString()));
    }
    ...
    if (0 < relevantIndices.size()) {
        ...
        while ((nextTaggedTree = isp.getNext()) && (out.size() < params.maxIndexedSolutions)) {
            LOGV2_DEBUG(20777, 5, "About to build solntree from tagged tree:\n{redact_nextTaggedTree_debugString}", "redact_nextTaggedTree_debugString"_attr = redact(nextTaggedTree->debugString()));
            ...
            if (!statusWithCacheData.isOK()) {
                LOGV2_DEBUG(20778, 5, "Query is not cachable: {redact_statusWithCacheData_getStatus_reason}", "redact_statusWithCacheData_getStatus_reason"_attr = redact(statusWithCacheData.getStatus().reason()));
            } else {
            ...
            auto soln = QueryPlannerAnalysis::analyzeDataAccess(query, params, std::move(solnRoot));
            if (soln) {
                LOGV2_DEBUG(20779, 5, "Planner: adding solution:\n{redact_soln_toString}", "redact_soln_toString"_attr = redact(soln->toString()));
                if (statusWithCacheData.isOK()) {
    ...
    LOGV2_DEBUG(20780, 5, "Planner: outputted {out_size} indexed solutions.", "out_size"_attr = out.size());
    ...
    if (!hintedIndex.isEmpty() && relevantIndices.size() == 1) {
        ...
        LOGV2_DEBUG(20781, 5, "Planner: outputting soln that uses hinted index as scan.");
        std::vector<std::unique_ptr<QuerySolution>> out;
        out.push_back(std::move(soln));
        return {std::move(out)};
    }
        ...
        if (!usingIndexToSort) {
            for (size_t i = 0; i < fullIndexList.size(); ++i) {
                ...
                if (providesSort(query, kp)) {
                    LOGV2_DEBUG(20782, 5, "Planner: outputting soln that uses index to provide sort.");
                ...
                if (providesSort(query, QueryPlannerCommon::reverseSortObj(kp))) {
                    LOGV2_DEBUG(20783, 5, "Planner: outputting soln that uses (reverse) index to provide sort.");
    ...
    if (params.options & QueryPlannerParams::GENERATE_COVERED_IXSCANS && out.size() == 0 &&
        ...
        for (auto&& index : *indicesToConsider) {
            ...
            if (soln && !soln->root->fetched()) {
                LOGV2_DEBUG(20784, 5, "Planner: outputting soln that uses index to provide projection.");
                PlanCacheIndexTree* indexTree = new PlanCacheIndexTree();
    ...
    if (possibleToCollscan && (collscanRequested || collScanRequired)) {
        ...
        if (collscan) {
            LOGV2_DEBUG(20785, 5, "Planner: outputting a collscan:\n{redact_collscan_toString}", "redact_collscan_toString"_attr = redact(collscan->toString()));
            SolutionCacheData* scd = new SolutionCacheData();
