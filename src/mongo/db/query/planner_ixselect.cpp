    ...
    if (firstHintElt.fieldNameStringData() == "$hint"_sd &&
        ...
        for (auto&& entry : allIndices) {
            if (entry.identifier.catalogName == hintName) {
                LOGV2_DEBUG(20753, 5, "Hint by name specified, restricting indices to {entry_keyPattern_toString}", "entry_keyPattern_toString"_attr = entry.keyPattern.toString());
                out.push_back(entry);
            }
        }
    } else {
        for (auto&& entry : allIndices) {
            if (SimpleBSONObjComparator::kInstance.evaluate(entry.keyPattern == hintedIndex)) {
                LOGV2_DEBUG(20754, 5, "Hint specified, restricting indices to {hintedIndex_toString}", "hintedIndex_toString"_attr = hintedIndex.toString());
                out.push_back(entry);
            }
        }
    }
...
bool QueryPlannerIXSelect::_compatible(const BSONElement& keyPatternElt,
    ...
    if (indexedFieldType.empty()) {
        ...
        LOGV2_WARNING(20755, "Unknown indexing for node {node_debugString} and field {keyPatternElt_toString}", "node_debugString"_attr = node->debugString(), "keyPatternElt_toString"_attr = keyPatternElt.toString());
        verify(0);
    }
}
