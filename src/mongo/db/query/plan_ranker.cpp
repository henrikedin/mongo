...
StatusWith<std::unique_ptr<PlanRankingDecision>> PlanRanker::pickBestPlan(
    ...
    for (size_t i = 0; i < statTrees.size(); ++i) {
        if (!candidates[i].failed) {
            LOGV2_DEBUG(20757, 5, "Scoring plan {i}:\n{redact_candidates_i_solution_toString}Stats:\n{redact_Explain_statsToBSON_statTrees_i_jsonString_ExtendedRelaxedV2_0_0_true}", "i"_attr = i, "redact_candidates_i_solution_toString"_attr = redact(candidates[i].solution->toString()), "redact_Explain_statsToBSON_statTrees_i_jsonString_ExtendedRelaxedV2_0_0_true"_attr = redact(Explain::statsToBSON(*statTrees[i])
                                 .jsonString(ExtendedRelaxedV2_0_0, true)));
            LOGV2_DEBUG(20758, 2, "Scoring query plan: {Explain_getPlanSummary_candidates_i_root} planHitEOF={statTrees_i_common_isEOF}", "Explain_getPlanSummary_candidates_i_root"_attr = Explain::getPlanSummary(candidates[i].root), "statTrees_i_common_isEOF"_attr = statTrees[i]->common.isEOF);

            double score = scoreTree(statTrees[i].get());
            LOGV2_DEBUG(20759, 5, "score = {score}", "score"_attr = score);
            if (statTrees[i]->common.isEOF) {
                LOGV2_DEBUG(20760, 5, "Adding +{eofBonus} EOF bonus to score.", "eofBonus"_attr = eofBonus);
                score += 1;
            }
            ...
            failed.push_back(i);
            LOGV2_DEBUG(20761, 2, "Not scording plan: {Explain_getPlanSummary_candidates_i_root} because the plan failed.", "Explain_getPlanSummary_candidates_i_root"_attr = Explain::getPlanSummary(candidates[i].root));
        }
    }
...
double PlanRanker::scoreTree(const PlanStageStats* stats) {
    ...
    LOGV2_DEBUG(20762, 2, "{sb_str}", "sb_str"_attr = sb.str());
    ...
    if (internalQueryForceIntersectionPlans.load()) {
        if (hasStage(STAGE_AND_HASH, stats) || hasStage(STAGE_AND_SORTED, stats)) {
            ...
            score += 3;
            LOGV2_DEBUG(20763, 5, "Score boosted to {score} due to intersection forcing.", "score"_attr = score);
        }
    }
