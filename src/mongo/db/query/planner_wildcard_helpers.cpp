 ...
 */
std::vector<size_t> findArrayIndexPathComponents(const boost::container::flat_set<std::size_t>& multikeyPaths,
...
std::set<FieldRef> generateFieldNameOrArrayIndexPathSet(const boost::container::flat_set<std::size_t>& multikeyPaths,
                                                        const FieldRef& queryPath,
...
void finalizeWildcardIndexScanConfiguration(IndexScanNode* scan) {
    ...
    index->multikeyPaths.insert(index->multikeyPaths.begin(), boost::container::flat_set<std::size_t>{});
