...
PlanEnumerator::MemoID PlanEnumerator::memoIDForNode(MatchExpression* node) {
    ...
    if (_nodeToId.end() == it) {
        LOGV2_ERROR(20746, "Trying to look up memo entry for node, none found.");
        MONGO_UNREACHABLE;
    }
...
unique_ptr<MatchExpression> PlanEnumerator::getNext() {
    ...
    _root->resetTag();
    LOGV2_DEBUG(20744, 5, "Enumerator: memo just before moving:\n{dumpMemo}", "dumpMemo"_attr = dumpMemo());
...
void PlanEnumerator::tagMemo(size_t id) {
    LOGV2_DEBUG(20745, 5, "Tagging memoID {id}", "id"_attr = id);
