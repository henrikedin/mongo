...
Status restoreMissingFeatureCompatibilityVersionDocument(OperationContext* opCtx,
    ...
    if (!db) {
        LOGV2(20798, "Re-creating admin database that was dropped.");
    }
    ...
    if (!CollectionCatalog::get(opCtx).lookupCollectionByNamespace(
        ...
        LOGV2(20799, "Re-creating the server configuration collection (admin.system.version) that was "
                 "dropped.");
        uassertStatusOK(
    ...
    if (!Helpers::findOne(opCtx,
        ...
        LOGV2(20800, "Re-creating featureCompatibilityVersion document that was deleted with version {FeatureCompatibilityVersionParser_kVersion42}.", "FeatureCompatibilityVersionParser_kVersion42"_attr = FeatureCompatibilityVersionParser::kVersion42);
    ...
    for (const auto& dbName : dbNames) {
        ...
        for (auto collIt = db->begin(opCtx); collIt != db->end(opCtx); ++collIt) {
            ...
            if (requiresIndex && !hasAutoIndexIdField &&
                ...
                if (!status.isOK()) {
                    LOGV2_ERROR(20819, "could not build an _id index on collection {coll_ns}: {status}", "coll_ns"_attr = coll->ns(), "status"_attr = status);
...
void rebuildIndexes(OperationContext* opCtx, StorageEngine* storageEngine) {
    ...
    if (!reconcileResult.indexesToRebuild.empty() && serverGlobalParams.indexBuildRetry) {
        LOGV2(20801, "note: restart the server with --noIndexBuildRetry to skip index rebuilds");
    }

    if (!serverGlobalParams.indexBuildRetry) {
        LOGV2(20802, "  not rebuilding interrupted indexes");
        return;
    }
    ...
    for (const auto& entry : nsToIndexNameObjMap) {
        ...
        for (const auto& indexName : entry.second.first) {
            LOGV2(20803, "Rebuilding index. Collection: {collNss} Index: {indexName}", "collNss"_attr = collNss, "indexName"_attr = indexName);
        }
...
bool repairDatabasesAndCheckVersion(OperationContext* opCtx) {
    ...
    if (storageGlobalParams.repair) {
        ...
        if (MONGO_unlikely(exitBeforeDataRepair.shouldFail())) {
            LOGV2(20804, "Exiting because 'exitBeforeDataRepair' fail point was set.");
        ...
        for (const auto& dbName : dbNames) {
            LOGV2_DEBUG(20805, 1, "    Repairing database: {dbName}", "dbName"_attr = dbName);
    ...
    if (storageGlobalParams.repair) {
        if (MONGO_unlikely(exitBeforeRepairInvalidatesConfig.shouldFail())) {
            LOGV2(20806, "Exiting because 'exitBeforeRepairInvalidatesConfig' fail point was set.");
            quickExit(EXIT_ABRUPT);
        }
        ...
        if (repairObserver->getModifications().size() > 0) {
            LOGV2_WARNING(20816, "Modifications made by repair:");
            const auto& mods = repairObserver->getModifications();
            for (const auto& mod : mods) {
                LOGV2_WARNING(20817, "  {mod_getDescription}", "mod_getDescription"_attr = mod.getDescription());
            }
        }
        if (repairObserver->isDataInvalidated()) {
            if (hasReplSetConfigDoc(opCtx)) {
                LOGV2_WARNING(20818, "WARNING: Repair may have modified replicated data. This node will no "
        ...
        if (repairObserver->getModifications().size() > 0 && !repairObserver->isDataInvalidated()) {
            LOGV2(20807, "Repair has made modifications to unreplicated data. The data is healthy and "
    ...
    for (const auto& dbName : dbNames) {
        ...
        LOGV2_DEBUG(20808, 1, "    Recovering database: {dbName}", "dbName"_attr = dbName);
        ...
        if (!status.isOK()) {
            ...
            LOGV2_FATAL(20820, "Unable to start mongod due to an incompatibility with the data files and"
                        " this version of mongod: {redact_status}", "redact_status"_attr = redact(status));
            LOGV2_FATAL(20821, "Please consult our documentation when trying to downgrade to a previous"
                        " major release");
            ...
            if (Collection* versionColl = CollectionCatalog::get(opCtx).lookupCollectionByNamespace(
                ...
                if (Helpers::findOne(
                    ...
                    if (version ==
                        ...
                        LOGV2_OPTIONS(20809, {logv2::LogTag::kStartupWarnings}, "** WARNING: A featureCompatibilityVersion upgrade did not complete. ");
                        LOGV2_OPTIONS(20810, {logv2::LogTag::kStartupWarnings}, "**          The current featureCompatibilityVersion is {FeatureCompatibilityVersionParser_toString_version}.", "FeatureCompatibilityVersionParser_toString_version"_attr = FeatureCompatibilityVersionParser::toString(version));
                        LOGV2_OPTIONS(20811, {logv2::LogTag::kStartupWarnings}, "**          To fix this, use the setFeatureCompatibilityVersion command to resume upgrade to 4.4.");
                    } else if (version ==
                               ServerGlobalParams::FeatureCompatibility::Version::
                                   kDowngradingTo42) {
                        LOGV2_OPTIONS(20812, {logv2::LogTag::kStartupWarnings}, "** WARNING: A featureCompatibilityVersion downgrade did not complete. ");
                        LOGV2_OPTIONS(20813, {logv2::LogTag::kStartupWarnings}, "**          The current featureCompatibilityVersion is {FeatureCompatibilityVersionParser_toString_version}.", "FeatureCompatibilityVersionParser_toString_version"_attr = FeatureCompatibilityVersionParser::toString(version));
                        LOGV2_OPTIONS(20814, {logv2::LogTag::kStartupWarnings}, "**          To fix this, use the setFeatureCompatibilityVersion command to resume downgrade to 4.2.");
                    }
                }
            }
        }
    ...
    if (!fcvDocumentExists && nonLocalDatabases && !needInitialSync) {
        LOGV2_FATAL(20822, "Unable to start up mongod due to missing featureCompatibilityVersion document.");
        LOGV2_FATAL(20823, "Please run with --repair to restore the document.");
        fassertFailedNoTrace(40652);
    }

    LOGV2_DEBUG(20815, 1, "done repairDatabases");
