...
bool repairDatabasesAndCheckVersion(OperationContext* opCtx) {
    ...
    for (const auto& dbName : dbNames) {
        ...
        if (!status.isOK()) {
            ...
            LOGV2_ERROR("Unable to start mongod due to an incompatibility with the data files and"
    ...
    if (!fcvDocumentExists && nonLocalDatabases && !needInitialSync) {
        LOGV2_ERROR("Unable to start up mongod due to missing featureCompatibilityVersion document.");
        LOGV2_FATAL(40652, "Please run with --repair to restore the document.");
