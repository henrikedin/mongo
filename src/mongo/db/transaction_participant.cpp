...
void fassertOnRepeatedExecution(const LogicalSessionId& lsid,
    ...
    LOGV2_FATAL(22232, "Statement id {stmtId} from transaction [ {lsid_toBSON}:{txnNumber} ] was committed once with opTime {firstOpTime} and a second time with opTime {secondOpTime}. This indicates possible data corruption or server bug and the process will be "
...
TransactionParticipant::OplogSlotReserver::~OplogSlotReserver() {
    if (MONGO_unlikely(hangBeforeReleasingTransactionOplogHole.shouldFail())) {
        LOGV2(22228, "transaction - hangBeforeReleasingTransactionOplogHole fail point enabled. Blocking "
...
Timestamp TransactionParticipant::Participant::prepareTransaction(
    ...
    auto abortGuard = makeGuard([&] {
        ...
        try {
            ...
            // instead.
            LOGV2_FATAL(22233, "Caught exception during abort of prepared transaction {opCtx_getTxnNumber} on {sessionId_toBSON}: {exceptionToStatus}", "opCtx_getTxnNumber"_attr = opCtx->getTxnNumber(), "sessionId_toBSON"_attr = _sessionId().toBSON(), "exceptionToStatus"_attr = exceptionToStatus());
            std::terminate();
        }
    });
    ...
    if (prepareOptime) {
        ...
        if (MONGO_unlikely(hangAfterReservingPrepareTimestamp.shouldFail())) {
            ...
            LOGV2(22229, "transaction - hangAfterReservingPrepareTimestamp fail point "
                     "enabled. Blocking until fail point is disabled. Prepare OpTime: {prepareOplogSlot}", "prepareOplogSlot"_attr = prepareOplogSlot);
    ...
    if (MONGO_unlikely(hangAfterSettingPrepareStartTime.shouldFail())) {
        LOGV2(22230, "transaction - hangAfterSettingPrepareStartTime fail point enabled. Blocking "
                 "until fail point is disabled.");
...
void TransactionParticipant::Participant::commitPreparedTransaction(
    ...
    try {
        ...
        LOGV2_FATAL(22234, "Caught exception during commit of prepared transaction {opCtx_getTxnNumber} on {sessionId_toBSON}: {exceptionToStatus}", "opCtx_getTxnNumber"_attr = opCtx->getTxnNumber(), "sessionId_toBSON"_attr = _sessionId().toBSON(), "exceptionToStatus"_attr = exceptionToStatus());
        std::terminate();
    }
}
...
void TransactionParticipant::Participant::_abortActiveTransaction(
    ...
    if (needToWriteAbortEntry) {
        ...
        try {
            ...
            LOGV2_FATAL(22235, "Caught exception during abort of transaction that must write abort oplog entry {opCtx_getTxnNumber} on {sessionId_toBSON}: {exceptionToStatus}", "opCtx_getTxnNumber"_attr = opCtx->getTxnNumber(), "sessionId_toBSON"_attr = _sessionId().toBSON(), "exceptionToStatus"_attr = exceptionToStatus());
            std::terminate();
        }
    } else {
        // Clean up the transaction resources on the opCtx even if the transaction resources on the
...
void TransactionParticipant::Participant::_logSlowTransaction(
    ...
    if (!o().txnState.isInRetryableWriteMode()) {
        ...
        if (shouldLog(logger::LogComponent::kTransaction, logger::LogSeverity::Debug(1)) ||
            ...
            LOGV2_OPTIONS(22231, {logComponentV1toV2(logger::LogComponent::kTransaction)}, "transaction {transactionInfoForLog_opCtx_lockStats_terminationCause_readConcernArgs}", "transactionInfoForLog_opCtx_lockStats_terminationCause_readConcernArgs"_attr = _transactionInfoForLog(opCtx, lockStats, terminationCause, readConcernArgs));
        }
    }
}
