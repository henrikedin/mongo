...
public:
    ...
    virtual void run() {
        ...
        while (!globalInShutdownDeprecated()) {
            ...
            LOGV2_DEBUG(22236, 3, "thread awake");

            if (!ttlMonitorEnabled.load()) {
                LOGV2_DEBUG(22237, 1, "disabled");
                continue;
            }
            ...
            if (lockedForWriting()) {
                ...
                LOGV2_DEBUG(22238, 3, "locked for writing");
                continue;
            }
            ...
            try {
                ...
                LOGV2_DEBUG(22239, 1, "got WriteConflictException");
            } catch (const ExceptionForCat<ErrorCategory::Interruption>& interruption) {
                LOGV2_DEBUG(22240, 1, "TTLMonitor was interrupted: {interruption}", "interruption"_attr = interruption);
            }
        }
    }
...
private:
    void doTTLPass() {
        ...
        for (const auto& it : ttlIndexes) {
            try {
                ...
                LOGV2_WARNING(22245, "TTLMonitor was interrupted, waiting {ttlMonitorSleepSecs_load} seconds before doing another pass", "ttlMonitorSleepSecs_load"_attr = ttlMonitorSleepSecs.load());
                return;
            } catch (const DBException& dbex) {
                LOGV2_ERROR(22246, "Error processing ttl index: {it_second} -- {dbex_toString}", "it_second"_attr = it.second, "dbex_toString"_attr = dbex.toString());
    ...
    void doTTLForIndex(OperationContext* opCtx, NamespaceString collectionNSS, BSONObj idx) {
        ...
        if (!userAllowedWriteNS(collectionNSS).isOK()) {
            LOGV2_ERROR(22247, "namespace '{collectionNSS}' doesn't allow deletes, skipping ttl job for: {idx}", "collectionNSS"_attr = collectionNSS, "idx"_attr = idx);
            return;
        }
        ...
        if (key.nFields() != 1) {
            LOGV2_ERROR(22248, "key for ttl index can only have 1 field, skipping ttl job for: {idx}", "idx"_attr = idx);
            return;
        }

        LOGV2_DEBUG(22241, 1, "ns: {collectionNSS} key: {key} name: {name}", "collectionNSS"_attr = collectionNSS, "key"_attr = key, "name"_attr = name);

        AutoGetCollection autoGetCollection(opCtx, collectionNSS, MODE_IX);
        if (MONGO_unlikely(hangTTLMonitorWithLock.shouldFail())) {
            LOGV2(22242, "Hanging due to hangTTLMonitorWithLock fail point");
        ...
        if (!desc) {
            LOGV2_DEBUG(22243, 1, "index not found (index build in progress? index dropped?), skipping ttl job for: {idx}", "idx"_attr = idx);
            return;
        }
        ...
        if (IndexType::INDEX_BTREE != IndexNames::nameToType(desc->getAccessMethodName())) {
            LOGV2_ERROR(22249, "special index can't be used as a ttl index, skipping ttl job for: {idx}", "idx"_attr = idx);
            return;
        }

        BSONElement secondsExpireElt = idx[secondsExpireField];
        if (!secondsExpireElt.isNumber()) {
            LOGV2_ERROR(22250, "ttl indexes require the {secondsExpireField} field to be numeric but received a type of {typeName_secondsExpireElt_type}, skipping ttl job for: {idx}", "secondsExpireField"_attr = secondsExpireField, "typeName_secondsExpireElt_type"_attr = typeName(secondsExpireElt.type()), "idx"_attr = idx);
            return;
        }
        ...
        if (!result.isOK()) {
            LOGV2_ERROR(22251, "ttl query execution for index {idx} failed with status: {redact_result}", "idx"_attr = idx, "redact_result"_attr = redact(result));
            return;
        }

        const long long numDeleted = DeleteStage::getNumDeleted(*exec);
        ttlDeletedDocuments.increment(numDeleted);
        LOGV2_DEBUG(22244, 1, "deleted: {numDeleted}", "numDeleted"_attr = numDeleted);
    }
