...
private:
    void doTTLPass() {
        ...
        for (const auto& it : ttlIndexes) {
            try {
                ...
                LOGV2_WARNING("TTLMonitor was interrupted, waiting {} seconds before doing another pass", "ttlMonitorSleepSecs_load"_attr = ttlMonitorSleepSecs.load());
                return;
            } catch (const DBException& dbex) {
                LOGV2_ERROR("Error processing ttl index: {} -- {}", "it_second"_attr = it.second, "dbex_toString"_attr = dbex.toString());
    ...
    void doTTLForIndex(OperationContext* opCtx, NamespaceString collectionNSS, BSONObj idx) {
        ...
        if (!userAllowedWriteNS(collectionNSS).isOK()) {
            LOGV2_ERROR("namespace '{}' doesn't allow deletes, skipping ttl job for: {}", "collectionNSS"_attr = collectionNSS, "idx"_attr = idx);
            return;
        }
        ...
        if (key.nFields() != 1) {
            LOGV2_ERROR("key for ttl index can only have 1 field, skipping ttl job for: {}", "idx"_attr = idx);
            return;
        }
        ...
        if (IndexType::INDEX_BTREE != IndexNames::nameToType(desc->getAccessMethodName())) {
            LOGV2_ERROR("special index can't be used as a ttl index, skipping ttl job for: {}", "idx"_attr = idx);
            return;
        }

        BSONElement secondsExpireElt = idx[secondsExpireField];
        if (!secondsExpireElt.isNumber()) {
            LOGV2_ERROR("ttl indexes require the {} field to be numeric but received a type of {}, skipping ttl job for: {}", "secondsExpireField"_attr = secondsExpireField, "typeName_secondsExpireElt_type"_attr = typeName(secondsExpireElt.type()), "idx"_attr = idx);
            return;
        }
        ...
        if (!result.isOK()) {
            LOGV2_ERROR("ttl query execution for index {} failed with status: {}", "idx"_attr = idx, "redact_result"_attr = redact(result));
            return;
        }
