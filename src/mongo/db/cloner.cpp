        ...
        while (i.moreInCurrentBatch()) {
            ...
            writeConflictRetry(opCtx, "cloner insert", to_collection.ns(), [&] {
                ...
                if (!status.isOK() && status.code() != ErrorCodes::DuplicateKey) {
                    LOGV2_ERROR("error: exception cloning object in {} {} obj:{}", "from_collection"_attr = from_collection, "redact_status"_attr = redact(status), "redact_doc"_attr = redact(doc));
...
void Cloner::copy(OperationContext* opCtx,
                  ...
                  Query query) {
    LOGV2_DEBUG(2, "\t\tcloning collection {} to {} on {} with filter {}", "from_collection"_attr = from_collection, "to_collection"_attr = to_collection, "conn_getServerAddress"_attr = _conn->getServerAddress(), "redact_query_toString"_attr = redact(query.toString()));
...
void Cloner::copyIndexes(OperationContext* opCtx,
                         ...
                         const NamespaceString& to_collection) {
    LOGV2_DEBUG(2, "\t\t copyIndexes {} to {} on {}", "from_collection"_attr = from_collection, "to_collection"_attr = to_collection, "conn_getServerAddress"_attr = _conn->getServerAddress());
...
bool Cloner::copyCollection(OperationContext* opCtx,
    ...
    if (shouldCreateCollection) {
        ...
        LOGV2_DEBUG(1, "No collection info found for ns:{}, host:{}", "nss_toString"_attr = nss.toString(), "conn_getServerAddress"_attr = _conn->getServerAddress());
    }
