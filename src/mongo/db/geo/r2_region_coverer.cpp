...
void R2RegionCoverer::getCovering(const R2Region& region, vector<GeoHash>* cover) {
    ...
    while (!_candidateQueue->empty()) {
        ...
        LOGV2_DEBUG(20593, 3, "Pop: {redact_candidate_cell_toString}", "redact_candidate_cell_toString"_attr = redact(candidate->cell.toString()));
        ...
        LOGV2_DEBUG(20594, 3, "Queue: {candidateQueue_size}", "candidateQueue_size"_attr = _candidateQueue->size());
    }
...
void R2RegionCoverer::addCandidate(Candidate* candidate) {
    ...
    if (candidate->numChildren == 0) {
        ...
        // REDACT??
        LOGV2_DEBUG(20595, 3, "Push: {redact_candidate_cell_toString} ({priority}) ", "redact_candidate_cell_toString"_attr = redact(candidate->cell.toString()), "priority"_attr = priority);
    }
}
