...
void CurOp::setMessage_inlock(StringData message) {
    if (_progressMeter.isActive()) {
        LOGV2_ERROR("old _message: {} new message:{}", "redact_message"_attr = redact(_message), "redact_message"_attr = redact(message));
...
bool CurOp::completeAndLogOperation(OperationContext* opCtx,
    ...
    if (shouldLogOp || (shouldSample && _debug.executionTimeMicros > slowMs * 1000LL)) {
        ...
        LOGV2_OPTIONS({logComponentV1toV2(component)}, "{}", "debug_report_opCtx_lockerInfo_lockerInfo_stats_nullptr"_attr = _debug.report(opCtx, (lockerInfo ? &lockerInfo->stats : nullptr)));
    }
