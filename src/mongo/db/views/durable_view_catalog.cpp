...
void DurableViewCatalogImpl::upsert(OperationContext* opCtx,
    ...
    if (!id.isValid() || !systemViews->findDoc(opCtx, id, &oldView)) {
        LOGV2_DEBUG(2, "insert view {} into {}", "view"_attr = view, "db_getSystemViewsName"_attr = _db->getSystemViewsName());
        uassertStatusOK(
...
void DurableViewCatalogImpl::remove(OperationContext* opCtx, const NamespaceString& name) {
    ...
    LOGV2_DEBUG(2, "remove view {} from {}", "name"_attr = name, "db_getSystemViewsName"_attr = _db->getSystemViewsName());
