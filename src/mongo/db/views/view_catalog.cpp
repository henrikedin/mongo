...
Status ViewCatalog::_reload(WithLock,
    ...
    LOGV2_DEBUG(1, "reloading view catalog for database {}", "durable_getName"_attr = _durable->getName());
    ...
    try {
        ...
        LOGV2("could not load view catalog for database {}: {}", "durable_getName"_attr = _durable->getName(), "status"_attr = status);
        return status;
    }
