...
Status ViewCatalog::_reload(WithLock,
    ...
    LOGV2_DEBUG(22254, 1, "reloading view catalog for database {durable_getName}", "durable_getName"_attr = _durable->getName());
    ...
    try {
        ...
        LOGV2(22255, "could not load view catalog for database {durable_getName}: {status}", "durable_getName"_attr = _durable->getName(), "status"_attr = status);
        return status;
    }
