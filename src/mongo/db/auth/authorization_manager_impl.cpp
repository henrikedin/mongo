...
Status AuthorizationManagerImpl::getAuthorizationVersion(OperationContext* opCtx, int* version) {
    ...
    if (schemaVersionInvalid == newVersion) {
        ...
        guard.endFetchPhase();
        if (!status.isOK()) {
            LOGV2_WARNING("Problem fetching the stored schema version of authorization data: {}", "redact_status"_attr = redact(status));
        ...
        for (const auto& userName : usersToPin) {
            ...
            if (swUser.isOK()) {
                ...
                if (status != ErrorCodes::UserNotFound) {
                    LOGV2_WARNING("Unable to fetch pinned user {}: {}", "userName_toString"_attr = userName.toString(), "status"_attr = status);
                } else {
