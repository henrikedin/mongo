...
public:
    ...
    boost::optional<User::CredentialData> generate(const std::string& password) {
        if (password.size() < kMinKeyLength || password.size() > kMaxKeyLength) {
            LOGV2_ERROR(" security key in {} has length {}, must be between 6 and 1024 chars", "_filename"_attr = _filename, "password_size"_attr = password.size());
            return boost::none;
        }

        auto swSaslPassword = icuSaslPrep(password);
        if (!swSaslPassword.isOK()) {
            LOGV2_ERROR("Could not prep security key file for SCRAM-SHA-256: {}", "swSaslPassword_getStatus"_attr = swSaslPassword.getStatus());
            return boost::none;
        }
...
private:
    ...
    bool _copyCredentials(CredsTarget&& target, const CredsSource&& source) {
        ...
        if (!target.isValid()) {
            LOGV2_ERROR("Could not generate valid credentials from key in {}", "_filename"_attr = _filename);
            return false;
        }
...
bool setUpSecurityKey(const string& filename) {
    ...
    if (keyStrings.size() > 2) {
        LOGV2_ERROR("Only two keys are supported in the security key file, {} are specified in {}", "keyStrings_size"_attr = keyStrings.size(), "filename"_attr = filename);
        return false;
    }
