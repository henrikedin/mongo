...
public:
    ...
    boost::optional<User::CredentialData> generate(const std::string& password) {
        if (password.size() < kMinKeyLength || password.size() > kMaxKeyLength) {
            LOGV2_ERROR(" security key in {} has length {}, must be between 6 and 1024 chars", "filename"_attr = _filename, "password_size"_attr = password.size());
            return boost::none;
        }
...
private:
    ...
    bool _copyCredentials(CredsTarget&& target, const CredsSource&& source) {
        ...
        if (!target.isValid()) {
            LOGV2_ERROR("Could not generate valid credentials from key in {}", "filename"_attr = _filename);
            return false;
        }
