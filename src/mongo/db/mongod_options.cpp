...
void sysRuntimeInfo() {
    ...
    LOGV2("  page size: {}", "int_sysconf_SC_PAGE_SIZE"_attr = (int)sysconf(_SC_PAGE_SIZE));
#endif
#if defined(_SC_PHYS_PAGES)
    LOGV2("  _SC_PHYS_PAGES: {}", "sysconf_SC_PHYS_PAGES"_attr = sysconf(_SC_PHYS_PAGES));
#endif
#if defined(_SC_AVPHYS_PAGES)
    LOGV2("  _SC_AVPHYS_PAGES: {}", "sysconf_SC_AVPHYS_PAGES"_attr = sysconf(_SC_AVPHYS_PAGES));
#endif
}
}  // namespace
...
bool handlePreValidationMongodOptions(const moe::Environment& params,
    ...
    if (params.count("master") || params.count("slave")) {
        LOGV2_ERROR("Master/slave replication is no longer supported");
        return false;
    }
...
Status storeMongodOptions(const moe::Environment& params) {
    ...
    if (params.count("sharding.clusterRole")) {
        ...
        if (clusterRoleParam == "configsvr") {
            ...
            if (params.count("replication.enableMajorityReadConcern") &&
                !params["replication.enableMajorityReadConcern"].as<bool>()) {
                LOGV2_WARNING("Config servers require majority read concern, but it was explicitly "
    ...
    if (sizeof(void*) == 4 && !params.count("storage.journal.enabled")) {
        ...
        LOGV2("");
        LOGV2_WARNING("32-bit servers don't have journaling enabled by default. Please use --journal if you want durability.");
        LOGV2("");
    }
