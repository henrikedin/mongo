...
StatusWithMatchExpression JSONSchemaParser::parse(
    ...
    BSONObj schema,
    bool ignoreUnknownKeywords) {
    LOGV2_DEBUG(20675, 5, "Parsing JSON Schema: {schema_jsonString_JsonStringFormat_LegacyStrict}", "schema_jsonString_JsonStringFormat_LegacyStrict"_attr = schema.jsonString(JsonStringFormat::LegacyStrict));
    try {
        auto translation = _parse(expCtx, ""_sd, schema, ignoreUnknownKeywords);
        if (shouldLog(logger::LogSeverity::Debug(5)) && translation.isOK()) {
            LOGV2_DEBUG(20676, 5, "Translated schema match expression: {translation_getValue_debugString}", "translation_getValue_debugString"_attr = translation.getValue()->debugString());
        }
        return translation;
