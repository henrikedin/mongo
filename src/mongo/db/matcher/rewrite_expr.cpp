...
RewriteExpr::RewriteResult RewriteExpr::rewrite(const boost::intrusive_ptr<Expression>& expression,
    ...
    LOGV2_DEBUG(20672, 5, "Expression prior to rewrite: {expression_serialize_false}", "expression_serialize_false"_attr = expression->serialize(false));
    ...
    if (auto matchTree = rewriteExpr._rewriteExpression(expression)) {
        ...
        LOGV2_DEBUG(20673, 5, "Post-rewrite MatchExpression: {matchExpression_debugString}", "matchExpression_debugString"_attr = matchExpression->debugString());
        matchExpression = MatchExpression::optimize(std::move(matchExpression));
        LOGV2_DEBUG(20674, 5, "Post-rewrite/post-optimized MatchExpression: {matchExpression_debugString}", "matchExpression_debugString"_attr = matchExpression->debugString());
    }
