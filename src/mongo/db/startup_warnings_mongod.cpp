...
void logMongodStartupWarnings(const StorageGlobalParams& storageParams,
    ...
    if (hasMultipleNumaNodes) {
        ...
        if (f.is_open()) {
            ...
            std::getline(f, line);
            if (f.fail()) {
                LOGV2_WARNING_OPTIONS({logv2::LogTag::kStartupWarnings}, "failed to read from /proc/self/numa_maps: {}", "errnoWithDescription"_attr = errnoWithDescription());
                warned = true;
            } else {
                // skip over pointer
    ...
    if (transparentHugePagesEnabledResult.isOK()) {
        if (transparentHugePagesEnabledResult.getValue() == "always") {
            ...
            warned = true;
               ...
               ErrorCodes::NonExistentPath) {
        LOGV2_WARNING_OPTIONS({logv2::LogTag::kStartupWarnings}, "");
        LOGV2_WARNING_OPTIONS({logv2::LogTag::kStartupWarnings}, "{}", "transparentHugePagesEnabledResult_getStatus_reason"_attr = transparentHugePagesEnabledResult.getStatus().reason());
        warned = true;
    }
    ...
    if (transparentHugePagesDefragResult.isOK()) {
        ...
        LOGV2_WARNING_OPTIONS({logv2::LogTag::kStartupWarnings}, "");
        LOGV2_WARNING_OPTIONS({logv2::LogTag::kStartupWarnings}, "{}", "transparentHugePagesDefragResult_getStatus_reason"_attr = transparentHugePagesDefragResult.getStatus().reason());
        warned = true;
    }
#endif  // __linux__
