...
void PeriodicThreadToDecreaseSnapshotHistoryCachePressure::_init(ServiceContext* serviceContext) {
        ...
        [](Client* client) {
            try {
                ...
                if (!ErrorCodes::isShutdownError(ex.toStatus().code())) {
                    LOGV2_WARNING(20697, "Periodic task to check for and decrease cache pressure caused by "
                                 "maintaining too much snapshot history failed! Caused by: {ex_toStatus}", "ex_toStatus"_attr = ex.toStatus());
                }
            }
        },
    ...
    SnapshotWindowParams::observeCheckCachePressurePeriodSeconds.addObserver([anchor = _anchor](
        ...
        try {
            ...
            LOGV2(20696, "Failed to update the period of the thread which decreases data history cache "
                     "target size if there is cache pressure.{ex_toStatus}", "ex_toStatus"_attr = ex.toStatus());
        }
    });
}
