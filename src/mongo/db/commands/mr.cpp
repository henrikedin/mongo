...

    {
        ...
        if (auto mapParamsElem = cmdObj["mapparams"]) {
            if (mapParamsDeprecationSampler.tick()) {
                LOGV2_WARNING(20460, "The mapparams option to MapReduce is deprecated.");
            }
...
State::~State() {
    if (_onDisk) {
        try {
            ...
            LOGV2_ERROR(20461, "Unable to drop temporary collection created by mapReduce: {config_tempNamespace}. This collection will be removed automatically "
                       "the next time the server starts up. {exceptionToStatus}", "config_tempNamespace"_attr = _config.tempNamespace, "exceptionToStatus"_attr = exceptionToStatus());
        }
    }
    if (_scope && !_scope->isKillPending() && _scope->getError().empty()) {
        ...
        try {
            ...
            LOGV2_DEBUG(20450, 1, "MapReduce terminated during state destruction");
        }
    }
}
...
void State::bailFromJS() {
    LOGV2_DEBUG(20451, 1, "M/R: Switching from JS mode to mixed mode");
...
void State::reduceAndSpillInMemoryStateIfNeeded() {
    ...
    if (_jsMode) {
        ...
        if (keyCt > _config.jsMaxKeys) {
            ...
            LOGV2_DEBUG(20452, 3, "  MR - did reduceAll: keys={keyCt} dups={dupCt} newKeys={scope_getNumberInt_keyCt} time={t_millis}ms", "keyCt"_attr = keyCt, "dupCt"_attr = dupCt, "scope_getNumberInt_keyCt"_attr = _scope->getNumberInt("_keyCt"), "t_millis"_attr = t.millis());
            return;
        }
    }
    ...
    if (_size > _config.maxInMemSize || _dupCount > (_temp->size() * _config.reduceTriggerRatio)) {
        ...
        Timer t;
        reduceInMemory();
        LOGV2_DEBUG(20453, 3, "  MR - did reduceInMemory: size={oldSize} dups={dupCount} newSize={size} time={t_millis}ms", "oldSize"_attr = oldSize, "dupCount"_attr = _dupCount, "size"_attr = _size, "t_millis"_attr = t.millis());

        // if size is still high, or values are not reducing well, dump
        if (_onDisk && (_size > _config.maxInMemSize || _size > oldSize / 2)) {
            dumpToInc();
            LOGV2_DEBUG(20454, 3, "  MR - dumping to db");
        }
    }
}
...
bool runMapReduce(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(20455, 1, "mr ns: {config_nss}", "config_nss"_attr = config.nss);
    ...
    try {
        ...
        LOGV2(20456, "mr detected stale config, should retry{redact_e}", "redact_e"_attr = redact(e));
        throw;
    }
    // TODO:  The error handling code for queries is v. fragile,
    // *requires* rethrow AssertionExceptions - should probably fix.
    catch (AssertionException& e) {
        LOGV2(20457, "mr failed, removing collection{redact_e}", "redact_e"_attr = redact(e));
        throw;
    } catch (std::exception& e) {
        LOGV2(20458, "mr failed, removing collection{causedBy_e}", "causedBy_e"_attr = causedBy(e));
        throw;
    } catch (...) {
        LOGV2(20459, "mr failed for unknown reason, removing collection");
        throw;
    }
