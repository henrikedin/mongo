    ...
    if (_onDisk) {
        try {
            ...
            LOGV2_ERROR("Unable to drop temporary collection created by mapReduce: {}. This collection will be removed automatically "
                       "the next time the server starts up. {}", "config_tempNamespace"_attr = _config.tempNamespace, "exceptionToStatus"_attr = exceptionToStatus());
        }
    }
...
void State::reduceAndSpillInMemoryStateIfNeeded() {
    ...
    if (_jsMode) {
        ...
        if (keyCt > _config.jsMaxKeys) {
            ...
            LOGV2_DEBUG(3, "  MR - did reduceAll: keys={} dups={} newKeys={} time={}ms", "keyCt"_attr = keyCt, "dupCt"_attr = dupCt, "scope_getNumberInt_keyCt"_attr = _scope->getNumberInt("_keyCt"), "t_millis"_attr = t.millis());
            return;
        }
    }
    ...
    if (_size > _config.maxInMemSize || _dupCount > (_temp->size() * _config.reduceTriggerRatio)) {
        ...
        Timer t;
        reduceInMemory();
        LOGV2_DEBUG(3, "  MR - did reduceInMemory: size={} dups={} newSize={} time={}ms", "oldSize"_attr = oldSize, "dupCount"_attr = _dupCount, "size"_attr = _size, "t_millis"_attr = t.millis());
