        ...
        Status generateBatch(OperationContext* opCtx,
            ...
            switch (*state) {
                case PlanExecutor::FAILURE: {
                    ...
                    LOGV2_WARNING(20445, "GetMore command executor error: {PlanExecutor_statestr_state}, status: {status}, stats: {redact_Explain_getWinningPlanStats_exec}", "PlanExecutor_statestr_state"_attr = PlanExecutor::statestr(*state), "status"_attr = status, "redact_Explain_getWinningPlanStats_exec"_attr = redact(Explain::getWinningPlanStats(exec)));
        ...
        void acquireLocksAndIterateCursor(OperationContext* opCtx,
            ...
            if (cursorPin->lockPolicy() == ClientCursorParams::LockPolicy::kLocksInternally) {
                ...
                if (MONGO_unlikely(GetMoreHangBeforeReadLock.shouldFail())) {
                    LOGV2(20444, "GetMoreHangBeforeReadLock fail point enabled. Blocking until fail "
                             "point is disabled.");
