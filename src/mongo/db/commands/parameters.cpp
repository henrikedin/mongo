    ...
    bool errmsgRun(OperationContext* opCtx,
        ...
        for (std::map<std::string, BSONElement>::iterator it = parametersToSet.begin();
            ...
            try {
                ...
                LOGV2("error setting parameter {} to {} errMsg: {}", "parameterName"_attr = parameterName, "redact_parameter_toString_false"_attr = redact(parameter.toString(false)), "redact_ex"_attr = redact(ex));
                throw;
            }
