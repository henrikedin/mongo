...
public:
    ...
    virtual bool errmsgRun(OperationContext* opCtx,
        ...
        LOGV2(20428, "CMD fsync: sync:{sync} lock:{lock}", "sync"_attr = sync, "lock"_attr = lock);
        ...
        if (lockCountAtStart == 0) {
            ...
            if (!status.isOK()) {
                releaseLock();
                LOGV2_WARNING(20435, "fsyncLock failed. Lock count reset to 0. Status: {status}", "status"_attr = status);
                uassertStatusOK(status);
            }
        }

        LOGV2(20429, "mongod is locked and no writes are allowed. db.fsyncUnlock() to unlock");
        LOGV2(20430, "Lock count is {getLockCount}", "getLockCount"_attr = getLockCount());
        LOGV2(20431, "    For more info see {FSyncCommand_url}", "FSyncCommand_url"_attr = FSyncCommand::url());
...
public:
    ...
    bool errmsgRun(OperationContext* opCtx,
        ...
        LOGV2(20432, "command: unlock requested");
        ...
        if (stateLock) {
            ...
            LOGV2(20433, "fsyncUnlock completed. Lock count is now {lockCount}", "lockCount"_attr = lockCount);
        } else {
            invariant(fsyncCmd.getLockCount() == 0);
            lockCount = 0;
            LOGV2(20434, "fsyncUnlock completed. mongod is now unlocked and free to accept writes");
        }
...
void FSyncLockThread::run() {
    ...
    try {
        ...
        if (!_shutdownTaskRegistered) {
            ...
            registerShutdownTask([&] {
                ...
                if (fsyncCmd.getLockCount_inLock() > 0) {
                    LOGV2_WARNING(20436, "Interrupting fsync because the server is shutting down.");
        ...
        try {
            ...
            LOGV2_ERROR(20439, "error doing flushAll: {e_what}", "e_what"_attr = e.what());
        ...
        try {
            ...
            if (_allowFsyncFailure) {
                LOGV2_WARNING(20437, "Locking despite storage engine being unable to begin backup : {e_toString}", "e_toString"_attr = e.toString());
            } else {
                LOGV2_ERROR(20440, "storage engine unable to begin backup : {e_toString}", "e_toString"_attr = e.toString());
        ...
        while (fsyncCmd.getLockCount_inLock() > 0) {
            LOGV2_WARNING(20438, "WARNING: instance is locked, blocking all writes. The fsync command has "
        ...
        LOGV2_FATAL(20441, "FSyncLockThread exception: {e_what}", "e_what"_attr = e.what());
        fassertFailed(40350);
    }
}
