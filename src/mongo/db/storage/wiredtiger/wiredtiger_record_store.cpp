...
void WiredTigerRecordStore::OplogStones::createNewStoneIfNeeded(RecordId lastRecord) {
    ...
    LOGV2_DEBUG(2, "create new oplogStone, current stones:{}", "stones_size"_attr = _stones.size());
...
void WiredTigerRecordStore::OplogStones::_calculateStonesBySampling(OperationContext* opCtx,
    ...
    for (int i = 0; i < numSamples; ++i) {
        ...
        if (samplingLogIntervalSeconds > 0 &&
            ...
            LOGV2("Oplog sampling progress: {} of {} samples taken", "i_1"_attr = (i + 1), "numSamples"_attr = numSamples);
            lastProgressLog = now;
        }
    }
    std::sort(oplogEstimates.begin(), oplogEstimates.end());
    LOGV2("Oplog sampling complete");
...
void WiredTigerRecordStore::reclaimOplog(OperationContext* opCtx, Timestamp mayTruncateUpTo) {
    ...
    while (auto stone = _oplogStones->peekOldestStoneIfNeeded()) {
        ...
        LOGV2_DEBUG(1, "Truncating the oplog between {} and {} to remove approximately {} records totaling to {} bytes", "oplogStones_firstRecord"_attr = _oplogStones->firstRecord, "stone_lastRecord"_attr = stone->lastRecord, "stone_records"_attr = stone->records, "stone_bytes"_attr = stone->bytes);
        ...
        try {
            ...
            if (firstRecord < _oplogStones->firstRecord || firstRecord > stone->lastRecord) {
                LOGV2_WARNING("First oplog record {} is not in truncation range ({}, {})", "firstRecord"_attr = firstRecord, "oplogStones_firstRecord"_attr = _oplogStones->firstRecord, "stone_lastRecord"_attr = stone->lastRecord);
            }
    ...
    LOGV2_DEBUG(1, "Finished truncating the oplog, it now contains approximately {} records totaling to {} bytes", "sizeInfo_numRecords_load"_attr = _sizeInfo->numRecords.load(), "sizeInfo_dataSize_load"_attr = _sizeInfo->dataSize.load());
...
public:
    ...
    virtual void rollback() {
        LOGV2_DEBUG(3, "WiredTigerRecordStore: rolling back NumRecordsChange {}", "diff"_attr = -_diff);
...
boost::optional<Record> WiredTigerRecordStoreCursorBase::next() {
    ...
    if (_forward && _lastReturnedId >= id) {
        LOGV2("WTCursor::next -- c->next_key ( {}) was not greater than _lastReturnedId ({}) which is a bug.", "id"_attr = id, "lastReturnedId"_attr = _lastReturnedId);
