...
Status applyMaxCacheOverflowSizeGBParameter(WiredTigerMaxCacheOverflowSizeGBParameter& param,
    ...
    LOGV2(22101, "Reconfiguring WiredTiger max cache overflow size with value: \"{valueMB}MB\'", "valueMB"_attr = valueMB);
    ...
    if (ret != 0) {
        ...
        LOGV2_ERROR(22103, "{result}", "result"_attr = result);
...
Status WiredTigerEngineRuntimeConfigParameter::setFromString(const std::string& str) {
    ...
    LOGV2(22102, "Reconfiguring WiredTiger storage engine with config string: \"{str}\"", "str"_attr = str);
    ...
    if (ret != 0) {
        string result = (str::stream() << "WiredTiger reconfiguration failed with error code ("
                                       << ret << "): " << wiredtiger_strerror(ret));
        LOGV2_ERROR(22104, "{result}", "result"_attr = result);
