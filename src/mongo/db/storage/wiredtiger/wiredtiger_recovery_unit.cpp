...
void WiredTigerRecoveryUnit::prepareUnitOfWork() {
    ...
    LOGV2_DEBUG(22136, 1, "preparing transaction at time: {prepareTimestamp}", "prepareTimestamp"_attr = _prepareTimestamp);
...
void WiredTigerRecoveryUnit::assertInActiveTxn() const {
    ...
    LOGV2_FATAL(22143, "Recovery unit is not active. Current state: {toString_getState}", "toString_getState"_attr = toString(getState()));
    fassertFailed(28575);
}
...
void WiredTigerRecoveryUnit::_txnClose(bool commit) {
    ...
    if (_timer) {
        ...
        if (transactionTime >= std::max(1, serverGlobalParams.slowMS)) {
            LOGV2_DEBUG(22137, logSeverityV1toV2(kSlowTransactionSeverity).toInt(), "Slow WT transaction. Lifetime of SnapshotId {getSnapshotId_toNumber} was {transactionTime}ms", "getSnapshotId_toNumber"_attr = getSnapshotId().toNumber(), "transactionTime"_attr = transactionTime);
        }
    }
    ...
    if (commit) {
        ...
        LOGV2_DEBUG(22138, 3, "WT commit_transaction for snapshot id {getSnapshotId_toNumber}", "getSnapshotId_toNumber"_attr = getSnapshotId().toNumber());
    } else {
        wtRet = s->rollback_transaction(s, nullptr);
        invariant(!wtRet);
        LOGV2_DEBUG(22139, 3, "WT rollback_transaction for snapshot id {getSnapshotId_toNumber}", "getSnapshotId_toNumber"_attr = getSnapshotId().toNumber());
    }
...
void WiredTigerRecoveryUnit::_txnOpen() {
    ...
    LOGV2_DEBUG(22140, 3, "WT begin_transaction for snapshot id {getSnapshotId_toNumber}", "getSnapshotId_toNumber"_attr = getSnapshotId().toNumber());
}
...
Status WiredTigerRecoveryUnit::setTimestamp(Timestamp timestamp) {
    _ensureSession();
    LOGV2_DEBUG(22141, 3, "WT set timestamp of future write operations to {timestamp}", "timestamp"_attr = timestamp);
    WT_SESSION* session = _session->getSession();
    invariant(_inUnitOfWork(), toString(getState()));
    invariant(_prepareTimestamp.isNull());
...
void WiredTigerRecoveryUnit::setTimestampReadSource(ReadSource readSource,
    ...
    LOGV2_DEBUG(22142, 3, "setting timestamp read source: {static_cast_int_readSource}, provided timestamp: {provided_provided_toString_none}", "static_cast_int_readSource"_attr = static_cast<int>(readSource), "provided_provided_toString_none"_attr = ((provided) ? provided->toString() : "none"));
