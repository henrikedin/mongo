...
void WiredTigerSizeStorer::store(StringData uri, std::shared_ptr<SizeInfo> sizeInfo) {
    ...
    LOGV2_DEBUG(22149, 2, "WiredTigerSizeStorer::store Marking {uri} dirty, numRecords: {sizeInfo_numRecords_load}, dataSize: {sizeInfo_dataSize_load}, use_count: {entry_use_count}", "uri"_attr = uri, "sizeInfo_numRecords_load"_attr = sizeInfo->numRecords.load(), "sizeInfo_dataSize_load"_attr = sizeInfo->dataSize.load(), "entry_use_count"_attr = entry.use_count());
}
...
std::shared_ptr<WiredTigerSizeStorer::SizeInfo> WiredTigerSizeStorer::load(StringData uri) const {
    ...
    LOGV2_DEBUG(22150, 2, "WiredTigerSizeStorer::load {uri} -> {redact_data}", "uri"_attr = uri, "redact_data"_attr = redact(data));
...
void WiredTigerSizeStorer::flush(bool syncToDisk) {
    ...
    stdx::lock_guard<Latch> cursorLock(_cursorMutex);
    {
        ...
        for (auto it = buffer.begin(); it != buffer.end(); ++it) {
            ...
            auto& uri = it->first;
            LOGV2_DEBUG(22151, 2, "WiredTigerSizeStorer::flush {uri} -> {redact_data}", "uri"_attr = uri, "redact_data"_attr = redact(data));
    ...
    LOGV2_DEBUG(22152, 2, "WiredTigerSizeStorer flush took {micros} µs", "micros"_attr = micros);
}
}  // namespace mongo
