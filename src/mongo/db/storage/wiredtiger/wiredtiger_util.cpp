...
StatusWith<std::string> WiredTigerUtil::getMetadataCreate(OperationContext* opCtx, StringData uri) {
    ...
    try {
        ...
        LOGV2_ERROR(22158, "{ex}", "ex"_attr = ex);
...
StatusWith<std::string> WiredTigerUtil::getMetadata(OperationContext* opCtx, StringData uri) {
    ...
    try {
        ...
        LOGV2_ERROR(22159, "{ex}", "ex"_attr = ex);
...
StatusWith<int64_t> WiredTigerUtil::checkApplicationMetadataFormatVersion(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22154, 2, "WiredTigerUtil::checkApplicationMetadataFormatVersion  uri: {uri} ok range {minimumVersion} -> {maximumVersion} current: {version}", "uri"_attr = uri, "minimumVersion"_attr = minimumVersion, "maximumVersion"_attr = maximumVersion, "version"_attr = version);
...
int mdb_handle_error_with_startup_suppression(WT_EVENT_HANDLER* handler,
    ...
    try {
        ...
        LOGV2_ERROR(22160, "WiredTiger error ({errorCode}) {redact_message} Raw: {message}", "errorCode"_attr = errorCode, "redact_message"_attr = redact(message), "message"_attr = message);
...
int mdb_handle_error(WT_EVENT_HANDLER* handler,
                     ...
                     const char* message) {
    try {
        LOGV2_ERROR(22161, "WiredTiger error ({errorCode}) {redact_message}", "errorCode"_attr = errorCode, "redact_message"_attr = redact(message));
...
int mdb_handle_message(WT_EVENT_HANDLER* handler, WT_SESSION* session, const char* message) {
    try {
        LOGV2(22155, "WiredTiger message {redact_message}", "redact_message"_attr = redact(message));
    } catch (...) {
        std::terminate();
    }
    return 0;
...
int mdb_handle_progress(WT_EVENT_HANDLER* handler,
                        ...
                        uint64_t progress) {
    try {
        LOGV2(22156, "WiredTiger progress {redact_operation} {progress}", "redact_operation"_attr = redact(operation), "progress"_attr = progress);
    } catch (...) {
        std::terminate();
    }
...
Status WiredTigerUtil::setTableLogging(WT_SESSION* session, const std::string& uri, bool on) {
    ...
    LOGV2_DEBUG(22157, 1, "Changing table logging settings. Uri: {uri} Enable? {on}", "uri"_attr = uri, "on"_attr = on);
    int ret = session->alter(session, uri.c_str(), setting.c_str());
    if (ret) {
        LOGV2_FATAL(22162, "Failed to update log setting. Uri: {uri} Enable? {on} Ret: {ret} MD: {redact_existingMetadata} Msg: {session_strerror_session_ret}", "uri"_attr = uri, "on"_attr = on, "ret"_attr = ret, "redact_existingMetadata"_attr = redact(existingMetadata), "session_strerror_session_ret"_attr = session->strerror(session, ret));
        fassertFailed(50756);
    }
