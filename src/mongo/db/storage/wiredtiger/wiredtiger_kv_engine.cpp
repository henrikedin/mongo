    ...
    virtual void run() {
        ...
        while (!_shuttingDown.load()) {
            ...
            try {
                ...
                LOGV2_WARNING("Checkpoint encountered a write conflict exception.");
    ...
    if (!_durable && !_readOnly) {
        ...
        if (boost::filesystem::exists(journalPath)) {
            ...
            try {
                ...
                LOGV2_ERROR("error removing journal dir {} {}", "journalPath_string"_attr = journalPath.string(), "e_what"_attr = e.what());
                throw;
            }
        }
...
void WiredTigerKVEngine::_openWiredTiger(const std::string& path, const std::string& wtOpenConfig) {
    ...
    LOGV2_WARNING("Failed to start up WiredTiger under any compatibility version.");
    if (ret == EINVAL) {
    ...
    LOGV2_FATAL(28595, "Reason: {}", "wtRCToStatus_ret_reason"_attr = wtRCToStatus(ret).reason());
    if (!_inRepairMode) {
    ...
    LOGV2_WARNING("Attempting to salvage WiredTiger metadata");
...
// for memory leaks.
    ...
    if (_fileVersion.shouldDowngrade(_readOnly, _inRepairMode, !_recoveryTimestamp.isNull())) {
        ...
        LOGV2_DEBUG(1, "Downgrade compatibility configuration: {}", "fileVersion_getDowngradeString"_attr = _fileVersion.getDowngradeString());
...
Status WiredTigerKVEngine::_salvageIfNeeded(const char* uri) {
    ...
    if (rc == EBUSY) {
        ...
        // is resolved.
        LOGV2_ERROR("Verify on {} failed with EBUSY. This means the collection was being accessed. No repair is necessary unless other "
               "errors are reported.", "uri"_attr = uri);
        return Status::OK();
    }

    if (rc == ENOENT) {
        LOGV2_WARNING("Data file is missing for {}. Attempting to drop and re-create the collection.", "uri"_attr = uri);
    ...
    LOGV2_WARNING("Salvage failed for uri {}: {}. The file will be moved out of the way and a new ident will be created.", "uri"_attr = uri, "status_reason"_attr = status.reason());
...
Status WiredTigerKVEngine::_rebuildIdent(WT_SESSION* session, const char* uri) {
    ...
    if (filePath) {
        ...
        LOGV2_WARNING("Moving data file {} to backup as {}", "filePath_string"_attr = filePath->string(), "corruptFile_string"_attr = corruptFile.string());
    ...
    LOGV2_WARNING("Rebuilding ident {}", "identName"_attr = identName);
    ...
    if (!swMetadata.isOK()) {
        LOGV2_ERROR("Failed to get metadata for {}", "uri"_attr = uri);
        return swMetadata.getStatus();
    }

    int rc = session->drop(session, uri, nullptr);
    if (rc != 0) {
        LOGV2_ERROR("Failed to drop {}", "uri"_attr = uri);
        return wtRCToStatus(rc);
    }

    rc = session->create(session, uri, swMetadata.getValue().c_str());
    if (rc != 0) {
        LOGV2_ERROR("Failed to create {} with config: {}", "uri"_attr = uri, "swMetadata_getValue"_attr = swMetadata.getValue());
        return wtRCToStatus(rc);
    }
...
Status WiredTigerKVEngine::disableIncrementalBackup(OperationContext* opCtx) {
    ...
    if (wtRet != 0) {
        LOGV2_ERROR("Could not open a backup cursor to disable incremental backups");
...
Status WiredTigerKVEngine::recoverOrphanedIdent(OperationContext* opCtx,
    ...
    LOGV2_WARNING("Could not salvage data. Rebuilding ident: {}", "status_reason"_attr = status.reason());
    ...
    while ((idx = ident.find('/', start)) != string::npos) {
        ...
        if (!boost::filesystem::exists(subdir)) {
            ...
            try {
                ...
                LOGV2_ERROR("error creating path {} {}", "subdir_string"_attr = subdir.string(), "e_what"_attr = e.what());
                throw;
            }
        }
