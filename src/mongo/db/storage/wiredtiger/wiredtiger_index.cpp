...
Status WiredTigerIndex::insert(OperationContext* opCtx,
    ...
    TRACE_INDEX(51781, "KeyString: {keyString}", "keyString"_attr = keyString);
...
void WiredTigerIndex::fullValidate(OperationContext* opCtx,
    ...
    long long count = 0;
    TRACE_INDEX(51783, "fullValidate");
    ...
    for (auto kv = cursor->seek(keyStringForSeek, requestedInfo); kv; kv = cursor->next()) {
        TRACE_INDEX(51784, "fullValidate {kv}", "kv"_attr=kv);
        count++;
    }
    if (numKeysOut) {
        *numKeysOut = count;
...
public:
    ...
    void restore() override {
        ...
        if (!_eof) {
            ...
            TRACE_CURSOR(51785, "restore _lastMoveSkippedKey: {lastMoveSkippedKey}", "lastMoveSkippedKey"_attr=_lastMoveSkippedKey);
        }
    }
...
protected:
    ...
    boost::optional<IndexKeyEntry> curr(RequestedInfo parts) const {
        ...
        if (TRACING_ENABLED || (parts & kWantKey)) {
            ...
            TRACE_CURSOR(51786, "returning {bson} {id}", "bson"_attr=bson, "id"_attr=_id);
        }
    ...
    bool seekWTCursor(const KeyString::Value& query) {
        ...
        if (ret == WT_NOTFOUND) {
            _cursorAtEof = true;
            TRACE_CURSOR(51787, "not found");
            return false;
        }
        invariantWTOK(ret);
        _cursorAtEof = false;

        TRACE_CURSOR(51788, "cmp: {cmp}", "cmp"_attr=cmp);
    ...
    KeyStringEntry getKeyStringEntry() {
        ...
        TRACE_CURSOR(51792, "returning {key} {id}", "key"_attr = _key, "id"_attr = _id);
...
public:
    ...
    void updateIdAndTypeBits() override {
        TRACE_INDEX(
            51793, "Unique Index KeyString: [{keyString}]", "keyString"_attr = _key.toString());
    ...
    void restore() override {
        ...
        if (_lastMoveSkippedKey && !_eof && !_cursorAtEof) {
            ...
            if (item.size >= keySize && std::memcmp(_key.getBuffer(), item.data, keySize) == 0) {
                _lastMoveSkippedKey = false;
                TRACE_CURSOR(51794, "restore _lastMoveSkippedKey changed to false.");
            }
        }
    }
...
Status WiredTigerIndexUnique::_insertTimestampSafe(OperationContext* opCtx,
                                                   ...
                                                   const KeyString::Value& keyString,
                                                   bool dupsAllowed) {
    TRACE_INDEX(51796, "Timestamp safe unique idx KeyString: {keyString}", "keyString"_attr=keyString);
