...
public:
    ...
    virtual void commit(boost::optional<Timestamp> commitTimestamp) {
        ...
        if (_engine->getStorageEngine()->supportsPendingDrops() && commitTimestamp) {
            LOGV2(21922, "Deferring table drop for index '{indexName}' on collection '{indexNss}{uuid_uuid_toString}. Ident: '{ident}', commit timestamp: '{commitTimestamp}'", "indexName"_attr = _indexName, "indexNss"_attr = _indexNss, "uuid_uuid_toString"_attr = (_uuid ? " (" + _uuid->toString() + ")'" : ""), "ident"_attr = _ident, "commitTimestamp"_attr = commitTimestamp);
...
DurableCatalogImpl::FeatureTracker::FeatureBits DurableCatalogImpl::FeatureTracker::getInfo(
    ...
    if (!nonRepairableFeaturesStatus.isOK()) {
        LOGV2_ERROR(21931, "error: exception extracting typed field with obj:{redact_obj}", "redact_obj"_attr = redact(obj));
    ...
    if (!repairableFeaturesStatus.isOK()) {
        LOGV2_ERROR(21932, "error: exception extracting typed field with obj:{redact_obj}", "redact_obj"_attr = redact(obj));
...
StatusWith<DurableCatalog::Entry> DurableCatalogImpl::_addEntry(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(21923, 1, "stored meta data for {nss_ns} @ {res_getValue}", "nss_ns"_attr = nss.ns(), "res_getValue"_attr = res.getValue());
...
BSONObj DurableCatalogImpl::_findEntry(OperationContext* opCtx, RecordId catalogId) const {
    LOGV2_DEBUG(21924, 3, "looking up metadata for: {catalogId}", "catalogId"_attr = catalogId);
    RecordData data;
...
BSONCollectionCatalogEntry::MetaData DurableCatalogImpl::getMetaData(OperationContext* opCtx,
    ...
    BSONObj obj = _findEntry(opCtx, catalogId);
    LOGV2_DEBUG(21925, 3, " fetched CCE metadata: {obj}", "obj"_attr = obj);
    BSONCollectionCatalogEntry::MetaData md;
    const BSONElement mdElement = obj["md"];
    if (mdElement.isABSONObj()) {
        LOGV2_DEBUG(21926, 3, "returning metadata: {mdElement}", "mdElement"_attr = mdElement);
...
void DurableCatalogImpl::putMetaData(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(21927, 3, "recording new metadata: {obj}", "obj"_attr = obj);
...
Status DurableCatalogImpl::_removeEntry(OperationContext* opCtx, RecordId catalogId) {
    ...
    LOGV2_DEBUG(21928, 1, "deleting metadata for {it_second_nss} @ {catalogId}", "it_second_nss"_attr = it->second.nss, "catalogId"_attr = catalogId);
...
StatusWith<std::string> DurableCatalogImpl::newOrphanedIdent(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(21929, 1, "stored meta data for orphaned collection {ns} @ {res_getValue}", "ns"_attr = ns, "res_getValue"_attr = res.getValue());
    return {ns.ns()};
}
    ...
    opCtx->recoveryUnit()->onCommit(
        [opCtx, ru, catalog = this, entry](boost::optional<Timestamp> commitTimestamp) {
            ...
            if (storageEngine->supportsPendingDrops() && commitTimestamp) {
                LOGV2(21930, "Deferring table drop for collection '{entry_nss}'. Ident: {entry_ident}, commit timestamp: {commitTimestamp}", "entry_nss"_attr = entry.nss, "entry_ident"_attr = entry.ident, "commitTimestamp"_attr = commitTimestamp);
