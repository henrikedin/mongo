...
TEST_F(DurableCatalogTest, MultikeyPathsForBtreeIndexInitializedToVectorOfEmptySets) {
    ...
    DurableCatalog* catalog = getCatalog();
    {
        ...
        assertMultikeyPathsAreEqual(multikeyPaths, {boost::container::flat_set<size_t>{}, boost::container::flat_set<size_t>{}});
    }
}
...
DEATH_TEST_REGEX_F(DurableCatalogTest,
    ...
    catalog->setIndexIsMultikey(
        opCtx.get(), getCatalogId(), indexName, {boost::container::flat_set<size_t>{}, boost::container::flat_set<size_t>{}});
}
