...
MobileKVEngine::MobileKVEngine(const std::string& path,
    ...
    LOGV2_DEBUG(21956, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Confirmed SQLite database opened in WAL mode");

    fassert(50869, queryPragmaInt(*session, "synchronous"_sd) == _options.durabilityLevel);
    LOGV2_DEBUG(21957, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Confirmed SQLite database has synchronous set to: {options_durabilityLevel}", "options_durabilityLevel"_attr = _options.durabilityLevel);

    fassert(50868, queryPragmaInt(*session, "fullfsync"_sd) == 1);
    LOGV2_DEBUG(21958, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Confirmed SQLite database is set to fsync with "
...
void MobileKVEngine::cleanShutdown() {
    try {
        ...
        LOGV2_DEBUG(21959, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Exception while doing vacuum at shutdown, surpressing. {e_what}", "e_what"_attr = e.what());
    }
}
...
void MobileKVEngine::maybeVacuum(Client* client, Date_t deadline) {
    ...
    LOGV2_DEBUG(21960, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Evaluating if we need to vacuum. page_count = {pageCount}, freelist_count = {freelistCount}", "pageCount"_attr = pageCount, "freelistCount"_attr = freelistCount);
    if ((pageCount > 0 && (float)freelistCount / pageCount >= _options.vacuumFreePageRatio) ||
        ...
        LOGV2_DEBUG(21961, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Performing incremental vacuum");
...
Status MobileKVEngine::dropIdent(OperationContext* opCtx, RecoveryUnit* ru, StringData ident) {
    ...
    try {
        ...
        LOGV2_DEBUG(21962, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Caught WriteConflictException while dropping table, "
