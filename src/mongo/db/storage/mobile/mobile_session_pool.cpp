...
void MobileDelayedOpQueue::enqueueOp(std::string& opQuery) {
    ...
    LOGV2_DEBUG(21963, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Enqueued operation for delayed execution: {opQuery}", "opQuery"_attr = opQuery);
    _queueMutex.unlock();
}
...
void MobileDelayedOpQueue::execAndDequeueOp(MobileSession* session) {
    ...
    LOGV2_DEBUG(21964, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Retrying previously enqueued operation: {opQuery}", "opQuery"_attr = opQuery);
    try {
        ...
        LOGV2_DEBUG(21965, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Caught WriteConflictException while executing "
...
void MobileSessionPool::shutDown() {
    ...
    if (!failedDropsQueue.isEmpty()) {
        ...
        LOGV2_DEBUG(21966, logSeverityV1toV2(MOBILE_LOG_LEVEL_LOW).toInt(), "MobileSE: Executing queued drops at shutdown");
