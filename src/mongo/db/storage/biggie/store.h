...
private:
    ...
    Node* _findNode(const Key& key) const {
        ...
        for (unsigned int i = 0; i < _root->_trieKey.size(); i++) {
            ...
            if (depth == key.size() &&
                _root->_data && (key.size() - initialDepthOffset) == _root->_trieKey.size()) {
                return _root.get();
            }
        }
            ...
            if (!node) {
                ...
                if (node->_trieKey == baseNode->_trieKey &&
                    ...
                    if (!updatedNode->_data) {
                        // Drop if leaf node without data, that is not valid. Otherwise we might need to compress if we have only one child.
