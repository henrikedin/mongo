...
void StorageEngineImpl::loadCatalog(OperationContext* opCtx) {
    ...
    if (_options.forRepair && catalogExists) {
        ...
        LOGV2(21976, "Repairing catalog metadata");
        Status status = _engine->repairIdent(opCtx, catalogInfo);

        if (status.code() == ErrorCodes::DataModifiedByRepair) {
            LOGV2_WARNING(21994, "Catalog data modified by repair: {status_reason}", "status_reason"_attr = status.reason());
            ...
            if (_catalog->isCollectionIdent(ident)) {
                ...
                if (isOrphan) {
                    ...
                    if (statusWithNs.isOK()) {
                        ...
                        LOGV2(21977, "Successfully created an entry in the catalog for the orphaned "
                                 "collection: {orphanCollNs}", "orphanCollNs"_attr = orphanCollNs);
                        LOGV2_WARNING(21995, "{orphanCollNs} does not have the _id index. Please manually "
                                     "build the index.", "orphanCollNs"_attr = orphanCollNs);
                        ...
                        LOGV2_ERROR(21998, "Cannot create an entry in the catalog for the orphaned "
                                   "collection ident: {ident} due to {statusWithNs_getStatus_reason}", "ident"_attr = ident, "statusWithNs_getStatus_reason"_attr = statusWithNs.getStatus().reason());
                        LOGV2_ERROR(21999, "Restarting the server will remove this ident.");
                    }
                }
            }
        }
    ...
    for (DurableCatalog::Entry entry : catalogEntries) {
        if (loadingFromUncleanShutdownOrRepair) {
            ...
            if (orphan) {
                ...
                if (!status.isOK()) {
                    LOGV2_WARNING(21996, "Failed to recover orphaned data file for collection '{entry_nss}': {status}", "entry_nss"_attr = entry.nss, "status"_attr = status);
                    WriteUnitOfWork wuow(opCtx);
        ...
        if (entry.nss.isOrphanCollection()) {
            LOGV2(21978, "Orphaned collection found: {entry_nss}", "entry_nss"_attr = entry.nss);
        }
    }
...
Status StorageEngineImpl::_recoverOrphanedCollection(OperationContext* opCtx,
    ...
    LOGV2(21979, "Storage engine is missing collection '{collectionName}' from its metadata. Attempting to locate and recover the data for {collectionIdent}", "collectionName"_attr = collectionName, "collectionIdent"_attr = collectionIdent);
...
StatusWith<StorageEngine::ReconcileResult> StorageEngineImpl::reconcileCatalogAndIdents(
    ...
    for (const auto& it : engineIdents) {
        ...
        if (dropPendingIdents.find(it) != dropPendingIdents.cend()) {
            LOGV2(21980, "Not removing ident for uncheckpointed collection or index drop: {it}", "it"_attr = it);
            continue;
        }

        const auto& toRemove = it;
        LOGV2(21981, "Dropping unknown ident: {toRemove}", "toRemove"_attr = toRemove);
        WriteUnitOfWork wuow(opCtx);
    ...
    for (DurableCatalog::Entry entry : catalogEntries) {
        ...
        for (const auto& indexMetaData : metaData.indexes) {
            ...
            if (!indexMetaData.multikey && hasMultiKeyPaths) {
                LOGV2_WARNING(21997, "The 'multikey' field for index {indexName} on collection {coll} was false with non-empty 'multikeyPaths'. This indicates corruption of the catalog. Consider either dropping and recreating the index, or rerunning with the --repair option. See http://dochub.mongodb.org/core/repair for more information.", "indexName"_attr = indexName, "coll"_attr = coll);
            }
            ...
            if (indexMetaData.ready && !foundIdent) {
                LOGV2(21982, "Expected index data is missing, rebuilding. Collection: {coll} Index: {indexName}", "coll"_attr = coll, "indexName"_attr = indexName);
            ...
            if (indexMetaData.buildUUID) {
                ...
                LOGV2(21983, "Found index from unfinished build. Collection: {coll} ({collUUID}), index: {indexName}, build UUID: {buildUUID}", "coll"_attr = coll, "collUUID"_attr = *collUUID, "indexName"_attr = indexName, "buildUUID"_attr = buildUUID);
            ...
            if (indexMetaData.isBackgroundSecondaryBuild && (!foundIdent || !indexMetaData.ready)) {
                if (!serverGlobalParams.indexBuildRetry) {
                    LOGV2(21984, "Dropping an unfinished index because --noIndexBuildRetry is set. "
                             "Collection: {coll} Index: {indexName}", "coll"_attr = coll, "indexName"_attr = indexName);
                    fassert(51197, _engine->dropIdent(opCtx, opCtx->recoveryUnit(), indexIdent));
                    indexesToDrop.push_back(indexName);
                    continue;
                }

                LOGV2(21985, "Expected background index build did not complete, rebuilding. "
                         "Collection: {coll} Index: {indexName}", "coll"_attr = coll, "indexName"_attr = indexName);
                ret.indexesToRebuild.push_back({entry.catalogId, coll, indexName});
                continue;
            }
            ...
            if (!indexMetaData.ready && !indexMetaData.isBackgroundSecondaryBuild) {
                LOGV2(21986, "Dropping unfinished index. Collection: {coll} Index: {indexName}", "coll"_attr = coll, "indexName"_attr = indexName);
    ...
    for (auto&& temp : internalIdentsToDrop) {
        LOGV2(21987, "Dropping internal ident: {temp}", "temp"_attr = temp);
        WriteUnitOfWork wuow(opCtx);
...
std::unique_ptr<TemporaryRecordStore> StorageEngineImpl::makeTemporaryRecordStore(
    ...
    LOGV2_DEBUG(21988, 1, "created temporary record store: {rs_getIdent}", "rs_getIdent"_attr = rs->getIdent());
...
StatusWith<Timestamp> StorageEngineImpl::recoverToStableTimestamp(OperationContext* opCtx) {
    ...
    LOGV2(21989, "recoverToStableTimestamp successful. Stable Timestamp: {swTimestamp_getValue}", "swTimestamp_getValue"_attr = swTimestamp.getValue());
...
void StorageEngineImpl::_onMinOfCheckpointAndOldestTimestampChanged(const Timestamp& timestamp) {
    ...
    if (auto earliestDropTimestamp = _dropPendingIdentReaper.getEarliestDropTimestamp()) {
        if (timestamp > *earliestDropTimestamp) {
            LOGV2(21990, "Removing drop-pending idents with drop timestamps before timestamp {timestamp}", "timestamp"_attr = timestamp);
...
StorageEngineImpl::TimestampMonitor::~TimestampMonitor() {
    LOGV2(21991, "Timestamp monitor shutting down");
...
void StorageEngineImpl::TimestampMonitor::startup() {
    ...
    LOGV2(21992, "Timestamp monitor starting");
    PeriodicRunner::PeriodicJob job(
        ...
        [&](Client* client) {
            ...
            try {
                ...
                LOGV2(21993, "{Timestamp_monitor_is_stopping_due_to_ex_reason}", "Timestamp_monitor_is_stopping_due_to_ex_reason"_attr = "Timestamp monitor is stopping due to: " + ex.reason());
                return;
            }
        },
        Seconds(1));
