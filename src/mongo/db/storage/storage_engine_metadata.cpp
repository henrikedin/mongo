...
std::unique_ptr<StorageEngineMetadata> StorageEngineMetadata::forPath(const std::string& dbpath) {
    ...
    if (boost::filesystem::exists(boost::filesystem::path(dbpath) / kMetadataBasename)) {
        ...
        if (!status.isOK()) {
            LOGV2_ERROR(22018, "Unable to read the storage engine metadata file: {status}", "status"_attr = status);
...
void flushMyDirectory(const boost::filesystem::path& file) {
    ...
    if (!file.has_branch_path()) {
        LOGV2(22013, "warning flushMyDirectory couldn't find parent dir for file: {file_string}", "file_string"_attr = file.string());
        return;
    }
    ...
    LOGV2_DEBUG(22014, 1, "flushing directory {dir_string}", "dir_string"_attr = dir.string());
    ...
    if (fsync(fd) != 0) {
        ...
        if (e == EINVAL) {  // indicates filesystem does not support synchronization
            if (!_warnedAboutFilesystem) {
                LOGV2_OPTIONS(22015, {logv2::LogTag::kStartupWarnings}, "\tWARNING: This file system is not supported. For further information see:");
                LOGV2_OPTIONS(22016, {logv2::LogTag::kStartupWarnings}, "\t\t\thttp://dochub.mongodb.org/core/unsupported-filesystems");
                LOGV2_OPTIONS(22017, {logv2::LogTag::kStartupWarnings}, "\t\tPlease notify MongoDB, Inc. if an unlisted filesystem generated this warning.");
