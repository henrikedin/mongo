...
Status fsyncParentDirectory(const boost::filesystem::path& file) {
    ...
    LOGV2_DEBUG(22019, 1, "flushing directory {dir_string}", "dir_string"_attr = dir.string());
    ...
    if (fsync(fd) != 0) {
        int e = errno;
        if (e == EINVAL) {
            LOGV2_WARNING(22020, "Could not fsync directory because this file system is not supported.");
        } else {
            close(fd);
