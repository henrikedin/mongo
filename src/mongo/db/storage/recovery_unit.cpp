...
void RecoveryUnit::commitRegisteredChanges(boost::optional<Timestamp> commitTimestamp) {
    ...
    for (auto& change : _changes) {
        try {
            ...
            LOGV2_DEBUG(21974, 3, "CUSTOM COMMIT {redact_demangleName_typeid_change}", "redact_demangleName_typeid_change"_attr = redact(demangleName(typeid(*change))));
...
void RecoveryUnit::abortRegisteredChanges() {
    ...
    try {
        for (Changes::const_reverse_iterator it = _changes.rbegin(), end = _changes.rend();
            ...
            LOGV2_DEBUG(21975, 2, "CUSTOM ROLLBACK {redact_demangleName_typeid_change}", "redact_demangleName_typeid_change"_attr = redact(demangleName(typeid(*change))));
            change->rollback();
        }
        _changes.clear();
    } catch (...) {
