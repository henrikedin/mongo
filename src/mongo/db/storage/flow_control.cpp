...
bool sustainerAdvanced(const std::vector<repl::MemberData>& prevMemberData,
    ...
    if (currMemberData.size() == 0 || currMemberData.size() != prevMemberData.size()) {
        LOGV2_WARNING("Flow control detected a change in topology. PrevMemberSize: {} CurrMemberSize: {}", "prevMemberData_size"_attr = prevMemberData.size(), "currMemberData_size"_attr = currMemberData.size());
        return false;
    }
    ...
    if (currSustainerAppliedTs < prevSustainerAppliedTs) {
        LOGV2_WARNING("Flow control's sustainer time decreased. PrevSustainer: {} CurrSustainer: {}", "prevSustainerAppliedTs"_attr = prevSustainerAppliedTs, "currSustainerAppliedTs"_attr = currSustainerAppliedTs);
        return false;
    }
...
int FlowControl::_calculateNewTicketsForLag(const std::vector<repl::MemberData>& prevMemberData,
    ...
    LOGV2_DEBUG({logComponentV1toV2(DEBUG_LOG_LEVEL)}, " PrevApplied: {} CurrApplied: {} NumSustainerApplied: {}", "prevSustainerAppliedTs"_attr = prevSustainerAppliedTs, "currSustainerAppliedTs"_attr = currSustainerAppliedTs, "sustainerAppliedCount"_attr = sustainerAppliedCount);
    if (sustainerAppliedCount > 0) {
        ...
        if (warnThresholdSeconds > 0 &&
            ...
            LOGV2_WARNING("Flow control is engaged and the sustainer point is not moving. Please "
...
void FlowControl::sample(Timestamp timestamp, std::uint64_t opsApplied) {
    ...
    LOGV2_DEBUG({logComponentV1toV2(DEBUG_LOG_LEVEL)}, "Sampling. Time: {} Applied: {} LockAcquisitions: {}", "timestamp"_attr = timestamp, "_numOpsSinceStartup"_attr = _numOpsSinceStartup, "lockAcquisitions"_attr = lockAcquisitions);
...
void FlowControl::_trimSamples(const Timestamp trimTo) {
    ...
    LOGV2_DEBUG({logComponentV1toV2(DEBUG_LOG_LEVEL)}, "Trimmed samples. Num: {}", "numTrimmed"_attr = numTrimmed);
}
