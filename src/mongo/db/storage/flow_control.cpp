...
void FlowControl::sample(Timestamp timestamp, std::uint64_t opsApplied) {
    ...
    LOGV2_DEBUG(::mongo::logger::LogSeverity(DEBUG_LOG_LEVEL).toInt(), "Sampling. Time: {} Applied: {} LockAcquisitions: {}", "timestamp"_attr = timestamp, "numOpsSinceStartup"_attr = _numOpsSinceStartup, "lockAcquisitions"_attr = lockAcquisitions);
