...
void LogicalSessionCacheImpl::_periodicRefresh(Client* client) {
    try {
        ...
        LOGV2("Failed to refresh session cache: {}, will try again at the next refresh interval", "exceptionToStatus"_attr = exceptionToStatus());
    }
}
...
void LogicalSessionCacheImpl::_periodicReap(Client* client) {
    ...
    if (!res.isOK()) {
        LOGV2("Failed to reap transaction table: {}", "res"_attr = res);
    }
    ...
    try {
        ...
        try {
            ...
            if (ex.code() != ErrorCodes::NamespaceNotFound ||
                ...
                LOGV2("{}: {}", "notSetUpWarning"_attr = notSetUpWarning, "ex_reason"_attr = ex.reason());
            } else {
                LOGV2("{}", "notSetUpWarning"_attr = notSetUpWarning);
            }
            return Status::OK();
        }
...
void LogicalSessionCacheImpl::_refresh(Client* client) {
    ...
    try {
        ...
        LOGV2("Failed to refresh session cache, will try again at the next refresh interval{}", "causedBy_redact_ex"_attr = causedBy(redact(ex)));
        return;
    }
...
Status LogicalSessionCacheImpl::_addToCacheIfNotFull(WithLock, LogicalSessionRecord record) {
    if (_activeSessions.size() >= size_t(maxSessions)) {
        ...
        LOGV2_DEBUG(::mongo::logger::LogSeverity(severity).toInt(), "{}", "status_toString"_attr = status.toString());
        return status;
    }
