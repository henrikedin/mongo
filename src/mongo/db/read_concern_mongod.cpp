...
Status makeNoopWriteIfNeeded(OperationContext* opCtx, LogicalTime clusterTime) {
    ...
    if (clusterTime > lastAppliedOpTime && replCoord->getMemberState().secondary()) {
        ...
        if (!waitStatus.isOK()) {
            LOGV2_DEBUG(20787, 1, "Wait for clusterTime: {clusterTime_toString} until deadline: {deadline} failed with {waitStatus_toString}", "clusterTime_toString"_attr = clusterTime.toString(), "deadline"_attr = deadline, "waitStatus_toString"_attr = waitStatus.toString());
        }
    }
    ...
    while (clusterTime > lastAppliedOpTime) {
        ...
        if (std::get<0>(myWriteRequest)) {  // Its a new request
            try {
                LOGV2_DEBUG(20788, 2, "New appendOplogNote request on clusterTime: {clusterTime_toString} remaining attempts: {remainingAttempts}", "clusterTime_toString"_attr = clusterTime.toString(), "remainingAttempts"_attr = remainingAttempts);
            ...
            LOGV2_DEBUG(20789, 2, "Join appendOplogNote request on clusterTime: {clusterTime_toString} remaining attempts: {remainingAttempts}", "clusterTime_toString"_attr = clusterTime.toString(), "remainingAttempts"_attr = remainingAttempts);
            try {
    ...
    if (!status.isOK()) {
        LOGV2_DEBUG(20790, 1, "Reached clusterTime {lastAppliedOpTime_toString} but failed noop write due to {status_toString}", "lastAppliedOpTime_toString"_attr = lastAppliedOpTime.toString(), "status_toString"_attr = status.toString());
    }
    return Status::OK();
}
...
Status waitForReadConcernImpl(OperationContext* opCtx,
    ...
    if (!readConcernArgs.isEmpty()) {
        ...
        if (targetClusterTime) {
            ...
            if (!status.isOK()) {
                LOGV2(20791, "Failed noop write at clusterTime: {targetClusterTime_toString} due to {status_toString}", "targetClusterTime_toString"_attr = targetClusterTime->toString(), "status_toString"_attr = status.toString());
            }
        }
    ...
    if (atClusterTime) {
        ...
        LOGV2_DEBUG(20792, logSeverityV1toV2(debugLevel).toInt(), "Waiting for 'committed' snapshot to be available for reading: {readConcernArgs}", "readConcernArgs"_attr = readConcernArgs);
        ...
        while (status == ErrorCodes::ReadConcernMajorityNotAvailableYet) {
            LOGV2_DEBUG(20793, logSeverityV1toV2(debugLevel).toInt(), "Snapshot not available yet.");
...
Status waitForLinearizableReadConcernImpl(OperationContext* opCtx, const int readConcernTimeout) {
    CurOpFailpointHelpers::waitWhileFailPointEnabled(
        &hangBeforeLinearizableReadConcern, opCtx, "hangBeforeLinearizableReadConcern", [opCtx]() {
            LOGV2(20795, "batch update - hangBeforeLinearizableReadConcern fail point enabled. "
...
Status waitForSpeculativeMajorityReadConcernImpl(
    ...
    LOGV2_DEBUG(20796, 1, "Servicing speculative majority read, waiting for timestamp {waitTs} to become committed, current commit point: {replCoord_getLastCommittedOpTime}", "waitTs"_attr = waitTs, "replCoord_getLastCommittedOpTime"_attr = replCoord->getLastCommittedOpTime());
    ...
    if (waitStatus.isOK()) {
        LOGV2_DEBUG(20797, 1, "Timestamp {waitTs} became majority committed, waited {t_millis}ms for speculative majority read to be satisfied.", "waitTs"_attr = waitTs, "t_millis"_attr = t.millis());
    }
    return waitStatus;
}
