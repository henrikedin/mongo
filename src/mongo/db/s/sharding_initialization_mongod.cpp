...
bool ShardingInitializationMongoD::initializeShardingAwarenessIfNeeded(OperationContext* opCtx) {
    ...
    if (serverGlobalParams.clusterRole == ClusterRole::ShardServer) {
        if (!foundShardIdentity) {
            LOGV2_WARNING("Started with --shardsvr, but no shardIdentity document was found on "
                         "disk in {}. This most likely means this server has not yet been added to a "
        ...
        if (!shardIdentityBSON.isEmpty()) {
            LOGV2_WARNING("Not started with --shardsvr, but a shardIdentity document was found "
                         "on disk in {}: {}", "NamespaceString_kServerConfigurationNamespace"_attr = NamespaceString::kServerConfigurationNamespace, "shardIdentityBSON"_attr = shardIdentityBSON);
        }
        return false;
    }
}
...
void ShardingInitializationMongoD::updateShardIdentityConfigString(
    ...
    try {
        ...
        if (result.numMatched == 0) {
            LOGV2_WARNING("failed to update config string of shard identity document because it does not exist. This shard could have been removed from the cluster");
        } else {
        ...
        if (!ErrorCodes::isNotMasterError(status.code())) {
            LOGV2_WARNING("Error encountered while trying to update config connection string to {}{}", "newConnectionString_toString"_attr = newConnectionString.toString(), "causedBy_redact_status"_attr = causedBy(redact(status)));
        }
    }
}
