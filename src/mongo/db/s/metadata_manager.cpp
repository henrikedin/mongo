...
void MetadataManager::setFilteringMetadata(CollectionMetadata remoteMetadata) {
    ...
    LOGV2("Updating metadata for collection {} from {} to {} due to version change", "nss_ns"_attr = _nss.ns(), "activeMetadata_toStringBasic"_attr = activeMetadata.toStringBasic(), "remoteMetadata_toStringBasic"_attr = remoteMetadata.toStringBasic());
    ...
    for (auto it = _receivingChunks.begin(); it != _receivingChunks.end();) {
        ...
        LOGV2_DEBUG(2, "Verified chunk {} for collection {} has been migrated to this shard earlier", "redact_receivingRange_toString"_attr = redact(receivingRange.toString()), "nss_ns"_attr = _nss.ns());
...
auto MetadataManager::beginReceive(ChunkRange const& range) -> CleanupNotification {
    ...
    LOGV2("Scheduling deletion of any documents in {} range {} before migrating in a chunk covering the range", "nss_ns"_attr = _nss.ns(), "redact_range_toString"_attr = redact(range.toString()));
...
auto MetadataManager::cleanUpRange(ChunkRange const& range, Date_t whenToDelete)
    ...
    if (!overlapMetadata) {
        ...
        LOGV2("Scheduling {} deletion of {} range {}", "whenStr"_attr = whenStr, "nss_ns"_attr = _nss.ns(), "redact_range_toString"_attr = redact(range.toString()));
        return _pushRangeToClean(lg, range, whenToDelete);
    }

    LOGV2("Deletion of {} range {} will be scheduled after all possibly dependent queries finish", "nss_ns"_attr = _nss.ns(), "redact_range_toString"_attr = redact(range.toString()));
