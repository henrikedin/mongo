...
Status ShardingLogging::logAction(OperationContext* opCtx,
    ...
    if (_actionLogCollectionCreated.load() == 0) {
        ...
        if (result.isOK()) {
            ...
            LOGV2(21793, "couldn't create config.actionlog collection:{causedBy_result}", "causedBy_result"_attr = causedBy(result));
            return result;
        }
    }
...
Status ShardingLogging::logChangeChecked(OperationContext* opCtx,
    ...
    if (_changeLogCollectionCreated.load() == 0) {
        ...
        if (result.isOK()) {
            ...
            LOGV2(21794, "couldn't create config.changelog collection:{causedBy_result}", "causedBy_result"_attr = causedBy(result));
            return result;
        }
    }
...
Status ShardingLogging::_log(OperationContext* opCtx,
    ...
    LOGV2(21795, "about to log metadata event into {logCollName}: {redact_changeLogBSON}", "logCollName"_attr = logCollName, "redact_changeLogBSON"_attr = redact(changeLogBSON));
    ...
    if (!result.isOK()) {
        LOGV2_WARNING(21796, "Error encountered while logging config change with ID [{changeId}] into collection {logCollName}: {redact_result}", "changeId"_attr = changeId, "logCollName"_attr = logCollName, "redact_result"_attr = redact(result));
    }
