            ...
            return txn::sendPrepare(
                ...
                .then([this](PrepareVoteConsensus consensus) mutable {
                    ...
                    if (_decision->getDecision() == CommitDecision::kCommit) {
                        LOGV2_DEBUG(3, "{} Advancing cluster time to the commit timestamp {}", "txn_txnIdToString_lsid_txnNumber"_attr = txn::txnIdToString(_lsid, _txnNumber), "decision_getCommitTimestamp"_attr = *_decision->getCommitTimestamp());
...
void TransactionCoordinator::_done(Status status) {
    ...
    LOGV2_DEBUG(3, "{} Two-phase commit completed with {}", "txn_txnIdToString_lsid_txnNumber"_attr = txn::txnIdToString(_lsid, _txnNumber), "redact_status"_attr = redact(status));
...
void TransactionCoordinator::_logSlowTwoPhaseCommit(
    ...
    LOGV2("{}", "twoPhaseCommitInfoForLog_decision"_attr = _twoPhaseCommitInfoForLog(decision));
}
