...
Status MovePrimarySourceManager::clone(OperationContext* opCtx) {
    ...
    LOGV2("Moving {} primary from: {} to: {}", "dbname"_attr = _dbname, "fromShard"_attr = _fromShard, "toShard"_attr = _toShard);
...
void MovePrimarySourceManager::cleanupOnError(OperationContext* opCtx) {
    ...
    try {
        ...
        LOGV2_WARNING("Failed to clean up movePrimary: {}due to: {}", "redact_requestArgsBSON_obj"_attr = redact(requestArgsBSON.obj()), "redact_ex"_attr = redact(ex));
    }
}
