...
vector<MigrateInfo> BalancerPolicy::balance(const ShardStatisticsVector& shardStats,
    ...
    // 1) Check for shards, which are in draining mode
    {
        for (const auto& stat : shardStats) {
            ...
            for (const auto& chunk : chunks) {
                ...
                if (!to.isValid()) {
                    if (migrations.empty()) {
                        LOGV2_WARNING("Chunk {} is on a draining shard, but no appropriate recipient found", "redact_chunk_toString"_attr = redact(chunk.toString()));
                    }
                    continue;
                }
            ...
            if (migrations.empty()) {
                LOGV2_WARNING("Unable to find any chunk to move from draining shard {}. numJumboChunks: {}", "stat_shardId"_attr = stat.shardId, "numJumboChunks"_attr = numJumboChunks);
            }
        }
    }
    ...
    if (!distribution.tags().empty()) {
        for (const auto& stat : shardStats) {
            ...
            for (const auto& chunk : chunks) {
                ...
                if (chunk.getJumbo()) {
                    LOGV2_WARNING("Chunk {} violates zone {}, but it is jumbo and cannot be moved", "redact_chunk_toString"_attr = redact(chunk.toString()), "redact_tag"_attr = redact(tag));
                    continue;
                }
                ...
                if (!to.isValid()) {
                    if (migrations.empty()) {
                        LOGV2_WARNING("Chunk {} violates zone {}, but no appropriate recipient found", "redact_chunk_toString"_attr = redact(chunk.toString()), "redact_tag"_attr = redact(tag));
                    }
                    continue;
                }
    ...
    for (const auto& tag : tagsPlusEmpty) {
        ...
        if (totalNumberOfShardsWithTag == 0) {
            if (!tag.empty()) {
                LOGV2_WARNING("Zone {} in collection {} has no assigned shards and chunks which fall into it cannot be "
...
bool BalancerPolicy::_singleZoneBalance(const ShardStatisticsVector& shardStats,
    ...
    if (numJumboChunks) {
        LOGV2_WARNING("Shard: {}, collection: {} has only jumbo chunks for zone \'{}\' and cannot be balanced. Jumbo chunks count: {}", "from"_attr = from, "distribution_nss_ns"_attr = distribution.nss().ns(), "tag"_attr = tag, "numJumboChunks"_attr = numJumboChunks);
    }
