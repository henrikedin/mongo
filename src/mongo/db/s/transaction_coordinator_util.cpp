...
repl::OpTime persistParticipantListBlocking(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22188, 3, "{txnIdToString_lsid_txnNumber} Going to write participant list", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber));

    if (MONGO_unlikely(hangBeforeWritingParticipantList.shouldFail())) {
        LOGV2(22189, "Hit hangBeforeWritingParticipantList failpoint");
    ...
    LOGV2_DEBUG(22190, 3, "{txnIdToString_lsid_txnNumber} Wrote participant list", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber));
...
Future<PrepareVoteConsensus> sendPrepare(ServiceContext* service,
    ...
    OperationContextFn operationContextFn = [lsid, txnNumber](OperationContext* opCtx) {
        ...
        if (MONGO_unlikely(hangBeforeSendingPrepare.shouldFail())) {
            LOGV2(22191, "Hit hangBeforeSendingPrepare failpoint");
...
repl::OpTime persistDecisionBlocking(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22192, 3, "{txnIdToString_lsid_txnNumber} Going to write decision {isCommit_commit_abort}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "isCommit_commit_abort"_attr = (isCommit ? "commit" : "abort"));

    if (MONGO_unlikely(hangBeforeWritingDecision.shouldFail())) {
        LOGV2(22193, "Hit hangBeforeWritingDecision failpoint");
    ...
    LOGV2_DEBUG(22194, 3, "{txnIdToString_lsid_txnNumber} Wrote decision {isCommit_commit_abort}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "isCommit_commit_abort"_attr = (isCommit ? "commit" : "abort"));
...
Future<void> sendCommit(ServiceContext* service,
    ...
    OperationContextFn operationContextFn = [lsid, txnNumber](OperationContext* opCtx) {
        ...
        if (MONGO_unlikely(hangBeforeSendingCommit.shouldFail())) {
            LOGV2(22195, "Hit hangBeforeSendingCommit failpoint");
...
Future<void> sendAbort(ServiceContext* service,
    ...
    OperationContextFn operationContextFn = [lsid, txnNumber](OperationContext* opCtx) {
        ...
        if (MONGO_unlikely(hangBeforeSendingAbort.shouldFail())) {
            LOGV2(22196, "Hit hangBeforeSendingAbort failpoint");
...
void deleteCoordinatorDocBlocking(OperationContext* opCtx,
                                  const LogicalSessionId& lsid,
                                  TxnNumber txnNumber) {
    LOGV2_DEBUG(22197, 3, "{txnIdToString_lsid_txnNumber} Going to delete coordinator doc", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber));

    if (MONGO_unlikely(hangBeforeDeletingCoordinatorDoc.shouldFail())) {
        LOGV2(22198, "Hit hangBeforeDeletingCoordinatorDoc failpoint");
    ...
    LOGV2_DEBUG(22199, 3, "{txnIdToString_lsid_txnNumber} Deleted coordinator doc", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber));

    hangAfterDeletingCoordinatorDoc.execute([&](const BSONObj& data) {
        LOGV2(22200, "Hit hangAfterDeletingCoordinatorDoc failpoint");
...
Future<PrepareResponse> sendPrepareToShard(ServiceContext* service,
    ...
    auto f = txn::doWhile(
        ...
        [&scheduler,
         ...
         operationContextFn] {
            LOGV2_DEBUG(22201, 3, "{txnIdToString_lsid_txnNumber} Coordinator going to send command {commandObj} to {isLocalShard_local}shard {shardId}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "commandObj"_attr = commandObj, "isLocalShard_local"_attr = (isLocalShard ? "local " : ""), "shardId"_attr = shardId);
                ...
                .then([lsid, txnNumber, shardId, commandObj = commandObj.getOwned()](
                    ...
                    if (status.isOK()) {
                        ...
                        if (prepareTimestampField.eoo() ||
                            ...
                            LOGV2(22202, "{txnIdToString_lsid_txnNumber} {redact_abortStatus}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "redact_abortStatus"_attr = redact(abortStatus));

                            return PrepareResponse{
                                shardId, PrepareVote::kAbort, boost::none, abortStatus};
                        }

                        LOGV2_DEBUG(22203, 3, "{txnIdToString_lsid_txnNumber} Coordinator shard received a vote to commit from shard {shardId} with prepareTimestamp: {prepareTimestampField_timestamp}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "shardId"_attr = shardId, "prepareTimestampField_timestamp"_attr = prepareTimestampField.timestamp());
                    ...
                    LOGV2_DEBUG(22204, 3, "{txnIdToString_lsid_txnNumber} Coordinator shard received {status} from shard {shardId} for {commandObj}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "status"_attr = status, "shardId"_attr = shardId, "commandObj"_attr = commandObj);
    ...
    return std::move(f).onError<ErrorCodes::TransactionCoordinatorReachedAbortDecision>(
        [lsid, txnNumber, shardId](const Status& status) {
            LOGV2_DEBUG(22205, 3, "{txnIdToString_lsid_txnNumber} Prepare stopped retrying due to retrying being cancelled", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber));
...
Future<void> sendDecisionToShard(ServiceContext* service,
    ...
    return txn::doWhile(
        ...
        [&scheduler,
         ...
         operationContextFn,
         commandObj = commandObj.getOwned()] {
            LOGV2_DEBUG(22206, 3, "{txnIdToString_lsid_txnNumber} Coordinator going to send command {commandObj} to {isLocalShard_local}shard {shardId}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "commandObj"_attr = commandObj, "isLocalShard_local"_attr = (isLocalShard ? "local " : ""), "shardId"_attr = shardId);
            ...
            return scheduler
                ...
                .then([lsid, txnNumber, shardId, commandObj = commandObj.getOwned()](
                    ...
                    LOGV2_DEBUG(22207, 3, "{txnIdToString_lsid_txnNumber} Coordinator shard received {status} in response to {commandObj} from shard {shardId}", "txnIdToString_lsid_txnNumber"_attr = txnIdToString(lsid, txnNumber), "status"_attr = status, "commandObj"_attr = commandObj, "shardId"_attr = shardId);
