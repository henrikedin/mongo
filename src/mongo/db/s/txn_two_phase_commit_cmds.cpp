...
public:
    ...
    public:
        ...
        Response typedRun(OperationContext* opCtx) {
            ...
            LOGV2_DEBUG(22208, 3, "Participant shard received prepareTransaction for transaction with txnNumber {opCtx_getTxnNumber} on session {opCtx_getLogicalSessionId_toBSON}", "opCtx_getTxnNumber"_attr = opCtx->getTxnNumber(), "opCtx_getLogicalSessionId_toBSON"_attr = opCtx->getLogicalSessionId()->toBSON());
...
std::set<ShardId> validateParticipants(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22209, 3, "Coordinator shard received request to coordinate commit with "
              "participant list {ss_str} for {opCtx_getLogicalSessionId_getId}:{opCtx_getTxnNumber}", "ss_str"_attr = ss.str(), "opCtx_getLogicalSessionId_getId"_attr = opCtx->getLogicalSessionId()->getId(), "opCtx_getTxnNumber"_attr = opCtx->getTxnNumber());
    ...
    public:
        ...
        void typedRun(OperationContext* opCtx) {
            ...
            if (MONGO_unlikely(hangAfterStartingCoordinateCommit.shouldFail())) {
                LOGV2(22210, "Hit hangAfterStartingCoordinateCommit failpoint");
            ...
            LOGV2_DEBUG(22211, 3, "Going to recover decision from local participant for {opCtx_getLogicalSessionId_getId}:{opCtx_getTxnNumber}", "opCtx_getLogicalSessionId_getId"_attr = opCtx->getLogicalSessionId()->getId(), "opCtx_getTxnNumber"_attr = opCtx->getTxnNumber());
