...
void TransactionCoordinatorCatalog::join() {
    ...
    while (!_noActiveCoordinatorsCV.wait_for(
        ...
        LOGV2("After 5 seconds of wait there are still {} sessions left with active coordinators which have not yet completed", "coordinatorsBySession_size"_attr = _coordinatorsBySession.size());
        LOGV2("{}", "toString_ul"_attr = _toString(ul));
    }
}
