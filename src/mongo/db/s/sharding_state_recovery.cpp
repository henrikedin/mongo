...
void ShardingStateRecovery::endMetadataOp(OperationContext* opCtx) {
    Status status =
        modifyRecoveryDocument(opCtx, RecoveryDocument::Decrement, WriteConcernOptions());
    if (!status.isOK()) {
        LOGV2_WARNING("Failed to decrement minOpTimeUpdaters due to {}", "redact_status"_attr = redact(status));
    }
}
...
Status ShardingStateRecovery::recover(OperationContext* opCtx) {
    ...
    if (!status.isOK()) {
        LOGV2_WARNING("Failed to reset sharding state recovery document due to {}", "redact_status"_attr = redact(status));
    }
