...
Status modifyRecoveryDocument(OperationContext* opCtx,
    ...
    try {
        ...
        LOGV2_DEBUG(21798, 1, "Changing sharding recovery document {redact_updateObj}", "redact_updateObj"_attr = redact(updateObj));
...
void ShardingStateRecovery::endMetadataOp(OperationContext* opCtx) {
    Status status =
        modifyRecoveryDocument(opCtx, RecoveryDocument::Decrement, WriteConcernOptions());
    if (!status.isOK()) {
        LOGV2_WARNING(21803, "Failed to decrement minOpTimeUpdaters due to {redact_status}", "redact_status"_attr = redact(status));
    }
}
...
Status ShardingStateRecovery::recover(OperationContext* opCtx) {
    ...
    LOGV2(21799, "Sharding state recovery process found document {redact_recoveryDoc_toBSON}", "redact_recoveryDoc_toBSON"_attr = redact(recoveryDoc.toBSON()));
    ...
    if (!recoveryDoc.getMinOpTimeUpdaters()) {
        ...
        if (prevOpTime) {
            LOGV2(21800, "No in flight metadata change operations, so config server optime updated from {prevOpTime} to {recoveryDoc_getMinOpTime}", "prevOpTime"_attr = *prevOpTime, "recoveryDoc_getMinOpTime"_attr = recoveryDoc.getMinOpTime());
        }
        return Status::OK();
    }

    LOGV2(21801, "Sharding state recovery document indicates there were {recoveryDoc_getMinOpTimeUpdaters} metadata change operations in flight. Contacting the config server primary in order "
    ...
    LOGV2(21802, "Sharding state recovered. New config server opTime is {grid_configOpTime}", "grid_configOpTime"_attr = grid->configOpTime());
    ...
    if (!status.isOK()) {
        LOGV2_WARNING(21804, "Failed to reset sharding state recovery document due to {redact_status}", "redact_status"_attr = redact(status));
    }
