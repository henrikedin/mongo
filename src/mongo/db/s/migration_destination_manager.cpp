...
void MigrationDestinationManager::_setStateFailWarn(StringData msg) {
    LOGV2_WARNING(21730, "{msg}", "msg"_attr = msg);
    {
        stdx::lock_guard<Latch> sl(_mutex);
        _errmsg = msg.toString();
        _state = FAIL;
...
repl::OpTime MigrationDestinationManager::cloneDocumentsFromDonor(
    ...
    stdx::thread inserterThread{[&] {
        ...
        try {
            ...
            LOGV2(21719, "Batch insertion failed {causedBy_redact_exceptionToStatus}", "causedBy_redact_exceptionToStatus"_attr = causedBy(redact(exceptionToStatus())));
        }
    }};
...
void MigrationDestinationManager::_migrateDriver(OperationContext* opCtx) {
    ...
    LOGV2(21720, "Starting receiving end of migration of chunk {redact_min} -> {redact_max} for collection {nss_ns} from {fromShard} at epoch {epoch_toString} with session id {sessionId}", "redact_min"_attr = redact(_min), "redact_max"_attr = redact(_max), "nss_ns"_attr = _nss.ns(), "fromShard"_attr = _fromShard, "epoch_toString"_attr = _epoch.toString(), "sessionId"_attr = *_sessionId);
    ...
    if (initialState == ABORT) {
        LOGV2_ERROR(21732, "Migration abort requested before it started");
        return;
    }
...

    {
        ...
        if (_useFCV44Protocol) {
            while (migrationutil::checkForConflictingDeletions(
                ...
                LOGV2(21721, "Migration paused because range overlaps with a "
                          "range that is scheduled for deletion: collection: {nss_ns} range: {redact_range_toString}", "nss_ns"_attr = _nss.ns(), "redact_range_toString"_attr = redact(range.toString()));
...

    {
        ...
        while (true) {
            ...
            for (i = 0; i < maxIterations; i++) {
                ...
                if (getState() == ABORT) {
                    LOGV2(21722, "Migration aborted while waiting for replication at catch up stage");
                    return;
                }
                ...
                if (i > 100) {
                    LOGV2(21723, "secondaries having hard time keeping up with migrate");
                }
...

    {
        ...
        LOGV2(21724, "Waiting for replication to catch up before entering critical section");
        ...
        LOGV2(21725, "Chunk data replicated successfully.");
    }

    {
        ...
        while (getState() == STEADY || getState() == COMMIT_START) {
            ...
            if (getState() == ABORT) {
                LOGV2(21726, "Migration aborted while transferring mods");
                return;
            }
    ...
    if (xfer["reload"].isABSONObj()) {
        ...
        while (i.more()) {
            ...
            if (willOverrideLocalId(opCtx,
                ...
                LOGV2_WARNING(21731, "{errMsg}", "errMsg"_attr = errMsg);
...
bool MigrationDestinationManager::_flushPendingWrites(OperationContext* opCtx,
    ...
    if (!opReplicatedEnough(opCtx, lastOpApplied, _writeConcern)) {
        ...
        if (sampler.tick()) {
            LOGV2(21727, "migrate commit waiting for a majority of slaves for '{nss_ns}' {redact_min} -> {redact_max} waiting for: {op}", "nss_ns"_attr = _nss.ns(), "redact_min"_attr = redact(_min), "redact_max"_attr = redact(_max), "op"_attr = op);
        }
        return false;
    }

    LOGV2(21728, "migrate commit succeeded flushing to secondaries for '{nss_ns}' {redact_min} -> {redact_max}", "nss_ns"_attr = _nss.ns(), "redact_min"_attr = redact(_min), "redact_max"_attr = redact(_max));
...
void MigrationDestinationManager::_forgetPending(OperationContext* opCtx, ChunkRange const& range) {
    ...
    if (!optMetadata || !(*optMetadata)->isSharded() ||
        (*optMetadata)->getCollVersion().epoch() != _epoch) {
        LOGV2(21729, "No need to forget pending chunk {redact_range_toString} because the epoch for {nss_ns} changed", "redact_range_toString"_attr = redact(range.toString()), "nss_ns"_attr = _nss.ns());
        return;
    }
