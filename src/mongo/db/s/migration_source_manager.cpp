...
MigrationSourceManager::MigrationSourceManager(OperationContext* opCtx,
    ...
    LOGV2("Starting chunk migration {} with expected collection version epoch {}", "redact_args_toString"_attr = redact(_args.toString()), "args_getVersionEpoch"_attr = _args.getVersionEpoch());
...
void MigrationSourceManager::cleanupOnError() {
    ...
    try {
        ...
        LOGV2_WARNING("Failed to clean up migration: {}due to: {}", "redact_args_toString"_attr = redact(_args.toString()), "redact_ex"_attr = redact(ex));
    }
}
