...
void safeClose(int fd) {
    ...
    if (close(fd) != 0) {
        ...
        LOGV2_ERROR(22516, "failed to close fd {fd}: {ewd}", "fd"_attr = fd, "ewd"_attr = ewd);
        fassertFailed(40318);
    }
}
...
void ProgramRunner::start() {
...

    {
        ...
        if (status != 0) {
            const auto ewd = errnoWithDescription();
            LOGV2_ERROR(22517, "failed to create pipe: {ewd}", "ewd"_attr = ewd);
            fassertFailed(16701);
        }
#ifndef _WIN32
        ...
        if (status != 0) {
            const auto ewd = errnoWithDescription();
            LOGV2_ERROR(22518, "failed to set FD_CLOEXEC on pipe end 0: {ewd}", "ewd"_attr = ewd);
            fassertFailed(40308);
        }
        status = fcntl(pipeEnds[1], F_SETFD, FD_CLOEXEC);
        if (status != 0) {
            const auto ewd = errnoWithDescription();
            LOGV2_ERROR(22519, "failed to set FD_CLOEXEC on pipe end 1: {ewd}", "ewd"_attr = ewd);
            fassertFailed(40317);
        }
#endif
...

    {
        ...
        LOGV2(22497, "{ss_str}", "ss_str"_attr = ss.str());
    }
}
...
bool wait_for_pid(ProcessId pid, bool block = true, int* exit_code = nullptr) {
    ...
    if (ret == WAIT_TIMEOUT) {
        ...
        const auto ewd = errnoWithDescription();
        LOGV2(22498, "wait_for_pid: WaitForSingleObject failed: {ewd}", "ewd"_attr = ewd);
    }
    ...
    if (GetExitCodeProcess(h, &tmp)) {
        ...
        const auto ewd = errnoWithDescription();
        LOGV2(22499, "GetExitCodeProcess failed: {ewd}", "ewd"_attr = ewd);
        return false;
    }
#else
    int tmp;
...
BSONObj WaitMongoProgram(const BSONObj& a, void* data) {
    ...
    if (!registry.isPortRegistered(port)) {
        LOGV2(22500, "No db started on port: {port}", "port"_attr = port);
...
BSONObj ResetDbpath(const BSONObj& a, void* data) {
    ...
    if (path.empty()) {
        LOGV2_WARNING(22511, "ResetDbpath(): nothing to do, path was empty");
        return undefinedReturn;
    }
...
BSONObj PathExists(const BSONObj& a, void* data) {
    ...
    string path = a.firstElement().valuestrsafe();
    if (path.empty()) {
        LOGV2_WARNING(22512, "PathExists(): path was empty");
...
void copyDir(const boost::filesystem::path& from, const boost::filesystem::path& to) {
    ...
    while (i != end) {
        ...
        if (p.leaf() == "metrics.interim" || p.leaf() == "metrics.interim.temp") {
            ...
            if (ec) {
                LOGV2(22501, "Skipping copying of file from '{p_generic_string}' to '{to_p_leaf_generic_string}' due to: {ec_message}", "p_generic_string"_attr = p.generic_string(), "to_p_leaf_generic_string"_attr = (to / p.leaf()).generic_string(), "ec_message"_attr = ec.message());
            }
...
BSONObj CopyDbpath(const BSONObj& a, void* data) {
    ...
    if (from.empty() || to.empty()) {
        LOGV2_WARNING(22513, "CopyDbpath(): nothing to do, source or destination path(s) were empty");
        return undefinedReturn;
    }
...
inline void kill_wrapper(ProcessId pid, int sig, int port, const BSONObj& opt) {
    ...
    if (event == nullptr) {
        ...
        if (gle != ERROR_FILE_NOT_FOUND) {
            const auto ewd = errnoWithDescription();
            LOGV2_WARNING(22514, "kill_wrapper OpenEvent failed: {ewd}", "ewd"_attr = ewd);
        } else {
            LOGV2(22502, "kill_wrapper OpenEvent failed to open event to the process {pid_asUInt32}. It has likely died already or server is running an older version. Attempting to shutdown through admin command.", "pid_asUInt32"_attr = pid.asUInt32());
    ...
    if (!result) {
        const auto ewd = errnoWithDescription();
        LOGV2_ERROR(22520, "kill_wrapper SetEvent failed: {ewd}", "ewd"_attr = ewd);
        return;
    }
#else
    ...
    if (x) {
        if (errno == ESRCH) {
            ...
            const auto ewd = errnoWithDescription();
            LOGV2(22503, "killFailed: {ewd}", "ewd"_attr = ewd);
            verify(x == 0);
        }
    }
...
int killDb(int port, ProcessId _pid, int signal, const BSONObj& opt, bool waitPid = true) {
    ...
    if (port > 0) {
        if (!registry.isPortRegistered(port)) {
            LOGV2(22504, "No db started on port: {port}", "port"_attr = port);
            return 0;
        }
        pid = registry.pidForPort(port);
    } else {
    ...
    if (!waitPid) {
        LOGV2(22505, "skip waiting for pid {pid} to terminate", "pid"_attr = pid);
        return 0;
    }

    int exitCode = EXIT_FAILURE;
    try {
        LOGV2(22506, "waiting for process {pid} to terminate.", "pid"_attr = pid);
        wait_for_pid(pid, true, &exitCode);
    } catch (...) {
        LOGV2_WARNING(22515, "process {pid} failed to terminate.", "pid"_attr = pid);
        return EXIT_FAILURE;
    }
...
BSONObj StopMongoProgram(const BSONObj& a, void* data) {
    ...
    LOGV2(22507, "shell: stopping mongo program, waitpid={getWaitPid_a}", "getWaitPid_a"_attr = getWaitPid(a));
    int code =
        killDb(port, ProcessId::fromNative(0), getSignal(a), getStopMongodOpts(a), getWaitPid(a));
    LOGV2(22508, "shell: stopped mongo program on port {port}", "port"_attr = port);
...
BSONObj StopMongoProgramByPid(const BSONObj& a, void* data) {
    ...
    LOGV2(22509, "shell: stopped mongo program with pid {pid}", "pid"_attr = pid);
    return BSON("" << (double)code);
}
...
int KillMongoProgramInstances() {
    ...
    for (auto&& pid : pids) {
        ...
        if (code != EXIT_SUCCESS) {
            LOGV2(22510, "Process with pid {pid} exited with error code {code}", "pid"_attr = pid, "code"_attr = code);
            returnCode = code;
        }
    }
    return returnCode;
