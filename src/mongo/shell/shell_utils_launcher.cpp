...
void safeClose(int fd) {
    ...
    if (close(fd) != 0) {
        ...
        LOGV2_ERROR("failed to close fd {}: {}", "fd"_attr = fd, "ewd"_attr = ewd);
        fassertFailed(40318);
    }
}
...
void ProgramRunner::start() {
...

    {
        ...
        if (status != 0) {
            const auto ewd = errnoWithDescription();
            LOGV2_ERROR("failed to create pipe: {}", "ewd"_attr = ewd);
            fassertFailed(16701);
        }
#ifndef _WIN32
        ...
        if (status != 0) {
            const auto ewd = errnoWithDescription();
            LOGV2_ERROR("failed to set FD_CLOEXEC on pipe end 0: {}", "ewd"_attr = ewd);
            fassertFailed(40308);
        }
        status = fcntl(pipeEnds[1], F_SETFD, FD_CLOEXEC);
        if (status != 0) {
            const auto ewd = errnoWithDescription();
            LOGV2_ERROR("failed to set FD_CLOEXEC on pipe end 1: {}", "ewd"_attr = ewd);
            fassertFailed(40317);
        }
#endif
...
BSONObj ResetDbpath(const BSONObj& a, void* data) {
    ...
    if (path.empty()) {
        LOGV2_WARNING("ResetDbpath(): nothing to do, path was empty");
        return undefinedReturn;
    }
...
BSONObj PathExists(const BSONObj& a, void* data) {
    ...
    string path = a.firstElement().valuestrsafe();
    if (path.empty()) {
        LOGV2_WARNING("PathExists(): path was empty");
...
BSONObj CopyDbpath(const BSONObj& a, void* data) {
    ...
    if (from.empty() || to.empty()) {
        LOGV2_WARNING("CopyDbpath(): nothing to do, source or destination path(s) were empty");
        return undefinedReturn;
    }
...
inline void kill_wrapper(ProcessId pid, int sig, int port, const BSONObj& opt) {
    ...
    if (event == nullptr) {
        ...
        if (gle != ERROR_FILE_NOT_FOUND) {
            const auto ewd = errnoWithDescription();
            LOGV2_WARNING("kill_wrapper OpenEvent failed: {}", "ewd"_attr = ewd);
        } else {
    ...
    if (!result) {
        const auto ewd = errnoWithDescription();
        LOGV2_ERROR("kill_wrapper SetEvent failed: {}", "ewd"_attr = ewd);
        return;
    }
#else
...
int killDb(int port, ProcessId _pid, int signal, const BSONObj& opt, bool waitPid = true) {
    ...
    try {
        ...
        LOGV2_WARNING("process {} failed to terminate.", "pid"_attr = pid);
        return EXIT_FAILURE;
    }
