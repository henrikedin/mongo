...
#include "mongo/log/ramlog_sink.h"
#include "mongo/log/text_formatter.h"
#include "mongo/log/uassert_sink.h"
...
public:
    LogDuringInitShutdownTester() {
        ...
        LOG(20001, "log during init");
        ASSERT_EQUALS(lines.back(), "log during init");
    }
    ~LogDuringInitShutdownTester() {
        enabled = true;
        LOG(4600800, "log during shutdown");
...
class logTypesTest : public logTest {
public:
    using logTest::logTest;
...
TEST_F(logTypesTest, Numeric) {
    auto testIntegral = [&](auto dummy) {
        using T = decltype(dummy);

        auto test = [&](auto value) {
            text.clear();
            LOG(20012, "{name}", "name"_attr = value);
    ...
    auto testFloatingPoint = [&](auto dummy) {
        ...
        auto test = [&](auto value) {
            text.clear();
            LOG(20013, "{name}", "name"_attr = value);
    ...
    bool b = true;
    LOG(20014, "bool {name}", "name"_attr = b);
    ASSERT_EQUALS(text.back(), "bool true");
    validateJSON(b);
    ASSERT(lastBSONElement().Bool() == b);

    char c = 1;
    LOG(20015, "char {name}", "name"_attr = c);
...
TEST_F(logTypesTest, Enums) {
    // enums
    enum UnscopedEnum { UnscopedEntry };
    LOG(20076, "{name}", "name"_attr = UnscopedEntry);
    ...
    LOG(20077, "{name}", "name"_attr = ScopedEnum::Entry);
    ...
    LOG(20078, "{name}", "name"_attr = UnscopedEntryWithToString);
...
TEST_F(logTypesTest, Stringlike) {
    const char* c_str = "a c string";
    LOG(20016, "c string {name}", "name"_attr = c_str);
    ...
    LOG(20017, "c string {name}", "name"_attr = c_str2);
    ...
    LOG(20018, "std::string {name}", "name"_attr = str);
    ...
    LOG(20019, "StringData {name}", "name"_attr = str_data);
...

    {
        ...
        LOG(4329200, "std::string_view {name}", "name"_attr = s);
...
TEST_F(logTypesTest, BSONObj) {
    BSONObj bsonObj = BSONObjBuilder()
                          ...
                          .obj();
    LOG(20020, "bson {name}", "name"_attr = bsonObj);
    ASSERT(text.back() ==
...
TEST_F(logTypesTest, BSONElement) {
    BSONObj bsonObj = BSONObjBuilder()
                          .append("int32"_sd, 1)
                          .append("int64"_sd, std::numeric_limits<int64_t>::max())
                          .append("double"_sd, 1.0)
                          .append("str"_sd, "a StringData"_sd)
                          .obj();
    LOG(20022, "bson element {name}", "name"_attr = bsonObj.getField("int32"_sd));
...
TEST_F(logTypesTest, DateT) {
    ...
    for (auto localTimezone : {true, false}) {
        ...
        LOG(20023, "Date_t {name}", "name"_attr = date);
...
TEST_F(logTypesTest, Decimal128) {
    LOG(20024, "Decimal128 {name}", "name"_attr = Decimal128::kPi);
...
TEST_F(logTypesTest, OID) {
    OID oid = OID::gen();
    LOG(20025, "OID {name}", "name"_attr = oid);
...
TEST_F(logTypesTest, Timestamp) {
    Timestamp ts = Timestamp::max();
    LOG(20026, "Timestamp {name}", "name"_attr = ts);
...
TEST_F(logTypesTest, UUID) {
    UUID uuid = UUID::gen();
    LOG(20027, "UUID {name}", "name"_attr = uuid);
...
TEST_F(logTypesTest, BoostOptional) {
    LOG(20028, "boost::optional empty {name}", "name"_attr = boost::optional<bool>());
    ASSERT_EQUALS(text.back(),
    ...
    LOG(20029, "boost::optional<bool> {name}", "name"_attr = boost::optional<bool>(true));
...
TEST_F(logTypesTest, Duration) {
    Milliseconds ms{12345};
    LOG(20033, "Duration {name}", "name"_attr = ms);
...
TEST_F(logTest, TextFormat) {
    ...
    LOG_OPTIONS(20065, {LogTag::kNone}, "warning");
    ASSERT(lines.back().rfind("** WARNING: warning") == std::string::npos);

    LOG_OPTIONS(20066, {LogTag::kStartupWarnings}, "warning");
    ASSERT(lines.back().rfind("** WARNING: warning") != std::string::npos);
...
// between them.
class logJsonBsonTest : public logTest {
public:
    using logTest::logTest;
...
TEST_F(logJsonBsonTest, Root) {
    ...
    validate([](const BSONObj& obj) {
        ...
        ASSERT_EQUALS(obj.getField(kComponentFieldName).String(),
                      LogComponent(MONGO_LOG_DEFAULT_COMPONENT).getNameForLog());
...
TEST_F(logJsonBsonTest, Attr) {
    LOG(20038, "test {name}", "name"_attr = 1);
    validate([](const BSONObj& obj) {
...
TEST_F(logJsonBsonTest, MessageReconstructionBasic) {
    LOG(20039, "test {name:d}", "name"_attr = 2);
    validate([](const BSONObj& obj) {
...
TEST_F(logJsonBsonTest, MessageReconstructionPadded) {
    LOG(20040, "test {name: <4}", "name"_attr = 2);
    validate([](const BSONObj& obj) {
...
TEST_F(logJsonBsonTest, Tags) {
    LOG_OPTIONS(20068, {LogTag::kStartupWarnings}, "warning");
    validate([](const BSONObj& obj) {
...
TEST_F(logJsonBsonTest, Component) {
    LOG_OPTIONS(20069, {LogComponent::kControl}, "different component");
    validate([](const BSONObj& obj) {
...
TEST_F(logJsonBsonTest, CustomFormatting) {
    TypeWithBSON t(1.0, 2.0);
    LOG(20041, "{name} custom formatting", "name"_attr = t);
...
TEST_F(logJsonBsonTest, BsonAttr) {
    TypeWithBSON t(1.0, 2.0);
    LOG(20042, "{name} bson", "name"_attr = t.toBSON());
    validate([&t](const BSONObj& obj) {
...
TEST_F(logJsonBsonTest, TypeWithoutBSON) {
    TypeWithoutBSON t2(1.0, 2.0);
    LOG(20043, "{name} custom formatting", "name"_attr = t2);
...
TEST_F(logJsonBsonTest, TypeWithBSONSerialize) {
    TypeWithBSONSerialize t3(1.0, 2.0);
    LOG(20044, "{name}", "name"_attr = t3);
...
TEST_F(logJsonBsonTest, TypeWithoutBSONFormatters) {
    TypeWithBothBSONFormatters t4(1.0, 2.0);
    LOG(20045, "{name}", "name"_attr = t4);
...
TEST_F(logJsonBsonTest, TypeWithBSONArray) {
    TypeWithBSONArray t5;
    LOG(20046, "{name}", "name"_attr = t5);
...
TEST_F(logJsonBsonTest, TypeWithNonMemberFormatting) {
    TypeWithNonMemberFormatting t6;
    LOG(20080, "{name}", "name"_attr = t6);
...
TEST_F(logJsonBsonTest, DynamicAttributes) {
...
TEST_F(logJsonBsonTest, AttrWrapperOne) {
    A a;
    LOG(4759400, "{}", logAttrs(a));
...
TEST_F(logJsonBsonTest, AttrWrapperTwo) {
    A a;
    B b;
    LOG(4759401, "{}", logAttrs(a), logAttrs(b));
...
TEST_F(logJsonBsonTest, AttrWrapperRvalue) {
    A a;
    B b;
    LOG(4759402, "{}", logAttrs(a), logAttrs(b), logAttrs(C()));
    validate([&a, &b](const BSONObj& obj) {
        ASSERT_EQUALS(obj.getField(kAttributesFieldName).Obj().getField("a").String(),
                      a.toString());
        ASSERT_EQUALS(obj.getField(kAttributesFieldName).Obj().getField("b").String(),
...
TEST_F(logJsonBsonTest, AttrWrapperComplex) {
    D d;
    LOG(4759403, "{}", logAttrs(d));
...
TEST_F(logJsonBsonTest, AttrWrapperComplexHierarchy) {
    E e;
    LOG(4759404, "{}", logAttrs(e));
...
class logContainerTest : public logTypesTest {
public:
    using logTypesTest::logTypesTest;
...
TEST_F(logContainerTest, StandardSequential) {
    std::vector<std::string> vectorStrings = {"str1", "str2", "str3"};
    LOG(20047, "{name}", "name"_attr = vectorStrings);
...
TEST_F(logContainerTest, Uint32Sequence) {
    std::vector<uint32_t> vectorUInt32s = {0, 1, std::numeric_limits<uint32_t>::max()};
    LOG(4684000, "{vectorUInt32s}", "vectorUInt32s"_attr = vectorUInt32s);
...
TEST_F(logContainerTest, OptionalsAsElements) {
    std::forward_list<boost::optional<bool>> listOptionalBool = {true, boost::none, false};
    LOG(20049, "{name}", "name"_attr = listOptionalBool);
...
TEST_F(logContainerTest, Nested) {
    std::array<std::deque<int>, 4> arrayOfDeques = {{{0, 1}, {2, 3}, {4, 5}, {6, 7}}};
    LOG(20050, "{name}", "name"_attr = arrayOfDeques);
...
TEST_F(logContainerTest, AssociativeWithOptionalSequential) {
    ...
    LOG(20052, "{name}", "name"_attr = mapOptionalVector);
...
TEST_F(logContainerTest, DurationVector) {
    std::vector<Nanoseconds> nanos = {Nanoseconds(10), Nanoseconds(100)};
    LOG(20081, "{name}", "name"_attr = nanos);
...
TEST_F(logContainerTest, StringMapUint32) {
    ...
    LOG(4684001, "{mapOfUInt32s}", "mapOfUInt32s"_attr = mapOfUInt32s);
...
TEST_F(logTest, Unicode) {
    ...
    for (const auto& pair : strs) {
        LOG(20053, "{name}", "name"_attr = pair.first);
...
TEST_F(logTest, JsonTruncation) {
    ...
    LOG(20085, "{name}{attr2}", "name"_attr = builder.done(), "attr2"_attr = true);
    ...
    LOG_OPTIONS(20086, {LogTruncation::Disabled}, "{name}", "name"_attr = builder.done());
    ...
    LOG(20087, "{name}", "name"_attr = arrToLog);
...
TEST_F(logTest, Ramlog_CircularBuffer) {
...
TEST_F(logTest, Ramlog_GiantLine) {
...
TEST_F(logTest, MultipleDomains) {
    ...
    LOG_OPTIONS(20070, {&other_domain}, "test");
    ASSERT(global_lines.lines().empty());
    ASSERT(other_lines.back() == "test");

    LOG(20060, "global domain log");
    ASSERT(global_lines.back() == "global domain log");
    ASSERT(other_lines.back() == "test");
}

TEST_F(logTest, FileLogging) {
    auto LOG_dir = std::make_unique<mongo::unittest::TempDir>("log");

    // Examples of some capabilities for file logging. Rotation, header/footer support.
    std::string file_name = LOG_dir->path() + "/file.log";
    std::string rotated_file_name = LOG_dir->path() + "/file-rotated.log";
...
TEST_F(logTest, UserAssert) {
    ...
    ASSERT_THROWS_WITH_CHECK(
        LOG_OPTIONS(4652000, {UserAssertAfterLog(ErrorCodes::BadValue)}, "uasserting log"),
        DBException,
    ...
    ASSERT_THROWS_WITH_CHECK(LOG_OPTIONS(4652001,
    ...
    ASSERT_THROWS_WITH_CHECK(LOG_OPTIONS(4716000, {UserAssertAfterLog()}, "uasserting log"),
                             DBException,
                             [&lines](const DBException& ex) {
...
class UnstructuredLoggingTest : public logJsonBsonTest {};
