...
public:
    LogDuringInitTester() {
        ...
        sink->set_filter(ComponentSettingsFilter(LogManager::global().getGlobalSettings()));
...
TEST_F(LogTestV2, Threads) {
    ...
    plainSink->set_filter(ComponentSettingsFilter(LogManager::global().getGlobalSettings()));
    plainSink->set_formatter(PlainFormatter());
    attach(plainSink);

    std::vector<std::string> linesText;
    auto textSink = LogTestBackend::create(linesText);
    textSink->set_filter(ComponentSettingsFilter(LogManager::global().getGlobalSettings()));
    textSink->set_formatter(TextFormatter());
    attach(textSink);

    std::vector<std::string> linesJson;
    auto jsonSink = LogTestBackend::create(linesJson);
    jsonSink->set_filter(ComponentSettingsFilter(LogManager::global().getGlobalSettings()));
...
TEST_F(LogTestV2, Ramlog) {
    ...
    testSink->set_filter(ComponentSettingsFilter(LogManager::global().getGlobalSettings()));
...
TEST_F(LogTestV2, MultipleDomains) {
    ...
    other_sink->set_filter(ComponentSettingsFilter(LogManager::global().getGlobalSettings()));
...
//    attach(sink);
//
//    // Example on how we could instead support multiple domains using just a single
//    boost::log::core
...
//        DomainId::Domain1, LogManager::global().getGlobalSettings()));
...
//        DomainId::Domain2, LogManager::global().getGlobalSettings()));
