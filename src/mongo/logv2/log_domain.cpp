...
LogDebugRecord LogDomain::openDebug(LogSeverity severity, LogComponent component, LogTag tags) {
    std::unique_ptr<LogDebugRecordImpl> debugRecord;
    auto record = impl().source().open_record(severity, component, tags, StringData{});
    if (record) {
        debugRecord = std::make_unique<LogDebugRecordImpl>(std::move(record));
    }
    return LogDebugRecord(std::move(debugRecord));
}
