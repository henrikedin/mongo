...
struct JSONValueExtractor {
    ...
    void operator()(StringData name, CustomAttributeValue const& val) {
        if (val.BSONAppend) {
            ...
            storeUnquoted(name);
            builder.obj().getField(name).jsonStringBuffer(
                JsonStringFormat::ExtendedRelaxedV200, false, 0, _buffer);
        } else if (val.toBSON) {
            // This is a JSON subobject, no quotes needed
            storeUnquoted(name);
            val.toBSON().jsonStringBuffer(JsonStringFormat::ExtendedRelaxedV200, 0, false, _buffer);
        } else {
    ...
    void operator()(StringData name, const BSONObj* val) {
        // This is a JSON subobject, no quotes needed
        storeUnquoted(name);
        val->jsonStringBuffer(JsonStringFormat::ExtendedRelaxedV200, 0, false, _buffer);
    }
...
void JSONFormatter::operator()(boost::log::record_view const& rec,
    ...
    if (tags != LogTag::kNone) {
        tag = fmt::format(",\"{}\":{}",
                          ...
                          tags.toBSON().jsonString(JsonStringFormat::ExtendedRelaxedV200, 0, true));
    }
