...
CompareResult compareAllShardVersions(const CachedCollectionRoutingInfo& routingInfo,
    ...
    for (const auto& shardVersionEntry : remoteShardVersions) {
        ...
        try {
            ...
            LOGV2_WARNING("could not lookup shard {} in local cache, shard metadata may have changed or be unavailable{}", "shardId"_attr = shardId, "causedBy_ex"_attr = causedBy(ex));
...
Status ChunkManagerTargeter::refreshIfNeeded(OperationContext* opCtx, bool* wasChanged) {
    ...
    *wasChanged = false;
    ...
    LOGV2_DEBUG(4, "ChunkManagerTargeter checking if refresh is needed, needsTargetingRefresh({}) remoteShardVersions empty ({})) remoteDbVersion empty ({})", "needsTargetingRefresh"_attr = _needsTargetingRefresh, "remoteShardVersions_empty"_attr = _remoteShardVersions.empty(), "remoteDbVersion"_attr = !_remoteDbVersion);
