...
void TransactionRouter::Router::processParticipantResponse(OperationContext* opCtx,
    ...
    if (txnResponseMetadata.getReadOnly()) {
        if (participant->readOnly == Participant::ReadOnly::kUnset) {
            LOGV2_DEBUG(22564, 3, "{txnIdToString} Marking {shardId} as read-only", "txnIdToString"_attr = txnIdToString(), "shardId"_attr = shardId);
    ...
    if (participant->readOnly != Participant::ReadOnly::kNotReadOnly) {
        LOGV2_DEBUG(22565, 3, "{txnIdToString} Marking {shardId} as having done a write", "txnIdToString"_attr = txnIdToString(), "shardId"_attr = shardId);

        _setReadOnlyForParticipant(opCtx, shardId, Participant::ReadOnly::kNotReadOnly);

        if (!p().recoveryShardId) {
            LOGV2_DEBUG(22566, 3, "{txnIdToString} Choosing {shardId} as recovery shard", "txnIdToString"_attr = txnIdToString(), "shardId"_attr = shardId);
...
BSONObj TransactionRouter::Router::attachTxnFieldsIfNeeded(OperationContext* opCtx,
    ...
    if (auto txnPart = getParticipant(shardId)) {
        LOGV2_DEBUG(22567, 4, "{txnIdToString} Sending transaction fields to existing participant: {shardId}", "txnIdToString"_attr = txnIdToString(), "shardId"_attr = shardId);
        return txnPart->attachTxnFieldsIfNeeded(cmdObj, false);
    }

    auto txnPart = _createParticipant(opCtx, shardId);
    LOGV2_DEBUG(22568, 4, "{txnIdToString} Sending transaction fields to new participant: {shardId}", "txnIdToString"_attr = txnIdToString(), "shardId"_attr = shardId);
...
void TransactionRouter::Router::onStaleShardOrDbError(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22569, 3, "{txnIdToString} Clearing pending participants after stale version error: {errorStatus}", "txnIdToString"_attr = txnIdToString(), "errorStatus"_attr = errorStatus);
...
void TransactionRouter::Router::onViewResolutionError(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22570, 3, "{txnIdToString} Clearing pending participants after view resolution error on namespace: {nss}", "txnIdToString"_attr = txnIdToString(), "nss"_attr = nss);
...
void TransactionRouter::Router::onSnapshotError(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22571, 3, "{txnIdToString} Clearing pending participants and resetting global snapshot "
              "timestamp after snapshot error: {errorStatus}, previous timestamp: {o_atClusterTime_getTime}", "txnIdToString"_attr = txnIdToString(), "errorStatus"_attr = errorStatus, "o_atClusterTime_getTime"_attr = o().atClusterTime->getTime());
...
void TransactionRouter::Router::_setAtClusterTime(
    ...
    LOGV2_DEBUG(22572, 2, "{txnIdToString} Setting global snapshot timestamp to {candidateTime} on statement {p_latestStmtId}", "txnIdToString"_attr = txnIdToString(), "candidateTime"_attr = candidateTime, "p_latestStmtId"_attr = p().latestStmtId);
    ...
    if (txnNumber < o().txnNumber) {
        ...
        switch (action) {
            case TransactionActions::kStart: {
                ...
                LOGV2_DEBUG(22573, 3, "{txnIdToString} New transaction started", "txnIdToString"_attr = txnIdToString());
                break;
            }
            case TransactionActions::kContinue: {
            ...
            case TransactionActions::kCommit: {
                ...
                LOGV2_DEBUG(22574, 3, "{txnIdToString} Commit recovery started", "txnIdToString"_attr = txnIdToString());
                break;
            }
        };
    }
...
BSONObj TransactionRouter::Router::_handOffCommitToCoordinator(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(22575, 3, "{txnIdToString} Committing using two-phase commit, coordinator: {o_coordinatorId}", "txnIdToString"_attr = txnIdToString(), "o_coordinatorId"_attr = *o().coordinatorId);
...
BSONObj TransactionRouter::Router::_commitTransaction(
    ...
    if (o().participants.size() == 1) {
        ...
        LOGV2_DEBUG(22576, 3, "{txnIdToString} Committing single-shard transaction, single participant: {shardId}", "txnIdToString"_attr = txnIdToString(), "shardId"_attr = shardId);
    ...
    if (writeShards.size() == 0) {
        LOGV2_DEBUG(22577, 3, "{txnIdToString} Committing read-only transaction on {readOnlyShards_size} shards", "txnIdToString"_attr = txnIdToString(), "readOnlyShards_size"_attr = readOnlyShards.size());
        {
            stdx::lock_guard<Client> lk(*opCtx->getClient());
    ...
    if (writeShards.size() == 1) {
        LOGV2_DEBUG(22578, 3, "{txnIdToString} Committing single-write-shard transaction with {readOnlyShards_size} read-only shards, write shard: {writeShards_front}", "txnIdToString"_attr = txnIdToString(), "readOnlyShards_size"_attr = readOnlyShards.size(), "writeShards_front"_attr = writeShards.front());
        {
            stdx::lock_guard<Client> lk(*opCtx->getClient());
...
BSONObj TransactionRouter::Router::abortTransaction(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(22579, 3, "{txnIdToString} Aborting transaction on {o_participants_size} shard(s)", "txnIdToString"_attr = txnIdToString(), "o_participants_size"_attr = o().participants.size());
...
void TransactionRouter::Router::implicitlyAbortTransaction(OperationContext* opCtx,
    ...
    if (o().commitType == CommitType::kTwoPhaseCommit ||
        ...
        LOGV2_DEBUG(22580, 3, "{txnIdToString} Router not sending implicit abortTransaction because commit "
    ...
    LOGV2_DEBUG(22581, 3, "{txnIdToString} Implicitly aborting transaction on {o_participants_size} shard(s) due to error: {errorStatus}", "txnIdToString"_attr = txnIdToString(), "o_participants_size"_attr = o().participants.size(), "errorStatus"_attr = errorStatus);
    ...
    try {
        ...
        LOGV2_DEBUG(22582, 3, "{txnIdToString} Implicitly aborting transaction failed {causedBy_ex_toStatus}", "txnIdToString"_attr = txnIdToString(), "causedBy_ex_toStatus"_attr = causedBy(ex.toStatus()));
...
void TransactionRouter::Router::_logSlowTransaction(OperationContext* opCtx,
    ...
    LOGV2(22583, "transaction {transactionInfoForLog_opCtx_terminationCause}", "transactionInfoForLog_opCtx_terminationCause"_attr = _transactionInfoForLog(opCtx, terminationCause));
}
