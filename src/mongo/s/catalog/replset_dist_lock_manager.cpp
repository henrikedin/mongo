...
void ReplSetDistLockManager::shutDown(OperationContext* opCtx) {
    ...
    if (!status.isOK()) {
        LOGV2_WARNING("error encountered while cleaning up distributed ping entry for {}{}", "_processID"_attr = _processID, "causedBy_redact_status"_attr = causedBy(redact(status)));
    }
}
    ...
    while (!isShutDown()) {
        {
            ...
            if (elapsed > 10 * _pingInterval) {
                LOGV2_WARNING("Lock pinger for proc: {} was inactive for {} ms", "_processID"_attr = _processID, "elapsed"_attr = elapsed);
            }
