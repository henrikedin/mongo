...
StatusWith<TaskExecutor::CallbackHandle> ShardingTaskExecutor::scheduleRemoteCommandOnAny(
    ...
    auto shardingCb = [timeTracker,
        ...
        if (!args.response.isOK()) {
            ...
            if (!shard) {
                LOGV2_DEBUG(22554, 1, "Could not find shard containing host: {target}", "target"_attr = target);
            }

            if (isMongos() && args.response.status == ErrorCodes::IncompatibleWithUpgradedServer) {
                LOGV2_FATAL(22557, "This mongos server must be upgraded. It is attempting to communicate "
                            "with "
                            "an upgraded cluster with which it is incompatible. Error: '{args_response_status_toString}' Crashing in order to bring attention to the incompatibility, rather "
            ...
            LOGV2_DEBUG(22555, 1, "Error processing the remote request, not updating operationTime or gLE");
        ...
        if (clusterGLE) {
            ...
            if (swShardingMetadata.isOK()) {
                ...
                if (!shardConn.isOK()) {
                    LOGV2_FATAL(22558, "got bad host string in saveGLEStats: {target}", "target"_attr = target);
                }
                ...
                LOGV2_WARNING(22556, "Got invalid sharding metadata {redact_swShardingMetadata_getStatus} metadata object was '{redact_args_response_data}'", "redact_swShardingMetadata_getStatus"_attr = redact(swShardingMetadata.getStatus()), "redact_args_response_data"_attr = redact(args.response.data));
            }
        }
    };
