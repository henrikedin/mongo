...
Status BalancerConfiguration::_refreshChunkSizeSettings(OperationContext* opCtx) {
    ...
    if (settings.getMaxChunkSizeBytes() != getMaxChunkSizeBytes()) {
        LOGV2(22328, "MaxChunkSize changing from {getMaxChunkSizeBytes_1024_1024}MB to {settings_getMaxChunkSizeBytes_1024_1024}MB", "getMaxChunkSizeBytes_1024_1024"_attr = getMaxChunkSizeBytes() / (1024 * 1024), "settings_getMaxChunkSizeBytes_1024_1024"_attr = settings.getMaxChunkSizeBytes() / (1024 * 1024));
...
Status BalancerConfiguration::_refreshAutoSplitSettings(OperationContext* opCtx) {
    ...
    if (settings.getShouldAutoSplit() != getShouldAutoSplit()) {
        LOGV2(22329, "ShouldAutoSplit changing from {getShouldAutoSplit} to {settings_getShouldAutoSplit}", "getShouldAutoSplit"_attr = getShouldAutoSplit(), "settings_getShouldAutoSplit"_attr = settings.getShouldAutoSplit());
