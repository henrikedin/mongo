...
StatusWith<Shard::CommandResponse> Shard::runCommand(OperationContext* opCtx,
    ...
    while (true) {
        ...
        if (isRetriableError(status.code(), retryPolicy)) {
            LOGV2_DEBUG(22407, 2, "Command {redact_cmdObj} failed with retriable error and will be retried{causedBy_redact_status}", "redact_cmdObj"_attr = redact(cmdObj), "causedBy_redact_status"_attr = causedBy(redact(status)));
            continue;
        }
...
StatusWith<Shard::CommandResponse> Shard::runCommandWithFixedRetryAttempts(
    ...
    for (int retry = 1; retry <= kOnErrorNumRetries; ++retry) {
        ...
        if (retry < kOnErrorNumRetries && isRetriableError(status.code(), retryPolicy)) {
            LOGV2_DEBUG(22408, 2, "Command {redact_cmdObj} failed with retriable error and will be retried{causedBy_redact_status}", "redact_cmdObj"_attr = redact(cmdObj), "causedBy_redact_status"_attr = causedBy(redact(status)));
            continue;
        }
...
BatchedCommandResponse Shard::runBatchWriteCommand(OperationContext* opCtx,
    ...
    for (int retry = 1; retry <= kOnErrorNumRetries; ++retry) {
        ...
        if (retry < kOnErrorNumRetries && isRetriableError(writeStatus.code(), retryPolicy)) {
            LOGV2_DEBUG(22409, 2, "Batch write command to {getId} failed with retriable error and will be retried{causedBy_redact_writeStatus}", "getId"_attr = getId(), "causedBy_redact_writeStatus"_attr = causedBy(redact(writeStatus)));
            continue;
        }
