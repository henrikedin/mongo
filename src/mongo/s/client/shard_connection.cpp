...
public:
    ...
    void done(const std::string& addr, DBClientBase* conn) {
        ...
        if (s->avail != nullptr) {
            LOGV2_WARNING("Detected additional sharded connection in the thread local pool for {}", "addr"_attr = addr);
    ...
    void checkVersions(OperationContext* opCtx, const std::string& ns) {
        ...
        for (const ShardId& shardId : all) {
            try {
                ...
                LOGV2_WARNING("Problem while initially checking shard versions on {}{}", "shardId"_attr = shardId, "causedBy_redact_ex"_attr = causedBy(redact(ex)));
...
ShardConnection::~ShardConnection() {
    if (_conn) {
        if (_conn->isFailed()) {
            ...
            LOGV2("sharded connection to {} not being returned to the pool", "conn_getServerAddress"_attr = _conn->getServerAddress());
