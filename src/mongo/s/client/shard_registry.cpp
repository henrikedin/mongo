...
void ShardRegistry::shutdown() {
    if (_executor && !_isShutdown) {
        LOGV2_DEBUG(22411, 1, "Shutting down task executor for reloading shard registry");
        _executor->shutdown();
        _executor->join();
        _isShutdown = true;
    }
...
void ShardRegistry::startup(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(22412, 1, "Starting up task executor for periodic reloading of ShardRegistry");
    _executor->startup();
    ...
    if (status.getStatus() == ErrorCodes::ShutdownInProgress) {
        LOGV2_DEBUG(22413, 1, "Cant schedule Shard Registry reload. Executor shutdown in progress");
        return;
    }

    if (!status.isOK()) {
        LOGV2_FATAL(22424, "Can't schedule ShardRegistry reload due to {causedBy_status_getStatus}", "causedBy_status_getStatus"_attr = causedBy(status.getStatus()));
        fassertFailed(40252);
    }
}

void ShardRegistry::_internalReload(const CallbackArgs& cbArgs) {
    LOGV2_DEBUG(22414, 1, "Reloading shardRegistry");
    if (!cbArgs.status.isOK()) {
        LOGV2_WARNING(22421, "cant reload ShardRegistry {causedBy_cbArgs_status}", "causedBy_cbArgs_status"_attr = causedBy(cbArgs.status));
        return;
    }
    ...
    if (status.getStatus() == ErrorCodes::ShutdownInProgress) {
        LOGV2_DEBUG(22415, 1, "Cant schedule ShardRegistry reload. Executor shutdown in progress");
        return;
    }

    if (!status.isOK()) {
        LOGV2_FATAL(22425, "Can't schedule ShardRegistry reload due to {causedBy_status_getStatus}", "causedBy_status_getStatus"_attr = causedBy(status.getStatus()));
        fassertFailed(40253);
    }
}
...
bool ShardRegistry::reload(OperationContext* opCtx) {
    ...
    if (_reloadState == ReloadState::Reloading) {
        ...
        try {
            ...
            LOGV2_DEBUG(22416, 1, "ShardRegistry reload is interrupted due to: {redact_e_toStatus}", "redact_e_toStatus"_attr = redact(e.toStatus()));
            return false;
        }
...
void ShardRegistry::updateReplicaSetOnConfigServer(ServiceContext* serviceContext,
    ...
    if (!s) {
        LOGV2_DEBUG(22417, 1, "shard not found for set: {connStr} when attempting to inform config servers of updated set membership", "connStr"_attr = connStr);
        return;
    }
    ...
    if (!status.isOK()) {
        LOGV2_ERROR(22423, "RSChangeWatcher: could not update config db with connection string {connStr}{causedBy_redact_status}", "connStr"_attr = connStr, "causedBy_redact_status"_attr = causedBy(redact(status)));
    }
}
...
ShardRegistryData::ShardRegistryData(OperationContext* opCtx, ShardFactory* shardFactory) {
    ...
    LOGV2_DEBUG(22418, 1, "found {shards_size} shards listed on config server(s) with lastVisibleOpTime: {reloadOpTime_toBSON}", "shards_size"_attr = shards.size(), "reloadOpTime_toBSON"_attr = reloadOpTime.toBSON());
    ...
    for (const auto& shardType : shards) {
        ...
        if (!shardHostStatus.isOK()) {
            LOGV2_WARNING(22422, "Unable to parse shard host {shardHostStatus_getStatus_toString}", "shardHostStatus_getStatus_toString"_attr = shardHostStatus.getStatus().toString());
            continue;
        }
...
void ShardRegistryData::_addShard(WithLock lk,
    ...
    if (currentShard) {
        ...
        if (oldConnString.toString() != connString.toString()) {
            LOGV2(22419, "Updating ShardRegistry connection string for shard {currentShard_getId} from: {oldConnString_toString} to: {connString_toString}", "currentShard_getId"_attr = currentShard->getId(), "oldConnString_toString"_attr = oldConnString.toString(), "connString_toString"_attr = connString.toString());
        }
    ...
    LOGV2_DEBUG(22420, 3, "Adding shard {shard_getId}, with CS {connString_toString}", "shard_getId"_attr = shard->getId(), "connString_toString"_attr = connString.toString());
