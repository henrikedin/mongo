...
void ParallelSortClusteredCursor::setupVersionAndHandleSlaveOk(
    ...
    if (allowShardVersionFailure) {
        ...
        if (!rsMonitor->isKnownToHaveGoodPrimary()) {
            ...
            if (sampler.tick()) {
                ...
                dassert(repl);
                LOGV2_WARNING(22383, "Primary for {repl_getServerAddress} was down before, bypassing setShardVersion. The local replica set view and targeting may be stale.", "repl_getServerAddress"_attr = repl->getServerAddress());
            }
    ...
    try {
        if (state->conn->setVersion()) {
            LOGV2_DEBUG(22366, 2, "pcursor: needed to set remote version on connection to value compatible with {vinfo}", "vinfo"_attr = vinfo);
        }
        ...
        if (allowShardVersionFailure &&
            ...
            if (sampler.tick()) {
                ...
                dassert(repl);
                LOGV2_WARNING(22384, "Cannot contact primary for {repl_getServerAddress} to check shard version. The local replica set view and targeting may be stale.", "repl_getServerAddress"_attr = repl->getServerAddress());
            }
        } else {
            throw;
        }
...
void ParallelSortClusteredCursor::startInit(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(22367, 2, "pcursor: {prefix} pcursor over {qSpec} and {cInfo}", "prefix"_attr = prefix, "qSpec"_attr = _qSpec, "cInfo"_attr = _cInfo);
    ...
    for (auto& cmEntry : _cursorMap) {
        ...
        if (shardIds.find(shardId) == shardIds.end()) {
            LOGV2_DEBUG(22368, 2, "pcursor: closing cursor on shard {shardId} as the connection is no longer required by {vinfo}", "shardId"_attr = shardId, "vinfo"_attr = vinfo);

            cmEntry.second.cleanup(true);
        }
    }

    LOGV2_DEBUG(22369, 2, "pcursor: initializing over {shardIds_size} shards required by {vinfo}", "shardIds_size"_attr = shardIds.size(), "vinfo"_attr = vinfo);
    ...
    for (const ShardId& shardId : shardIds) {
        ...
        LOGV2_DEBUG(22370, 2, "pcursor: initializing on shard {shardId}, current connection state is {mdata_toBSON}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON());
        ...
        try {
            if (mdata.initialized) {
                ...
                if (primary && !state->primary)
                    LOGV2_WARNING(22385, "Collection becoming unsharded detected");
                if (manager && !state->manager)
                    LOGV2_WARNING(22386, "Collection becoming sharded detected");
                if (primary && state->primary && primary != state->primary)
                    LOGV2_WARNING(22387, "Weird shift of primary detected");
            ...
            LOGV2_DEBUG(22371, 2, "pcursor: initialized {isCommand_command_query}{lazyInit_lazily_full}on shard {shardId}, current connection state is {mdata_toBSON}", "isCommand_command_query"_attr = (isCommand() ? "command " : "query "), "lazyInit_lazily_full"_attr = (lazyInit ? "(lazily) " : "(full) "), "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON());
            ...
            LOGV2_DEBUG(22372, 1, "stale config of ns {staleNS} during initialization, will retry{causedBy_redact_e}", "staleNS"_attr = staleNS, "causedBy_redact_e"_attr = causedBy(redact(e)));

            // This is somewhat strange
            if (staleNS != nss) {
                LOGV2_WARNING(22388, "versioned ns {nss_ns} doesn't match stale config namespace {staleNS}", "nss_ns"_attr = nss.ns(), "staleNS"_attr = staleNS);
            }
            ...
            LOGV2_WARNING(22389, "socket exception when initializing on {shardId}, current connection state is {mdata_toBSON}{causedBy_redact_e}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON(), "causedBy_redact_e"_attr = causedBy(redact(e)));
            mdata.errored = true;
            if (returnPartial) {
                mdata.cleanup(true);
                continue;
            ...
            LOGV2_WARNING(22390, "db exception when initializing on {shardId}, current connection state is {mdata_toBSON}{causedBy_redact_e}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON(), "causedBy_redact_e"_attr = causedBy(redact(e)));
            mdata.errored = true;
            ...
            LOGV2_WARNING(22391, "exception when initializing on {shardId}, current connection state is {mdata_toBSON}{causedBy_e}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON(), "causedBy_e"_attr = causedBy(e));
            mdata.errored = true;
            throw;
        } catch (...) {
            LOGV2_WARNING(22392, "unknown exception when initializing on {shardId}, current connection state is {mdata_toBSON}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON());
            mdata.errored = true;
            throw;
        }
    }
...
void ParallelSortClusteredCursor::finishInit(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(22373, 2, "pcursor: finishing over {cursorMap_size} shards", "cursorMap_size"_attr = _cursorMap.size());

    for (auto& cmEntry : _cursorMap) {
        const auto& shardId = cmEntry.first;
        auto& mdata = cmEntry.second;

        LOGV2_DEBUG(22374, 2, "pcursor: finishing on shard {shardId}, current connection state is {mdata_toBSON}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON());
        ...
        try {
            ...
            if (!mdata.completed) {
                ...
                LOGV2_DEBUG(22375, 2, "pcursor: finished on shard {shardId}, current connection state is {mdata_toBSON}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON());
            }
        } catch (StaleConfigException& e) {
            retry = true;
            ...
            LOGV2_WARNING(22393, "socket exception when finishing on {shardId}, current connection state is {mdata_toBSON}{causedBy_redact_e}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON(), "causedBy_redact_e"_attr = causedBy(redact(e)));
            mdata.errored = true;
            if (returnPartial) {
                mdata.cleanup(true);
                continue;
            ...
            // ABOVE
            if (e.code() == 15988) {
                LOGV2_WARNING(22394, "exception when receiving data from {shardId}, current connection state is {mdata_toBSON}{causedBy_redact_e}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON(), "causedBy_redact_e"_attr = causedBy(redact(e)));
                ...
                if (e.code() == ErrorCodes::InvalidBSON)
                    LOGV2_WARNING(22395, "bson is malformed :: db exception when finishing on {shardId}{causedBy_redact_e}", "shardId"_attr = shardId, "causedBy_redact_e"_attr = causedBy(redact(e)));
                else
                    LOGV2_WARNING(22396, "db exception when finishing on {shardId}, current connection state is {mdata_toBSON}{causedBy_redact_e}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON(), "causedBy_redact_e"_attr = causedBy(redact(e)));
                mdata.errored = true;
                throw;
            }
        } catch (std::exception& e) {
            LOGV2_WARNING(22397, "exception when finishing on {shardId}, current connection state is {mdata_toBSON}{causedBy_e}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON(), "causedBy_e"_attr = causedBy(e));
            mdata.errored = true;
            throw;
        } catch (...) {
            LOGV2_WARNING(22398, "unknown exception when finishing on {shardId}, current connection state is {mdata_toBSON}", "shardId"_attr = shardId, "mdata_toBSON"_attr = mdata.toBSON());
            mdata.errored = true;
            throw;
        }
    }
    ...
    if (retry) {
        ...
        if (staleNSExceptions.size()) {
            for (const auto& exEntry : staleNSExceptions) {
                ...
                LOGV2_DEBUG(22376, 1, "stale config of ns {staleNS} on finishing query, will retry{causedBy_redact_ex}", "staleNS"_attr = staleNS, "causedBy_redact_ex"_attr = causedBy(redact(ex)));

                // This is somewhat strange
                if (staleNS.ns() != ns) {
                    LOGV2_WARNING(22399, "versioned ns {ns} doesn't match stale config namespace {staleNS}", "ns"_attr = ns, "staleNS"_attr = staleNS);
                }
            }
        }
    ...
    for (auto i = _cursorMap.begin(); i != _cursorMap.end();) {
        ...
        // Erase empty stuff
        if (!mdata.pcState) {
            LOGV2(22377, "PCursor erasing empty state {mdata_toBSON}", "mdata_toBSON"_attr = mdata.toBSON());
            _cursorMap.erase(i++);
            continue;
        } else {
            ++i;
...
void ParallelSortClusteredCursor::_oldInit(OperationContext* opCtx) {
    ...
    do {
        ...
        if (!firstPass) {
            LOGV2(22378, "retrying {returnPartial_partial}parallel connection to ", "returnPartial_partial"_attr = (returnPartial ? "(partial) " : ""));
            for (set<int>::const_iterator it = retryQueries.begin(); it != retryQueries.end();
                 ++it) {
                LOGV2(22379, "{serverHosts_it}, ", "serverHosts_it"_attr = serverHosts[*it]);
            }
            LOGV2(22380, "{finishedQueries} finished queries.", "finishedQueries"_attr = finishedQueries);
        }
        ...
        for (vector<string>::const_iterator it = serverHosts.begin(); it != serverHosts.end();
            ...
            LOGV2_DEBUG(22381, 5, "ParallelSortClusteredCursor::init server:{serverHost} ns:{ns} query:{redact_query} fields:{redact_fields} options: {options}", "serverHost"_attr = serverHost, "ns"_attr = _ns, "redact_query"_attr = redact(_query), "redact_fields"_attr = redact(_fields), "options"_attr = _options);
        ...
        for (size_t i = 0; i < num; i++) {
            ...
            try {
                if (!_cursors[i].get()->initLazyFinish(retry)) {
                    LOGV2_WARNING(22400, "invalid result from {conns_i_getHost}{retry_retrying}", "conns_i_getHost"_attr = conns[i]->getHost(), "retry_retrying"_attr = (retry ? ", retrying" : ""));
                    _cursors[i].reset(nullptr, nullptr);
    ...
    if (socketExs.size() > 0 || staleConfigExs.size() > 0 || otherExs.size() > 0) {
        ...
        if (throwException && staleConfigExs.size() > 0) {
            ...
            LOGV2_WARNING(22401, "{redact_errMsg_str}", "redact_errMsg_str"_attr = redact(errMsg.str()));
        }
    }

    if (retries > 0)
        LOGV2(22382, "successfully finished parallel query after {retries} retries", "retries"_attr = retries);
}
        ...
        if (initialized && !errored) {
            ...
            if (!finished && pcState->conn->ok()) {
                try {
                    ...
                    LOGV2_WARNING(22402, "exception closing cursor");
                } catch (...) {
                    LOGV2_WARNING(22403, "unknown exception closing cursor");
                }
            }
        }
