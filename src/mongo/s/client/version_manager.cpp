...
bool initShardVersionEmptyNS(OperationContext* opCtx, DBClientBase* conn_in) {
    try {
        ...
        LOGV2_DEBUG(22428, 3, "initial sharding result : {result}", "result"_attr = result);
        ...
        if (sampler.tick()) {
            LOGV2_WARNING(22434, "failed to initialize new replica set connection version, will initialize on first use");
        }
...
bool checkShardVersion(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(22429, 1, "setting shard version of {version} for {ns} on shard {shard_toString}", "version"_attr = version, "ns"_attr = ns, "shard_toString"_attr = shard->toString());

    LOGV2_DEBUG(22430, 3, "last version sent with chunk manager iteration {sequenceNumber}, current chunk manager iteration is {officialSequenceNumber}", "sequenceNumber"_attr = sequenceNumber, "officialSequenceNumber"_attr = officialSequenceNumber);
    ...
    if (setShardVersion(opCtx,
                        ...
                        authoritative,
                        result)) {
        LOGV2_DEBUG(22431, 1, "      setShardVersion success: {result}", "result"_attr = result);
    ...
    LOGV2_DEBUG(22432, 1, "       setShardVersion failed!\n{result}", "result"_attr = result);
    ...
    string errmsg = str::stream() << "setShardVersion failed shard: " << shard->toString() << " "
                                  << result;
    LOGV2(22433, "     {errmsg}", "errmsg"_attr = errmsg);
