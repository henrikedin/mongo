...
auto AsyncRequestsSender::RemoteData::handleResponse(RemoteCommandOnAnyCallbackArgs&& rcr)
    ...
    if (!shard) {
        ...
        if (!_ars->_stopRetrying && shard->isRetriableError(status.code(), _ars->_retryPolicy) &&
            ...
            LOGV2_DEBUG(1, "Command to remote {}{}{}failed with retriable error and will be retried {}", "shardId"_attr = _shardId, "failedTargets_empty_failedTargets_size_1_for_hosts_at_host"_attr = (failedTargets.empty()
                           ? " "
