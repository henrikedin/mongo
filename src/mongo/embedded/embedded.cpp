...
void shutdown(ServiceContext* srvContext) {
    ...
    LOGV2_OPTIONS(22259, {logComponentV1toV2(LogComponent::kControl)}, "now exiting");
}
...
ServiceContext* initialize(const char* yaml_config) {
    ...
    if (kDebugBuild)
        LOGV2_OPTIONS(22260, {logComponentV1toV2(LogComponent::kControl)}, "DEBUG build (which is slower)");
    ...
    if (serverGlobalParams.parsedOpts.hasField("storage")) {
        ...
        for (auto&& e : storageElement.Obj()) {
            ...
            if (isRegisteredStorageEngine(serviceContext, e.fieldName())) {
                LOGV2_WARNING(22262, "Detected configuration for non-active storage engine {e_fieldName} when current storage engine is {storageGlobalParams_engine}", "e_fieldName"_attr = e.fieldName(), "storageGlobalParams_engine"_attr = storageGlobalParams.engine);
            }
        }
    }
    ...
    try {
        ...
        LOGV2_FATAL_OPTIONS(22263, {logComponentV1toV2(LogComponent::kControl)}, "** IMPORTANT: {error_toStatus_reason}", "error_toStatus_reason"_attr = error.toStatus().reason());
    ...
    if (storageGlobalParams.upgrade) {
        LOGV2(22261, "finished checking dbs");
