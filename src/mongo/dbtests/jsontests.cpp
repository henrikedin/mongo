...
void checkJsonStringEach(const std::vector<std::pair<BSONObj, std::string>>& pairs) {
    for (const auto& pair : pairs) {
        ASSERT_JSON_EQUALS(pair.first.jsonString(ExtendedCanonicalV200), pair.second);
        ASSERT_JSON_EQUALS(pair.first.jsonString(ExtendedRelaxedV200), pair.second);
...
TEST(JsonStringTest, UnicodeTest) {
    ...
    // Ascii
    ASSERT_JSON_EQUALS(B().append("a", "\x80").obj().jsonString(ExtendedCanonicalV200),
                       R"({ "a" : "\u0080" })");
    ASSERT_JSON_EQUALS(B().append("a", "\x80").obj().jsonString(ExtendedRelaxedV200),
                       R"({ "a" : "\u0080" })");
...
TEST(JsonStringTest, NumbersTest) {
    ...
    ASSERT(str::contains(B().append("a", qNaN).obj().jsonString(ExtendedCanonicalV200), "NaN"));
    ASSERT(str::contains(B().append("a", sNaN).obj().jsonString(ExtendedCanonicalV200), "NaN"));

    ASSERT_JSON_EQUALS(B().append("a", 1).obj().jsonString(ExtendedCanonicalV200),
                       R"({ "a" : {"$numberInt": 1 }})");
    ASSERT_JSON_EQUALS(B().append("a", 1).obj().jsonString(ExtendedRelaxedV200), R"({ "a" : 1 })");
    ASSERT_EQUALS(B().append("a", 1).obj().jsonString(LegacyStrict), R"({ "a" : 1 })");

    ASSERT_JSON_EQUALS(B().append("a", -1).obj().jsonString(ExtendedCanonicalV200),
                       R"({ "a" : {"$numberInt": -1 }})");
    ASSERT_JSON_EQUALS(B().append("a", -1).obj().jsonString(ExtendedRelaxedV200),
                       R"({ "a" : -1 })");
    ASSERT_EQUALS(B().append("a", -1).obj().jsonString(LegacyStrict), R"({ "a" : -1 })");

    ASSERT_JSON_EQUALS(B().append("a", 1.5).obj().jsonString(ExtendedCanonicalV200),
                       R"({ "a" : {"$numberDouble": 1.5 }})");
    ASSERT_JSON_EQUALS(B().append("a", 1.5).obj().jsonString(ExtendedRelaxedV200),
                       R"({ "a" : 1.5 })");
...
TEST(JsonStringTest, NumberLongStrictZero) {
    ...
    ASSERT_JSON_EQUALS("{ \"a\" : { \"$numberLong\" : \"0\" } }",
                       b.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS("{ \"a\" : 0 }", b.done().jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, NumberLongStrict) {
    ...
    ASSERT_JSON_EQUALS("{ \"a\" : { \"$numberLong\" : \"20000\" } }",
                       b.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS("{ \"a\" : 20000 }", b.done().jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, NumberLongStrictLarge) {
    ...
    ASSERT_JSON_EQUALS("{ \"a\" : { \"$numberLong\" : \"9223372036854775807\" } }",
                       b.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS("{ \"a\" : 9223372036854775807 }", b.done().jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, NumberDoubleNaN) {
    ...
    ASSERT_JSON_EQUALS(R"({ "a" : { "$numberDouble": "NaN" }})",
                       b.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS(R"({ "a" : { "$numberDouble": "NaN" }})",
                       b.done().jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, TwoMembers) {
    ...
    ASSERT_JSON_EQUALS(R"({ "a" : {"$numberInt" : 1}, "b" : {"$numberInt" : 2} })",
                       b.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS(R"({ "a" : 1, "b" : 2 })", b.done().jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, BinData) {
    ...
    ASSERT_JSON_EQUALS(R"({ "a" : { "$binary" : { "base64": "YWI=", "subType" : "0" } } })",
                       c.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS(R"({ "a" : { "$binary" : { "base64": "YWI=", "subType" : "0" } } })",
                       c.done().jsonString(ExtendedRelaxedV200));
    ...
    ASSERT_JSON_EQUALS(R"({ "a" : { "$binary" : { "base64": "YQ==", "subType" : "0" } } })",
                       d.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS(R"({ "a" : { "$binary" : { "base64": "YQ==", "subType" : "0" } } })",
                       d.done().jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, Symbol) {
    ...
    ASSERT_JSON_EQUALS(R"({ "a" : { "$symbol": "b" } })",
                       b.done().jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS(R"({ "a" : { "$symbol": "b" } })", b.done().jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, Date) {
    ...
    ASSERT_JSON_EQUALS(R"({ "a" : { "$date" : { "$numberLong" : "0" } } })",
                       built.jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS(R"({ "a" : { "$date" : "1969-12-31T19:00:00.000-0500" } })",
                       built.jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, RegexEscape) {
    ...
    ASSERT_JSON_EQUALS(expected, built.jsonString(ExtendedCanonicalV200));
    ASSERT_JSON_EQUALS(expected, built.jsonString(ExtendedRelaxedV200));
...
TEST(JsonStringTest, AllTypesTest) {
    ...
    BSONObj o = b.obj();
    o.jsonString(ExtendedCanonicalV200);
    o.jsonString(ExtendedRelaxedV200);
...
void checkEquivalence(const std::string& json, const BSONObj& bson) {
    ...
    assertEquals(json, bson, fromjson(tojson(bson, ExtendedCanonicalV200)), "mode: canonical");
    assertEquals(json, bson, fromjson(tojson(bson, ExtendedRelaxedV200)), "mode: relaxed");
}
