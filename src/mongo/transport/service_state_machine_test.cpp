...
TEST_F(ServiceStateMachineFixture, TerminateWorksForAllStates) {
    ...
    auto waitForHook = [this, &waitFor, &atDesiredState, &okayToContinue]() {
        LOGV2("Checking for wakeup at {}. Expecting {}", "stateToString_ssm_state"_attr = stateToString(_ssm->state()), "stateToString_waitFor"_attr = stateToString(waitFor));
    ...
    for (const auto testState : states) {
        ...
        atDesiredState.wait();
        LOGV2("Terminating session at {}", "stateToString_ssm_state"_attr = stateToString(_ssm->state()));
