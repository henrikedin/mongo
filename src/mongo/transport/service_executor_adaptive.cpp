...
void ServiceExecutorAdaptive::_controllerThreadRoutine() {
    ...
    while (_isRunning.load()) {
        ...
        if (threadsRunning < _config->reservedThreads()) {
            LOGV2("Starting {} to replenish reserved worker threads", "config_reservedThreads_threadsRunning"_attr = _config->reservedThreads() - threadsRunning);
...
void ServiceExecutorAdaptive::_startWorkerThread(ThreadCreationReason reason) {
    ...
    if (!launchResult.isOK()) {
        LOGV2_WARNING("Failed to launch new worker thread: {}", "launchResult"_attr = launchResult);
        lk.lock();
