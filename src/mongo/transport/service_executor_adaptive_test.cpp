...
protected:
    ...
    std::unique_ptr<ServiceExecutorAdaptive> makeAndStartExecutor() {
        ...
        LOGV2(22644, "wait for executor to finish starting");
        waitFor.store(1);
...
TEST_F(ServiceExecutorAdaptiveFixture, TestStuckTask) {
    ...
    LOGV2(22645, "Scheduling blocked task");
    waitFor.store(3);
    ...
    LOGV2(22646, "Scheduling task stuck on blocked task");
    ASSERT_OK(exec->schedule(
        notifyCallback, ServiceExecutor::kEmptyFlags, ServiceExecutorTaskName::kSSMProcessMessage));

    LOGV2(22647, "Waiting for second thread to start");
    waitForCallback(1);
    ASSERT_EQ(exec->threadsRunning(), 2);

    LOGV2(22648, "Waiting for unstuck task to run");
    blockedLock.unlock();
    waitForCallback(0);
    ASSERT_EQ(exec->threadsRunning(), 2);

    LOGV2(22649, "Waiting for second thread to idle out");
...
TEST_F(ServiceExecutorAdaptiveFixture, TestStuckThreads) {
    ...
    auto blockedTask = [this, &blockedMutex] {
        LOGV2(22650, "waiting on blocked mutex");
        notifyCallback();
        stdx::unique_lock<Latch> lk(blockedMutex);
        notifyCallback();
    };
    ...
    LOGV2(22651, "Scheduling {tasks} blocked tasks", "tasks"_attr = tasks);
    ...
    LOGV2(22652, "Waiting for executor to start new threads");
    waitForCallback(3);

    LOGV2(22653, "All threads blocked, wait for executor to detect block and start a new thread.");
    ...
    LOGV2(22654, "Waiting for unstuck task to run");
    blockedLock.unlock();
    waitForCallback(0);
}
...
TEST_F(ServiceExecutorAdaptiveFixture, TestRecursion) {
    ...
    task = [this, &task, &exec, &mutex, &cv, &remainingTasks] {
        if (remainingTasks.subtractAndFetch(1) == 0) {
            LOGV2(22655, "Signaling job done");
            cv.notify_one();
            return;
        }

        LOGV2(22656, "Starting task recursively");
        ...
        LOGV2(22657, "Completing task recursively");
    };
...
TEST_F(ServiceExecutorAdaptiveFixture, TestDeferredTasks) {
    ...
    waitFor.store(3);
    LOGV2(22658, "Scheduling a blocking task");
    ASSERT_OK(exec->schedule(
        [this, &blockedMutex] {
            stdx::unique_lock<Latch> lk(blockedMutex);
            notifyCallback();
    ...
    LOGV2(22659, "Scheduling deferred task");
    ASSERT_OK(exec->schedule(notifyCallback,
    ...
    LOGV2(22660, "Scheduling non-deferred task");
    ASSERT_OK(exec->schedule(
        notifyCallback, ServiceExecutor::kEmptyFlags, ServiceExecutorTaskName::kSSMProcessMessage));
