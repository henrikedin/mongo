...
public:
    ...
    void cancel(const BatonHandle& baton = nullptr) override {
        ...
        if (baton && baton->networking() && baton->networking()->cancelTimer(*this)) {
            LOGV2_DEBUG(22693, 2, "Canceled via baton, skipping asio cancel.");
            return;
        }
        ...
        try {
            ...
            return _timer->async_wait(UseFuture{}).tapError([timer = _timer](const Status& status) {
                if (status != ErrorCodes::CallbackCanceled) {
                    LOGV2_DEBUG(22694, 2, "Timer received error: {status}", "status"_attr = status);
                }
            });
...
public:
    ...
    void drain() override {
        ...
        while (_ioContext.poll()) {
            LOGV2_DEBUG(22695, 2, "Draining remaining work in reactor.");
        }
        _ioContext.stop();
    }
    ...
    connector->resolver.asyncResolve(connector->peer, _listenerOptions.enableIPv6)
        .then([connector, timeBefore](WrappedResolver::EndpointVector results) {
            try {
                ...
                if (timeAfter - timeBefore > Seconds(1)) {
                    LOGV2_WARNING(22702, "DNS resolution while connecting to {connector_peer} took {timeAfter_timeBefore}", "connector_peer"_attr = connector->peer, "timeAfter_timeBefore"_attr = timeAfter - timeBefore);
                }
        ...
        .getAsync([connector](Status connectResult) {
            if (MONGO_unlikely(transportLayerASIOasyncConnectTimesOut.shouldFail())) {
                LOGV2(22696, "asyncConnectTimesOut fail point is active. simulating timeout.");
                return;
            }
...
Status TransportLayerASIO::setup() {
    ...
    if (auto foStatus = validateFastOpen(); !foStatus.isOK()) {
        if (tcpFastOpenIsConfigured) {
            ...
            LOGV2(22697, "{foStatus_reason}", "foStatus_reason"_attr = foStatus.reason());
        }
    }
    ...
    for (auto& ip : listenAddrs) {
        if (ip.empty()) {
            LOGV2_WARNING(22703, "Skipping empty bind address");
            continue;
        }
        ...
        if (!swAddrs.isOK()) {
            LOGV2_WARNING(22704, "Found no addresses for {swAddrs_getStatus}", "swAddrs_getStatus"_attr = swAddrs.getStatus());
            continue;
        }
    ...
    for (auto& addr : endpoints) {
        ...
        if (addr.family() == AF_UNIX) {
            if (::unlink(addr.toString().c_str()) == -1 && errno != ENOENT) {
                LOGV2_ERROR(22707, "Failed to unlink socket file {addr_toString_c_str} {errnoWithDescription_errno}", "addr_toString_c_str"_attr = addr.toString().c_str(), "errnoWithDescription_errno"_attr = errnoWithDescription(errno));
                fassertFailedNoTrace(40486);
            }
        }
#endif
        if (addr.family() == AF_INET6 && !_listenerOptions.enableIPv6) {
            LOGV2_ERROR(22708, "Specified ipv6 bind address, but ipv6 is disabled");
        ...
        if (addr.family() == AF_UNIX) {
            if (::chmod(addr.toString().c_str(), serverGlobalParams.unixSocketPermissions) == -1) {
                LOGV2_ERROR(22709, "Failed to chmod socket file {addr_toString_c_str} {errnoWithDescription_errno}", "addr_toString_c_str"_attr = addr.toString().c_str(), "errnoWithDescription_errno"_attr = errnoWithDescription(errno));
...
void TransportLayerASIO::_runListener() noexcept {
    ...
    for (auto& acceptor : _acceptors) {
        ...
        if (ec) {
            LOGV2_FATAL(22710, "Error listening for new connections on {acceptor_first}: {ec_message}", "acceptor_first"_attr = acceptor.first, "ec_message"_attr = ec.message());
            fassertFailed(31339);
        }

        _acceptConnection(acceptor.second);
        LOGV2(22698, "Listening on {acceptor_first_getAddr}", "acceptor_first_getAddr"_attr = acceptor.first.getAddr());
    }
...
#endif
    LOGV2(22699, "waiting for connections on port {listenerPort}{ssl}", "listenerPort"_attr = _listenerPort, "ssl"_attr = ssl);
    ...
    for (auto& acceptor : _acceptors) {
        ...
        if (addr.getType() == AF_UNIX && !addr.isAnonymousUNIXSocket()) {
            ...
            LOGV2(22700, "removing socket file: {path}", "path"_attr = path);
            if (::unlink(path.c_str()) != 0) {
                const auto ewd = errnoWithDescription();
                LOGV2_WARNING(22705, "Unable to remove UNIX socket {path}: {ewd}", "path"_attr = path, "ewd"_attr = ewd);
            }
        }
    }
}
...
void TransportLayerASIO::_acceptConnection(GenericAcceptor& acceptor) {
    auto acceptCb = [this, &acceptor](const std::error_code& ec, GenericSocket peerSocket) mutable {
        ...
        if (ec) {
            LOGV2(22701, "Error accepting new connection on {endpointToHostAndPort_acceptor_local_endpoint}: {ec_message}", "endpointToHostAndPort_acceptor_local_endpoint"_attr = endpointToHostAndPort(acceptor.local_endpoint()), "ec_message"_attr = ec.message());
        ...
        try {
            ...
            LOGV2_WARNING(22706, "Error accepting new connection {e}", "e"_attr = e);
        }
