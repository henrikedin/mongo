...
StatusWith<Message> MessageCompressorManager::compressMessage(
    ...
    LOGV2_DEBUG(22609, 3, "Compressing message with {compressor_getName}", "compressor_getName"_attr = compressor->getName());
    ...
    if (bufferSize > MaxMessageSizeBytes) {
        LOGV2_DEBUG(22610, 3, "Compressed message would be larger than {MaxMessageSizeBytes}, returning original uncompressed message", "MaxMessageSizeBytes"_attr = MaxMessageSizeBytes);
        return {msg};
    }
...
StatusWith<Message> MessageCompressorManager::decompressMessage(const Message& msg,
    ...
    LOGV2_DEBUG(22611, 3, "Decompressing message with {compressor_getName}", "compressor_getName"_attr = compressor->getName());
...
void MessageCompressorManager::clientBegin(BSONObjBuilder* output) {
    LOGV2_DEBUG(22612, 3, "Starting client-side compression negotiation");
    ...
    for (const auto e : _registry->getCompressorNames()) {
        LOGV2_DEBUG(22613, 3, "Offering {e} compressor to server", "e"_attr = e);
        sub.append(e);
    }
    sub.doneFast();
}
...
void MessageCompressorManager::clientFinish(const BSONObj& input) {
    ...
    LOGV2_DEBUG(22614, 3, "Finishing client-side compression negotiation");
    ...
    if (elem.eoo()) {
        LOGV2_DEBUG(22615, 3, "No compression algorithms were sent from the server. This connection will be uncompressed");
        return;
    }

    LOGV2_DEBUG(22616, 3, "Received message compressors from server");
    for (const auto& e : elem.Obj()) {
        auto algoName = e.checkAndGetStringData();
        auto ret = _registry->getCompressor(algoName);
        LOGV2_DEBUG(22617, 3, "Adding compressor {ret_getName}", "ret_getName"_attr = ret->getName());
        _negotiated.push_back(ret);
    }
}

void MessageCompressorManager::serverNegotiate(const BSONObj& input, BSONObjBuilder* output) {
    LOGV2_DEBUG(22618, 3, "Starting server-side compression negotiation");
    ...
    if (elem.eoo()) {
        ...
        if (_negotiated.size() > 0) {
            ...
            LOGV2_DEBUG(22619, 3, "Compression negotiation not requested by client");
        }
        return;
    }
    ...
    if (!theirObj.nFields()) {
        LOGV2_DEBUG(22620, 3, "No compressors provided");
        return;
    }
    ...
    for (const auto& elem : theirObj) {
        ...
        if ((cur = _registry->getCompressor(curName))) {
            LOGV2_DEBUG(22621, 3, "{cur_getName} is supported", "cur_getName"_attr = cur->getName());
            _negotiated.push_back(cur);
        } else {  // Otherwise the compressor is not supported and we skip over it.
            LOGV2_DEBUG(22622, 3, "{curName} is not supported", "curName"_attr = curName);
        }
    }
    ...
    if (_negotiated.size() > 0) {
        ...
        LOGV2_DEBUG(22623, 3, "Could not agree on compressor to use");
    }
}
