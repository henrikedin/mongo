...
public:
    ...
    explicit EmbeddedLogAppender(std::function<void(const char*, const char*, int)> callback,
                                 std::unique_ptr<EventEncoder> encoder)
        : _encoder(std::move(encoder)), _callback(std::move(callback)) {}
    virtual Status append(const Event& event) {
        std::stringstream output;
        _encoder->encode(event, output);
        _callback(output.str().c_str(),
...
private:
    ...
    std::function<void(const char*, const char*, int)> _callback;
};
