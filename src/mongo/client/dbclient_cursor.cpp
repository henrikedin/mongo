...
bool DBClientCursor::init() {
    ...
    if (!_client->call(toSend, reply, false, &_originalHost)) {
        // log msg temp?
        log() << "DBClientCursor::init call() failed" ;
        return false;
    }
    if (reply.empty()) {
        // log msg temp?
        log() << "DBClientCursor::init message from call() was empty" ;
        return false;
    }
    dataReceived(reply);
    return true;
...
bool DBClientCursor::initLazyFinish(bool& retry) {
    ...
    if (!recvd || reply.empty()) {
        if (!recvd)
            log() << "DBClientCursor::init lazy say() failed" ;
        if (reply.empty())
            log() << "DBClientCursor::init message from say() was empty" ;
