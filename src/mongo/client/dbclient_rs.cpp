...
string DBClientReplicaSet::getServerAddress() const {
    if (!_rsm) {
        LOGV2_WARNING("Trying to get server address for DBClientReplicaSet, but no "
                     "ReplicaSetMonitor exists for {}", "_setName"_attr = _setName);
...
void DBClientReplicaSet::_authConnection(DBClientConnection* conn) {
    if (_internalAuthRequested) {
        ...
        if (!status.isOK()) {
            LOGV2_WARNING("cached auth failed for set {}: {}", "_setName"_attr = _setName, "status"_attr = status);
        }
        return;
    }
    ...
    for (map<string, BSONObj>::const_iterator i = _auths.begin(); i != _auths.end(); ++i) {
        try {
            ...
            LOGV2_WARNING("cached auth failed for set: {} db: {} user: {}", "_setName"_attr = _setName, "i_second_saslCommandUserDBFieldName_str"_attr = i->second[saslCommandUserDBFieldName].str(), "i_second_saslCommandUserFieldName_str"_attr = i->second[saslCommandUserFieldName].str());
        }
    }
}
    ...
    if (_lazyState._lastOp == dbQuery && _lazyState._secondaryQueryOk) {
        ...
        if (networkError ||
            ...
            if (_lazyState._lastClient == _lastSlaveOkConn.get()) {
                ...
                LOGV2_WARNING("passed {} but last rs client {} is not master or secondary", "redact_dataObj"_attr = redact(dataObj), "_lazyState__lastClient_toString"_attr = _lazyState._lastClient->toString());
            }
