...
string DBClientReplicaSet::getServerAddress() const {
    if (!_rsm) {
        LOGV2_WARNING(20144, "Trying to get server address for DBClientReplicaSet, but no "
                     "ReplicaSetMonitor exists for {setName}", "setName"_attr = _setName);
...
void DBClientReplicaSet::_authConnection(DBClientConnection* conn) {
    if (_internalAuthRequested) {
        ...
        if (!status.isOK()) {
            LOGV2_WARNING(20145, "cached auth failed for set {setName}: {status}", "setName"_attr = _setName, "status"_attr = status);
        }
        return;
    }
    ...
    for (map<string, BSONObj>::const_iterator i = _auths.begin(); i != _auths.end(); ++i) {
        try {
            ...
            LOGV2_WARNING(20146, "cached auth failed for set: {setName} db: {i_second_saslCommandUserDBFieldName_str} user: {i_second_saslCommandUserFieldName_str}", "setName"_attr = _setName, "i_second_saslCommandUserDBFieldName_str"_attr = i->second[saslCommandUserDBFieldName].str(), "i_second_saslCommandUserFieldName_str"_attr = i->second[saslCommandUserFieldName].str());
        }
    }
}
...
void DBClientReplicaSet::logoutAll(DBClientConnection* conn) {
    ...
    for (map<string, BSONObj>::const_iterator i = _auths.begin(); i != _auths.end(); ++i) {
        ...
        try {
            ...
            LOGV2_WARNING(20147, "Failed to logout: {conn_getServerAddress} on db: {i_first}{causedBy_redact_ex}", "conn_getServerAddress"_attr = conn->getServerAddress(), "i_first"_attr = i->first, "causedBy_redact_ex"_attr = causedBy(redact(ex)));
        }
    }
}
...
Status DBClientReplicaSet::_runAuthLoop(Authenticate authCb) {
    ...
    LOGV2_DEBUG(20129, 3, "dbclient_rs authentication of {getMonitor_getName}", "getMonitor_getName"_attr = _getMonitor()->getName());
...
unique_ptr<DBClientCursor> DBClientReplicaSet::query(const NamespaceStringOrUUID& nsOrUuid,
    ...
    if (_isSecondaryQuery(ns, query.obj, *readPref)) {
        LOGV2_DEBUG(20130, 3, "dbclient_rs query using secondary or tagged node selection in {getMonitor_getName}, read pref is {readPref_toString} (primary : {master_get_nullptr_master_getServerAddress_not_cached}, lastTagged : {lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached})", "getMonitor_getName"_attr = _getMonitor()->getName(), "readPref_toString"_attr = readPref->toString(), "master_get_nullptr_master_getServerAddress_not_cached"_attr = (_master.get() != nullptr ? _master->getServerAddress() : "[not cached]"), "lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached"_attr = (_lastSlaveOkConn.get() != nullptr ? _lastSlaveOkConn->getServerAddress()
                                                     : "[not cached]"));
    ...
    LOGV2_DEBUG(20131, 3, "dbclient_rs query to primary node in {getMonitor_getName}", "getMonitor_getName"_attr = _getMonitor()->getName());
...
BSONObj DBClientReplicaSet::findOne(const string& ns,
    ...
    shared_ptr<ReadPreferenceSetting> readPref(_extractReadPref(query.obj, queryOptions));
    if (_isSecondaryQuery(ns, query.obj, *readPref)) {
        LOGV2_DEBUG(20132, 3, "dbclient_rs findOne using secondary or tagged node selection in {getMonitor_getName}, read pref is {readPref_toString} (primary : {master_get_nullptr_master_getServerAddress_not_cached}, lastTagged : {lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached})", "getMonitor_getName"_attr = _getMonitor()->getName(), "readPref_toString"_attr = readPref->toString(), "master_get_nullptr_master_getServerAddress_not_cached"_attr = (_master.get() != nullptr ? _master->getServerAddress() : "[not cached]"), "lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached"_attr = (_lastSlaveOkConn.get() != nullptr ? _lastSlaveOkConn->getServerAddress()
                                                     : "[not cached]"));
    ...
    LOGV2_DEBUG(20133, 3, "dbclient_rs findOne to primary node in {getMonitor_getName}", "getMonitor_getName"_attr = _getMonitor()->getName());
...
DBClientConnection* DBClientReplicaSet::selectNodeUsingTags(
    ...
    if (checkLastHost(readPref.get())) {
        LOGV2_DEBUG(20134, 3, "dbclient_rs selecting compatible last used node {lastSlaveOkHost}", "lastSlaveOkHost"_attr = _lastSlaveOkHost);
    ...
    if (!selectedNodeStatus.isOK()) {
        LOGV2_DEBUG(20135, 3, "dbclient_rs no compatible node found{causedBy_redact_selectedNodeStatus_getStatus}", "causedBy_redact_selectedNodeStatus_getStatus"_attr = causedBy(redact(selectedNodeStatus.getStatus())));
        return nullptr;
    }
    ...
    if (monitor->isPrimary(selectedNode)) {
        ...
        LOGV2_DEBUG(20136, 3, "dbclient_rs selecting primary node {selectedNode}", "selectedNode"_attr = selectedNode);
    ...
    LOGV2_DEBUG(20137, 3, "dbclient_rs selecting node {lastSlaveOkHost}", "lastSlaveOkHost"_attr = _lastSlaveOkHost);
...
void DBClientReplicaSet::say(Message& toSend, bool isRetry, string* actualServer) {
    ...
    if (lastOp == dbQuery) {
        ...
        if (_isSecondaryQuery(qm.ns, qm.query, *readPref)) {
            LOGV2_DEBUG(20138, 3, "dbclient_rs say using secondary or tagged node selection in {getMonitor_getName}, read pref is {readPref_toString} (primary : {master_get_nullptr_master_getServerAddress_not_cached}, lastTagged : {lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached})", "getMonitor_getName"_attr = _getMonitor()->getName(), "readPref_toString"_attr = readPref->toString(), "master_get_nullptr_master_getServerAddress_not_cached"_attr = (_master.get() != nullptr ? _master->getServerAddress() : "[not cached]"), "lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached"_attr = (_lastSlaveOkConn.get() != nullptr ? _lastSlaveOkConn->getServerAddress()
                                                         : "[not cached]"));
    ...
    LOGV2_DEBUG(20139, 3, "dbclient_rs say to primary node in {getMonitor_getName}", "getMonitor_getName"_attr = _getMonitor()->getName());
...
Status DBClientReplicaSet::recv(Message& m, int lastRequestId) {
    ...
    try {
        ...
        LOGV2(20140, "could not receive data from {lazyState_lastClient_toString}{causedBy_redact_e}", "lazyState_lastClient_toString"_attr = _lazyState._lastClient->toString(), "causedBy_redact_e"_attr = causedBy(redact(e)));
        return e.toStatus();
    }
}
    ...
    if (_lazyState._lastOp == dbQuery && _lazyState._secondaryQueryOk) {
        ...
        if (networkError ||
            ...
            if (_lazyState._lastClient == _lastSlaveOkConn.get()) {
                ...
                LOGV2_WARNING(20148, "passed {redact_dataObj} but last rs client {lazyState_lastClient_toString} is not master or secondary", "redact_dataObj"_attr = redact(dataObj), "lazyState_lastClient_toString"_attr = _lazyState._lastClient->toString());
            }
            ...
            if (_lazyState._retries < static_cast<int>(MAX_RETRY)) {
                ...
                *retry = true;
                ...
                LOGV2(20141, "too many retries ({lazyState_retries}), could not get data from replica set", "lazyState_retries"_attr = _lazyState._retries);
            }
        }
...
bool DBClientReplicaSet::call(Message& toSend,
    ...
    if (toSend.operation() == dbQuery) {
        ...
        if (_isSecondaryQuery(ns, qm.query, *readPref)) {
            LOGV2_DEBUG(20142, 3, "dbclient_rs call using secondary or tagged node selection in {getMonitor_getName}, read pref is {readPref_toString} (primary : {master_get_nullptr_master_getServerAddress_not_cached}, lastTagged : {lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached})", "getMonitor_getName"_attr = _getMonitor()->getName(), "readPref_toString"_attr = readPref->toString(), "master_get_nullptr_master_getServerAddress_not_cached"_attr = (_master.get() != nullptr ? _master->getServerAddress() : "[not cached]"), "lastSlaveOkConn_get_nullptr_lastSlaveOkConn_getServerAddress_not_cached"_attr = (_lastSlaveOkConn.get() != nullptr ? _lastSlaveOkConn->getServerAddress()
                                                         : "[not cached]"));
    ...
    LOGV2_DEBUG(20143, 3, "dbclient_rs call to primary node in {getMonitor_getName}", "getMonitor_getName"_attr = _getMonitor()->getName());
