...
Status DBClientConnection::connect(const HostAndPort& serverAddress, StringData applicationName) {
    ...
    if (!validateStatus.isOK()) {
        LOGV2_WARNING(20124, "remote host has incompatible wire version: {validateStatus}", "validateStatus"_attr = validateStatus);
...
Status DBClientConnection::connectSocketOnly(const HostAndPort& serverAddress) {
    ...
    LOGV2_DEBUG(20117, 1, "connected to server {toString}", "toString"_attr = toString());
    return Status::OK();
}
...
void DBClientConnection::_checkConnection() {
    ...
    LOGV2_DEBUG(20118, logSeverityV1toV2(_logLevel).toInt(), "trying reconnect to {toString}", "toString"_attr = toString());
    string errmsg;
    auto connectStatus = connect(_serverAddress, _applicationName);
    if (!connectStatus.isOK()) {
        _markFailed(kSetFlag);
        LOGV2_DEBUG(20119, logSeverityV1toV2(_logLevel).toInt(), "reconnect {toString} failed {errmsg}", "toString"_attr = toString(), "errmsg"_attr = errmsg);
    ...
    LOGV2_DEBUG(20120, logSeverityV1toV2(_logLevel).toInt(), "reconnect {toString} ok", "toString"_attr = toString());
    if (_internalAuthOnReconnect) {
        ...
        for (const auto& kv : authCache) {
            try {
                ...
                LOGV2_DEBUG(20121, logSeverityV1toV2(_logLevel).toInt(), "reconnect: auth failed {kv_second_auth_getSaslCommandUserDBFieldName}{kv_second_auth_getSaslCommandUserFieldName} {ex_what}", "kv_second_auth_getSaslCommandUserDBFieldName"_attr = kv.second[auth::getSaslCommandUserDBFieldName()], "kv_second_auth_getSaslCommandUserFieldName"_attr = kv.second[auth::getSaslCommandUserFieldName()], "ex_what"_attr = ex.what());
            }
        }
    }
}
...
bool DBClientConnection::call(Message& toSend,
    ...
    if (!sinkStatus.isOK()) {
        LOGV2(20122, "DBClientConnection failed to send message to {getServerAddress} - {redact_sinkStatus}", "getServerAddress"_attr = getServerAddress(), "redact_sinkStatus"_attr = redact(sinkStatus));
    ...
    if (swm.isOK()) {
        ...
        LOGV2(20123, "DBClientConnection failed to receive message from {getServerAddress} - {redact_swm_getStatus}", "getServerAddress"_attr = getServerAddress(), "redact_swm_getStatus"_attr = redact(swm.getStatus()));
