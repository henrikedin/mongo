...
TEST(BSONValidate, RandomData) {
    ...
    LOGV2(20103, "RandomData: didn't crash valid/total: {numValid}/{numToRun} (want few valid ones) jsonSize: {jsonSize}", "numValid"_attr = numValid, "numToRun"_attr = numToRun, "jsonSize"_attr = jsonSize);
}
...
TEST(BSONValidate, MuckingData1) {
    ...
    LOGV2(20104, "MuckingData1: didn't crash valid/total: {numValid}/{numToRun} (want few valid ones)  jsonSize: {jsonSize}", "numValid"_attr = numValid, "numToRun"_attr = numToRun, "jsonSize"_attr = jsonSize);
}

TEST(BSONValidate, Fuzz) {
    int64_t seed = time(nullptr);
    LOGV2(20105, "BSONValidate Fuzz random seed: {seed}", "seed"_attr = seed);
