...
void BSONObj::jsonStringGenerator(const Generator& g,
                                  int pretty,
                                  bool isArray,
...
// Instantiate jsonStringGenerator with our available generators so it can be called from
// BSONElement
template void BSONObj::jsonStringGenerator<ExtendedCanonicalV200Generator>(
    ExtendedCanonicalV200Generator const&, int, bool, fmt::memory_buffer&) const;
template void BSONObj::jsonStringGenerator<ExtendedRelaxedV200Generator>(
    ExtendedRelaxedV200Generator const&, int, bool, fmt::memory_buffer&) const;
template void BSONObj::jsonStringGenerator<LegacyStrictGenerator>(LegacyStrictGenerator const&,
                                                                  int,
                                                                  bool,
...
void BSONObj::jsonStringBuffer(JsonStringFormat format,
    ...
    if (format == ExtendedCanonicalV200) {
        jsonStringGenerator(ExtendedCanonicalV200Generator(), pretty, isArray, buffer);
    } else if (format == ExtendedRelaxedV200) {
        jsonStringGenerator(ExtendedRelaxedV200Generator(), pretty, isArray, buffer);
    } else if (format == LegacyStrict) {
        jsonStringGenerator(LegacyStrictGenerator(), pretty, isArray, buffer);
    } else {
        MONGO_UNREACHABLE;
    }
}
