...
void BSONElement::jsonStringBuffer(JsonStringFormat format,
    ...
    if (format == ExtendedCanonicalV200) {
        jsonStringGenerator(ExtendedCanonicalV200Generator(), includeFieldNames, pretty, buffer);
    } else if (format == ExtendedRelaxedV200) {
        jsonStringGenerator(ExtendedRelaxedV200Generator(), includeFieldNames, pretty, buffer);
    } else if (format == LegacyStrict) {
        jsonStringGenerator(LegacyStrictGenerator(), includeFieldNames, pretty, buffer);
    } else {
        MONGO_UNREACHABLE;
    }
}
...
void BSONElement::jsonStringGenerator(const Generator& g,
                                      bool includeFieldNames,
                                      int pretty,
                                      fmt::memory_buffer& buffer) const {
    ...
    switch (type()) {
        ...
        case BinData: {
            ...
            g.writeBinData(buffer, reader.view(), len, type);
        }
...
// Instantiate jsonStringGenerator with our available generators so it can be called from BSONObj
template void BSONElement::jsonStringGenerator<ExtendedCanonicalV200Generator>(
    ExtendedCanonicalV200Generator const&, bool, int, fmt::memory_buffer&) const;
template void BSONElement::jsonStringGenerator<ExtendedRelaxedV200Generator>(
    ExtendedRelaxedV200Generator const&, bool, int, fmt::memory_buffer&) const;
template void BSONElement::jsonStringGenerator<LegacyStrictGenerator>(LegacyStrictGenerator const&,
                                                                      bool,
                                                                      int,
                                                                      fmt::memory_buffer&) const;
