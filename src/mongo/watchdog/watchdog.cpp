...

        {
            ...
            try {
                ...
                if (!opCtx->getServiceContext()->getKillAllOperations()) {
                    LOGV2_FATAL(23081, "Watchdog was interrupted, shutting down, reason: {e_toStatus}", "e_toStatus"_attr = e.toStatus());
                ...
                LOGV2_DEBUG(23072, 1, "WatchdogPeriodicThread interrupted by: {e}", "e"_attr = e);
                return;
            }
...
void WatchdogCheckThread::run(OperationContext* opCtx) {
    for (auto& check : _checks) {
        ...
        LOGV2_DEBUG(23073, 1, "Watchdog test '{check_getDescriptionForLogging}' took {duration_cast_Milliseconds_micros}", "check_getDescriptionForLogging"_attr = check->getDescriptionForLogging(), "duration_cast_Milliseconds_micros"_attr = duration_cast<Milliseconds>(micros));
...
void WatchdogMonitor::start() {
    LOGV2(23074, "Starting Watchdog Monitor");
...
void WatchdogMonitor::setPeriod(Milliseconds duration) {
    {
        ...
        if (duration > Milliseconds(0)) {
            ...
            LOGV2(23075, "WatchdogMonitor period changed to {duration_cast_Seconds_duration}", "duration_cast_Seconds_duration"_attr = duration_cast<Seconds>(duration));
        } else {
            _watchdogMonitorThread.setPeriod(duration);
            _watchdogCheckThread.setPeriod(duration);

            LOGV2(23076, "WatchdogMonitor disabled");
        }
    }
}
...
void checkFile(OperationContext* opCtx, const boost::filesystem::path& file) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        ...
        LOGV2_FATAL(23082, "CreateFile failed for '{file_generic_string}' with error: {errnoWithDescription_gle}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
    ...
    if (!WriteFile(hFile, nowStr.c_str(), nowStr.size(), &bytesWrittenTotal, NULL)) {
        std::uint32_t gle = ::GetLastError();
        LOGV2_FATAL(23083, "WriteFile failed for '{file_generic_string}' with error: {errnoWithDescription_gle}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
        fassertNoTrace(4075, gle == 0);
    }

    if (bytesWrittenTotal != nowStr.size()) {
        LOGV2_WARNING(23077, "partial write for '{file_generic_string}' expected {nowStr_size} bytes but wrote {bytesWrittenTotal} bytes", "file_generic_string"_attr = file.generic_string(), "nowStr_size"_attr = nowStr.size(), "bytesWrittenTotal"_attr = bytesWrittenTotal);
    } else {

        if (!FlushFileBuffers(hFile)) {
            std::uint32_t gle = ::GetLastError();
            LOGV2_FATAL(23084, "FlushFileBuffers failed for '{file_generic_string}' with error: {errnoWithDescription_gle}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
        ...
        if (newOffset != 0) {
            std::uint32_t gle = ::GetLastError();
            LOGV2_FATAL(23085, "SetFilePointer failed for '{file_generic_string}' with error: {errnoWithDescription_gle}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
        ...
        if (!ReadFile(hFile, readBuffer.get(), nowStr.size(), &bytesRead, NULL)) {
            std::uint32_t gle = ::GetLastError();
            LOGV2_FATAL(23086, "ReadFile failed for '{file_generic_string}' with error: {errnoWithDescription_gle}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
            fassertNoTrace(4078, gle == 0);
        }

        if (bytesRead != bytesWrittenTotal) {
            LOGV2_FATAL(23087, "Read wrong number of bytes for '{file_generic_string}' expected {bytesWrittenTotal} bytes but read {bytesRead} bytes", "file_generic_string"_attr = file.generic_string(), "bytesWrittenTotal"_attr = bytesWrittenTotal, "bytesRead"_attr = bytesRead);
            fassertNoTrace(50724, false);
        }

        if (memcmp(nowStr.c_str(), readBuffer.get(), nowStr.size()) != 0) {
            LOGV2_FATAL(23088, "Read wrong string from file '{file_generic_string}{nowStr_size} bytes (in hex) '{toHexLower_nowStr_c_str_nowStr_size}' but read bytes '{toHexLower_readBuffer_get_bytesRead}'", "file_generic_string"_attr = file.generic_string(), "nowStr_size"_attr = nowStr.size(), "toHexLower_nowStr_c_str_nowStr_size"_attr = toHexLower(nowStr.c_str(), nowStr.size()), "toHexLower_readBuffer_get_bytesRead"_attr = toHexLower(readBuffer.get(), bytesRead));
    ...
    if (!CloseHandle(hFile)) {
        std::uint32_t gle = ::GetLastError();
        LOGV2_FATAL(23089, "CloseHandle failed for '{file_generic_string}' with error: {errnoWithDescription_gle}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
...
void checkFile(OperationContext* opCtx, const boost::filesystem::path& file) {
    ...
    if (fd == -1) {
        auto err = errno;
        LOGV2_FATAL(23090, "open failed for '{file_generic_string}' with error: {errnoWithDescription_err}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_err"_attr = errnoWithDescription(err));
    ...
    while (bytesWrittenTotal < nowStr.size()) {
        ...
        if (bytesWrittenInWrite == -1) {
            ...
            LOGV2_FATAL(23091, "write failed for '{file_generic_string}' with error: {errnoWithDescription_err}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_err"_attr = errnoWithDescription(err));
        ...
        if (bytesWrittenTotal == 0 && static_cast<size_t>(bytesWrittenInWrite) != nowStr.size()) {
            LOGV2_WARNING(23078, "parital write for '{file_generic_string}' expected {nowStr_size} bytes but wrote {bytesWrittenInWrite} bytes", "file_generic_string"_attr = file.generic_string(), "nowStr_size"_attr = nowStr.size(), "bytesWrittenInWrite"_attr = bytesWrittenInWrite);
        }
    ...
    if (fsync(fd)) {
        auto err = errno;
        LOGV2_FATAL(23092, "fsync failed for '{file_generic_string}' with error: {errnoWithDescription_err}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_err"_attr = errnoWithDescription(err));
    ...
    while (bytesReadTotal < nowStr.size()) {
        ...
        if (bytesReadInRead == -1) {
            ...
            LOGV2_FATAL(23093, "read failed for '{file_generic_string}' with error: {errnoWithDescription_err}", "file_generic_string"_attr = file.generic_string(), "errnoWithDescription_err"_attr = errnoWithDescription(err));
            fassertNoTrace(4083, err == 0);
        } else if (bytesReadInRead == 0) {
            LOGV2_FATAL(23094, "read failed for '{file_generic_string}' with unexpected end of file", "file_generic_string"_attr = file.generic_string());
        ...
        if (bytesReadTotal == 0 && static_cast<size_t>(bytesReadInRead) != nowStr.size()) {
            LOGV2_WARNING(23079, "partial read for '{file_generic_string}' expected {nowStr_size} bytes but read {bytesReadInRead} bytes", "file_generic_string"_attr = file.generic_string(), "nowStr_size"_attr = nowStr.size(), "bytesReadInRead"_attr = bytesReadInRead);
        }
...
void DirectoryCheck::run(OperationContext* opCtx) {
    ...
    if (ec) {
        LOGV2_WARNING(23080, "Failed to delete file '{file_generic_string}', error: {ec_message}", "file_generic_string"_attr = file.generic_string(), "ec_message"_attr = ec.message());
    }
}
