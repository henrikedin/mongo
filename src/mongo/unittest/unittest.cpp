...
int Suite::run(const std::vector<std::string>& suites,
    ...
    if (!totals._fails.empty()) {
        ...
        LOGV2("FAILURE - {} tests in {} suites failed", "totals_fails_size"_attr = totals._fails.size(), "failedSuites_size"_attr = failedSuites.size());
    } else {
...
TestAssertionFailure::~TestAssertionFailure() noexcept(false) {
    ...
    LOGV2_ERROR("Throwing exception: {}", "exception"_attr = _exception);
    throw _exception;
}
