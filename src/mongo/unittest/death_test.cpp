...
void DeathTestBase::_doTest() {
#if defined(_WIN32)
    LOGV2(22733, "Skipping death test on Windows");
    return;
#elif defined(__APPLE__) && (TARGET_OS_TV || TARGET_OS_WATCH)
    LOGV2(22734, "Skipping death test on tvOS/watchOS");
    return;
#else
    int pipes[2];
    ...
    if (child) {
        ...
        ssize_t bytesRead;
        LOGV2(22735, "========== Beginning of interleaved output of death test ==========");
        ...
        LOGV2(22736, "========== End of interleaved output of death test ==========");
        ...
        while (child != (pid = waitpid(child, &stat, 0))) {
            ...
            switch (err) {
                case EINTR:
                    continue;
                default:
                    LOGV2_FATAL(22738, "Unrecoverable error while waiting for {child}: {errnoWithDescription_err}", "child"_attr = child, "errnoWithDescription_err"_attr = errnoWithDescription(err));
                    MONGO_UNREACHABLE;
            }
        }
    ...
    try {
        ...
        LOGV2(22737, "Caught test exception while expecting death: {tafe}", "tafe"_attr = tafe);
