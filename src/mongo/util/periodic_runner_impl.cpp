...
void PeriodicRunnerImpl::PeriodicJobImpl::start() {
    LOGV2_DEBUG(23001, 2, "Starting periodic job {job_name}", "job_name"_attr = _job.name);
...
void PeriodicRunnerImpl::PeriodicJobImpl::stop() {
    ...
    if (lastExecStatus != ExecutionStatus::CANCELED) {
        LOGV2_DEBUG(23002, 2, "Stopping periodic job {job_name}", "job_name"_attr = _job.name);
