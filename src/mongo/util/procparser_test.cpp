...
TEST(FTDCProcStat, TestLocalStat) {
    ...
    auto stringMap = toStringMap(obj);
    LOGV2(23042, "OBJ:{obj}", "obj"_attr = obj);
    ASSERT_KEY("user_ms");
    ASSERT_KEY("nice_ms");
    ASSERT_KEY("idle_ms");
...
TEST(FTDCProcMemInfo, TestLocalMemInfo) {
    ...
    BSONObj obj = builder.obj();
    auto stringMap = toStringMap(obj);
    LOGV2(23043, "OBJ:{obj}", "obj"_attr = obj);
...
TEST(FTDCProcNetstat, TestLocalNetstat) {
    ...
    BSONObj obj = builder.obj();
    auto stringMap = toStringMap(obj);
    LOGV2(23044, "OBJ:{obj}", "obj"_attr = obj);
...
TEST(FTDCProcNetstat, TestLocalNetSnmp) {
    ...
    BSONObj obj = builder.obj();
    auto stringMap = toStringMap(obj);
    LOGV2(23045, "OBJ:{obj}", "obj"_attr = obj);
...
TEST(FTDCProcDiskStats, TestLocalDiskStats) {
    ...
    for (const auto& disk : disks) {
        LOGV2(23046, "DISK:{disk}", "disk"_attr = disk);
    ...
    BSONObj obj = builder.obj();
    auto stringMap = toNestedStringMap(obj);
    LOGV2(23047, "OBJ:{obj}", "obj"_attr = obj);
