...
int ProcessInfo::getVirtualMemorySize() {
    ...
    if ((kd = kvm_openfiles(NULL, NULL, NULL, KVM_NO_FILES, err)) == NULL) {
        LOGV2(23021, "Unable to get virt mem size: {err}", "err"_attr = err);
        return -1;
    }
...
int ProcessInfo::getResidentSize() {
    ...
    char err[_POSIX2_LINE_MAX] = {0};
    if ((kd = kvm_openfiles(NULL, NULL, NULL, KVM_NO_FILES, err)) == NULL) {
        LOGV2(23022, "Unable to get res mem size: {err}", "err"_attr = err);
        return -1;
    }
    kinfo_proc* task = kvm_getprocs(kd, KERN_PROC_PID, _pid.toNative(), sizeof(kinfo_proc), &cnt);
...
void ProcessInfo::SystemInfo::collectSystemInfo() {
    ...
    if (status != 0)
        LOGV2(23023, "Unable to collect OS Version. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));

    mib[0] = CTL_HW;
    mib[1] = HW_MACHINE;
    status = getSysctlByIDWithDefault(mib, 2, std::string("unknown"), &cpuArch);
    if (status != 0)
        LOGV2(23024, "Unable to collect Machine Architecture. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));
    ...
    memLimit = memSize;
    if (status != 0)
        LOGV2(23025, "Unable to collect Physical Memory. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));
    ...
    numCores = numBuffer;
    if (status != 0)
        LOGV2(23026, "Unable to collect Number of CPUs. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));
...
bool ProcessInfo::blockInMemory(const void* start) {
    ...
    if (mincore((void*)alignToStartOfPage(start), getPageSize(), &x)) {
        LOGV2(23027, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return 1;
    }
    return x & 0x1;
}
...
bool ProcessInfo::pagesInMemory(const void* start, size_t numPages, std::vector<char>* out) {
    ...
    if (mincore((void*)alignToStartOfPage(start), numPages * getPageSize(), &(out->front()))) {
        LOGV2(23028, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return false;
    }
