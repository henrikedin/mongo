...
void BlockedOp::start(ServiceContext* serviceContext) {
    ...
    _latchState.thread = stdx::thread([this, serviceContext]() mutable {
        ...
        LOGV2(22806, "Entered currentOpSpawnsThreadWaitingForLatch thread");

        stdx::lock_guard testLock(_latchState.mutex);

        LOGV2(22807, "Joining currentOpSpawnsThreadWaitingForLatch thread");
    });
    ...
    _interruptibleState.thread = stdx::thread([this, serviceContext]() mutable {
        ...
        LOGV2(22808, "Entered currentOpSpawnsThreadWaitingForLatch thread for interruptibles");
        ...
        LOGV2(22809, "Joining currentOpSpawnsThreadWaitingForLatch thread for interruptibles");
    });


    _cv.wait(lk, [this] { return _latchState.isContended && _interruptibleState.isWaiting; });
    LOGV2(22810, "Started threads for currentOpSpawnsThreadWaitingForLatch");
}
...
void BlockedOp::setIsContended(bool value) {
    LOGV2(22811, "Setting isContended to {value_true_false}", "value_true_false"_attr = (value ? "true" : "false"));
...
void BlockedOp::setIsWaiting(bool value) {
    LOGV2(22812, "Setting isWaiting to {value_true_false}", "value_true_false"_attr = (value ? "true" : "false"));
    stdx::lock_guard lk(_m);
