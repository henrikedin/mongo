    ...
    static unsigned long long getSystemMemorySize() {
        ...
        if (!meminfo.empty() && (lineOff = meminfo.find("MemTotal")) != std::string::npos) {
            ...
            if (mongo::NumberParser{}(meminfo, &systemMem).isOK()) {
                ...
                LOGV2(23016, "Unable to collect system memory information");
        }
        return 0;
    }
...
void ProcessInfo::SystemInfo::collectSystemInfo() {
    ...
    if (uname(&unameData) == -1) {
        LOGV2(23017, "Unable to collect detailed system information: {strerror_errno}", "strerror_errno"_attr = strerror(errno));
    }
...
bool ProcessInfo::checkNumaEnabled() {
    ...
    try {
        ...
        LOGV2(23018, "WARNING: Cannot detect if NUMA interleaving is enabled. Failed to probe \"{e_path1_string}\": {e_code_message}", "e_path1_string"_attr = e.path1().string(), "e_code_message"_attr = e.code().message());
        return false;
    }
...
bool ProcessInfo::blockInMemory(const void* start) {
    ...
    if (mincore(const_cast<void*>(alignToStartOfPage(start)), getPageSize(), &x)) {
        LOGV2(23019, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return 1;
    }
    return x & 0x1;
}
...
bool ProcessInfo::pagesInMemory(const void* start, size_t numPages, std::vector<char>* out) {
    ...
    if (mincore(const_cast<void*>(alignToStartOfPage(start)),
        ...
        LOGV2(23020, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return false;
    }
