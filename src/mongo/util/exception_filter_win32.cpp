...
LONG WINAPI exceptionFilter(struct _EXCEPTION_POINTERS* excPointers) {
    ...
    LOGV2_ERROR("*** unhandled exception {} at {}, terminating", "exceptionString"_attr = exceptionString, "addressString"_attr = addressString);
    if (excPointers->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) {
        ...
        LOGV2_ERROR("*** access violation was a {}{}", "acTypeString"_attr = acTypeString, "addressString"_attr = addressString);
    }

    LOGV2_ERROR("*** stack trace for unhandled exception:");
