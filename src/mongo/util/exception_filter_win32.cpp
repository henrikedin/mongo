...
void doMinidumpWithException(struct _EXCEPTION_POINTERS* exceptionInfo) {
    ...
    if (ret == 0) {
        ...
        LOGV2(22813, "GetModuleFileName failed {errnoWithDescription_gle}", "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
    ...
    if (INVALID_HANDLE_VALUE == hFile) {
        ...
        LOGV2(22814, "failed to open minidump file {toUtf8String_dumpName_c_str} : {errnoWithDescription_lasterr}", "toUtf8String_dumpName_c_str"_attr = toUtf8String(dumpName.c_str()), "errnoWithDescription_lasterr"_attr = errnoWithDescription(lasterr));
        return;
    }
...
#endif
    LOGV2(22815, "writing minidump diagnostic file {toUtf8String_dumpName_c_str}", "toUtf8String_dumpName_c_str"_attr = toUtf8String(dumpName.c_str()));
    ...
    if (FALSE == bstatus) {
        DWORD lasterr = GetLastError();
        LOGV2(22816, "failed to create minidump : {errnoWithDescription_lasterr}", "errnoWithDescription_lasterr"_attr = errnoWithDescription(lasterr));
    }
...
LONG WINAPI exceptionFilter(struct _EXCEPTION_POINTERS* excPointers) {
    ...
    LOGV2_FATAL(22817, "*** unhandled exception {exceptionString} at {addressString}, terminating", "exceptionString"_attr = exceptionString, "addressString"_attr = addressString);
    if (excPointers->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) {
        ...
        LOGV2_FATAL(22818, "*** access violation was a {acTypeString}{addressString}", "acTypeString"_attr = acTypeString, "addressString"_attr = addressString);
    }

    LOGV2_FATAL(22819, "*** stack trace for unhandled exception:");
    ...
    LOGV2_FATAL(22820, "*** immediate exit due to unhandled exception");
