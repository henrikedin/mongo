...
void doMinidumpWithException(struct _EXCEPTION_POINTERS* exceptionInfo) {
    ...
    if (ret == 0) {
        ...
        LOG(23130, "GetModuleFileName failed", "error"_attr = errnoWithDescription(gle));
    ...
    if (FALSE == bstatus) {
        DWORD lasterr = GetLastError();
        LOG(23133, "Failed to create minidump", "error"_attr = errnoWithDescription(lasterr));
    }
...
LONG WINAPI exceptionFilter(struct _EXCEPTION_POINTERS* excPointers) {
    ...
    LOG_FATAL_CONTINUE(23136, "*** stack trace for unhandled exception:");
    ...
    LOG_FATAL_CONTINUE(23137, "*** immediate exit due to unhandled exception");
