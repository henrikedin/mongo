...
void consoleTerminate(const char* controlCodeName) {
    ...
    LOGV2(23048, "got {controlCodeName}, will terminate after current cmd ends", "controlCodeName"_attr = controlCodeName);
...
BOOL WINAPI CtrlHandler(DWORD fdwCtrlType) {
    switch (fdwCtrlType) {
        case CTRL_C_EVENT:
            LOGV2(23049, "Ctrl-C signal");
            consoleTerminate("CTRL_C_EVENT");
            return TRUE;

        case CTRL_CLOSE_EVENT:
            LOGV2(23050, "CTRL_CLOSE_EVENT signal");
            consoleTerminate("CTRL_CLOSE_EVENT");
            return TRUE;

        case CTRL_BREAK_EVENT:
            LOGV2(23051, "CTRL_BREAK_EVENT signal");
        ...
        case CTRL_SHUTDOWN_EVENT:
            LOGV2(23052, "CTRL_SHUTDOWN_EVENT signal");
...
void eventProcessingThread() {
    ...
    if (event == nullptr) {
        LOGV2_WARNING(23059, "eventProcessingThread CreateEvent failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return;
    }
    ...
    if (returnCode != WAIT_OBJECT_0) {
        if (returnCode == WAIT_FAILED) {
            LOGV2_WARNING(23060, "eventProcessingThread WaitForSingleObject failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
            return;
        } else {
            LOGV2_WARNING(23061, "eventProcessingThread WaitForSingleObject failed: {errnoWithDescription_returnCode}", "errnoWithDescription_returnCode"_attr = errnoWithDescription(returnCode));
            return;
        }
    }
    ...
    LOGV2(23053, "shutdown event signaled, will terminate after current cmd ends");
...
bool waitForSignal(const sigset_t& sigset, SignalWaitResult* result) {
    ...
    while (true) {
        ...
        if (result->sig == -1) {
            if (errsv == EINTR)
                continue;
            LOGV2_FATAL(23062, "sigwaitinfo failed with error:{strerror_errsv}", "strerror_errsv"_attr = strerror(errsv));
            return false;
        }
        return true;
    }
...
void handleOneSignal(const SignalWaitResult& waited, LogRotationState* rotation) {
    int sig = waited.sig;
    LOGV2(23054, "got signal {sig} ({strsignal_sig})", "sig"_attr = sig, "strsignal_sig"_attr = strsignal(sig));
#ifdef __linux__
    const siginfo_t& si = waited.si;
    switch (si.si_code) {
        case SI_USER:
        case SI_QUEUE:
            LOGV2(23055, "kill from pid:{si_si_pid} uid:{si_si_uid}", "si_si_pid"_attr = si.si_pid, "si_si_uid"_attr = si.si_uid);
            break;
        case SI_TKILL:
            LOGV2(23056, "tgkill");
            break;
    ...
    LOGV2(23058, "will terminate after current cmd ends");
    exitCleanly(EXIT_CLEAN);
}
...
void signalProcessingThread(LogFileStatus rotate) {
    ...
    if (int r = pthread_sigmask(SIG_SETMASK, &waitSignals, nullptr); r != 0) {
        int errsv = errno;
        LOGV2_FATAL(23063, "pthread_sigmask failed with error:{strerror_errsv}", "strerror_errsv"_attr = strerror(errsv));
        fassertFailed(31377);
    }
