...
template<typename T, typename FindKey, typename ConvertTraits, typename ArgumentHash, typename Hasher, typename ArgumentEqual, typename Equal, typename ArgumentAlloc, typename ByteAlloc, uint8_t BlockSize>
...
private:
    ...
    SKA_NOINLINE(std::pair<iterator, bool>) emplace_direct_hit(LinkedListIt block, Args &&... args)
    {
        ...
        {
            AllocatorTraits::construct(*this, std::addressof(*block), convert_traits.emplace(std::forward<Args>(args)...));
        ...
        else
        {
            ...
            value_type new_value(convert_traits.emplace(std::forward<Args>(args)...));
    ...
    SKA_NOINLINE(std::pair<iterator, bool>) emplace_new_key(LinkedListIt parent, Args &&... args)
    {
        ...
        AllocatorTraits::construct(*this, std::addressof(*free_block.second), convert_traits.emplace(std::forward<Args>(args)...));
        free_block.second.set_metadata(Constants::magic_for_list_entry);
...
struct noop_converter
{
	...
	K toLookup(const K& s) {
		return s;
	}
};

template<typename K, typename V, typename C = noop_converter<K>, typename H = std::hash<K>, typename E = std::equal_to<K>, typename A = std::allocator<std::pair<K, V> > >
...
template<typename T, typename C = noop_converter<T>, typename H = std::hash<T>, typename E = std::equal_to<T>, typename A = std::allocator<T> >
