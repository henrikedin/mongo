...
void networkWarnWithDescription(const Socket& socket, StringData call, int errorCode = -1) {
    ...
    LOGV2_WARNING("Failed to connect to {}:{}, in({}), reason: {}", "socket_remoteAddr_getAddr"_attr = socket.remoteAddr().getAddr(), "socket_remoteAddr_getPort"_attr = socket.remoteAddr().getPort(), "call"_attr = call, "ewd"_attr = ewd);
}
...
void disableNagle(int sock) {
    ...
    if (setsockopt(sock, level, TCP_NODELAY, (char*)&x, sizeof(x)))
        LOGV2_ERROR("disableNagle failed: {}", "errnoWithDescription"_attr = errnoWithDescription());

#ifdef SO_KEEPALIVE
    if (setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (char*)&x, sizeof(x)))
        LOGV2_ERROR("SO_KEEPALIVE failed: {}", "errnoWithDescription"_attr = errnoWithDescription());
#endif
...
SockAddr getLocalAddrForBoundSocketFd(int fd) {
    ...
    if (rc != 0) {
        LOGV2_WARNING("Could not resolve local address for socket with fd {}: {}", "fd"_attr = fd, "getAddrInfoStrError_socketGetLastError"_attr = getAddrInfoStrError(socketGetLastError()));
        result = SockAddr();
    }
    return result;
}
...
bool Socket::connect(SockAddr& remote, Milliseconds connectTimeoutMillis) {
    ...
    if (!connectSucceeded) {
        ...
        while (true) {
            ...
            if (pollReturn == 0) {
                LOGV2_WARNING("Failed to connect to {}:{} after {} milliseconds, giving up.", "_remote_getAddr"_attr = _remote.getAddr(), "_remote_getPort"_attr = _remote.getPort(), "connectTimeoutMillis"_attr = connectTimeoutMillis);
                return false;
            }
...
bool Socket::isStillConnected() {
    ...
    if (nEvents == 0) {
        ...
        LOGV2_WARNING("Socket poll() failed during connectivity check (idle {} secs, remote host {}){}", "idleTimeSecs"_attr = idleTimeSecs, "remoteString"_attr = remoteString(), "causedBy_errnoWithDescription"_attr = causedBy(errnoWithDescription()));
    ...
    if (pollInfo.revents & POLLIN) {
        ...
        if (recvd < 0) {
            ...
            LOGV2_WARNING("Socket recv() failed during connectivity check (idle {} secs, remote host {}){}", "idleTimeSecs"_attr = idleTimeSecs, "remoteString"_attr = remoteString(), "causedBy_errnoWithDescription"_attr = causedBy(errnoWithDescription()));
        } else if (recvd > 0) {
            ...
            LOGV2_ERROR("Socket found pending {} bytes of data during connectivity check (idle {} secs, remote host {})", "recvd"_attr = recvd, "idleTimeSecs"_attr = idleTimeSecs, "remoteString"_attr = remoteString());
            if (kDebugBuild) {
                std::string hex = hexdump(testBuf, recvd);
                LOGV2_ERROR("Hex dump of stale log data: {}", "hex"_attr = hex);
            }
            dassert(false);
        } else {
        ...
        // Log and warn at runtime, log and abort at devtime
        LOGV2_ERROR("Socket descriptor detected as invalid (idle {} secs, remote host {})", "idleTimeSecs"_attr = idleTimeSecs, "remoteString"_attr = remoteString());
        dassert(false);
    } else {
        // Don't know what poll is saying here
        // Log and warn at runtime, log and abort at devtime
        LOGV2_ERROR("Socket had unknown event ({}) (idle {} secs, remote host {})", "static_cast_int_pollInfo_revents"_attr = static_cast<int>(pollInfo.revents), "idleTimeSecs"_attr = idleTimeSecs, "remoteString"_attr = remoteString());
        dassert(false);
    }
