...
bool Socket::connect(SockAddr& remote, Milliseconds connectTimeoutMillis) {
    ...
    if (!connectSucceeded) {
        ...
        while (true) {
            ...
            if (pollReturn == 0) {
                LOGV2_WARNING("Failed to connect to {}:{} after {} milliseconds, giving up.", "remote_getAddr"_attr = _remote.getAddr(), "remote_getPort"_attr = _remote.getPort(), "connectTimeoutMillis"_attr = connectTimeoutMillis);
                return false;
            }
