...
bool enableECDHE(SSL_CTX* const ctx) {
    ...
    if (SSL_CTX_ctrl(ctx, 94, 1, nullptr) != 1) {
        ...
        if (!useDefaultECKey(ctx)) {
            LOGV2_WARNING(22907, "Failed to enable ECDHE due to a lack of support from system libraries.");
            return false;
        }
    }
#endif
...
SSLConnectionOpenSSL::SSLConnectionOpenSSL(SSL_CTX* context,
    ...
    if (len > 0) {
        ...
        if (toBIO != len) {
            LOGV2_DEBUG(22900, 3, "Failed to write initial network data to the SSL BIO layer");
...
int SSLManagerOpenSSL::password_cb(char* buf, int num, int rwflag, void* userdata) {
    ...
    if (!swPassword.isOK()) {
        LOGV2_ERROR(22917, "Unable to fetch password: {swPassword_getStatus}", "swPassword_getStatus"_attr = swPassword.getStatus());
        return -1;
    }
...
Status verifyPeerCertWithOCSP(SSL* conn, X509* peerCert) {
    ...
    for (auto& [host, ocspRequestAndIDs] : ocspRequestMap) {
        ...
        if (swResponse.getStatus().code() == ErrorCodes::InternalErrorNotSupported) {
            LOGV2_WARNING(22908, "Could not perform OCSP validation: {swResponse_getStatus}", "swResponse_getStatus"_attr = swResponse.getStatus());
            return Status::OK();
...
int ocspClientCallback(SSL* ssl, void* arg) {
    ...
    if (!peerCert) {
        LOGV2_DEBUG(22901, 1, "Could not get peer certificate from SSL object in OCSP verification callback. Will continue with the connection.");
    ...
    if (!swStapleOK.isOK()) {
        if (swStapleOK.getStatus() == ErrorCodes::OCSPCertificateStatusRevoked) {
            LOGV2_DEBUG(22902, 1, "Stapled Certificate validation failed: {swStapleOK_getStatus_reason}", "swStapleOK_getStatus_reason"_attr = swStapleOK.getStatus().reason());
            return OCSP_CLIENT_RESPONSE_NOT_ACCEPTABLE;
        }

        return OCSP_CLIENT_RESPONSE_ERROR;
    } else if (!swStapleOK.getValue()) {
        LOGV2_DEBUG(22903, 1, "Stapled Certificate validation failed: Stapled response does not contain status information regarding the peer certificate.");
        return OCSP_CLIENT_RESPONSE_NOT_ACCEPTABLE;
    }
...
Status stapleOCSPResponse(SSL_CTX* context) {
    ...
    if (!cert) {
        ...
        LOGV2_WARNING(22909, "Could not staple because could not get certificate from SSL Context.");
        return Status::OK();
    }
    ...
    for (auto host : leafResponders) {
        ...
        if (!swResponse.isOK()) {
            if (swResponse.getStatus() == ErrorCodes::InternalErrorNotSupported) {
                LOGV2_WARNING(22910, "Could not perform OCSP validation: {swResponse_getStatus}", "swResponse_getStatus"_attr = swResponse.getStatus());
                return Status::OK();
            }
    ...
    LOGV2_WARNING(22911, "Could not staple OCSP response to outgoing certificate.");
    return Status::OK();
}
#endif
    ...
    if (enableECDHE(context)) {
        ...
        if (params.sslPEMTempDHParam.empty()) {
            ...
            if (!dhparams || SSL_CTX_set_tmp_dh(context, dhparams.get()) != 1) {
                LOGV2_ERROR(22918, "Failed to set default DH parameters: {getSSLErrorMessage_ERR_get_error}", "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
            }
        }
    }
...
unsigned long long SSLManagerOpenSSL::_convertASN1ToMillis(ASN1_TIME* asn1time) {
    ...
    if (timeError <= 0) {
        LOGV2_ERROR(22919, "ASN1_TIME_print failed or wrote no data.");
        return 0;
    }
    ...
    if (timeError <= 0) {
        LOGV2_ERROR(22920, "BIO_gets call failed to transfer contents to buf");
        return 0;
    }
...
bool SSLManagerOpenSSL::_parseAndValidateCertificate(const std::string& keyFile,
    ...
    if (inBIO == nullptr) {
        LOGV2_ERROR(22921, "failed to allocate BIO object: {getSSLErrorMessage_ERR_get_error}", "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }

    ON_BLOCK_EXIT([&] { BIO_free(inBIO); });
    if (BIO_read_filename(inBIO, keyFile.c_str()) <= 0) {
        LOGV2_ERROR(22922, "cannot read key file when setting subject name: {keyFile} {getSSLErrorMessage_ERR_get_error}", "keyFile"_attr = keyFile, "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }
    ...
    if (x509 == nullptr) {
        LOGV2_ERROR(22923, "cannot retrieve certificate from keyfile: {keyFile} {getSSLErrorMessage_ERR_get_error}", "keyFile"_attr = keyFile, "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }
    ...
    *subjectName = getCertificateSubjectX509Name(x509);
    if (serverCertificateExpirationDate != nullptr) {
        ...
        if (notBeforeMillis == 0) {
            LOGV2_ERROR(22924, "date conversion failed");
            return false;
        }

        unsigned long long notAfterMillis = _convertASN1ToMillis(X509_get_notAfter(x509));
        if (notAfterMillis == 0) {
            LOGV2_ERROR(22925, "date conversion failed");
            return false;
        }

        if ((notBeforeMillis > curTimeMillis64()) || (curTimeMillis64() > notAfterMillis)) {
            LOGV2_FATAL(22943, "The provided SSL certificate is expired or not yet valid.");
...
bool SSLManagerOpenSSL::_setupPEM(SSL_CTX* context,
    ...
    if (SSL_CTX_use_certificate_chain_file(context, keyFile.c_str()) != 1) {
        LOGV2_ERROR(22926, "cannot read certificate file: {keyFile} {getSSLErrorMessage_ERR_get_error}", "keyFile"_attr = keyFile, "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }

    BIO* inBio = BIO_new(BIO_s_file());
    if (!inBio) {
        LOGV2_ERROR(22927, "failed to allocate BIO object: {getSSLErrorMessage_ERR_get_error}", "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }
    const auto bioGuard = makeGuard([&inBio]() { BIO_free(inBio); });

    if (BIO_read_filename(inBio, keyFile.c_str()) <= 0) {
        LOGV2_ERROR(22928, "cannot read PEM key file: {keyFile} {getSSLErrorMessage_ERR_get_error}", "keyFile"_attr = keyFile, "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }
    ...
    if (!privateKey) {
        LOGV2_ERROR(22929, "cannot read PEM key file: {keyFile} {getSSLErrorMessage_ERR_get_error}", "keyFile"_attr = keyFile, "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }
    const auto privateKeyGuard = makeGuard([&privateKey]() { EVP_PKEY_free(privateKey); });

    if (SSL_CTX_use_PrivateKey(context, privateKey) != 1) {
        LOGV2_ERROR(22930, "cannot use PEM key file: {keyFile} {getSSLErrorMessage_ERR_get_error}", "keyFile"_attr = keyFile, "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }

    // Verify that the certificate and the key go together.
    if (SSL_CTX_check_private_key(context) != 1) {
        LOGV2_ERROR(22931, "SSL certificate validation: {getSSLErrorMessage_ERR_get_error}", "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }
...
bool SSLManagerOpenSSL::_setupCRL(SSL_CTX* context, const std::string& crlFile) {
    ...
    if (status == 0) {
        LOGV2_ERROR(22932, "cannot read CRL file: {crlFile} {getSSLErrorMessage_ERR_get_error}", "crlFile"_attr = crlFile, "getSSLErrorMessage_ERR_get_error"_attr = getSSLErrorMessage(ERR_get_error()));
        return false;
    }
    LOGV2(22904, "ssl imported {status} revoked certificate{status_1_s} from the revocation list.", "status"_attr = status, "status_1_s"_attr = ((status == 1) ? "" : "s"));
    return true;
}
...
void SSLManagerOpenSSL::_flushNetworkBIO(SSLConnectionOpenSSL* conn) {
    ...
    while ((wantRead = BIO_ctrl_get_read_request(conn->networkBIO)) > 0) {
        ...
        if (toBIO != numRead) {
            LOGV2_DEBUG(22905, 3, "Failed to write network data to the SSL BIO layer");
...
StatusWith<SSLPeerInfo> SSLManagerOpenSSL::parseAndValidatePeerCertificate(
    ...
    if (nullptr == peerCert) {  // no certificate presented by peer
        if (_weakValidation) {
            ...
            if (!_suppressNoCertificateWarning) {
                LOGV2_WARNING(22912, "no SSL certificate provided by peer");
            }
            return SSLPeerInfo(sni);
        } else {
            auto msg = "no SSL certificate provided by peer; connection rejected";
            LOGV2_ERROR(22933, "{msg}", "msg"_attr = msg);
    ...
    if (result != X509_V_OK) {
        if (_allowInvalidCertificates) {
            LOGV2_WARNING(22913, "SSL peer certificate validation failed: {X509_verify_cert_error_string_result}", "X509_verify_cert_error_string_result"_attr = X509_verify_cert_error_string(result));
            return SSLPeerInfo(sni);
        } else {
            str::stream msg;
            msg << "SSL peer certificate validation failed: "
                << X509_verify_cert_error_string(result);
            LOGV2_ERROR(22934, "{msg_ss_str}", "msg_ss_str"_attr = msg.ss.str());
            return Status(ErrorCodes::SSLHandshakeFailed, msg);
        }
    }
    ...
    LOGV2_DEBUG(22906, 2, "Accepted TLS connection from peer: {peerSubject}", "peerSubject"_attr = peerSubject);
    ...
    if (remoteHost.empty()) {
        ...
        if (_sslConfiguration.serverSubjectName() == peerSubject) {
            LOGV2_WARNING(22914, "Client connecting with server's own TLS certificate");
        }
        ...
        for (int i = 0; i < sanNamesList; i++) {
            ...
            if (currentName && currentName->type == GEN_DNS) {
                ...
                if (swCIDRDNSName.isOK()) {
                    LOGV2_WARNING(22915, "You have an IP Address in the DNS Name field on your "
    ...
    if (!sanMatch && !cnMatch) {
        ...
        if (_allowInvalidCertificates || _allowInvalidHostnames || isUnixDomainSocket(remoteHost)) {
            LOGV2_WARNING(22916, "{msg}", "msg"_attr = msg);
        } else {
            LOGV2_ERROR(22935, "{msg}", "msg"_attr = msg);
            return Status(ErrorCodes::SSLHandshakeFailed, msg);
        }
    }
...
void SSLManagerOpenSSL::_handleSSLError(SSLConnectionOpenSSL* conn, int ret) {
    ...
    switch (code) {
        ...
        case SSL_ERROR_WANT_WRITE:
            ...
            LOGV2_ERROR(22936, "SSL: {code}, possibly timed out during connect", "code"_attr = code);
            break;

        case SSL_ERROR_ZERO_RETURN:
            // TODO: Check if we can avoid throwing an exception for this condition
            // If so, change error() back to LOG(3)
            LOGV2_ERROR(22937, "SSL network connection closed");
            break;
        case SSL_ERROR_SYSCALL:
            // If ERR_get_error returned 0, the error queue is empty
            // check the return value of the actual SSL operation
            if (err != 0) {
                LOGV2_ERROR(22938, "SSL: {getSSLErrorMessage_err}", "getSSLErrorMessage_err"_attr = getSSLErrorMessage(err));
            } else if (ret == 0) {
                LOGV2_ERROR(22939, "Unexpected EOF encountered during SSL communication");
            } else {
                LOGV2_ERROR(22940, "The SSL BIO reported an I/O error {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
            }
            break;
        case SSL_ERROR_SSL: {
            LOGV2_ERROR(22941, "SSL: {getSSLErrorMessage_err}", "getSSLErrorMessage_err"_attr = getSSLErrorMessage(err));
            break;
        }

        default:
            LOGV2_ERROR(22942, "unrecognized SSL error");
            break;
    }
