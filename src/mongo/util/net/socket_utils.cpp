...
const struct WinsockInit {
    WinsockInit() {
        ...
        if (WSAStartup(MAKEWORD(2, 2), &d) != 0) {
            LOGV2(22878, "ERROR: wsastartup failed {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
...
void setSocketKeepAliveParams(int sock,
    ...
    const auto getKey = [](const CString& key, DWORD default_value) {
        ...
        LOGV2_ERROR(22880, "can't get KeepAlive parameter: {withval_getStatus}", "withval_getStatus"_attr = withval.getStatus());
    ...
    if ((keepIdleSecs > maxKeepIdleSecs) || (keepIntvlSecs > maxKeepIntvlSecs)) {
        ...
        if (WSAIoctl(sock,
                     ...
                     nullptr,
                     nullptr)) {
            LOGV2_ERROR(22881, "failed setting keepalive values: {WSAGetLastError}", "WSAGetLastError"_attr = WSAGetLastError());
        }
    }
    ...
    const auto updateSockOpt =
        [sock](int level, int optnum, unsigned int maxval, StringData optname) {
            ...
            if (getsockopt(sock, level, optnum, (char*)&optval, &len)) {
                LOGV2_ERROR(22882, "can't get {optname}: {errnoWithDescription}", "optname"_attr = optname, "errnoWithDescription"_attr = errnoWithDescription());
            }

            if (optval > maxval) {
                optval = maxval;
                if (setsockopt(sock, level, optnum, (char*)&optval, sizeof(optval))) {
                    LOGV2_ERROR(22883, "can't set {optname}: {errnoWithDescription}", "optname"_attr = optname, "errnoWithDescription"_attr = errnoWithDescription());
                }
            }
        };
...
std::string getHostName() {
    ...
    if (ec || *buf == 0) {
        LOGV2(22879, "can't get this server's hostname {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return "";
    }
    return buf;
}
