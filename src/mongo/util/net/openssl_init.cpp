...
// Turn on FIPS mode if requested, OPENSSL_FIPS must be defined by the OpenSSL headers
    ...
    if (!status) {
        LOGV2_FATAL(22850, "can't activate FIPS mode: {SSLManagerInterface_getSSLErrorMessage_ERR_get_error}", "SSLManagerInterface_getSSLErrorMessage_ERR_get_error"_attr = SSLManagerInterface::getSSLErrorMessage(ERR_get_error()));
        fassertFailedNoTrace(16703);
    }
    LOGV2(22849, "FIPS 140-2 mode activated");
#else
    LOGV2_FATAL(22851, "this version of mongodb was not compiled with FIPS support");
