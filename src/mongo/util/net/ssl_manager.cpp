...
bool SSLConfiguration::isClusterMember(StringData subjectName) const {
    ...
    if (!swClient.isOK()) {
        LOGV2_WARNING("Unable to parse client subject name: {}", "swClient_getStatus"_attr = swClient.getStatus());
        return false;
    }
    auto& client = swClient.getValue();
    auto status = client.normalizeStrings();
    if (!status.isOK()) {
        LOGV2_WARNING("Unable to normalize client subject name: {}", "status"_attr = status);
        return false;
    }
...
void tlsEmitWarningExpiringClientCertificate(const SSLX509Name& peer) {
    LOGV2_WARNING("Peer certificate '{}' expires soon", "peer"_attr = peer);
}

void tlsEmitWarningExpiringClientCertificate(const SSLX509Name& peer, Days days) {
    LOGV2_WARNING("Peer certificate '{}' expires in {}", "peer"_attr = peer, "days"_attr = days);
}
