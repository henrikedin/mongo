...
void ProcessInfo::SystemInfo::collectSystemInfo() {
    ...
    if (uname(&unameData) == -1) {
        LOGV2(23034, "Unable to collect detailed system information: {strerror_errno}", "strerror_errno"_attr = strerror(errno));
    }
    ...
    if (sysinfo(SI_ARCHITECTURE_64, buf_64, sizeof(buf_64)) != -1 &&
        ...
        LOGV2(23035, "Unable to determine system architecture: {strerror_errno}", "strerror_errno"_attr = strerror(errno));
    }
    ...
    if (str::startsWith(osName, "Oracle Solaris")) {
        ...
        if (versionComponents.size() > 1) {
            ...
            if (!majorStatus.isOK() || !minorStatus.isOK()) {
                LOGV2_WARNING(23038, "Could not parse OS version numbers from uname: {osVersion}", "osVersion"_attr = osVersion);
            } else if ((majorInt == 11 && minorInt >= 2) || majorInt > 11) {
                preferMsyncOverFSync = true;
            }
        } else {
            LOGV2_WARNING(23039, "Could not parse OS version string from uname: {osVersion}", "osVersion"_attr = osVersion);
        }
    }
...
bool ProcessInfo::checkNumaEnabled() {
    ...
    if (cookie == LGRP_COOKIE_NONE) {
        LOGV2_WARNING(23040, "lgrp_init failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return false;
    }
    ...
    if (groups == -1) {
        LOGV2_WARNING(23041, "lgrp_nlgrps failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return false;
    }
...
bool ProcessInfo::blockInMemory(const void* start) {
    ...
    if (mincore(
        ...
        LOGV2(23036, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return 1;
    }
    return x & 0x1;
}
...
bool ProcessInfo::pagesInMemory(const void* start, size_t numPages, std::vector<char>* out) {
    ...
    if (mincore(static_cast<char*>(const_cast<void*>(alignToStartOfPage(start))),
                ...
                &out->front())) {
        LOGV2(23037, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return false;
    }
