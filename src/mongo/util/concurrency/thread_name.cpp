...
void setThreadName(StringData name) {
    ...
    if (error) {
        LOGV2(22789, "Ignoring error from setting thread name: {errnoWithDescription_error}", "errnoWithDescription_error"_attr = errnoWithDescription(error));
    }
    ...
    if (getpid() != syscall(SYS_gettid)) {
        ...
        if (error) {
            LOGV2(22790, "Ignoring error from setting thread name: {errnoWithDescription_error}", "errnoWithDescription_error"_attr = errnoWithDescription(error));
        }
    }
#endif
}
