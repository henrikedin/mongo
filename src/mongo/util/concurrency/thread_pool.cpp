...
void ThreadPool::startup() {
    ...
    if (_state != preStart) {
        LOGV2_FATAL(28698, "Attempting to start pool {}, but it has already started", "options_poolName"_attr = _options.poolName);
        fassertFailed(28698);
    }
...
void ThreadPool::_join_inlock(stdx::unique_lock<Latch>* lk) {
    _stateChange.wait(*lk, [this] {
        switch (_state) {
            ...
            case joining:
            case shutdownComplete:
                LOGV2_FATAL(28700, "Attempted to join pool {} more than once", "options_poolName"_attr = _options.poolName);
                fassertFailed(28700);
        }
        MONGO_UNREACHABLE;
    });
...
void ThreadPool::_consumeTasks() {
    ...
    if (_state != running) {
        LOGV2_FATAL(28701, "State of pool {} is {}, but expected {}", "options_poolName"_attr = _options.poolName, "static_cast_int32_t_state"_attr = static_cast<int32_t>(_state), "static_cast_int32_t_running"_attr = static_cast<int32_t>(running));
...
void ThreadPool::_doOneTask(stdx::unique_lock<Latch>* lk) noexcept {
    invariant(!_pendingTasks.empty());
    LOGV2_DEBUG(3, "Executing a task on behalf of pool {}", "options_poolName"_attr = _options.poolName);
...
void ThreadPool::_startWorkerThread_inlock() {
    switch (_state) {
        ...
        case joining:
        case shutdownComplete:
            LOGV2_DEBUG(1, "Not starting new thread in pool {} while shutting down", "options_poolName"_attr = _options.poolName);
            return;
        case running:
            break;
        default:
    ...
    if (_threads.size() == _options.maxThreads) {
        LOGV2_DEBUG(2, "Not starting new thread in pool {} because it already has {}, its maximum", "options_poolName"_attr = _options.poolName, "options_maxThreads"_attr = _options.maxThreads);
        return;
    }
