...
void ProcessInfo::SystemInfo::collectSystemInfo() {
    ...
    if (status != 0)
        LOGV2(23010, "Unable to collect OS Version. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));

    status = getSysctlByNameWithDefault("hw.machine_arch", std::string("unknown"), &cpuArch);
    if (status != 0)
        LOGV2(23011, "Unable to collect Machine Architecture. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));
    ...
    memLimit = memSize;
    if (status != 0)
        LOGV2(23012, "Unable to collect Physical Memory. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));

    status = getSysctlByNameWithDefault("hw.ncpu", defaultNum, &numBuffer);
    numCores = numBuffer;
    if (status != 0)
        LOGV2(23013, "Unable to collect Number of CPUs. (errno: {status} msg: {strerror_status})", "status"_attr = status, "strerror_status"_attr = strerror(status));
...
bool ProcessInfo::blockInMemory(const void* start) {
    ...
    if (mincore(alignToStartOfPage(start), getPageSize(), &x)) {
        LOGV2(23014, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return 1;
    }
    return x & 0x1;
}
...
bool ProcessInfo::pagesInMemory(const void* start, size_t numPages, std::vector<char>* out) {
    ...
    if (mincore(alignToStartOfPage(start), numPages * getPageSize(), &(out->front()))) {
        LOGV2(23015, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return false;
    }
