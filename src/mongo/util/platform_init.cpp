...
int __cdecl crtDebugCallback(int nRptType, char* originalMessage, int* returnValue) noexcept {
    *returnValue = 0;  // Returned by _CrtDbgReport. (1: starts the debugger).
    ...
    LOGV2("*** C runtime {}: {}{}", "severity_nRptType"_attr = severity(nRptType), "firstLine_originalMessage"_attr = firstLine(originalMessage), "die_terminating_sd_sd"_attr = (die ? ", terminating"_sd : ""_sd));
    if (die) {
        fassertFailed(17006);
    }
    return TRUE;
...
MONGO_INITIALIZER(Behaviors_Win32)(InitializerContext*) {
    ...
    if (_setmaxstdio(2048) == -1) {
        LOGV2_WARNING("Failed to increase max open files limit from default of 512 to 2048");
    }
    ...
    if (timeGetDevCaps(&tc, sizeof(TIMECAPS)) != TIMERR_NOERROR) {
        LOGV2_WARNING("Failed to read timer resolution range.");
        if (timeBeginPeriod(1) != TIMERR_NOERROR) {
            LOGV2_WARNING("Failed to set minimum timer resolution to 1 millisecond.");
        }
    } else {
        timerResolution =
