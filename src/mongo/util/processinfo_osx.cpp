...
Variant getSysctlByName(const char* sysctlName) {
    ...
    if (status == -1) {
        ...
        MONGO_BOOST_LOG << sysctlName << " unavailable";
        return "";
    }
...
long long getSysctlByName<NumberVal>(const char* sysctlName) {
    ...
    if (sysctlbyname(sysctlName, &value, &len, NULL, 0) < 0) {
        MONGO_BOOST_LOG << "Unable to resolve sysctl " << sysctlName << " (number) ";
    }
    if (len > 8) {
        MONGO_BOOST_LOG << "Unable to resolve sysctl " << sysctlName << " as integer.  System returned "
              << len << " bytes.";
    }
    return value;
}
