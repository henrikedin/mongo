...
Variant getSysctlByName(const char* sysctlName) {
    ...
    if (status == -1) {
        ...
        LOGV2(23029, "{sysctlName} unavailable", "sysctlName"_attr = sysctlName);
        return "";
    }
...
long long getSysctlByName<NumberVal>(const char* sysctlName) {
    ...
    if (sysctlbyname(sysctlName, &value, &len, nullptr, 0) < 0) {
        LOGV2(23030, "Unable to resolve sysctl {sysctlName} (number) ", "sysctlName"_attr = sysctlName);
    }
    if (len > 8) {
        LOGV2(23031, "Unable to resolve sysctl {sysctlName} as integer.  System returned {len} bytes.", "sysctlName"_attr = sysctlName, "len"_attr = len);
    }
    return value;
}
...
bool ProcessInfo::blockInMemory(const void* start) {
    ...
    if (mincore(alignToStartOfPage(start), getPageSize(), &x)) {
        LOGV2(23032, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return 1;
    }
    return x & 0x1;
}
...
bool ProcessInfo::pagesInMemory(const void* start, size_t numPages, std::vector<char>* out) {
    ...
    if (mincore(alignToStartOfPage(start), numPages * getPageSize(), &out->front())) {
        LOGV2(23033, "mincore failed: {errnoWithDescription}", "errnoWithDescription"_attr = errnoWithDescription());
        return false;
    }
