...
void installServiceOrDie(const wstring& serviceName,
    ...
    if (!ret) {
        ...
        LOGV2_ERROR("Failed to set timeout for pre-shutdown notification with error: {}", "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
...
void startService() {
    ...
    LOGV2("Trying to start Windows service '{}'", "toUtf8String_serviceName"_attr = toUtf8String(_serviceName));
