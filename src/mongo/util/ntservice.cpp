...
void configureService(ServiceCallback serviceCallback,
    ...
    if (params.count("install")) {
        if (badOption != -1) {
            LOGV2(22965, "--install cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
        if (!params.count("systemLog.destination") ||
            params["systemLog.destination"].as<std::string>() != "file") {
            LOGV2(22966, "--install has to be used with a log file for server output");
            quickExit(EXIT_BADOPTIONS);
        }
        installService = true;
    }
    if (params.count("reinstall")) {
        if (badOption != -1) {
            LOGV2(22967, "--reinstall cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
        if (!params.count("systemLog.destination") ||
            params["systemLog.destination"].as<std::string>() != "file") {
            LOGV2(22968, "--reinstall has to be used with a log file for server output");
            quickExit(EXIT_BADOPTIONS);
        }
    ...
    if (params.count("remove")) {
        if (badOption != -1) {
            LOGV2(22969, "--remove cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
        removeService = true;
    }
    if (params.count("service")) {
        if (badOption != -1) {
            LOGV2(22970, "--service cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
        _startService = true;
    }
    ...
    if (params.count("processManagement.windowsService.serviceName")) {
        if (badOption != -1) {
            LOGV2(22971, "--serviceName cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
    ...
    if (params.count("processManagement.windowsService.displayName")) {
        if (badOption != -1) {
            LOGV2(22972, "--serviceDisplayName cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
    ...
    if (params.count("processManagement.windowsService.description")) {
        if (badOption != -1) {
            LOGV2(22973, "--serviceDescription cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
    ...
    if (params.count("processManagement.windowsService.serviceUser")) {
        if (badOption != -1) {
            LOGV2(22974, "--serviceUser cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
    ...
    if (params.count("processManagement.windowsService.servicePassword")) {
        if (badOption != -1) {
            LOGV2(22975, "--servicePassword cannot be used with --{disallowedOptions_badOption}", "disallowedOptions_badOption"_attr = disallowedOptions[badOption]);
            quickExit(EXIT_BADOPTIONS);
        }
...
void installServiceOrDie(const wstring& serviceName,
                         ...
                         const std::vector<std::string>& argv,
                         const bool reinstall) {
    LOGV2(22976, "Trying to install Windows service '{toUtf8String_serviceName}'", "toUtf8String_serviceName"_attr = toUtf8String(serviceName));
    ...
    if (schSCManager == nullptr) {
        ...
        LOGV2(22977, "Error connecting to the Service Control Manager: {windows_GetErrMsg_err}", "windows_GetErrMsg_err"_attr = windows::GetErrMsg(err));
    ...
    while (true) {
        ...
        if (schService != nullptr) {
            LOGV2(22978, "There is already a service named '{toUtf8String_serviceName}{retryCount_0_sleeping_and_retrying_aborting}", "toUtf8String_serviceName"_attr = toUtf8String(serviceName), "retryCount_0_sleeping_and_retrying_aborting"_attr = (retryCount > 0 ? "', sleeping and retrying" : "', aborting"));
    ...
    if (schService == nullptr) {
        DWORD err = ::GetLastError();
        LOGV2(22979, "Error creating service: {windows_GetErrMsg_err}", "windows_GetErrMsg_err"_attr = windows::GetErrMsg(err));
        ::CloseServiceHandle(schSCManager);
        quickExit(EXIT_NTSERVICE_ERROR);
    }

    LOGV2(22980, "Service '{toUtf8String_serviceName}' ({toUtf8String_displayName}) installed with command line '{commandLine}'", "toUtf8String_serviceName"_attr = toUtf8String(serviceName), "toUtf8String_displayName"_attr = toUtf8String(displayName), "commandLine"_attr = commandLine);
    string typeableName((serviceName.find(L' ') != wstring::npos)
                            ? "\"" + toUtf8String(serviceName) + "\""
                            : toUtf8String(serviceName));
    LOGV2(22981, "Service can be started from the command line with 'net start {typeableName}'", "typeableName"_attr = typeableName);
    ...
    if (!serviceUser.empty()) {
        ...
        LOGV2(22982, "Setting service login credentials for user: {toUtf8String_actualServiceUser}", "toUtf8String_actualServiceUser"_attr = toUtf8String(actualServiceUser));
        ...
        if (!serviceInstalled) {
            LOGV2(22983, "Setting service login failed, service has 'LocalService' permissions");
        }
    }
...
#endif
        LOGV2(22984, "Could not set service description. Check the Windows Event Log for more details.");
    }
    ...
    if (!ret) {
        ...
        LOGV2_ERROR(22995, "Failed to set timeout for pre-shutdown notification with error: {errnoWithDescription_gle}", "errnoWithDescription_gle"_attr = errnoWithDescription(gle));
...
void removeServiceOrDie(const wstring& serviceName) {
    LOGV2(22985, "Trying to remove Windows service '{toUtf8String_serviceName}'", "toUtf8String_serviceName"_attr = toUtf8String(serviceName));

    SC_HANDLE schSCManager = ::OpenSCManager(nullptr, nullptr, SC_MANAGER_ALL_ACCESS);
    if (schSCManager == nullptr) {
        DWORD err = ::GetLastError();
        LOGV2(22986, "Error connecting to the Service Control Manager: {windows_GetErrMsg_err}", "windows_GetErrMsg_err"_attr = windows::GetErrMsg(err));
        quickExit(EXIT_NTSERVICE_ERROR);
    }

    SC_HANDLE schService = ::OpenService(schSCManager, serviceName.c_str(), SERVICE_ALL_ACCESS);
    if (schService == nullptr) {
        LOGV2(22987, "Could not find a service named '{toUtf8String_serviceName}' to remove", "toUtf8String_serviceName"_attr = toUtf8String(serviceName));
        ::CloseServiceHandle(schSCManager);
        quickExit(EXIT_NTSERVICE_ERROR);
    }
    ...
    if (::ControlService(schService, SERVICE_CONTROL_STOP, &serviceStatus)) {
        LOGV2(22988, "Service {toUtf8String_serviceName} is currently running, stopping service", "toUtf8String_serviceName"_attr = toUtf8String(serviceName));
        ...
        LOGV2(22989, "Service '{toUtf8String_serviceName}' stopped", "toUtf8String_serviceName"_attr = toUtf8String(serviceName));
    }
    ...
    if (serviceRemoved) {
        LOGV2(22990, "Service '{toUtf8String_serviceName}' removed", "toUtf8String_serviceName"_attr = toUtf8String(serviceName));
    } else {
        LOGV2(22991, "Failed to remove service '{toUtf8String_serviceName}'", "toUtf8String_serviceName"_attr = toUtf8String(serviceName));
    }
...
static void serviceStop() {
    ...
    while (exitedCleanly.wait_for(timeout.toSystemDuration()) != stdx::future_status::ready) {
        ...
        LOGV2(22992, "Service Stop is waiting for storage engine to finish shutdown");
    }
}
...
void startService() {
    ...
    LOGV2(22994, "Trying to start Windows service '{toUtf8String_serviceName}'", "toUtf8String_serviceName"_attr = toUtf8String(_serviceName));
